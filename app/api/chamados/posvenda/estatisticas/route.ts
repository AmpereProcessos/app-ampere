...
async function getPosVendaCallsStatistics(request: NextRequest) {
	...
	const stats = calls.reduce(
		(acc: { createdQty: number; concludedQty: number; totalTimeToConclusion: number; totalValue: number }, call) => {
			...
			const concludedWithinPeriod = conclusionDate && conclusionDate.getTime() >= periodAfterDate.getTime() && conclusionDate.getTime() <= periodBeforeDate.getTime();
			...
			if (call.metadados) {
				...
				const paymentWithinPeriod = paymentDate && paymentDate.getTime() >= periodAfterDate.getTime() && paymentDate.getTime() <= periodBeforeDate.getTime();
