				...
				<div style={MODAL_STYLES}>
					<div className="flex flex-col h-full">
						<div className="flex items-center justify-between border-b border-gray-300 pb-2">
							<h1 className="text-[#15599a] pl-6  font-bold">{info.nomeUsina}</h1>
							<button>
						...
						<div className="flex items-center justify-center gap-2 flex-wrap my-2">
							{infoHolder.created ? (
								infoHolder.status == "RESOLVIDO" ? (
									<h1 className={`p-2 rounded   text-xs border border-green-400 text-green-400 font-bold`}>RESOLVIDO</h1>
								) : (
									<>
										<h1
						...
						<div className="flex flex-col overflow-y-auto overscroll-y scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
							<div className="flex flex-col lg:items-center lg:flex-row gap-x-2 border border-gray-300 p-2 mt-4">
								...
								<div className={"grow"}>
									{!info.created ? (
										...
										<p className="text-gray-700 uppercase text-center">{info.nomeCliente}</p>
									)}
								</div>
							</div>
							<div className="flex flex-col lg:flex-row gap-x-2 border border-gray-300 p-2 mt-4">
								<span className="text-center uppercase font-bold">CÓDIGO DO PROJETO</span>
								<p className="text-sm text-center grow">{infoHolder.codProjeto ? infoHolder.codProjeto : "-"}</p>
							</div>
							<div className="flex flex-col lg:flex-row gap-x-2 border border-gray-300 p-2 mt-4">
							...
							<div className="flex items-center justify-center w-full mt-4">
								{info.created ? (
									<div className="flex flex-col gap-2">
										...
										<button onClick={closeCall} className="p-2 bg-green-300 rounded font-bold hover:bg-green-500 hover:text-white transition duration-300 ease-in-out hover:scale-105">
											FINALIZAR CHAMADO
										</button>
									</div>
								) : (
									<button onClick={createCall} className="p-2 bg-green-300 rounded font-bold hover:bg-green-500 hover:text-white transition duration-300 ease-in-out hover:scale-105">
