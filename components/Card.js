...
function Card({ propose, fetchProposes }) {
	...
	return (
		<div {...conditionalProp} className={`flex w-full flex-col gap-y-2 ${propose.rejected && "bg-gray-300"} py-2 mt-2 border border-gray-300 rounded shadow-lg`}>
			<div className="grid grid-cols-4">
				<p className="col-span-2 text-sm text-center mx-2">{propose.clientName}</p>
				<div className="flex col-span-1">
					<Link href={`/oem/pdf/propose/${propose._id}`}>
						<button className="bg-[#f6c228] h-[24px] w-[40px] px-2 rounded">Ver</button>
					</Link>
				</div>
				<p className="bg-green-400 text-xs py-1 col-span-1 mx-1 align-middle h-fit text-center rounded px-1">R$ {getCurrentPlanPrice()}</p>
			</div>
			<div className="w-full hidden xl:flex justify-around">
				<div className="flex flex-col items-center">
					<span className="text-xs uppercase text-gray-400">Plano:</span>
					<select onChange={(e) => handlePlanChange(e.target.value)} className="outline-none rounded bg-transparent text-center text-[#15599b] text-sm">
				...
				<div className="flex flex-col items-center">
					...
					<p className="text-xs text-center uppercase text-[#15599b]">{propose.rejected ? "Proposta rejeitada" : stages[propose.negotiationStage]}</p>
				</div>
				{propose.negotiationStage == 4 && !propose.closed && (
					<div className="flex items-center">
						<button onClick={handleProjectInitiation} className="bg-[#f6c228] h-[24px] w-[40px] px-2 rounded">
							&#128221;
						</button>
					</div>
				)}
				...
				{propose.negotiationStage != 4 && !propose.rejected && (
					<div className="flex items-center">
						<button onClick={() => setProposeAsRejected()} className="bg-red-400 h-[24px] w-[40px] px-2 rounded">
							&#x2716;
						</button>
					</div>
				)}
