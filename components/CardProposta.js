...
function CardProposta({ propose, fetchProposes }) {
	...
	return (
		<div {...conditionalProp} key={propose._id} className="flex gap-3 flex-col p-3 w-full min-h-[175px] h-[175px] border border-gray-300 shadow-md">
			...
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-2">
					...
					<h1 className="text-gray-700 font-medium text-xs">{propose.vendedor}</h1>
				</div>
				<div className="flex items-center gap-2">
					<BsTelephoneFill style={{ color: "#16B010" }} />
					<h1 className="text-gray-700 font-medium text-xs">{propose.telefoneVendedor ? propose.telefoneVendedor : "NÃO FORNECIDO"}</h1>
				</div>
			</div>
			<div className="flex items-center justify-between">
				<div className="w-1/3 flex items-center justify-start gap-2">
					<FaSolarPanel style={{ color: " rgb(217,119,6)" }} />
					<h1 className="text-gray-700 font-medium text-xs">{propose.qtdeModulos} MÓDULOS</h1>
				</div>
				<div className="w-1/3 flex items-center justify-center gap-2">
					<GoGraph style={{ color: "blue" }} />
					<h1 className="text-gray-700 font-medium text-xs">{propose.eficienciaAtual}%</h1>
				</div>
				<div className="w-1/3 flex items-end justify-end gap-2">
					<AiFillThunderbolt style={{ color: "red" }} />
					<h1 className="text-gray-700 font-medium text-xs">{propose.potModulos} W</h1>
				</div>
			</div>
			<div className="w-full grid grid-cols-3 items-center  px-0 lg:px-4">
				{propose.estagio > 1 ? (
					<div onClick={() => moveStage(propose.estagio - 1)} className="flex items-center justify-start cursor-pointer">
						<AiOutlineArrowLeft />
					</div>
				) : (
					<div></div>
				...
				<div className="flex gap-3 items-center justify-center">
					<a href={`https://app.ampereenergias.com.br/oem/pdfProposta/${propose._id}`} className="text-sm text-blue-300 font-medium cursor-pointer">
						PROPOSTA
					</a>
					<BsFolderFill style={{ color: "rgb(30,64,175)" }} />
				</div>
				{propose.estagio < 4 || !propose.estagio ? (
					<div onClick={() => moveStage(propose.estagio + 1)} className="flex items-center justify-end cursor-pointer">
						<AiOutlineArrowRight />
					</div>
				) : (
					<></>
