...
function ConferenciaManPreventivaOS({ session, order, closeModal, queryKey }) {
  ...
  function validateOSClosing() {
    ...
    if (!images.kitInversor) {
      toast.error(
        "Por favor, adicione foto(s) do quadro(s), string box (se houver) e inversor(res).",
      );
    ...
    try {
      if (images.paineisPreLimpeza) {
        for (let i = 0; i < images.paineisPreLimpeza.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido.nome}/paineisPreLimpeza${i + 1}`,
          );
      ...
      if (images.paineisPosLimpeza) {
        for (let i = 0; i < images.paineisPosLimpeza.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido.nome}/paineisPosLimpeza${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          const url = await getDownloadURL(ref(storage, res.metadata.fullPath));
      ...
      if (images.irregularidadesCorrigidas) {
        for (let i = 0; i < images.irregularidadesCorrigidas.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido.nome}/irregularidadesCorrigidas${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          const url = await getDownloadURL(ref(storage, res.metadata.fullPath));
      ...
      if (images.infraEletromecanica) {
        for (let i = 0; i < images.infraEletromecanica.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido.nome}/infraEletromecanica${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          const url = await getDownloadURL(ref(storage, res.metadata.fullPath));
      ...
      if (images.testesTensaoCAeCC) {
        for (let i = 0; i < images.testesTensaoCAeCC.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido.nome}/testesTensaoCAeCC${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          const url = await getDownloadURL(ref(storage, res.metadata.fullPath));
      ...
      if (images.ordemServicoAssinada) {
        for (let i = 0; i < images.ordemServicoAssinada.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido.nome}/ordemServicoAssinada${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          const url = await getDownloadURL(ref(storage, res.metadata.fullPath));
      ...
      if (images.conferenciaCabosSolares) {
        for (let i = 0; i < images.conferenciaCabosSolares.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido.nome}/conferenciaCabosSolares${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          const url = await getDownloadURL(ref(storage, res.metadata.fullPath));
      ...
      if (images.conferenciaCabosCA) {
        for (let i = 0; i < images.conferenciaCabosCA.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido.nome}/conferenciaCabosCA${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          const url = await getDownloadURL(ref(storage, res.metadata.fullPath));
      ...
      if (images.conferenciaConectores) {
        for (let i = 0; i < images.conferenciaConectores.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido.nome}/conferenciaConectores${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          const url = await getDownloadURL(ref(storage, res.metadata.fullPath));
      ...
      if (images.conferenciaMadeiramento) {
        for (let i = 0; i < images.conferenciaMadeiramento.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido.nome}/conferenciaMadeiramento${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          const url = await getDownloadURL(ref(storage, res.metadata.fullPath));
      ...
      if (images.configuracaoConcluida) {
        for (let i = 0; i < images.configuracaoConcluida.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido.nome}/configuracaoConcluida${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          const url = await getDownloadURL(ref(storage, res.metadata.fullPath));
  ...
  return (
    <div className="flex w-full flex-col items-center">
      ...
      <div className="mt-3 flex w-full flex-col gap-2">
        <div className="border-border dark:border-primary/70 flex w-full items-center justify-center rounded border p-2 pl-4">
          <input
          ...
          />
          <label className="dark:text-primary/20 ml-2 text-sm font-medium text-gray-900">
        ...
        <div className="flex flex-wrap justify-around gap-2">
          ...
          <div className="flex w-fit flex-col items-center">
            <label className="ml-2 text-center font-bold text-[#15599a]">
              FOTO DOS TESTES DE TENSÃO CC E CA
            </label>
            <div className="bg-primary/20 relative mt-2 flex h-fit items-center justify-center rounded-lg border-2 border-dotted border-blue-700 p-2">
              <div className="absolute">
                {images.testesTensaoCAeCC ? (
                  <div className="flex flex-col items-center">
                    <i className="fa fa-folder-open fa-4x text-blue-700"></i>
                    <span className="block text-center font-normal text-gray-400">
                      {images.testesTensaoCAeCC.length == 1
                        ? images.testesTensaoCAeCC[0].name
                        : `${images.testesTensaoCAeCC[0].name}...`}
                    </span>
                  </div>
                ) : (
              ...
              <div className="absolute">
                {images.ordemServicoAssinada ? (
                  <div className="flex flex-col items-center">
                    <i className="fa fa-folder-open fa-4x text-blue-700"></i>
                    <span className="block text-center font-normal text-gray-400">
                      {images.ordemServicoAssinada.length == 1
                        ? images.ordemServicoAssinada[0].name
                        : `${images.ordemServicoAssinada[0].name}...`}
                    </span>
                  </div>
                ) : (
              ...
              <div className="absolute">
                {images.reapertoGrampos ? (
                  <div className="flex flex-col items-center">
                    <i className="fa fa-folder-open fa-4x text-blue-700"></i>
                    <span className="block text-center font-normal text-gray-400">
                      {images.reapertoGrampos.length == 1
                        ? images.reapertoGrampos[0].name
                        : `${images.reapertoGrampos[0].name}...`}
                    </span>
                  </div>
                ) : (
          ...
          <div className="flex w-fit flex-col items-center">
            <label className="ml-2 text-center font-bold text-[#15599a]">
              PRINT DA CONFIGURAÇÃO CONCLUIDA (SE NECESSÁRIA)
            </label>
            <div className="bg-primary/20 relative mt-2 flex h-fit items-center justify-center rounded-lg border-2 border-dotted border-blue-700 p-2">
              <div className="absolute">
                {images.configuracaoConcluida ? (
                  <div className="flex flex-col items-center">
                    <i className="fa fa-folder-open fa-4x text-blue-700"></i>
                    <span className="block text-center font-normal text-gray-400">
                      {images.configuracaoConcluida.length == 1
                        ? images.configuracaoConcluida[0].name
                        : `${images.configuracaoConcluida[0].name}...`}
                    </span>
                  </div>
                ) : (
        ...
        <div className="flex flex-col items-center">
          ...
          <textarea
            ...
            className={
              "border-border min-h-[200px] w-full resize-none border p-2 text-center text-xs outline-hidden lg:w-[600px]"
            }
          />
        </div>
      </div>
