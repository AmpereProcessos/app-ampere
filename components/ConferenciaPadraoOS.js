...
function ConferenciaPadraoOS({ session, order, closeModal, queryKey }) {
	...
	function validateOSClosing() {
		if (!infoHolder.anotacoes || infoHolder.anotacoes?.trim().length < 5) {
			toast.error("Por favor, adicione anotações acerca da OS, dificultades encontradas, se o ramal não foi passado, o tamanho de ramal a ser levado e afins.");
	...
	return (
		<div className="flex w-full flex-col items-center">
			...
			<div className="mt-3 flex w-full flex-col gap-2">
				...
				<div className="flex w-full items-center justify-center rounded border border-gray-300 p-2 pl-4 dark:border-gray-700">
					<input checked={infoHolder.ramalPassado} onChange={(e) => setInfo({ ...infoHolder, ramalPassado: e.target.checked })} type="checkbox" value="" className="outline-none" />
