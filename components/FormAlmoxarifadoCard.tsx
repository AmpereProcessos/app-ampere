...
function FormAlmoxarifadoCard({ form, openModal }: FormAlmoxarifadoCardProps) {
    ...
    <div className="border-primary/20 flex w-full flex-col rounded-md border p-3 lg:w-[450px]">
      ...
      <div className="mt-1 flex w-full flex-wrap items-center justify-between">
        <div className="flex items-center gap-2">
          <FaCity />
          <p className="text-primary/60 text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">{form.cidade}</p>
        </div>
        <div className="flex items-center gap-2 text-blue-800">
          <FaUsers color={'rgb(30,64,175)'} />
          <p className="text-primary/60 text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">{form.equipeResp}</p>
        </div>
      </div>
      <div className="mt-2 flex w-full items-center justify-between">
        <div className={`flex items-center gap-2`}>
          <BsCalendarPlus />
          <p className="text-primary/60 text-xs font-medium">{formatDateAsLocale(form.abertura)}</p>
        </div>
        <div className="flex items-center justify-center gap-2">
          <FaUser />
          <p className="text-primary/60 text-xs font-medium">{form.responsavel}</p>
        </div>
      </div>
    </div>
  )
  ...
  //       }}
  //       className={`shdadow-lg flex w-full cursor-pointer flex-col justify-between border border-primary/20 p-3 hover:bg-blue-100 dark:hover:bg-primary/10 lg:w-[450px] ${getCardColor(
  //         formInfo.efetivado
  //       )}`}
  //     >
  //       <div className="mb-1 flex items-center justify-between font-medium">
  //         <p className="text-xs text-primary/70">{formInfo.nomeDoContrato ? formInfo.nomeDoContrato : formInfo.nomeTerceiro}</p>
  ...
  //               <p className="text-xs text-primary/80">{dayjs(formInfo.saidaDeObra).add(3, 'hour').format('DD/MM/YYYY')}</p>
  ...
  //             </div>
  //           ) : (
  //             <p className="text-xs text-primary/80">-</p>
  //           )}
  //         </div>
  //       </div>
  //     </div>
  ...
  //             <p className={`text-xs italic ${msg.color}`}>{msg.text}</p>
  //           ) : formInfo.saidaDeObra ? (
  //             <div className="flex items-center justify-end gap-2 text-primary/60">
  ...
  //         <p className="text-xs italic text-primary/60">ABERTO EM:</p>
  //         <p className="text-xs italic text-primary/60">{form.abertura ? dayjs(form.abertura).format('DD/MM/YYYY HH:mm') : ''}</p>
  //       </div>
  //     </div>
  //   )
}
