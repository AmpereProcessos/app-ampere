...
import { cidadesAtendidas, customersAcquisitionChannels, formatDate, vendedores } from '../utils/constants'
...
function FormSolicitacaoDadosContrato({ dados, setDados, avancar }) {
  ...
  function validarCamposObrigatorios() {
    ...
    if (dados.tipoDeServico == 'SISTEMA FOTOVOLTAICO (OFF GRID)' && dados.tipoDoTitular == 'NÃO DEFINIDO') {
    ...
    if (dados.possuiDeficiencia == 'SIM' && dados.qualDeficiencia.trim().length < 3) {
      setMessage('Por favor, preencha a deficiência.')
      return false
    }
    if (dados.canalVenda == 'INDICAÇÃO DE AMIGO' && dados.nomeIndicador.trim().length < 3) {
      ...
      <div className="flex flex-col flex-wrap justify-around gap-2 p-2 lg:grid lg:grid-cols-3">
        ...
        <div className="flex items-center justify-center">
          <DateFloatingInput
            ...
            editable={true}
            value={dados.dataDeNascimento ? formatDate(dados.dataDeNascimento) : null}
            handleChange={(value) =>
              setDados({
                ...dados,
        ...
        <div className="flex items-center justify-center">
          <TextFloatingInput
            ...
            editable={true}
            handleChange={(value) => setDados({ ...dados, numeroResCobranca: value })}
          />
        </div>
        <div className="flex items-center justify-center">
          <TextFloatingInput
