...
function FormSolicitacaoDadosInstalacaoRemontagem({ avancar, setDados, dados, voltar }) {
  ...
  async function findCPFRemontagem() {
    axios.get(`https://viacep.com.br/ws/${dados.cepInstalacaoRemontagem.replace('-', '')}/json/`).then((res) => {
      if (res.data.erro) {
        return
      } else {
        setDados({
          ...
          cidadeInstalacaoRemontagem: cidadesAtendidas.includes(res.data.localidade.toUpperCase()) ? res.data.localidade.toUpperCase() : 'ITUIUTABA',
  ...
  function validarCamposObrigatorios() {
    ...
    if (dados.numeroInstalacao == null || dados.numeroInstalacao.trim().length < 7) {
    ...
    if (dados.mudancaLocal == 'SIM') {
      ...
      if (dados.tipoDaInstalacaoRemontagem == null || dados.tipoDaInstalacaoRemontagem == 'NÃO DEFINIDO') {
      ...
      if (dados.numeroResInstalacaoRemontagem == null) {
        setMessage('Por favor, preencha o número da residência para o novo local.')
        return false
      }
      if (dados.tipoDaLigacao == 'EXISTENTE' && (dados.numeroInstalacaoRemontagem == null || dados.numeroInstalacao.trim().length < 7)) {
        setMessage('Por favor, preencha um número de instalação válido para o novo local.')
        return false
      }
      if (dados.tipoDaInstalacaoRemontagem == 'RURAL' && dados.longitudeRemontagem?.trim().length < 6) {
        setMessage('Por favor, preencha uma longitude válida para o novo local.')
        return false
      }
      if (dados.tipoDaInstalacaoRemontagem == 'RURAL' && dados.latitudeRemontagem?.trim().length < 6) {
        setMessage('Por favor, preencha uma latitude válida para o novo local.')
        return false
      }
    }
    setMessage('')
        ...
        {dados.mudancaLocal == 'SIM' ? (
          <>
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-2">
              ...
              />
              <button onClick={() => findCPFRemontagem('enderecoInstalacao')} className="flex h-[30px] items-center rounded bg-[#fead61] p-1">
                <AiOutlineSearch />
              </button>
            </div>
            <div className="flex items-center justify-center">
