...
function FormSolicitacaoEquipamentosBombaSolar({ dados, setDados, avancar, voltar }) {
	...
	const [inversoresMsg, setInversoresMsg] = useState("");
	...
	function addInversor() {
		if (!dadosInversores.marca || !dadosInversores.qtde || !dadosInversores.pot) {
			setInversoresMsg("Por favor, preencha todas as informações.");
		} else {
			setInversoresMsg("");
	...
	function addModulos() {
		if (!dadosModulos.marca || !dadosModulos.qtde || !dadosInversores.pot) {
			setModulosMsg("Por favor, preencha todas as informações.");
		} else {
	...
	function validarCamposObrigatorios() {
		if (dados.marcaInversor.trim().length == 0 || dados.qtdeInversor.trim().length == 0 || dados.potInversor.trim().length == 0) {
			setMessage("Por favor, adicione ao menos um driver.");
			return false;
		}
		if (dados.marcaModulos.trim().length == 0 || dados.qtdeModulos.trim().length == 0 || dados.potModulos.trim().length == 0) {
		...
		if (!dados.qtdeBateria || dados.qtdeBateria == 0) {
			setMessage("Por favor, preencha a quantidade de baterias.");
			return false;
		}
		if (!dados.tipoBateria || !dados.tipoBateria == "NÃO DEFINIDO") {
			setMessage("Por favor, preencha o tipo da bateria.");
			return false;
		}
		if (!dados.capacidadeBateria || dados.capacidadeBateria == 0) {
	...
	return (
		<div className="w-full flex flex-col border border-[#15599a] pb-2 shadow-lg bg-[#fff]">
			...
			<div className="flex flex-col border-t border-gray-300 p-2">
				<h1 className="text-center text-[#15599a] text-sm font-bold mt-2">ADICIONE DRIVERS</h1>
				<div className="flex flex-col mb-1 px-2">
					<p className="text-center italic text-xs">Você agora pode adicionar drivers de potência e/ou marca diferentes.</p>
					<p className="text-center italic text-xs text-[#fead61] font-bold">Preencha as informações dos drivers e clique em adicionar.</p>
				</div>
				<div className="flex flex-col lg:grid lg:grid-cols-4 gap-x-2 gap-y-1 mt-3">
					...
					<div className="flex items-center justify-center">
						<NumberFloatingInput label={"QTDE DE DRIVES"} editable={true} value={dadosInversores.qtde} handleChange={(value) => setDadosInvesores({ ...dadosInversores, qtde: value })} />
					</div>
					<div className="flex items-center justify-center">
						<NumberFloatingInput
					...
					<div className="flex items-center justify-center">
						<div onClick={addInversor} className="bg-green-300 hover:bg-green-500 text-white flex justify-center items-center h-fit p-2 rounded cursor-pointer">
							<MdOutlineAddCircle style={{ fontSize: "15px" }} />
						</div>
					</div>
				</div>
				{inversoresMsg && <p className="text-center italic text-sm text-red-500">{inversoresMsg}</p>}
			</div>
			{arrInv.length > 0 && (
				<div className="flex flex-col mt-2 font-bold">
					<h1 className="text-center text-[#15599a] text-xs">DRIVERS ADICIONADOS</h1>
			...
			<div className="flex flex-col border-t border-gray-300 p-2">
				<h1 className="text-center text-[#15599a] text-sm font-bold mt-2">ADICIONE MÓDULOS</h1>
				<div className="flex flex-col mb-1 px-2">
					<p className="text-center italic text-xs">Você agora pode adicionar módulos de potência e/ou marca diferentes.</p>
					<p className="text-center italic text-xs text-[#fead61] font-bold">Preencha as informações do módulos e clique em adicionar.</p>
				</div>
				<div className="flex flex-col lg:grid lg:grid-cols-4 gap-x-2 gap-y-1 mt-3">
					<div className="flex items-center justify-center">
						<TextFloatingInput
					...
					<div className="flex items-center justify-center">
						<div onClick={addModulos} className="bg-green-300 hover:bg-green-500 text-white flex justify-center items-center h-fit p-2 rounded cursor-pointer">
							<MdOutlineAddCircle style={{ fontSize: "15px" }} />
						</div>
					</div>
				</div>
				{modulosMsg && <p className="text-center italic text-sm text-red-500">{modulosMsg}</p>}
			</div>
			{arrModulos.length > 0 && (
				<div className="flex flex-col mt-2 font-bold">
					<h1 className="text-center text-[#15599a] text-xs">MÓDULOS ADICIONADOS</h1>
					{arrModulos.map((inv, index) => (
						<div key={index} className="flex flex-col lg:grid lg:grid-cols-4 gap-x-2 gap-y-1 mt-3">
			...
			<div className="flex flex-col border-t border-gray-300 p-2">
				<h1 className="text-center text-[#15599a] text-sm font-bold mt-2">BATERIAS</h1>
			...
			<div className="flex flex-col border-t border-gray-300 p-2">
				...
				<div className="flex flex-col lg:grid lg:grid-cols-3 items-center mt-2">
					...
					<div className="flex items-center justify-center">
						<NumberFloatingInput label={"POTÊNCIA BOMBA"} editable={true} value={dados.potBomba} handleChange={(value) => setDados({ ...dados, potBomba: Number(value) })} />
					</div>
				</div>
			</div>
			...
			<div className="flex w-full justify-center gap-2 flex-wrap mt-2">
				...
				<button onClick={proximaEtapa} className="w-fit text-center p-2 rounded bg-[#fead61] hover:bg-[#15599a] hover:text-white font-bold ">
					PRÓXIMA ETAPA
				</button>
			</div>
		</div>
