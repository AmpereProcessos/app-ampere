	...
	return (
		<div className="w-full flex flex-col border border-[#15599a] p-4 shadow-lg bg-[#fff]">
			...
			<div className="flex gap-2 justify-around flex-wrap mt-2">
				...
				/>
				<TextFloatingInput label={"CEP"} editable={true} width={"450px"} value={dados.cep} handleChange={(value) => setDados({ ...dados, cep: formatCEP(value) })} />
				<button onClick={() => findCPF()} className="flex items-center p-1 h-[30px] bg-[#fead61] rounded">
					<AiOutlineSearch />
				</button>
				<TextFloatingInput label={"BAIRRO"} editable={true} width={"450px"} value={dados.bairro} handleChange={(value) => setDados({ ...dados, bairro: value.toUpperCase() })} />
				<TextFloatingInput
					label={"LOGRADOURO"}
					editable={true}
					width={"450px"}
			...
			<div className="flex flex-col mt-2">
				<h1 className="text-[#fead61] col-span-3 text-center font-bold py-2">EQUIPAMENTO</h1>
				...
				<div className="flex flex-col border-t border-gray-300 p-2">
					<h1 className="text-center text-[#15599a] text-sm font-bold mt-2">ADICIONE MICRO/INVERSORES</h1>
					<div className="flex flex-col mb-1 px-2">
						<p className="text-center italic text-xs">Você agora pode adicionar micro/inversores de potência e/ou marca diferentes.</p>
						<p className="text-center italic text-xs text-[#fead61] font-bold">Preencha as informações do micro/inversor e clique em adicionar.</p>
					</div>
					...
					<div className="absolute">
						{images.localizacao ? (
							<div className="flex flex-col items-center">
								...
								<span className="block text-gray-400 font-normal text-center">{images.localizacao.file.name}</span>
							</div>
						) : (
							<div className="flex flex-col items-center">
								<i className="fa fa-folder-open fa-4x text-blue-700"></i>
			...
			{["VISITA TÉCNICA IN LOCO - URBANA", "ALTERAÇÃO DE PROJETO", "AUMENTO DE SISTEMA AMPÈRE", "VISITA TÉCNICA IN LOCO - RURAL"].includes(dados.tipoDeSolicitacao) ? (
				...
				<div className="flex justify-center items-center mt-3">
					<button onClick={goToNext} className="bg-[#fead61] hover:bg-[#15599a] text-center hover:text-white font-bold p-2 rounded w-fit">
						PRÓXIMA ETAPA
					</button>
				</div>
			)}
