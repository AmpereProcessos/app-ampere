...
export default function CostsInformation({
	...
	const {
		...
		isError,
		error,
	} = useProjectExpenses({ projectId, enabled: true, identifier: "CUSTOS-HOMOLOGAÇÃO" });

	const { mutate: handleCreateHomologationExpense, isPending } = useMutation({
		mutationKey: ["create-homologation-expense"],
		mutationFn: async () => {
			return await createExpense({
				identificador: "CUSTOS-HOMOLOGAÇÃO",
				rateio: "CUSTOS DIRETOS",
				categoria: "ART",
				descricao: "Custo de ART do projeto...",
				projeto: {
					id: projectId,
					nome: projectName,
	...
	return (
		<ResponsiveDialogDrawerSection sectionTitleText="CUSTOS" sectionTitleIcon={<DollarSign size={15} />}>
			<div className="w-full flex items-center justify-end gap-2">
				{isMissingARTCost ? (
					<LoadingButton
						...
						size="fit"
						loading={isPending}
						onClick={() => handleCreateHomologationExpense()}
					>
						GERAR CUSTO DE ART
					</LoadingButton>
				) : null}
			...
			{newExpenseMenuIsOpen ? <NewExpense session={sessionUser} closeModal={() => setNewExpenseMenuIsOpen(false)} /> : null}
