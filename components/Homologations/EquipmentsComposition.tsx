...
function EquipmentsComposition({ infoHolder, setInfoHolder, changes, setChanges }: EquipmentsCompositionProps) {
	...
	function addInverterToEquipments() {
		...
		const newProductsArr = [...infoHolder.homologacao.equipamentos, productInfo].sort((a, b) => a.categoria.localeCompare(b.categoria));
		setInfoHolder((prev) => ({ ...prev, homologacao: { ...prev.homologacao, equipamentos: newProductsArr } }));
	...
	function removeEquipment(index: number) {
		const currentProductList = [...infoHolder.homologacao.equipamentos].filter((_, i) => i !== index);
		...
		<div className="flex w-full flex-col gap-2">
			...
			<div className="flex w-full flex-col gap-1">
				...
				<div className="flex items-center justify-end">
					<Button variant={"ghost"} onClick={() => addInverterToEquipments()} size={"fit"} className="flex items-center gap-2 px-2 py-1 text-xs">
			...
			<div className="flex w-full flex-col gap-1">
				...
				<div className="flex items-center justify-end">
					<Button variant={"ghost"} onClick={() => addModuleToEquipments()} size={"fit"} className="flex items-center gap-2 px-2 py-1 text-xs">
						<Plus className="h-4 w-4" />
						ADICIONAR MÓDULO
					</Button>
				</div>
			...
			<div className="flex w-full flex-col gap-1">
				...
				<div className="flex items-center justify-end">
					<Button variant={"ghost"} onClick={() => addPersonalizedEquipment()} size={"fit"} className="flex items-center gap-2 px-2 py-1 text-xs">
						<Plus className="h-4 w-4" />
				...
				<div className="flex items-center gap-2">
					<div className="flex items-center gap-1 rounded-md bg-[#15599a] px-2 py-1 text-white">
						...
						<p className="text-xs font-medium tracking-tight">
							{getModulesPeakPotByProducts(infoHolder.homologacao.equipamentos as TProductItem[])} kWp EM MÓDULOS
						</p>
					</div>
					<div className="flex items-center gap-1 rounded-md bg-[#fead41] px-2 py-1 text-white">
						...
						<p className="tracking-tigh text-xs font-medium">
							{getInverterPeakPowerByProducts(infoHolder.homologacao.equipamentos as TProductItem[])} kWp EM INVERSORES
						</p>
					</div>
				</div>
			</div>
				...
				{infoHolder.homologacao.equipamentos.length > 0 ? (
					infoHolder.homologacao.equipamentos.map((product, index) => (
						<div
							...
							className="border-primary/20 flex w-full flex-col rounded-md border p-2 lg:w-[450px]"
						>
							<div className="flex w-full items-center justify-between gap-2">
