...
function EquipmentsComposition({ infoHolder, setInfoHolder, changes, setChanges }: EquipmentsCompositionProps) {
	...
	function addInverterToEquipments() {
		...
		var productsArr = [...infoHolder.homologacao.equipamentos];
		...
		setInfoHolder((prev) => ({ ...prev, homologacao: { ...prev.homologacao, equipamentos: orderProducts } }));
		setChanges((prev) => ({ ...prev, "homologacao.equipamentos": orderProducts }));
		setInverterHolder({
			id: "",
			fabricante: "",
			modelo: "",
	...
	function removeEquipment(index: number) {
		const currentProductList = [...infoHolder.homologacao.equipamentos];
		currentProductList.splice(index, 1);
		setInfoHolder((prev) => ({ ...prev, homologacao: { ...prev.homologacao, equipamentos: currentProductList } }));
		...
		<div className="flex w-full flex-col gap-2">
			...
			<div className="flex w-full flex-col gap-1">
				...
				<div className="flex items-center justify-end">
					<button className="rounded bg-black p-1 px-4 text-sm font-medium text-white duration-300 ease-in-out hover:bg-gray-700" onClick={() => addInverterToEquipments()}>
						ADICIONAR INVERSOR
					</button>
				</div>
			</div>
			<div className="flex w-full flex-col gap-1">
				...
				<div className="flex items-center justify-end">
					<button className="rounded bg-black p-1 px-4 text-sm font-medium text-white duration-300 ease-in-out hover:bg-gray-700" onClick={() => addModuleToEquipments()}>
						ADICIONAR MÓDULO
					</button>
				</div>
			</div>
			<div className="flex w-full flex-col gap-1">
				...
				<div className="flex items-center justify-end">
					<button className="rounded bg-black p-1 px-4 text-sm font-medium text-white duration-300 ease-in-out hover:bg-gray-700" onClick={() => addPersonalizedEquipment()}>
			...
			<div className="mt-2 flex w-full items-center justify-between gap-2">
				<h1 className="font-Inter mb-2 text-start font-bold leading-none tracking-tight">EQUIPAMENTOS A SEREM HOMOLOGADOS</h1>
				<div className="flex items-center gap-2">
					<div className="flex items-center gap-1 rounded-md bg-[#15599a] px-2 py-1 text-white">
						<FaSolarPanel />
						<p className="text-xs font-medium tracking-tight">{getModulesPeakPotByProducts(infoHolder.homologacao.equipamentos as TProductItem[])} kWp EM MÓDULOS</p>
					</div>
					<div className="flex items-center gap-1 rounded-md bg-[#fead41] px-2 py-1 text-white">
						<ImPower />
						<p className="tracking-tigh text-xs font-medium">{getInverterPeakPowerByProducts(infoHolder.homologacao.equipamentos as TProductItem[])} kWp EM INVERSORES</p>
					</div>
				</div>
			</div>
						...
						<div className="flex w-full flex-col rounded-md border border-gray-300 p-2 lg:w-[450px]">
							<div className="flex w-full items-center justify-between gap-2">
								<div className="flex  items-center gap-1">
									<div className="flex h-[25px] w-[25px] items-center justify-center rounded-full border border-black p-1">{renderProductCategoryIcon(product.categoria, 15)}</div>
