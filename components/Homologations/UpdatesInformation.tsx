...
function UpdatesInformation({
  session,
  infoHolder,
  setInfoHolder,
  setChanges,
}: UpdatesInformationProps) {
  const [updateHolder, setUpdateHolder] = useState<
    TProjectDTOWithHomologation["homologacao"]["atualizacoes"][number]
  >({
  ...
  function addUpdate(info: TProjectDTOWithHomologation["homologacao"]["atualizacoes"][number]) {
    ...
    if (updateHolder.descricao.trim().length < 3)
    ...
    return setInfoHolder((prev) => ({
      ...prev,
      homologacao: { ...prev.homologacao, atualizacoes: updates },
    }));
  }
    ...
    <div className="flex w-full flex-col gap-2">
      ...
      <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
        ...
        <div className="w-full lg:w-1/2">
          <DateTimeInput
            ...
            handleChange={(value) => {
              console.log(value);
              setUpdateHolder((prev) => ({
                ...prev,
                data: formatDateInputChange(value, "string") || new Date().toISOString(),
              }));
            }}
            width="100%"
          />
      ...
      <div className="mt-2 flex w-full flex-col gap-1">
        <h1 className="font-raleway mb-2 text-start leading-none font-bold tracking-tight">
          ATUALIZAÇÕES DA HOMOLOGAÇÃO
        </h1>
        {infoHolder.homologacao.atualizacoes.length > 0 ? (
          infoHolder.homologacao.atualizacoes.map((update, index) => (
            <div
              key={`${update.data}-${index}`}
              className="border-primary/60 flex w-full flex-col gap-1 rounded-md border p-3"
            >
              <div className="flex w-full items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-1">
                    <BsCalendarPlus />
                    <h1 className="cursor-pointer text-xs leading-none font-black tracking-tight">
                  ...
                  <div className="flex items-center gap-1">
                    <Avatar
                      ...
                      width={20}
                    />
                    <p className="text-foreground text-xs font-medium">{update.autor.nome}</p>
                  </div>
                </div>
                <button
              ...
              <div className="text-foreground bg-primary/20 flex w-full items-center justify-center rounded-md p-2 text-center text-xs font-medium">
                {update.descricao}
              </div>
            </div>
          ))
