...
import { GeneralVisibleHiddenExitMotionVariants, formatDecimalPlaces } from "@/utils/constants";
...
function InstallationDependentsBlock({
  infoHolder,
  setInfoHolder,
  changes,
  setChanges,
}: InstallationDependentsBlockProps) {
  const [newDependentMenuIsOpen, setNewDependentMenuIsOpen] = useState<boolean>(false);
  const [newDependentHolder, setNewDependentHolder] = useState<
    THomologation["instalacao"]["dependentes"][number]
  >({
    numeroInstalacao: "",
  ...
  function addDependent(info: THomologation["instalacao"]["dependentes"][number]) {
    ...
    setInfoHolder((prev) => ({
      ...
      homologacao: {
        ...prev.homologacao,
        instalacao: { ...prev.homologacao.instalacao, dependentes: dependents },
      },
    }));
  ...
  function updateDependent({
    ...
    item: THomologation["instalacao"]["dependentes"][number];
    index: number;
  }) {
    const dependents = [...infoHolder.homologacao.instalacao.dependentes];
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
        <h1 className="text-start font-sans font-bold text-primary">DEPENDENTES/RECEBEDORAS</h1>
        {infoHolder.homologacao.instalacao.dependentes.length > 0 ? (
          <div className="flex items-center gap-1">
            <h1 className="rounded-lg bg-green-500 px-2 py-1 text-[0.65rem] font-bold text-white">
            ...
            <h1 className="bg-primary/80 rounded-lg px-2 py-1 text-[0.65rem] font-bold text-white">
              ...
              {infoHolder.homologacao.instalacao.dependentes.length > 1
                ? "RECEBEDORAS"
                : "RECEBEDORA"}
            </h1>
          </div>
        ) : (
          <h1 className="rounded-lg bg-red-500 px-2 py-1 text-[0.65rem] font-bold text-white">
      ...
      {infoHolder.homologacao.instalacao.dependentes.length > 0 ? (
        <div className="flex w-full flex-col gap-1">
          <div className="border-primary/80 flex w-full flex-col items-center rounded border-0 lg:border">
            <div className="bg-primary/60 hidden w-full items-center gap-2 rounded rounded-br-none rounded-bl-none p-1 lg:flex">
              ...
              <h1 className="w-[40%] text-center text-sm font-bold text-white">
            ...
            {infoHolder.homologacao.instalacao.dependentes.map(
              ...
              //         <div className="flex items-center gap-1">
              //           <BsCode size={12} />
              //           <p className="text-[0.65rem] font-medium text-foreground">INSTALAÇÃO</p>
              //         </div>
              ...
              //           <p className="text-[0.65rem] font-medium text-foreground">PORCENTAGEM</p>
              //         </div>
              ...
              //   {/* <h1 className="text-xs font-medium text-foreground">INSTALAÇÃO Nº {dependent.numeroInstalacao}</h1>
        ...
        <p className="text-foreground w-full text-center text-sm font-medium">
          Não há recebedoras definidas para essa geradora.
        </p>
      )}
      <div className="flex w-full items-center justify-end">
          ...
          <div className="flex w-full flex-col gap-2 lg:flex-row">
            <div className="w-full lg:w-1/2">
              <TextInput
                ...
                handleChange={(value) =>
                  setNewDependentHolder((prev) => ({ ...prev, numeroInstalacao: value }))
                }
                width="100%"
              />
            </div>
            <div className="w-full lg:w-1/2">
              <NumberInput
                ...
                handleChange={(value) =>
                  setNewDependentHolder((prev) => ({ ...prev, recebimentoPercentual: value }))
                }
                width="100%"
              />
            </div>
...
function DependenteTableItem({
  index,
  dependent,
  handleRemove,
  handleUpdate,
}: DependenteTableItemProps) {
  const [editMenuIsOpen, setEditMenuIsOpen] = useState<boolean>(false);
  const [dependentHolder, setDependentHolder] =
    useState<THomologation["instalacao"]["dependentes"][number]>(dependent);
  return (
    <>
      <AnimatePresence>
        <div className="hidden w-full flex-col gap-1 lg:flex">
          <div className="flex w-full items-center gap-2 p-1">
            ...
            <h1 className="w-[45%] text-center text-xs tracking-tight">
              {dependent.numeroInstalacao}
            </h1>
            <h1 className="w-[45%] text-center text-xs tracking-tight">
              {dependent.recebimentoPercentual}%
            </h1>
          </div>
        </div>
        <div className="group border-primary/80 flex w-full flex-col items-center rounded border shadow-xs lg:hidden">
          <h1 className="bg-primary/80 w-full p-1 text-center text-[0.65rem] font-bold text-white">
            RECEBEDORA {index + 1}
          </h1>
          <div className="flex w-full flex-col p-2">
            <div className="flex w-full items-start justify-between gap-4 p-2">
              <div className="flex flex-col items-center gap-1 lg:items-start">
                <div className="flex items-center gap-1">
                  <BsCode size={12} />
                  <p className="text-foreground text-[0.65rem] font-medium">INSTALAÇÃO</p>
                </div>
              ...
              <div className="flex flex-col items-center gap-1 lg:items-end">
                <div className="flex items-center gap-1">
                  <FaPercentage size={12} />
                  <p className="text-foreground text-[0.65rem] font-medium">PORCENTAGEM</p>
                </div>
                <p className="text-[0.6rem] leading-none font-medium tracking-tight">
                  {dependent.recebimentoPercentual}%
                </p>
            ...
            <div className="flex w-full flex-col gap-2 lg:flex-row">
              <div className="w-full lg:w-1/2">
                <TextInput
                  ...
                  handleChange={(value) =>
                    setDependentHolder((prev) => ({ ...prev, numeroInstalacao: value }))
                  }
                  width="100%"
                />
              </div>
              <div className="w-full lg:w-1/2">
                <NumberInput
                  ...
                  handleChange={(value) =>
                    setDependentHolder((prev) => ({ ...prev, recebimentoPercentual: value }))
                  }
                  width="100%"
                />
              </div>
