...
function LeadCard({ lead, getLeads }) {
	...
	return (
		<div
			key={lead._id}
			className={`border-primary/20 dark:hover:bg-primary/10 flex w-full flex-col border p-3 hover:bg-blue-100 ${lead.perdido ? "bg-red-100" : ""} items-center`}
		>
			<div className="border-primary/20 relative grid w-full grid-cols-1 items-center border-b pb-1 lg:grid-cols-10">
				...
				<div className="hidden flex-col items-center lg:flex">
					<p className="text-primary/80 text-center text-sm font-bold">CIDADE</p>
					<p className="text-primary/80 text-center text-sm">{lead.cidade ? lead.cidade : "-"}</p>
				</div>
				<div className="hidden flex-col items-center lg:flex">
					<p className="text-primary/80 text-center text-sm font-bold">BRANDING SCORE</p>
					<p className="text-primary/80 text-center text-sm">{lead.leadscoreBranding ? lead.leadscoreBranding : "-"}</p>
				</div>
				<div className="hidden flex-col items-center lg:flex">
					<p className="text-primary/80 text-center text-sm font-bold">PRODUTO SCORE</p>
					<p className="text-primary/80 text-center text-sm">{lead.leadscoreProduto ? lead.leadscoreProduto : "-"}</p>
				</div>
				<div className="hidden flex-col items-center lg:flex">
					<p className="text-primary/80 text-center text-sm font-bold">NICHO</p>
					<p className="text-primary/80 text-center text-sm">{lead.nicho ? lead.nicho : "-"}</p>
				</div>
				<div className="hidden lg:block">
					<SelectInput
						label="CANAL"
				...
				<div className="grid w-full grid-cols-1 grid-rows-4 gap-2 lg:grid-cols-10 lg:grid-rows-1">
					...
					<div className="col-span-1 flex items-start justify-center gap-4 lg:col-span-4">
						<div className="flex flex-col items-center">
							<p className="text-primary/80 text-center text-sm font-bold">DATA DE AQUISIÇÃO</p>
							<input
								type={"date"}
								className="text-primary/80 bg-transparent text-center text-sm outline-hidden"
						...
						<div className="flex flex-col items-center">
							<p className="text-primary/80 text-center text-sm font-bold">DATA DE ENVIO</p>
							<p className="text-primary/80 text-center text-sm">{dayjs(infoHolder.dataDeEnvio).add(4, "hour").format("DD/MM/YYYY")}</p>
						</div>

						<div className="hidden flex-col items-center lg:flex">
							<p className="text-primary/80 text-center text-sm font-bold">TEMPO P/ENVIO</p>
							<p className="text-primary/80 text-center text-sm">
								{lead.dataDeAquisicao && lead.dataDeEnvio ? dayjs(lead.dataDeEnvio).diff(dayjs(lead.dataDeAquisicao), "day") : "-"}
							</p>
						</div>
					</div>
					<div className="col-span-1 flex items-center justify-center gap-3 lg:col-span-4">
						...
						<div className="flex flex-col items-center">
							<p className="text-primary/80 text-center text-sm font-bold">STATUS</p>
							<p className="text-primary/80 text-center text-sm uppercase">
								{lead.perdido ? "PERDIDO" : lead.estagioFunil ? stages[lead.estagioFunil] : stages[1]}
							</p>
						</div>
					</div>
						...
						{deleteMenu ? (
							<div className="flex flex-col items-center justify-center">
								...
								<div className="bg-background border-primary/20 absolute top-8 z-2 w-fit rounded border shadow-lg">
									<button onClick={deleteLead} className="text-primary/70 p-2 text-xs font-bold hover:bg-red-200">
										EXCLUIR
									</button>
								</div>
							</div>
