...
import { customersAcquisitionChannels, AcquisitionChannels, vendedores, formatDate } from "../utils/constants";
...
function LeadCard({ lead, getLeads }) {
	...
	return (
		<div key={lead._id} className={`flex flex-col w-full border border-gray-300 p-3 hover:bg-blue-100 ${lead.perdido ? "bg-red-100" : ""} items-center`}>
			<div className="relative grid grid-cols-1 lg:grid-cols-10 items-center border-b border-gray-300 pb-1 w-full">
				...
				<div className="hidden lg:flex flex-col items-center">
					...
					<p className="text-gray-600 text-center text-sm">{lead.cidade ? lead.cidade : "-"}</p>
				</div>
				<div className="hidden lg:flex flex-col items-center">
					<p className="text-gray-600 text-center font-bold text-sm">BRANDING SCORE</p>
					<p className="text-gray-600 text-center text-sm">{lead.leadscoreBranding ? lead.leadscoreBranding : "-"}</p>
				</div>
				<div className="hidden lg:flex flex-col items-center">
					<p className="text-gray-600 text-center font-bold text-sm">PRODUTO SCORE</p>
					<p className="text-gray-600 text-center text-sm">{lead.leadscoreProduto ? lead.leadscoreProduto : "-"}</p>
				</div>
				<div className="hidden lg:flex flex-col items-center">
					<p className="text-gray-600 text-center font-bold text-sm">NICHO</p>
					<p className="text-gray-600 text-center text-sm">{lead.nicho ? lead.nicho : "-"}</p>
				</div>
				<div className="hidden lg:block">
					<SelectInput
						label="CANAL"
				...
				<div className="grid grid-cols-1 grid-rows-4 gap-2 lg:grid-cols-10 lg:grid-rows-1 w-full">
					...
					<div className="flex justify-center items-start col-span-1  lg:col-span-4 gap-4">
						<div className="flex flex-col items-center">
							<p className="text-gray-600 text-center font-bold text-sm">DATA DE AQUISIÇÃO</p>
							<input
								type={"date"}
						...
						<div className="flex flex-col items-center">
							<p className="text-gray-600 text-center font-bold text-sm">DATA DE ENVIO</p>
							<p className="text-gray-600 text-center text-sm">{dayjs(infoHolder.dataDeEnvio).add(4, "hour").format("DD/MM/YYYY")}</p>
						</div>

						<div className="hidden lg:flex flex-col items-center">
							<p className="text-gray-600 text-center font-bold text-sm">TEMPO P/ENVIO</p>
							<p className="text-gray-600 text-center text-sm">{lead.dataDeAquisicao && lead.dataDeEnvio ? dayjs(lead.dataDeEnvio).diff(dayjs(lead.dataDeAquisicao), "day") : "-"}</p>
						</div>
					</div>
					<div className="flex justify-center items-center col-span-1 lg:col-span-4 gap-3">
						<SelectInput
							...
							editable={true}
							widthFit={true}
							value={vendedores.filter((x) => x.nome == infoHolder.vendedor).length > 0 ? infoHolder.vendedor : "NÃO DEFINIDO"}
						...
						<div className="flex flex-col items-center">
							<p className="text-gray-600 text-center font-bold text-sm">STATUS</p>
							<p className="text-gray-600 uppercase text-center text-sm">{lead.perdido ? "PERDIDO" : lead.estagioFunil ? stages[lead.estagioFunil] : stages[1]}</p>
						</div>
					</div>
					<div className="grid grid-cols-1 relative mt-1 lg:mt-0">
						{deleteMenu ? (
							<div className="flex flex-col items-center justify-center">
								...
								<div className="w-fit rounded bg-[#fff] z-2 shadow-lg border border-gray-300 absolute top-8">
									<button onClick={deleteLead} className="text-gray-700 font-bold text-xs hover:bg-red-200 p-2">
										EXCLUIR
									</button>
								</div>
							</div>
							...
							<div className="flex items-center justify-center">
								{lead.perdido ? (
									showLostLeadJustification ? (
										<AiFillEye title="ESCONDER MOTIVO DA PERDA" onClick={() => setShowJustification(false)} style={{ color: "#fead61", cursor: "pointer" }} />
									) : (
										<AiFillEyeInvisible title="MOSTRAR MOTIVO DA PERDA" onClick={() => setShowJustification(true)} style={{ color: "#fead61", cursor: "pointer" }} />
									)
								) : null}
								<div
			...
			{showLostLeadJustification ? (
				<div className="flex flex-col w-full border border-red-500">
					<h1 className="font-bold text-center text-sm">MOTIVO DA PERDA DO LEAD</h1>
					<p className="w-full text-center py-1 text-xs italic text-red-500 font-medium">{lead.motivoPerda}</p>
				</div>
			) : null}
		</div>
	);
