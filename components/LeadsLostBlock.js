    ...
    <div className="bg-background flex w-full flex-col rounded-md p-3 shadow-md">
      <div className="flex w-full items-center justify-between">
        <div className="flex flex-wrap items-center justify-center gap-2">
          <h1 className="text-foreground font-medium">PERDIDOS</h1>
        </div>
        {dropdownMenuVisible ? (
      ...
      {dropdownMenuVisible ? (
        <div className="grid w-full grid-cols-2 gap-2">
          {lostLeads.map((lead, index) => (
            <div key={index.toString()} className="flex w-full flex-col rounded-md bg-red-100 p-2">
              <h1 className="text-foreground font-medium">{lead.nome}</h1>
