...
function PurchaseSolicitationItemRow({ item, index, infoHolder, setInfo }) {
	...
	return (
		<div className="border-primary/20 flex w-full flex-col gap-2 rounded-tl-sm rounded-tr-sm border-b bg-gray-50 py-1">
			<div className="grid w-full grid-cols-10 items-center">
				<h1 className="text-primary/70 col-span-2 w-full p-1 text-center text-xs font-medium">
					{item.descricao}
				...
				<div className="col-span-1 w-full p-1">
					<input
						...
						type="number"
						className="text-primary/70 h-full w-full bg-transparent text-center text-xs outline-hidden"
					/>
				</div>
				...
				<div className="col-span-2 flex h-full w-full items-center p-1">
					<input
						...
						type="date"
						className="text-primary/70 h-full w-full grow bg-transparent text-center text-xs outline-hidden"
					/>
				...
				<div className="col-span-1 flex w-full items-center justify-center gap-4 p-1 font-medium">
					{item.anotacoes ? (
						showDescription ? (
							<AiFillEye onClick={() => setShowDescription(false)} style={{ color: "#fead61", cursor: "pointer" }} />
						) : (
							<AiFillEyeInvisible onClick={() => setShowDescription(true)} style={{ color: "#fead61", cursor: "pointer" }} />
						)
					) : null}
					<MdCancel
						onClick={() => {
			...
			{showDescription ? (
				<div className="flex flex-col items-center">
					...
					<p className="text-primary/80 w-full grow text-center text-xs italic">{item.anotacoes}</p>
				</div>
			) : null}
		</div>
	);
