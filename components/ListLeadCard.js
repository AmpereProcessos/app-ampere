...
function ListLeadCard({ lead, fetchLeads }) {
	...
	return (
		<div
			{...conditionalProp}
			key={lead._id}
			className={`flex gap-3 flex-col p-3 w-full min-h-[190px] h-[190px] ${lead.perdido ? "bg-red-100" : "bg-[#fff]"} border border-gray-300 shadow-md`}
		>
			...
			<div className="w-full flex items-center justify-between">
				...
				<div className="flex items-center gap-2 text-gray-700">
					...
					<h1 className="font-medium text-xs">{lead.dataDeEnvio ? dayjs(lead.dataDeEnvio).add(4, "hour").format("DD/MM/YYYY") : "-"}</h1>
				</div>
			</div>
			<div className="w-full flex items-center justify-between">
				...
				<div className="flex flex-col items-end text-gray-700">
					...
					<h1 className="font-medium text-xs">{lead.nicho ? lead.nicho : "-"}</h1>
				</div>
			</div>
			<div className="w-full grid grid-cols-3 items-center  px-0 lg:px-4">
				{lead.estagioFunil > 1 ? (
					<div onClick={() => moveStage(lead.estagioFunil - 1)} className="flex items-center justify-start cursor-pointer">
						<AiOutlineArrowLeft />
					</div>
				) : (
					<div></div>
				...
				<div className="flex items-start justify-center relative">
					{!lead.perdido ? (
						loseLeadInfo.open ? (
							<div className="p-2 absolute w-[200%] h-[150px] bg-[#fff] shadow-lg border border-gray-300 flex flex-col">
								<div className="w-full flex items-center justify-between text-xs border-b border-gray-300 pb-1">
								...
								<div className="w-full flex items-center justify-between mt-1">
									...
									<button onClick={() => setAsLost(lead._id)} className="bg-red-300 text-xs text-white hover:bg-red-500 font-medium p-1 rounded">
										PERDER
									</button>
								</div>
							</div>
				...
				{lead.estagioFunil < 3 || !lead.estagioFunil ? (
					<div onClick={() => moveStage(lead.estagioFunil ? lead.estagioFunil + 1 : 2)} className="flex items-center justify-end cursor-pointer">
						<AiOutlineArrowRight />
					</div>
				) : (
					<></>
