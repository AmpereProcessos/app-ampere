...
function ListLeads({ leads, title, listId, fetchLeads }) {
	...
	return (
		<div
			ref={dropRef}
			id={listId}
			className="flex flex-col min-w-[400px] w-[400px]  max-h-[750px] py-2 items-center pt-2 px-1 grow bg-white h-full rounded border border-gray-300 shadow-lg"
		>
			<div className="border-b pb-2 h-fit w-full text-center border-blue-300 text-xl font-bold">
				<h1>{title}</h1>
				<div className="flex justify-center gap-x-2">
					<p className="text-xs text-gray-500">{filteredLeads.length} lead(s)</p>
