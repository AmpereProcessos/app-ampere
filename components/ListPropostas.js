...
function ListPropostas({ proposes, title, listId, fetchProposes }) {
	...
	return (
		<div
			ref={dropRef}
			id={listId}
			className="flex flex-col min-w-[400px] w-[400px] max-h-[550px] py-2 items-center pt-2 px-1 grow bg-white h-full rounded border border-gray-300 shadow-lg"
		>
			<div className="border-b pb-2 h-fit w-full text-center border-blue-300 text-xl font-bold">
				<h1>{title}</h1>
				<div className="flex justify-center gap-x-2">
					<p className="text-xs text-gray-500">{filteredPropostas.length} proposta(s)</p>
					<p className="text-xs text-gray-500">{getListCumulativeModules()} módulos</p>
					<p className="text-xs text-gray-500">{getListCumulativePeakPot()} kWp</p>
				</div>
				<input
					value={searchPropostas}
					onChange={(e) => handleSearchFilter(e.target.value)}
					className={"outline-none border border-gray-300 p-1 w-full mt-2 text-sm text-gray-600 text-center font-semibold"}
