	...
	function renderResponsibles(current) {
		...
		return filtered.map((resp, index) => (
			...
			>
				...
				<p className="text-primary/60 text-xs font-medium">{resp.apelido}</p>
			</div>
		));
	}
				...
				<div className="flex h-full flex-col">
					<div className="border-primary/20 flex items-center justify-between border-b px-2 pb-2 text-lg">
						<div className="flex flex-col">
							...
							<p className="text-primary/60 text-center text-xs">#{call?._id || "..."}</p>
						</div>
						<button>
							<VscChromeClose
								onClick={() => {
					...
					{callFetched && infoHolder ? (
						<div className="overscroll-y scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex flex-col gap-2 overflow-y-auto px-2 lg:px-0">
							...
							<div className="mt-4 flex items-center justify-center gap-4">
								<div className="flex items-center gap-2">
									...
									<p className="text-primary/60 text-xs font-medium">{call.requerente?.apelido || "Requerente não identificado"}</p>
								</div>
							...
							<h1 className="bg-primary/80 mt-4 w-full rounded-md p-2 text-center font-bold text-white">PROJETO</h1>
							{call.projeto ? (
							...
							<h1 className="bg-primary/80 mt-4 w-full rounded-md p-2 text-center font-bold text-white">CLIENTE</h1>
							{call.cliente ? (
                      ...
                      <p className="font-raleway font-medium text-sm">{call.cliente.tipo || 'N/A'}</p>
								...
								<p className="text-primary/60 w-full py-2 text-center text-sm italic">Sem informações do cliente...</p>
							)}
							<h1 className="bg-primary/80 mt-4 w-full rounded-md p-2 text-center font-bold text-white">PREMISSAS</h1>
							{call.premissas ? (
								<div className="flex w-full flex-col items-center justify-center gap-2 md:flex-row md:gap-4">
									<div className="flex items-center gap-2">
								...
								<p className="text-primary/60 w-full py-2 text-center text-sm italic">Sem premissas fornecidas...</p>
							)}
							{call.premissas?.cargas ? (
								<div className="text-primary/80 flex w-full flex-col">
									...
									<div className="grid grid-cols-5 lg:grid-cols-6">
										<h1 className="bg-primary/80 text-xxs border-r border-white p-1 text-center font-bold text-white lg:text-xs">NOME</h1>
										<h1 className="bg-primary/80 text-xxs hidden border-r border-white p-1 text-center font-bold text-white lg:flex lg:text-xs">
											POTÊNCIA NOMINAL
										</h1>
										<h1 className="bg-primary/80 text-xxs border-r border-white p-1 text-center font-bold text-white lg:text-xs">QTDE</h1>
										<h1 className="bg-primary/80 text-xxs border-r border-white p-1 text-center font-bold text-white lg:text-xs">POTÊNCIA TOTAL</h1>
										<h1 className="bg-primary/80 text-xxs border-r border-white p-1 text-center font-bold text-white lg:text-xs">HORAS DE USO</h1>
										<h1 className="bg-primary/80 text-xxs p-1 text-center font-bold text-white lg:text-xs">Wh DIÁRIO</h1>
									</div>
									{call.premissas.cargas.map((charge, index) => (
										<div key={index} className="border-primary/20 grid grid-cols-5 border border-t-0 lg:grid-cols-6">
											<h1 className="text-xxs text-primary/80 border-primary/20 border-r p-2 text-center font-bold">{charge.descricao}</h1>
											<h1 className="text-xxs text-primary/80 border-primary/20 hidden border-r p-2 text-center font-bold lg:flex">{charge.qtde}</h1>
											<h1 className="text-xxs text-primary/80 border-primary/20 border-r p-2 text-center font-bold">{charge.potencia}</h1>
											<h1 className="text-xxs text-primary/80 border-primary/20 border-r p-2 text-center font-bold">{charge.qtde * charge.potencia}</h1>
											<h1 className="text-xxs text-primary/80 border-primary/20 border-r p-2 text-center font-bold">{charge.horasFuncionamento}</h1>
											<h1 className="text-xxs text-primary/80 p-2 text-center font-bold">{charge.qtde * charge.potencia * charge.horasFuncionamento}</h1>
										</div>
									))}
									<div className="border-primary/20 grid grid-cols-5 items-center border border-t-0 lg:grid-cols-6">
										...
										<p className="text-primary/80 col-span-1 p-2 text-center text-xs font-bold">
							...
							<div className="flex w-full flex-col">
								<h1 className="bg-primary/80 mt-4 w-full rounded-tl-md rounded-tr-md p-2 text-center font-bold text-white">OBSERVAÇÕES</h1>
							...
							<h1 className="bg-primary/80 mt-4 w-full rounded-md p-2 text-center font-bold text-white">RESPONSÁVEL</h1>
							<div className="flex w-full flex-wrap justify-around gap-2">{renderResponsibles(call.responsavel?.apelido)}</div>
							<div className="flex w-full flex-col">
								<h1 className="bg-primary/80 mt-4 w-full rounded-tl-md rounded-tr-md p-2 text-center font-bold text-white">ANOTAÇÕES</h1>
								<textarea
							...
							<h1 className="bg-primary/80 mt-4 w-full rounded-tl-md rounded-tr-md p-2 text-center font-bold text-white">ARQUIVOS</h1>
							<div className="flex w-full flex-col items-center gap-2">
								{call.links && call.links.length > 0 ? (
									...
									<p className="text-primary/60 py-2 text-sm italic">Sem arquivos anexados...</p>
								)}
							</div>
							{editor ? (
