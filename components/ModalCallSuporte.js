				...
				<div className="flex h-full flex-col">
					<div className="border-primary/20 flex items-center justify-between border-b px-2 pb-2 text-lg">
						<div className="flex flex-col items-center">
							<h1 className="pl-6 font-bold text-[#15599a] uppercase">
								{info.tipoChamado} {info.equipamento && `- ${info.equipamento}`}
							</h1>
							<p className="text-primary/60 text-xs">{info._id}</p>
						</div>

						{info.demanda && <span className="text-primary/80 border-primary/20 border p-2 text-xs font-bold">DEMANDA {info.demanda}</span>}
						<button>
							<VscChromeClose
								onClick={() => {
									setMessage({ text: "", color: "" });
					...
					<p className="text-primary/70 mt-2 text-center text-xs italic">
						{info.fechamento
					...
					<div className="overscroll-y scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 overflow-y-auto">
						{!!session?.user.permissoes.posVenda.visualizar && info.fechamento ? (
							<div className="border-primary/20 mt-4 flex flex-col items-center gap-x-2 border p-2 lg:flex-row">
								<span className="font-raleway text-center font-bold">COLETA DE FEEDBACK</span>
								<input
								...
								{info.statusChamado == "ABERTO" ? (
									<>
										<p
											...
											className={`${infoHolder.statusChamado != "ABERTO" && "opacity-30"} w-fit cursor-pointer rounded-lg border p-3 text-center text-xs font-bold ${
										...
										<p
											...
											className={`${infoHolder.statusChamado != "EM ANDAMENTO" && "opacity-30"} w-fit cursor-pointer rounded-lg border p-3 text-center text-xs font-bold hover:opacity-100 ${
						...
						<div className="border-primary/20 mt-4 flex flex-col items-center gap-x-2 border p-2 lg:flex-row">
							<span className="font-raleway text-center font-bold">NOME DO CLIENTE</span>
							<p className="font-raleway grow text-center">{info.nomeCliente ? info.nomeCliente : "-"}</p>
						</div>
						<div className="border-primary/20 mt-4 flex flex-col items-center gap-x-2 border p-2 lg:flex-row">
							<span className="font-raleway text-center font-bold">NOME DA USINA</span>
							<p className="font-raleway grow text-center">{info.nomeUsina ? info.nomeUsina : "-"}</p>
						</div>
						...
						<div className="border-primary/20 mt-4 flex flex-col items-center gap-x-2 border p-2 lg:flex-row">
							...
							<p className="font-raleway grow text-center">{new Date(info.abertura).toLocaleString()}</p>
						</div>
						{info.fechamento && (
							<div className="border-primary/20 mt-4 flex flex-col items-center gap-x-2 border p-2 lg:flex-row">
								<span className="font-raleway text-center font-bold">FECHAMENTO</span>
								<p className="font-raleway grow text-center">{new Date(info.fechamento).toLocaleString()}</p>
							</div>
						)}
						<div className="border-primary/20 mt-4 flex flex-col items-center gap-x-2 border p-2 lg:flex-row">
							<span className="font-raleway text-center font-bold">CIDADE</span>
							<select
						...
						<div className="border-primary/20 mt-4 flex flex-col items-center justify-center gap-x-2 border p-2 lg:flex-row">
						...
						<div className="border-primary/20 mt-4 flex flex-col gap-x-2 border p-2">
							<span className="font-raleway text-center font-bold">DESCRIÇÃO DO PROBLEMA</span>
							<span className="font-raleway bg-primary/20 grow p-4 text-center text-sm italic">{info.descricaoProblema ? info.descricaoProblema : ""}</span>
						</div>
						{info.tipoChamado.includes("GARANTIA") && (
							<>
								<div className="border-primary/20 mt-4 flex flex-col gap-1 border p-2">
									<span className="font-raleway text-center font-bold">ÚLTIMA ATUALIZAÇÃO DO CLIENTE</span>
									<input
										...
										type={"date"}
										className="font-raleway grow text-center outline-hidden"
									/>
								</div>
								<div className="border-primary/20 mt-4 flex flex-col gap-1 border p-2">
									<span className="font-raleway text-center font-bold">STATUS DA GARANTIA</span>
									<select
						...
						<div className="border-primary/20 mt-4 flex flex-col gap-x-2 border p-2">
							...
							<textarea
								...
								className="bg-primary/20 mt-1 h-fit min-h-[175px] grow resize-none rounded p-3 text-center text-sm outline-hidden placeholder:italic"
							/>
						</div>
                ...
                /> */}
						...
						{info.idPai ? (
							<div className="mt-2 flex w-full flex-col gap-2">
								<h1 className="bg-primary/80 w-full rounded-md p-2 text-center font-medium text-white">ORDENS DE SERVIÇO</h1>
								<OSCreationBlock
								...
								/>
								<ProjectServiceOrders projectId={info.idPai} session={session} />
							</div>
						) : null}
						{info.statusChamado == "RESOLVIDO" ? (
							<div className="text-center">
								<button onClick={reopenCall} className="font-raleway mt-4 rounded-lg bg-yellow-400 p-3 font-bold hover:bg-[#f18701] hover:text-white">
									REABRIR CHAMADO
								</button>
							</div>
						) : (
							<div className="text-center">
								<button onClick={closeCall} className="font-raleway mt-4 rounded-lg bg-green-400 p-3 font-bold hover:bg-[#06d6a0] hover:text-white">
									FINALIZAR CHAMADO
								</button>
							</div>
						)}
