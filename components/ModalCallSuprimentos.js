...
function ModalCallSuprimentos({ setModalIsOpen, info, getCalls, modalIsOpen }) {
  ...
  function handleChanges() {
    axios
      .put('/api/chamados/suprimentos/mainData', {
        id: info._id,
        mudancas: changes,
      })
      .then((res) => {
    ...
    <>
      <AnimatedModalWrapper modalIsOpen={modalIsOpen} width={'40%'} height={'87%'}>
        <div className="flex h-full flex-col">
          ...
          <div className="overscroll-y scrollbar-thin scrollbar-thumb-primary/20 flex h-full flex-col gap-2 overflow-y-auto py-3">
              ...
              {modalInfo.status != 'FECHADO' ? (
                <>
                  <h1
                    ...
                    className={`cursor-pointer rounded border-2 border-yellow-500 p-2 text-sm font-bold text-yellow-500 ${
                      modalInfo.status == 'ABERTO' ? 'opacity-100' : 'hover::opacity-100 opacity-30'
                    }`}
                  >
                    ABERTO
                  </h1>
                  <h1
                    ...
                    className={`cursor-pointer rounded border-2 border-[#15599a] p-2 text-sm font-bold text-[#15599a] ${
                      modalInfo.status == 'EM ANDAMENTO' ? 'opacity-100' : 'hover::opacity-100 opacity-30'
                    }`}
                  >
                    EM ANDAMENTO
                  </h1>
                ...
                <h1 className={`rounded border-2 border-green-500 p-2 text-sm font-bold text-green-500`}>FECHADO</h1>
              )}
            </div>
            <div className="border-primary/20 flex w-full items-center justify-between border-y p-4">
              <h1 className="text-sm font-bold">FORNECEDOR</h1>
              <h1 className="text-sm font-bold text-[#15599a]">{modalInfo.fornecedor}</h1>
            </div>
            <div className="border-primary/20 flex w-full items-center justify-between border-y p-4">
              <h1 className="text-sm font-bold">DATA DE ENTREGA</h1>
              <h1 className="text-sm font-bold">
                {modalInfo.dataEntrega ? `${dayjs().diff(new Date(modalInfo.dataEntrega), 'day')} dias desde entrega` : '-'}
              </h1>
              <h1 className="text-sm font-bold text-[#15599a]">{modalInfo.dataEntrega ? dayjs(modalInfo.dataEntrega).format('DD/MM/YYYY') : '-'}</h1>
            </div>
            <div className="border-primary/20 flex w-full items-center justify-between border-y p-4">
              <h1 className="text-sm font-bold">ENTREGA FALTANDO</h1>
              <h1 className="text-sm font-bold text-[#15599a]">{modalInfo.entregaFaltando ? 'SIM' : 'NÃO'}</h1>
            </div>
            <div className="border-primary/20 flex w-full items-center justify-between border-y p-4">
              <h1 className="text-sm font-bold">AVARIAS</h1>
              <h1 className="text-sm font-bold text-[#15599a]">{modalInfo.avarias ? 'SIM' : 'NÃO'}</h1>
            </div>
            <div className="mt-3 flex flex-col items-center gap-1">
              ...
              <textarea
                ...
                readOnly={true}
                className="border-primary/20 overscroll-y scrollbar-thin scrollbar-thumb-primary/20 flex h-[150px] w-full resize-none flex-col overflow-y-auto border p-2 text-center text-xs outline-hidden"
              />
            </div>
            <div className="mt-3 flex flex-col items-center gap-1">
            ...
            <div className="mt-3 flex flex-col items-center gap-1">
              <h1 className="text-sm font-bold text-[#15599a]">OBSERVAÇÕES E ANOTAÇÕES</h1>
              <textarea
            ...
            <div className="flex flex-col">
              ...
              <AnexoArquivo
                id={modalInfo.idPai}
                prevLinks={modalInfo.links ? { chamadosSuprimentos: modalInfo.links } : {}}
            ...
            {message.text && <p className={`text-center ${message.color} text-xs italic`}>{message.text}</p>}
            ...
            {modalInfo.status != 'FECHADO' ? (
              <button
                onClick={() => closeCall()}
                className="mt-3 w-fit self-center rounded border border-green-500 p-2 text-green-500 hover:bg-green-500 hover:font-bold hover:text-white"
              >
                FINALIZAR CHAMADO
              </button>
            ) : (
              <button
                onClick={() => reopenCall()}
                className="mt-3 w-fit self-center rounded border border-yellow-500 p-2 font-bold text-yellow-500 hover:bg-yellow-500 hover:text-white"
              >
                REABRIR CHAMADO
              </button>
            )}
