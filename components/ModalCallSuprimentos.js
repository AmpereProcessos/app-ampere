					...
					<div className="flex flex-col gap-2 h-full overflow-y-auto overscroll-y scrollbar-thin scrollbar-thumb-gray-300 py-3">
						<div className="flex items-center justify-center gap-2 w-full  p-2">
							{modalInfo.status != "FECHADO" ? (
								<>
									<h1
										...
										className={`cursor-pointer text-sm font-bold text-yellow-500 border-2 border-yellow-500 p-2 rounded ${
											modalInfo.status == "ABERTO" ? "opacity-100" : "opacity-30 hover::opacity-100"
										}`}
									>
										ABERTO
									</h1>
									<h1
										...
										className={`cursor-pointer text-sm font-bold text-[#15599a] border-2 border-[#15599a] p-2 rounded ${
											modalInfo.status == "EM ANDAMENTO" ? "opacity-100" : "opacity-30 hover::opacity-100"
										}`}
									>
										EM ANDAMENTO
									</h1>
								...
								<h1 className={`text-sm font-bold text-green-500 border-2 border-green-500 p-2 rounded`}>FECHADO</h1>
							)}
						</div>
						...
						<div className="flex items-center justify-between w-full border-y border-gray-300 p-4">
							...
							<h1 className="text-sm font-bold">{modalInfo.dataEntrega ? `${dayjs().diff(new Date(modalInfo.dataEntrega), "day")} dias desde entrega` : "-"}</h1>
						...
						{modalInfo.status != "FECHADO" ? (
							...
							<button onClick={() => reopenCall()} className="mt-3 self-center rounded w-fit p-2 border border-yellow-500 text-yellow-500 hover:text-white hover:bg-yellow-500 font-bold">
								REABRIR CHAMADO
							</button>
						)}
					</div>
