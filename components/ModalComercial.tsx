...
function ModalComercial({ session, projectId, closeModal }: ModalComercialProps) {
	...
	async function handleProjectUpdate({
		...
		changes,
		queryClient,
	}: { previousData: TProjectDTO; newData: TProjectDTO; changes: Record<string, any>; queryClient: QueryClient }) {
				...
				{isSuccess && infoHolder ? (
					<>
						...
						<DialogFooter>
							...
							<LoadingButton onClick={() => handleProjectUpdateMutation({ previousData: project, newData: infoHolder, changes: changes, queryClient: queryClient })} loading={isPending}>
								{BUTTON_TEXT}
							</LoadingButton>
						</DialogFooter>
					</>
...
function ModalComercialContent({ session, projectId, project, infoHolder, setInfoHolder, changes, setChanges, clientChanges, setClientChanges }: ModalComercialContentProps) {
	...
	return (
		<div className="flex h-full w-full flex-col gap-6 px-4 lg:px-0">
			...
			<InfoClienteBlock editable={true} infoHolder={infoHolder} setInfo={setInfoHolder} clientChanges={clientChanges} setClientChanges={setClientChanges} />
			<InfoVendaBlock editor={true} infoHolder={infoHolder} setInfo={setInfoHolder} changes={changes} setChanges={setChanges} updateLogs={updateLogs || []} project={project} />
			...
			<InfoReceitasBlock session={session} project={infoHolder} projectId={projectId} projectName={infoHolder.nomeDoContrato} projectIdentificator={project.qtde.toString()} />
			<InfoPagamentoBlock editor={true} infoHolder={infoHolder} setInfo={setInfoHolder} changes={changes} setChanges={setChanges} updateLogs={updateLogs || []} showADMOnly={false} />
