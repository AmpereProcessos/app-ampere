        ...
        <div className="flex h-full flex-col">
          ...
          <div className="scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex grow flex-col overflow-y-auto overscroll-y-auto px-2">
            ...
            <div className="flex w-full flex-col gap-2 pt-4 lg:flex-row">
              <div className="w-full lg:w-1/2">
                <div className="flex w-full flex-col">
                  <h1 className="text-foreground text-sm">NOME DA OPERAÇÃO</h1>
                  <p className="font-arial border-border text-foreground border-b-2 text-center text-sm">
                    {operationInfo.nome}
                  </p>
                </div>
              </div>
              <div className="w-full lg:w-1/2">
                <div className="flex w-full flex-col">
                  <h1 className="text-foreground text-sm">DESCRIÇÃO</h1>
                  <p className="font-arial border-border text-foreground border-b-2 text-center text-sm">
            ...
            <div className="flex w-full flex-col gap-2 pt-4 lg:flex-row">
              <div className="w=full lg:w-1/2">
                <div className="flex w-full flex-col">
                  <h1 className="text-foreground text-sm">DATA DE INÍCIO</h1>
                  <p className="font-arial border-border text-foreground border-b-2 text-center text-sm">
              ...
              <div className="w-full lg:w-1/2">
                <div className="flex w-full flex-col">
                  <h1 className="text-foreground text-sm">PREVISÃO DE CONCLUSÃO</h1>
                  <p className="font-arial border-border text-foreground border-b-2 text-center text-sm">
                ...
                operationInfo.atividades.map((activity, index, arr) => (
                  ...
                  >
                    <div className="flex w-full items-center justify-around">
                      <p className="text-foreground w-3/5 text-start font-medium lg:w-2/5">
                        {activity.nome}
                      </p>
                      <div className="hidden w-2/6 items-center gap-2 lg:flex">
                        <div className="text-foreground flex items-center gap-2 text-center font-medium">
                          <p>INICIO EM:</p>
                          <p>
                            {activity.dataInicio
                      ...
                      <div className="flex w-2/5 items-center justify-center gap-4 lg:w-1/5">
                        ...
                        {activity.dataConclusao ? (
                          <BsPatchCheckFill style={{ color: "rgb(34,197,94)" }} />
                        ) : null}
                      </div>
                    </div>
                    {activity.subAtividades
                      ? activity.subAtividades.map((subActivity, subIndex) => (
                          <div
                            key={subIndex}
                            className="my-1 flex w-full items-start justify-around"
                          >
                            <p className="text-foreground w-3/5 text-center text-xs lg:w-2/5">
                              {subActivity.nome}
                            </p>

                            <div className="hidden w-2/5 items-center gap-2 lg:flex">
                              <p className="text-foreground hidden text-center text-xs lg:block">
                              ...
                              {subActivity.status != "CONCLUIDO" ? (
                                <button
                                  onClick={() => {
                                    ...
                                    activitiesArr[index].subAtividades[subIndex].status =
                                      "CONCLUIDO";
                                    activitiesArr[index].subAtividades[subIndex].dataConclusao =
                                      new Date().toISOString();
          ...
          {/* <div className="w-full py-2 border-t border-border flex items-center justify-end justify-self-end">
            {operationMsg.text ? (
