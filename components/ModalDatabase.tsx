...
export default function ModalDatabase({ session, projectId, closeModal, callbacks }: ModalDatabaseProps) {
	...
	const userHasOverallAccess = useMemo(
		() =>
			["Projetos", "Obras", "Suprimentos", "O&M", "Marketing", "Vendas", "Pós-Venda", "PPS", "InsideSales", "Financeiro", "ADM", "RH"].every((el) => permissions?.rotas?.includes(el)),
		[permissions],
	);
	...
	const userHasComissionValuesAccess = useMemo(() => ["PPS", "Financeiro", "ADM", "RH"].every((el) => permissions?.rotas?.includes(el)), [permissions]);
	...
	async function handleProjectUpdate({
		...
		clientChanges,
		project,
	}: { projectId: string; changes: Record<string, any>; clientChanges: Record<string, any>; project: TProjectDTO }) {
...
function ModalDatabaseContent({
	...
	return (
		<div className="flex h-full w-full flex-col gap-6 px-4 lg:px-0">
			...
			<InfoClienteBlock editable={userHasOverallAccess} infoHolder={infoHolder} setInfo={setInfoHolder} clientChanges={clientChanges} setClientChanges={setClientChanges} />
			<InfoVendaBlock
			...
			<InfoJornadaBlock editor={userHasOverallAccess} infoHolder={infoHolder} setInfo={setInfoHolder} changes={changes} setChanges={setChanges} updateLogs={updateLogs || []} />
