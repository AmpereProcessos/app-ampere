        ...
        {isSuccess && infoHolder ? (
          <>
            ...
            <DialogFooter>
              ...
              <LoadingButton
                onClick={() =>
                  handleProjectUpdateMutation({ projectId, changes, clientChanges, project })
                }
                loading={isPending}
              >
                {BUTTON_TEXT}
...
function createNonNullableSetter(
  setter: Dispatch<SetStateAction<TProjectDTO | null>>,
): Dispatch<SetStateAction<TProjectDTO>> {
  return (update) => {
    if (typeof update === "function") {
      setter((prev) =>
        (update as (prevState: TProjectDTO) => TProjectDTO)(prev as unknown as TProjectDTO),
      );
    } else {
      setter(update);
    }
  ...
  return (
    <div className="flex h-full w-full flex-col gap-6 px-4 lg:px-0">
      <NotificationCreationBlock
        ...
        codProjeto={infoHolder.qtde.toString()}
      />
      <InfoAtividadesBlock
        projectId={projectId}
      ...
      {!["OPERAÇÃO E MANUTENÇÃO", "BOMBA SOLAR", "SISTEMA FOTOVOLTAICO (OFF GRID)"].includes(
      ...
      {!["BOMBA SOLAR", "SISTEMA FOTOVOLTAICO (OFF GRID)", "OPERAÇÃO E MANUTENÇÃO"].includes(
        infoHolder.tipoDeServico,
      ) ? (
      ...
      <ProjectServiceOrders
        ...
        session={session}
        projectMainServiceOrderId={project.idOrdemServico}
      />
