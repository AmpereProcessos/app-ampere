...
function ModalFormSolicitacao({ solicitacaoId, closeModal, editor, financeiroEditor, getFormularios }) {
  ...
  async function vinculateVisitaTecnica() {
    try {
      ...
      await editContractRequest({ id: solicitacaoId, changes: updateData })
      setDados(updateData)
    } catch (error) {
  ...
  function getInsertObject(dados) {
    var insertObj = {
  ...
  async function handleSendNotificationToCobrancas(requestInfo) {
    try {
      let data = await axios.post('/api/notificacoes/1', {
        destinatario: '6353eb83ef4e1a367a877949',
        remetente: 'SISTEMA',
        mensagem: `Olá, acabo de aprovar uma solicitação de contrato do cliente ${requestInfo.nomeDoContrato}. Desde já agradeço, Volts.`,
      })
    } catch (error) {
      console.log(error)
    }
  }
  async function handleSendEmailToCobrancas(requestInfo) {
    try {
      await axios.post('/api/integracao/email', {
        ...
        message: `Olá, acabo de aprovar uma solicitação de contrato do cliente ${requestInfo.nomeDoContrato}. Formulário disponível no link: https://app.ampereenergias.com.br/comercial/publicoFormulario/${requestInfo._id} . Desde já agradeço, Volts.`,
        copy: [
  ...
  function validateCreation(requestInfo) {
    if (!requestInfo) return { validation: false, msg: 'Oops, houve um erro.' }
    const insertObj = getInsertObject(requestInfo)
      ...
      if (typeof entry[1] == 'object' && entry[1] != null) {
        ...
        Object.keys(entry[1]).forEach((x) => {
          if (validation[`${tag}.${x}`] != undefined) {
            if (validation[`${tag}.${x}`].test(insertObj[tag][x]) == true) {
              return { validation: false, msg: validation[`${tag}.${x}`].msg }
              // return toast.error(validation[`${tag}.${x}`].msg || 'Oops, houve um erro.')
              // setCreationMsg({
        ...
        if (validation[tag] != undefined) {
          if (validation[tag].test(insertObj[tag]) == true) {
            return { validation: false, msg: validation[tag].msg }
          }
        }
      }
    })
  ...
  async function addProject(requestInfo) {
    if (!requestInfo) return toast.error('Oops, houve um erro.')
    try {
      const { validation, msg } = validateCreation(requestInfo)
      if (!validation) return toast.error(msg)
      // Updating the contract request instance
      await editContractRequest({ id: solicitacaoId, changes: { aprovacao: true, dataAprovacao: new Date().toISOString() } })
      // Adding a new operational project
      const insertObj = getInsertObject(dados)
      await axios.post('/api/projects/add', insertObj)
      // Notifying and emailing cobrancas sector
      handleSendEmailToCobrancas(requestInfo)
      handleSendNotificationToCobrancas(requestInfo)
  ...
  const {
    mutate: handleAddNewProject,
    isLoading: addProjectLoading,
    isError: addProjectError,
    isSuccess: addProjectSuccess,
  } = useMutationWithFeedback({
    mutationKey: ['add-new-project'],
    mutationFn: addProject,
    queryClient: queryClient,
    affectedQueryKey: ['contract-requests'],
    callbackFn: async () => await queryClient.invalidateQueries({ queryKey: ['contract-request-by-id', solicitacaoId] }),
  })
  const {
    mutate: handleRequestUpdate,
    isLoading: updateLoading,
    isError: updateError,
    isSuccess: updateSuccess,
  } = useMutationWithFeedback({
    mutationKey: ['update-contract-request', solicitacaoId],
          ...
          <div className="flex h-full flex-col">
            <div className="flex flex-col items-center justify-between gap-2 border-b border-gray-200 px-2 pb-2 lg:flex-row">
              <div className="flex items-center justify-center gap-2">
                <h1 className="pl-6 font-bold  text-[#15599a]">SOLICITAÇÃO DE CONTRATO</h1>
                <p className="text-xxs italic text-gray-600">#{solicitacaoId}</p>
              </div>
              <div className="flex w-full items-center justify-between gap-2 lg:w-fit">
                {dados?.aprovacao && !dados?.confeccionado ? (
                  <button
                    disabled={updateLoading}
                    onClick={() => handleRequestUpdate({ id: solicitacaoId, changes: { confeccionado: true } })}
                    className="group flex items-center gap-2 rounded border border-red-500 p-2 text-sm font-bold text-red-500 disabled:bg-gray-500 disabled:text-white hover:bg-green-500 enabled:hover:border-green-500 enabled:hover:text-white"
                  >
                ...
                <div className="flex grow items-center justify-center gap-2 lg:grow-0 lg:justify-end">
                  ...
                  {editor && (
                    <SaveButton
                      ...
                      icon={<FaSave />}
                      handleClick={() => handleRequestUpdate({ id: solicitacaoId, changes: dados })}
                    />
                  )}
                  <button>
            ...
            {isSuccess && dados ? (
              <div className="flex grow flex-col gap-y-2 overflow-y-auto overscroll-y-auto px-2 py-1 scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300">
                <>
                  ...
                  <FilesBlock
                    files={dados.links}
                    tag={dados.nomeDoContrato}
                    vinculateFiles={(newLinks) => handleRequestUpdate({ id: solicitacaoId, changes: { links: newLinks } })}
                    ...
                    <div className="flex w-full items-center justify-around">
                      <div className="flex w-full justify-center">
                        <button
                          ...
                          onClick={() => handleAddNewProject(dados)}
                          className="rounded bg-green-500 p-2 font-bold text-white duration-300 ease-in-out disabled:bg-gray-500 enabled:hover:bg-green-600"
                        >
                          ADICIONAR PROJETO
                        </button>
                      </div>
                      <div className="flex w-full justify-center">
                        <button
                          disabled={updateLoading}
                          onClick={() => handleRequestUpdate({ id: solicitacaoId, changes: { aprovacao: false } })}
                          className="rounded bg-red-500 p-2 font-bold text-white duration-300 ease-in-out disabled:bg-gray-500 enabled:hover:bg-red-600"
                        >
