			...
			<AnimatedModalWrapper width={"90%"} height={"80%"} modalIsOpen={isOpen}>
				<div className="h-full flex flex-col">
					...
					<div className="flex flex-col grow overflow-y-auto overscroll-y-auto">
						<h1 className="w-full p-1 text-center bg-[#15599a] text-white font-medium">INFORMAÇÕES GERAIS DA OPERAÇÃO</h1>
						...
						<h1 className="w-full p-1 text-center bg-[#15599a] text-white font-medium">ATIVIDADES</h1>
						<div className="w-full h-[200px] min-h-[200px] max-h-[200px] flex flex-col overflow-y-auto overscroll-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
							{operationInfo.atividades.length > 0 ? (
								operationInfo.atividades.map((activity, index, arr) => (
									<div key={index} className={`w-full items-start flex flex-col py-2 ${arr.length > 1 ? "border-b border-gray-300" : ""}`}>
										<div className="w-full items-start flex justify-around">
											<p className="w-2/3 lg:w-1/4 text-gray-700 font-medium text-center">{activity.nome}</p>
											...
											<div className="w-1/3 lg:w-1/4 flex items-center justify-center gap-4">
												...
												<button onClick={() => removeActivity(index)} className="text-red-500 text-sm flex items-center justify-center hover:scale-110 duration-300 ease-in-out">
										...
										{activity.subAtividades
											? activity.subAtividades.map((subActivity, subIndex) => (
													<div key={subIndex} className="w-full items-start flex  justify-around">
														<p className="w-2/3 lg:w-1/4 text-xs text-gray-500 text-center">{subActivity.nome}</p>
														...
														<div onClick={() => removeSubActivity(index, subIndex)} className="w-1/3 lg:w-1/4 flex items-center justify-center gap-4">
								...
								<div className="grow w-full flex items-center justify-center py-2">
									<p className="text-gray-500 italic font-medium">Sem atividades vinculadas...</p>
								</div>
							)}
						</div>
						{activityMsg.text ? <p className={`w-full text-center text-sm italic ${activityMsg.color}`}>{activityMsg.text}</p> : <p className="h-[21px] w-full"></p>}
						<div className="w-full flex flex-col lg:flex-row pb-2 pt-4">
							...
							<div className="w-full lg:w-[10%] flex items-start justify-center">
								<button onClick={addActivity} className="bg-green-300 hover:bg-green-500 hover:text-white hover:scale-105 duration-300 ease-in-out font-medium text-sm p-3 rounded">
									<IoMdAdd />
								</button>
							</div>
						</div>
						<h1 className="w-full p-1 text-center bg-[#15599a] text-white font-medium">SUBTAREFAS</h1>
						<div className="flex items-center py-1">
							<div className="flex flex-col">
								<h1 className="text-gray-500 italic text-xs">ADICIONAR SUBTAREFA À:</h1>
								<h1 className="text-gray-700 text-sm font-medium">{subactivityHolder.activityIndex != null ? operationInfo.atividades[subactivityHolder.activityIndex].nome : null}</h1>
							</div>
						</div>
						{subActivityMsg.text ? <p className={`w-full text-center text-sm italic ${subActivityMsg.color}`}>{subActivityMsg.text}</p> : <p className="h-[21px] w-full"></p>}
					...
					<div className="w-full py-2 border-t border-gray-300 flex items-center justify-end justify-self-end">
						{operationMsg.text ? (
							...
							<button onClick={createOperation} className="p-2 rounded bg-green-300 hover:bg-green-500 hover:text-white font-medium">
								CRIAR OPERAÇÃO
							</button>
						)}
					</div>
