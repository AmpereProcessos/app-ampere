    ...
    <>
      <AnimatedModalWrapper width={"90%"} height={"80%"} modalIsOpen={isOpen}>
        <div className="flex h-full flex-col">
          ...
          <div className="flex grow flex-col overflow-y-auto overscroll-y-auto">
            <h1 className="w-full bg-[#15599a] p-1 text-center font-medium text-white">
            ...
            <div className="scrollbar-thin scrollbar-thumb-primary/20 scrollbar-track-primary/20 flex h-[200px] max-h-[200px] min-h-[200px] w-full flex-col overflow-y-auto overscroll-y-auto">
              {operationInfo.atividades.length > 0 ? (
                operationInfo.atividades.map((activity, index, arr) => (
                  <div
                    key={index}
                    className={`flex w-full flex-col items-start py-2 ${arr.length > 1 ? "border-border border-b" : ""}`}
                  >
                    <div className="flex w-full items-start justify-around">
                      <p className="text-foreground w-2/3 text-center font-medium lg:w-1/4">
                        {activity.nome}
                      </p>
                      <div className="text-foreground hidden w-1/4 items-center gap-2 text-center font-medium lg:flex">
                        <p className="hidden lg:block">INICIO EM:</p>
                        <p>
                          {activity.dataInicio
                            ? dayjs(activity.dataInicio).add(4, "hours").format("DD/MM/YYYY")
                            : "-"}
                        </p>
                      </div>
                      <div className="text-foreground hidden w-1/4 items-center gap-2 text-center font-medium lg:flex">
                        ...
                        <p>
                          {activity.previsaoConclusao
                            ? dayjs(activity.previsaoConclusao).add(4, "hours").format("DD/MM/YYYY")
                            : "-"}
                        </p>
                      </div>
                    ...
                    {activity.subAtividades
                      ? activity.subAtividades.map((subActivity, subIndex) => (
                          <div key={subIndex} className="flex w-full items-start justify-around">
                            <p className="text-foreground w-2/3 text-center text-xs lg:w-1/4">
                              {subActivity.nome}
                            </p>

                            <p className="text-foreground hidden w-1/4 text-center text-xs lg:block">
                              {subActivity.dataInicio
                                ? `INICIO EM: ${dayjs(subActivity.dataInicio).add(4, "hours").format("DD/MM/YYYY")}`
                                : "-"}
                            </p>
                            <p className="text-foreground hidden w-1/4 text-center text-xs lg:block">
                              {subActivity.previsaoConclusao
                                ? `FIM EM: ${dayjs(subActivity.previsaoConclusao).add(4, "hours").format("DD/MM/YYYY")}`
                                : "-"}
                            </p>
                            <div
                              onClick={() => removeSubActivity(index, subIndex)}
                              className="flex w-1/3 items-center justify-center gap-4 lg:w-1/4"
                            >
                ...
                <div className="flex w-full grow items-center justify-center py-2">
                  <p className="text-foreground font-medium italic">Sem atividades vinculadas...</p>
                </div>
              )}
            </div>
            {activityMsg.text ? (
              <p className={`w-full text-center text-sm italic ${activityMsg.color}`}>
                {activityMsg.text}
              </p>
            ) : (
            ...
            <div className="flex items-center py-1">
              <div className="flex flex-col">
                <h1 className="text-foreground text-xs italic">ADICIONAR SUBTAREFA À:</h1>
                <h1 className="text-foreground text-sm font-medium">
                  {subactivityHolder.activityIndex != null
                    ? operationInfo.atividades[subactivityHolder.activityIndex].nome
                    : null}
                </h1>
              </div>
            </div>
            {subActivityMsg.text ? (
              <p className={`w-full text-center text-sm italic ${subActivityMsg.color}`}>
                {subActivityMsg.text}
              </p>
            ) : (
              <p className="h-[21px] w-full"></p>
          ...
          <div className="border-border flex w-full items-center justify-end justify-self-end border-t py-2">
            {operationMsg.text ? (
