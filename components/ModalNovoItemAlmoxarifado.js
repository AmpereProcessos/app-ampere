			...
			<div style={OVERLAY_STYLES}>
				<div className="h-[90%] w-[90%] lg:h-[80%] lg:w-[50%]" style={MODAL_STYLES}>
					<div className="flex h-full flex-col">
						<div className="flex min-h-[30px] justify-between border-b border-gray-300 px-2 pb-2 text-lg">
						...
						<div className="grow overflow-y-auto overscroll-y-auto py-2 scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300">
							<div className="flex h-full w-full flex-col px-4">
								...
								<NumberFloatingInput label={"QUANTIDADE"} editable={true} value={quantidade} handleChange={(value) => setQuantidade(Number(value))} width={"100%"} />
								<NumberFloatingInput label={"QUANTIDADE MÍNIMA"} editable={true} value={quantidadeMinima} handleChange={(value) => setQuantidadeMinima(Number(value))} width={"100%"} />
								<NumberFloatingInput label={"QUANTIDADE MÁXIMA"} editable={true} value={quantidadeMaxima} handleChange={(value) => setQuantidadeMaxima(Number(value))} width={"100%"} />
								<SelectFoatingInput label={"GRANDEZA"} editable={true} value={grandeza} options={units} handleChange={(value) => setGrandeza(value)} width={"100%"} />
								...
								<TextFloatingInput label={"LOCALIZAÇÃO"} editable={true} value={localizacao} handleChange={(value) => setLocalizacao(value)} width={"100%"} />
								<div className="flex w-full flex-col gap-1">
									...
									<textarea
										...
										className="h-[80px] w-full resize-none border border-gray-300 bg-gray-100 p-1 text-center text-sm outline-blue-200"
									/>
								</div>
								{msg.text && <p className={`text-center italic ${msg.color}`}>{msg.text}</p>}
								<div className="mt-4 flex justify-center">
									<button onClick={addItem} className="flex items-center gap-x-2 rounded bg-[#15599a] p-2 text-sm font-bold text-white hover:bg-blue-500">
										<p>ADICIONAR</p>
										<FaSave />
									</button>
								</div>
