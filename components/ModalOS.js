		...
		<>
			<AnimatedModalWrapper modalIsOpen={modalIsOpen}>
				<div className="flex h-full w-full flex-col overflow-y-auto overscroll-y-auto">
					<div className="border-primary/20 flex flex-row items-center justify-between border-b px-2 pb-2 text-lg">
						<div className="flex flex-col">
							<h1 className="p-0 text-center font-bold text-[#15599a] lg:pl-6">FINALIZAÇÃO DE ORDEM DE SERVIÇO</h1>
							<h1 className="text-xxs text-primary/60 p-0 text-start font-bold lg:pl-6">#{order?._id || "..."}</h1>
						</div>
					...
					{isError ? (
						<div className="flex grow flex-col items-center justify-center">
							...
							<p className="text-primary/60 text-center italic">Erro ao buscar informações da Ordem de Serviço...</p>
						</div>
					) : null}
					{isSuccess ? (
						<div className="overscroll-y border-primary/20 scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex w-full flex-col overflow-y-auto border-r p-3">
							...
							<div className="mt-2 flex w-full flex-col items-center justify-center gap-2 lg:flex-row">
								<div className="flex items-center gap-2">
									<MdEngineering />
									<p className="text-primary/60 text-xs font-medium uppercase lg:text-lg">{order?.responsavel?.nome || "RESPONSÁVEL NÃO DEFINIDO"}</p>
								</div>
								<div className="flex items-center gap-2">
									<IoMdAlert />
									<p className="text-primary/60 text-xs font-medium uppercase lg:text-lg">{order?.urgencia}</p>
								</div>
							</div>

							<div className={`mt-2 flex w-full items-center justify-center gap-2 ${order.dataEfetivacao ? "text-green-500" : "text-primary/60"}`}>
							...
							<h1 className="bg-primary/80 mt-4 w-full rounded-md p-2 text-center font-bold text-white">FAVORECIDO</h1>
							...
							<div className="mt-2 flex w-full flex-col items-center justify-center gap-2 md:flex-row lg:gap-4">
								<div className="flex items-center gap-2">
									...
									<p className="font-raleway text-sm font-medium">{order?.localizacao ? `${order?.localizacao.cidade} - ${order?.localizacao.uf} ` : "N/A"}</p>
								</div>
								<div className="flex items-center gap-2">
							...
							<div className="mt-2 flex w-full flex-col gap-2 px-2">
								{order.observacoes.length > 0 ? (
									order.observacoes.map((obs, index) => (
										<div key={index} className="border-primary/60 flex w-full flex-col rounded-md border">
											...
											<h1 className="text-primary/60 w-full p-2 text-center text-xs font-medium tracking-tight">{obs.descricao}</h1>
										</div>
									))
								) : (
							...
							<div className="mt-2 flex w-full flex-row flex-wrap items-center justify-center gap-2 lg:gap-4">
								<div className="border-primary/60 flex flex-col rounded-md border p-3">
									...
									<h1 className="text-primary/60 text-center text-xs font-medium uppercase">{order.detalhes.pontoAgua || "N/A"}</h1>
								</div>
								<div className="border-primary/60 flex flex-col rounded-md border p-3">
									<div className="flex items-center gap-2">
									...
									<h1 className="text-primary/60 text-center text-xs font-medium uppercase">{order.detalhes.senhaWifi || "N/A"}</h1>
								</div>
								<div className="border-primary/60 flex flex-col rounded-md border p-3">
									<div className="flex items-center gap-2">
										<BsFillGearFill />
									...
									<h1 className="text-primary/60 text-center text-xs font-medium uppercase">{order.detalhes.configuracaoMonitoramento ? "SIM" : "N/A"}</h1>
								</div>
								<div className="border-primary/60 flex flex-col rounded-md border p-3">
									<div className="flex items-center gap-2">
										<IoMdResize />
									...
									<h1 className="text-primary/60 text-center text-xs font-medium uppercase">{order.detalhes.possuiTrafo ? "SIM" : "N/A"}</h1>
								</div>
								<div className="border-primary/60 flex flex-col rounded-md border p-3">
									<div className="flex items-center gap-2">
										<BsHouse />
									...
									<h1 className="text-primary/60 text-center text-xs font-medium uppercase">{order.detalhes.tipoEstrutura || "N/A"}</h1>
								</div>
								<div className="border-primary/60 flex flex-col rounded-md border p-3">
									<div className="flex items-center gap-2">
									...
									<h1 className="text-primary/60 text-center text-xs font-medium uppercase">{order.detalhes.topologia || "N/A"}</h1>
								</div>
							</div>
