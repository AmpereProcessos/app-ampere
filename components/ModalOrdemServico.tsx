      ...
      <div className="flex h-full flex-col">
        <div className="border-border flex items-center justify-between border-b px-2 pb-2 text-lg">
          <div className="flex flex-col items-center gap-2 lg:flex-row">
            <h1 className="pl-6 text-xs font-bold text-[#15599a] lg:text-base">
              {order?.favorecido?.nome || "..."}
            </h1>
            <p className="text-foreground text-center text-xs">#{order?._id || "..."}</p>
          </div>
          <button type="button">
            <VscChromeClose
              onClick={() => {
          ...
          <>
            <div className="overscroll-y scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex flex-col overflow-y-auto px-2 py-2 lg:px-0">
              <div className="flex w-full items-center justify-center">
                <h1 className="rounded border border-[#15599a] p-1 text-sm font-black text-[#15599a]">
                  {order.categoria}
                </h1>
              </div>
              ...
              <div className="mt-2 flex items-center justify-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-foreground text-xs font-medium">
                    {order?.autor?.nome || "Autor não identificado"}
                  </p>
                </div>
                <div className="text-foreground flex items-center gap-2">
                  <BsCalendarFill />
                  <p className="text-xs font-medium">
                    {formatDateAsLocale(order?.dataInsercao, true)}
                  </p>
                </div>
              </div>
              <div className="mt-2 flex w-full items-center justify-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <IoMdAlert />
                  <p className="text-foreground text-xs font-medium uppercase">{order?.urgencia}</p>
                </div>
              </div>
                ...
                <div className="w-full lg:w-[25%]">
                  <Select
                    ...
                    handleChange={(value) =>
                      setInfoHolder((prev) => ({
                        ...prev,
                        responsavel: { ...prev.responsavel, tipo: value },
                      }))
                    }
                    onReset={() =>
                      setInfoHolder((prev) => ({
                        ...prev,
                        responsavel: { ...prev.responsavel, tipo: "EXTERNO" },
                      }))
                    }
                    width={"100%"}
                  />
                  ...
                  {infoHolder.responsavel.tipo === "EXTERNO" ? (
                    <TextInput
                      ...
                      handleChange={(value) =>
                        setInfoHolder((prev) => ({
                          ...prev,
                          responsavel: { ...prev.responsavel, nome: value },
                        }))
                      }
                      width={"100%"}
                    />
              ...
              <div className="mt-1 flex w-full items-center justify-center">
                <div className="w-full lg:w-1/2">
                  <Select
                    ...
                    selectedItemLabel={"NÃO DEFINIDO"}
                    handleChange={(value) =>
                      setInfoHolder((prev) => ({ ...prev, urgencia: value }))
                    }
            ...
            <div className="border-border mt-2 flex w-full items-center justify-between border-t px-4 py-1">
