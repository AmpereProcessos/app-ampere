...
function ModalProjetos({ projectId, session, modalIsOpen, closeModal }: ModalProjetosProps) {
	...
	const { mutate: updateProjectMutation, isPending } = useMutation({
		...
		mutationFn: async ({
			...
			await handleUpdateClientPersonalized({ id: previousData.idClienteCRM || "", changes: clientChanges });
				...
				{isSuccess && infoHolder ? (
					<>
						<div className="scrollbar-thin scrollbar-track-primary/10 scrollbar-thumb-primary/30 flex-1 overflow-auto">
							<NotificationCreationBlock session={session} nomeDoProjeto={project.nomeDoContrato} codProjeto={project.qtde.toString()} />
						...
						<DialogFooter>
							...
							<LoadingButton
								onClick={() => updateProjectMutation({ previousData: project, newData: infoHolder, changes: changes, queryClient: queryClient })}
								loading={isPending}
							>
								{BUTTON_TEXT}
							</LoadingButton>
