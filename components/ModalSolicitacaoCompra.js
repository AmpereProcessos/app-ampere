  ...
  return (
    <AnimatedModalWrapper modalIsOpen={isOpen} width={"70%"} height={"90%"}>
      <div className="flex h-full flex-col">
        <div className="border-border flex w-full items-center justify-between border-b pb-2">
          <div className="flex items-center gap-2">
            <h1 className="text-sm font-bold text-[#15599a] lg:text-lg">SOLICITAÇÃO DE COMPRA</h1>
            <p className="text-foreground hidden text-xs lg:block">#{info._id}</p>
          </div>
        ...
        <div className="scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex w-full grow flex-col gap-2 overflow-y-auto overscroll-y-auto px-2 py-2">
          <div className="flex w-full flex-wrap items-center justify-center gap-4 pb-4">
            <div className="flex flex-col items-center">
              <h1 className="text-foreground text-start text-xs font-medium">
                SOLICITAÇÃO FEITA EM:
              </h1>
              <div className="flex items-center gap-2">
                <AiOutlineCalendar style={{ color: "#15599a" }} />
                <h1 className="text-foreground text-xs font-medium lg:text-base">
                  {info.dataInsercao ? dayjs(info.dataInsercao).format("DD/MM/YY HH:mm") : null}
                </h1>
              </div>
            </div>
            <div className="flex flex-col items-center">
              <h1 className="text-foreground text-end text-xs font-medium">PRAZO:</h1>
            ...
            {infoHolder.dataEfetivacao ? (
              <div className="flex flex-col items-center">
                <h1 className="text-foreground text-end text-xs font-medium">EFETIVAÇÃO</h1>
                <div className="flex items-center justify-end gap-2">
                  <BsCalendarCheckFill
                    style={{
                      color: "rgb(34,197,94)",
                  ...
                  <h1 className="text-foreground text-xs font-medium lg:text-base">
                    {infoHolder.dataEfetivacao
                      ? dayjs(infoHolder.dataEfetivacao).format("DD/MM/YY HH:mm")
                      : "NÃO DEFINIDO"}
                  </h1>
                </div>
              </div>
          ...
          <h1 className="font-raleway text-foreground w-full text-center text-sm font-normal">
            CRITÉRIO DE CUSTO
          </h1>
          ...
          <div className="flex w-full flex-col gap-1">
            ...
            <textarea
              ...
              className="border-border bg-primary/20 h-[90px] resize-none border p-1 text-center text-sm outline-hidden"
            />
          </div>
          <div className="flex grow flex-col">
            ...
            <div className="bg-primary/60 grid w-full grid-cols-10 items-center rounded-tl-md rounded-tr-md">
              <h1 className="col-span-2 p-2 text-center text-xs font-medium text-white lg:text-base">
                NOME
              </h1>
              <h1 className="col-span-1 p-2 text-center text-xs font-medium text-white lg:text-base">
                QTDE
              </h1>
              <h1 className="col-span-2 p-2 text-center text-xs font-medium text-white lg:text-base">
                COTAÇÃO
            ...
            <h1 className="border-border w-full rounded-br rounded-bl border-x border-b px-2 py-2 text-center text-lg font-black">
          ...
          {infoHolder.projeto ? (
            <>
              <div className="my-2 flex w-full flex-col">
                ...
                <p className="font-raleway text-foreground w-full text-center text-lg font-black">
                ...
                <div className="flex items-center justify-center gap-2">
                  ...
                  <p className="text-foreground text-xs italic">#{infoHolder.projeto.id}</p>
                </div>
              </div>
            </>
          ) : null}
          <div className="border-border flex items-center justify-between border-t px-2 pt-2">
            {!infoHolder.dataEfetivacao ? (
              <div className="flex items-center gap-2">
                <button
                  onClick={() => finishPurchaseSolicitation()}
                  className="rounded bg-green-300 p-2 text-sm font-bold text-white hover:bg-green-500"
                >
                  EFETIVAR
                </button>
              </div>
