		...
		<div style={OVERLAY_STYLES}>
			<div style={MODAL_STYLES}>
				<div className="flex h-full flex-col">
					<div className="border-primary/20 grid grid-cols-3 items-center border-b px-2 pb-2 text-lg">
						<h1 className="p-0 text-xs font-bold text-[#15599a] lg:pl-6 lg:text-base">{dados.nomeDoCliente}</h1>
						{dados.status != "CONCLUIDO" ? (
							<div className="flex items-center justify-evenly gap-2">
								<select
									...
									className="p-2 text-xs font-bold text-[#15599a] outline-hidden lg:text-base"
								>
							...
							<div className="flex items-center justify-center">
								<p className="w-fit self-center rounded bg-green-400 p-1 text-xs font-bold text-white lg:text-base">CONCLUIDO</p>
							</div>
						)}
						<div className="flex items-center justify-end gap-x-2">
							{msg.text && <p className={`italic ${msg.color}`}>{msg.text}</p>}
							{dados.status == "REJEITADA" ? <SaveButton text={"Salvar alterações"} icon={<FaSave />} handleClick={saveChanges} /> : null}
					...
					<div className="flex h-full flex-col gap-y-2 overflow-y-auto overscroll-y-auto">
						<div className="bg-background flex w-full flex-col border border-[#15599a] p-4 shadow-lg">
							<span className="py-2 text-center text-sm font-bold text-[#15599a] uppercase">INFORMAÇÕES INICIAIS</span>
							...
							<span className="py-2 text-center text-sm font-bold text-[#15599a] uppercase">EQUIPAMENTO</span>
							<div className="mt-2 flex flex-wrap justify-around gap-2">
								<SelectInput
								...
								<NumberInput
									...
									handleChange={(value) => setDados({ ...dados, qtdeInversor: Number(value) })}
								/>
								<NumberInput
									...
									handleChange={(value) => setDados({ ...dados, potInversor: Number(value) })}
								/>
								<TextInput
									label={"MARCA DO INVERSOR"}
									editable={dados.status == "REJEITADA" ? true : false}
									value={dados.marcaInversor}
									handleChange={(value) => setDados({ ...dados, marcaInversor: value.toUpperCase() })}
								/>
								<NumberInput
									label={"QTDE DE MODULOS"}
									editable={dados.status == "REJEITADA" ? true : false}
									value={dados.qtdeModulos}
									handleChange={(value) => setDados({ ...dados, qtdeModulos: Number(value) })}
								/>
								<NumberInput
									label={"POTÊNCIA DOS MÓDULOS"}
									editable={dados.status == "REJEITADA" ? true : false}
									value={dados.potModulos}
									handleChange={(value) => setDados({ ...dados, potModulos: Number(value) })}
								/>
								<TextInput
									label={"MARCA DOS MÓDULOS"}
									editable={dados.status == "REJEITADA" ? true : false}
									value={dados.marcaModulos}
									handleChange={(value) => setDados({ ...dados, marcaModulos: value.toUpperCase() })}
								/>
									...
									<div className="mt-2 flex flex-col items-center">
										...
										<div className="mt-2 flex flex-wrap items-center justify-around gap-2">
											...
											<TextInput
												...
												handleChange={(value) => setDados({ ...dados, numeroTrafo: value })}
											/>
											<TextInput
												label={"NÚMERO POSTE DO TRANSFORMADOR"}
												editable={dados.status == "REJEITADA" ? true : false}
												value={dados.numeroPosteTrafo}
												handleChange={(value) => setDados({ ...dados, numeroPosteTrafo: value })}
											/>
											<SelectInput
												label={"PENDÊNCIAS"}
												editable={dados.status == "REJEITADA" ? true : false}
												value={dados.pendenciasTrafo ? dados.pendenciasTrafo : "NÃO DEFINIDO"}
												options={[
													{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
													{ label: "SIM", value: "SIM" },
													{ label: "NÃO", value: "NÃO" },
												]}
												handleChange={(value) => setDados({ ...dados, pendenciasTrafo: value })}
											/>
										</div>
									</div>
									<div className="mt-2 flex flex-col items-center">
						...
						<div className="bg-background flex w-full flex-col border border-[#15599a] p-4 shadow-lg">
							<span className="text-md py-2 text-center font-bold text-[#15599a] uppercase">ARQUIVOS</span>
							<div className="flex flex-wrap items-center justify-around gap-2">
								{dados.links.map((link) => (
									<a key={link.link} href={link.link} className="cursor-pointer font-bold text-blue-400">
						...
						<div className="bg-background flex w-full flex-col border border-[#15599a] p-4 shadow-lg">
							<div className="flex flex-col">
								<span className="py-2 text-center text-sm font-bold text-[#15599a] uppercase">CAMPOS ADICIONAIS P/OBRAS</span>
								<div className="flex w-full flex-wrap items-center justify-center gap-2">
									...
									<SelectInput
										...
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.adaptacaoQGBT ? dados.adaptacaoQGBT : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
									...
									<SelectInput
										...
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.avaliarTelhado ? dados.avaliarTelhado : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
											{ label: "NÃO", value: "NÃO" },
											{ label: "SIM", value: "SIM" },
									...
									<TextInput
										...
										handleChange={(value) => setDados({ ...dados, distanciaSistemaInversor: value })}
									/>
									<SelectInput
										label={"REALIMENTAR A FAZENDA ?"}
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.realimentar ? dados.realimentar : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
											{ label: "NÃO", value: "NÃO" },
											{ label: "SIM", value: "SIM" },
									...
									<SelectInput
										...
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.temEstudoDeCaso ? dados.temEstudoDeCaso : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
											{ label: "NÃO", value: "NÃO" },
											{ label: "SIM", value: "SIM" },
								...
								<div className="mt-2 flex w-full flex-col items-center self-center">
									<span className="font-raleway text-center text-sm font-bold uppercase">OBSERVAÇÕES P/OBRAS</span>
									<textarea
										...
										className="border-primary/80 h-[100px] w-full resize-none border bg-gray-200 p-2 text-center outline-hidden"
									/>
								</div>
							</div>
							<div className="flex flex-col">
								<span className="py-2 text-center text-sm font-bold text-[#15599a] uppercase">CAMPOS ADICIONAIS P/SUPRIMENTOS</span>
                  ...
                  />
                  <p className="text-primary/80 text-xs text-center">
                    {suprimentoHolder.medida}
                  </p>
                </div> */}
								<div className="mx-12 mt-2 flex flex-col gap-2">
									<div className="grid w-full grid-cols-4">
										<p className="text-md text-center font-bold text-[#fead61]">INSUMO</p>
										<p className="text-md text-center font-bold text-[#fead61]">TIPO</p>
										<p className="text-md text-center font-bold text-[#fead61]">QUANTIDADE</p>
										<p className="text-md text-center font-bold text-[#fead61]">UNIDADE</p>
									</div>
									{dados.suprimentos?.map((suprimento, index) => (
										<div key={index} className="grid w-full grid-cols-4">
											<p className="text-primary/80 text-center text-xs font-bold">{suprimento.insumo}</p>
											<p className="text-primary/80 text-center text-xs font-bold">{suprimento.tipo}</p>
											<p className="text-primary/80 text-center text-xs font-bold">{suprimento.qtde}</p>
											<p className="text-primary/80 text-center text-xs font-bold">{suprimento.medida}</p>
										</div>
									))}
								</div>
								<div className="mt-2 flex w-full flex-col items-center self-center">
									<span className="font-raleway text-center text-sm font-bold uppercase">OBSERVAÇÕES P/SUPRIMENTOS</span>
									<textarea
							...
							<div className="flex flex-col">
								<span className="py-2 text-center text-sm font-bold text-[#15599a] uppercase">CAMPOS ADICIONAIS P/PROJETOS</span>
								<div className="flex flex-wrap items-center justify-around">
									<SelectInput
										label={"CONCESSIONÁRIA"}
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.concessionaria ? dados.concessionaria : "NÃO DEFINIDO"}
										options={distributionCompanies}
										handleChange={(value) => setDados({ ...dados, concessionaria: value })}
									/>
									<SelectInput
										label={"TIPO DE PROJETO"}
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.tipoProjeto ? dados.tipoProjeto : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
										...
										handleChange={(value) => setDados({ ...dados, tipoProjeto: value })}
									/>
									<SelectInput
										label={"MODELO DA CAIXA"}
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.modeloCaixa ? dados.modeloCaixa : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
										...
										handleChange={(value) => setDados({ ...dados, modeloCaixa: value })}
									/>
									<SelectInput
										label={"PENDÊNCIAS"}
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.pendenciasProjetos ? dados.pendenciasProjetos : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
											{
										...
										handleChange={(value) => setDados({ ...dados, pendenciasProjetos: value })}
									/>
									<TextInput
										label={"NÚMERO POSTE DE DERIVAÇÃO(GOIÁS)"}
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.numeroPosteDerivacao}
										handleChange={(value) => setDados({ ...dados, numeroPosteDerivacao: value })}
									/>
									<NumberInput
										label={"POTÊNCIA DO FUSÍVEL"}
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.potFusivel}
										handleChange={(value) => setDados({ ...dados, potFusivel: value })}
									/>
									<SelectInput
										label={"SE AUMENTO, NOVO TIPO DE LIGAÇÃO DO PADRÃO"}
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.novaLigacaoPadrao ? dados.novaLigacaoPadrao : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
											{ label: "MONOFÁSICO", value: "MONOFÁSICO" },
											{ label: "BIFÁSICO", value: "BIFÁSICO" },
										...
										handleChange={(value) => setDados({ ...dados, novaLigacaoPadrao: value })}
									/>
									<SelectInput
										label={"SE AUMENTO, NOVA AMPERAGEM"}
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.novaAmperagem ? dados.novaAmperagem : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
											{ label: "40A", value: "40A" },
											{ label: "63A", value: "63A" },
										...
										handleChange={(value) => setDados({ ...dados, novaAmperagem: value })}
									/>
									<SelectInput
										label="FOTOS DO DRONE"
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.fotoDroneDesenho ? dados.fotoDroneDesenho : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
											{ label: "NÃO", value: "NÃO" },
											{ label: "SIM", value: "SIM" },
										]}
										handleChange={(value) => setDados({ ...dados, fotoDroneDesenho: value })}
									/>
									<SelectInput
										label="FOTO CLARA DA FACHADA"
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.fotoFaixada ? dados.fotoFaixada : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
											{ label: "NÃO", value: "NÃO" },
											{ label: "SIM", value: "SIM" },
										]}
										handleChange={(value) => setDados({ ...dados, fotoFaixada: value })}
									/>
									<SelectInput
										label="MEDIDAS DO LOCAL"
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.medidasLocal ? dados.medidasLocal : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
											{ label: "NÃO", value: "NÃO" },
											{ label: "SIM", value: "SIM" },
										]}
										handleChange={(value) => setDados({ ...dados, medidasLocal: value })}
									/>
									<SelectInput
										label="GOOGLE EARTH"
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.googleEarth ? dados.googleEarth : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
											{ label: "NÃO", value: "NÃO" },
											{ label: "SIM", value: "SIM" },
										]}
										handleChange={(value) => setDados({ ...dados, googleEarth: value })}
									/>
								</div>
								<div className="mt-2 flex w-full flex-col items-center self-center">
									<span className="font-raleway text-center text-sm font-bold uppercase">OBSERVAÇÕES P/PROJETOS</span>
									<textarea
								...
								<div className="mt-2 flex w-full flex-col items-center self-center">
									<span className="font-raleway text-center text-sm font-bold uppercase">DESCRITIVO</span>
									...
									<div className="border-primary/20 my-2 flex h-[100px] w-full flex-col items-center overflow-y-auto overscroll-y-auto border p-2">
										{dados.descritivo?.length ? (
											dados.descritivo?.map((item, index) => (
												<div key={index} className="grid w-full grid-cols-10 items-center gap-3">
													<p className="text-xxs col-span-1 font-bold text-[#15599a] lg:text-xs">{item.topico}</p>
													<p className={`${item.texto.length > 100 ? "text-xxs" : "text-xs"} text-primary/80 col-span-8 text-center font-bold`}>{item.texto}</p>
													<div
														onClick={() => {
													...
													>
														<div className="col-span-1 w-fit rounded bg-red-500 p-1">
											...
											<div className="text-primary/80 flex h-full items-center justify-center text-center italic">SEM DESCRITIVO</div>
										)}
									</div>
								</div>
							</div>
							<div className="flex flex-col">
								<span className="py-2 text-center text-sm font-bold text-[#15599a] uppercase">RESPOSTA VISITA TÉCNICA</span>
								<div className="flex flex-wrap justify-around gap-2">
									<SelectInput
										label={"PADRÃO"}
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.respostaPadrao ? dados.respostaPadrao : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
										...
										handleChange={(value) => setDados({ ...dados, respostaPadrao: value })}
									/>
									<SelectInput
										label={"ESPAÇO PARA PROJETO"}
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.respostaEspacoProjeto ? dados.respostaEspacoProjeto : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
											{ label: "SIM", value: "SIM" },
											{ label: "NÃO", value: "NÃO" },
										]}
										handleChange={(value) => setDados({ ...dados, respostaEspacoProjeto: value })}
									/>
									<SelectInput
										label={"ESTRUTURA DE INCLINAÇÃO"}
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.respostaEstruturaInclinacao ? dados.respostaEstruturaInclinacao : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
											{ label: "SIM", value: "SIM" },
											{ label: "NÃO", value: "NÃO" },
										]}
										handleChange={(value) => setDados({ ...dados, respostaEstruturaInclinacao: value })}
									/>
									<SelectInput
										label={"POSSUI SOMBRA ?"}
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.respostaPossuiSombra ? dados.respostaPossuiSombra : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
											{ label: "SIM", value: "SIM" },
											{ label: "NÃO", value: "NÃO" },
										]}
										handleChange={(value) => setDados({ ...dados, respostaPossuiSombra: value })}
									/>
									<SelectInput
										label={"MADERAMENTO"}
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.respostaMaderamento ? dados.respostaMaderamento : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
											{ label: "APTO", value: "APTO" },
										...
										handleChange={(value) => setDados({ ...dados, respostaMaderamento: value })}
									/>
									<SelectInput
										label={"NECESSÁRIO EXPLICAÇÃO DETALHADA"}
										editable={dados.status == "REJEITADA" ? true : false}
										value={dados.respostaExplicacaoDetalhada ? dados.respostaExplicacaoDetalhada : "NÃO DEFINIDO"}
										options={[
											{ label: "NÃO DEFINIDO", value: "NÃO DEFINIDO" },
											{ label: "SIM", value: "SIM" },
											{ label: "NÃO", value: "NÃO" },
										]}
										handleChange={(value) => setDados({ ...dados, respostaExplicacaoDetalhada: value })}
									/>
								</div>
								<div className="mt-2 flex w-full flex-col items-center self-center">
									<span className="font-raleway text-center text-sm font-bold uppercase">CONCLUSÃO</span>
									<textarea
							...
							<div className="flex flex-col">
								<span className="py-2 text-center text-sm font-bold text-[#15599a] uppercase">CUSTOS ADICIONAIS</span>
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-primary/80 text-xs text-center">
                      ...
                      {(custoAdicionalHolder.qtde * custoAdicionalHolder.valor)
                        ...
                        .replace(".", ",")}
								...
								<div className="mx-12 mt-2 flex flex-col gap-2">
									<div className="grid w-full grid-cols-5">
										<p className="text-md text-center font-bold text-[#fead61]">DESCRIÇÃO</p>
										<p className="text-md text-center font-bold text-[#fead61]">QUANTIDADE</p>
										<p className="text-md text-center font-bold text-[#fead61]">GRANDEZA</p>
										<p className="text-md text-center font-bold text-[#fead61]">VALOR</p>
										<p className="text-md text-center font-bold text-[#fead61]">VALOR TOTAL</p>
									</div>
									{dados.custosAdicionais?.map((custo, index) => (
										<div key={index} className="grid w-full grid-cols-5">
											<p className="text-primary/80 text-center text-xs font-bold">{custo.descricao}</p>
											<p className="text-primary/80 text-center text-xs font-bold">{custo.qtde}</p>
											<p className="text-primary/80 text-center text-xs font-bold">{custo.grandeza}</p>
											<p className="text-primary/80 text-center text-xs font-bold">{custo.valor}</p>
											<p className="text-primary/80 text-center text-xs font-bold">{(custo.valor * custo.qtde).toFixed(2)}</p>
										</div>
									))}
								</div>
							</div>
						...
						<div className="flex flex-col items-center">
							<span className="py-2 text-center text-sm font-bold text-[#15599a] uppercase">ESTUDO DE GERAÇÃO - DESVIO AZIMUTAL</span>
							<div className="grid grid-cols-2 grid-rows-4">
								<div className="grid grid-cols-3 items-center">
									<p className="text-primary/70 text-center font-bold">NORTE</p>
									<input
										className="text-primary/70 p-2 text-center font-bold outline-hidden"
										type={"number"}
										readOnly={true}
										value={dados.modNorte}
										onChange={(e) => setDados({ ...dados, modNorte: Number(e.target.value) })}
									/>
									<p className="text-primary/70 text-center font-bold">
										{dados.modNorte && fatorDeGeracaoPorOrientacao[dados.cidade]
											? ((Number(fatorDeGeracaoPorOrientacao[dados.cidade]["NORTE"]) * dados.modNorte * dados.potModulos) / 1000).toFixed(2)
											: false}{" "}
										kWh
									</p>
								</div>
								<div className="grid grid-cols-3 items-center">
									<p className="text-primary/70 text-center font-bold">NORDESTE</p>
									<input
										className="text-primary/70 p-2 text-center font-bold outline-hidden"
										type={"number"}
										readOnly={true}
									...
									/>
									<p className="text-primary/70 text-center font-bold">
										{dados.modNordeste && fatorDeGeracaoPorOrientacao[dados.cidade]
											? ((Number(fatorDeGeracaoPorOrientacao[dados.cidade]["NORDESTE"]) * dados.modNordeste * dados.potModulos) / 1000).toFixed(2)
											: false}{" "}
										kWh
									</p>
								</div>
								<div className="grid grid-cols-3 items-center">
									<p className="text-primary/70 text-center font-bold">LESTE</p>
									<input
										className="text-primary/70 p-2 text-center font-bold outline-hidden"
										type={"number"}
										readOnly={true}
										value={dados.modLeste}
										onChange={(e) => setDados({ ...dados, modLeste: Number(e.target.value) })}
									/>
									<p className="text-primary/70 text-center font-bold">
										{dados.modLeste && fatorDeGeracaoPorOrientacao[dados.cidade]
											? ((Number(fatorDeGeracaoPorOrientacao[dados.cidade]["LESTE"]) * dados.modLeste * dados.potModulos) / 1000).toFixed(2)
											: false}{" "}
										kWh
									</p>
								</div>
								<div className="grid grid-cols-3 items-center">
									<p className="text-primary/70 text-center font-bold">SUDESTE</p>
									<input
										className="text-primary/70 p-2 text-center font-bold outline-hidden"
										type={"number"}
										readOnly={true}
										value={dados.modSudeste}
										onChange={(e) => setDados({ ...dados, modSudeste: Number(e.target.value) })}
									/>
									<p className="text-primary/70 text-center font-bold">
										{dados.modSudeste && fatorDeGeracaoPorOrientacao[dados.cidade]
											? ((Number(fatorDeGeracaoPorOrientacao[dados.cidade]["SUDESTE"]) * dados.modSudeste * dados.potModulos) / 1000).toFixed(2)
											: false}{" "}
										kWh
									</p>
								</div>
								<div className="grid grid-cols-3 items-center">
									<p className="text-primary/70 text-center font-bold">SUL</p>
									<input
										className="text-primary/70 p-2 text-center font-bold outline-hidden"
										type={"number"}
										readOnly={true}
										value={dados.modSul}
										onChange={(e) => setDados({ ...dados, modSul: Number(e.target.value) })}
									/>
									<p className="text-primary/70 text-center font-bold">
										{dados.modSul && fatorDeGeracaoPorOrientacao[dados.cidade]
											? ((Number(fatorDeGeracaoPorOrientacao[dados.cidade]["SUL"]) * dados.modSul * dados.potModulos) / 1000).toFixed(2)
											: false}{" "}
										kWh
									</p>
								</div>
								<div className="grid grid-cols-3 items-center">
									<p className="text-primary/70 text-center font-bold">SUDOESTE</p>
									<input
										className="text-primary/70 p-2 text-center font-bold outline-hidden"
										type={"number"}
										readOnly={true}
									...
									/>
									<p className="text-primary/70 text-center font-bold">
										{dados.modSudoeste && fatorDeGeracaoPorOrientacao[dados.cidade]
											? ((Number(fatorDeGeracaoPorOrientacao[dados.cidade]["SUDOESTE"]) * dados.modSudoeste * dados.potModulos) / 1000).toFixed(2)
											: false}{" "}
										kWh
									</p>
								</div>
								<div className="grid grid-cols-3 items-center">
									<p className="text-primary/70 text-center font-bold">OESTE</p>
									<input
										className="text-primary/70 p-2 text-center font-bold outline-hidden"
										type={"number"}
										readOnly={true}
										value={dados.modOeste}
										onChange={(e) => setDados({ ...dados, modOeste: Number(e.target.value) })}
									/>
									<p className="text-primary/70 text-center font-bold">
										{dados.modOeste && fatorDeGeracaoPorOrientacao[dados.cidade]
											? ((Number(fatorDeGeracaoPorOrientacao[dados.cidade]["OESTE"]) * dados.modOeste * dados.potModulos) / 1000).toFixed(2)
											: false}{" "}
										kWh
									</p>
								</div>
								<div className="grid grid-cols-3 items-center">
									<p className="text-primary/70 text-center font-bold">NOROESTE</p>
									<input
										className="text-primary/70 p-2 text-center font-bold outline-hidden"
										type={"number"}
										readOnly={true}
									...
									/>
									<p className="text-primary/70 text-center font-bold">
										{dados.modNoroeste && fatorDeGeracaoPorOrientacao[dados.cidade]
											? ((Number(fatorDeGeracaoPorOrientacao[dados.cidade]["NOROESTE"]) * dados.modNoroeste * dados.potModulos) / 1000).toFixed(2)
											: false}{" "}
										kWh
									</p>
								</div>
						...
						<div className="bg-background flex w-full flex-col items-center justify-center gap-2 border border-[#15599a] p-4 shadow-lg">
							<span className="py-2 text-center text-sm font-bold text-[#15599a] uppercase">IMAGEM PARA VISUALIZAÇÃO DO PROJETO</span>
							{dados.linkVisualizacaoProjeto && (
								<div className="flex flex-col items-center">
									<a href={dados.linkVisualizacaoProjeto} className="cursor-pointer font-bold text-green-400">
										IMAGEM ATUAL
									</a>
						...
						{dados.status == "CONCLUIDO" ? (
							<div className="bg-background flex w-full items-center justify-center gap-2 border border-[#15599a] p-4 shadow-lg">
								<SelectInput
								...
								<Link href={`/projetos/laudo/pdf/${dados._id}?tipo=${laudoType}`}>
									<button className="rounded bg-[#fead61] p-2 font-bold hover:bg-[#15599a] hover:text-white">LAUDO</button>
								</Link>
							</div>
						) : null}
						...
						<div className="flex w-full items-center justify-center gap-2">
							{!dados.solicitacaoContrato && (
								<Link href={`/publico/formSolicitacao?cliente=${dados.nomeDoCliente}-${dados.codigoSVB}&id=${dados._id}`}>
									<button className="rounded bg-[#fead61] p-2 font-bold hover:bg-[#15599a] hover:text-white">SOLICITAR CONTRATO</button>
								</Link>
							)}
						</div>
					</div>
