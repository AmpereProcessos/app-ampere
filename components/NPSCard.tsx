...
function NPSCard({ project }: NPSCardProps) {
	...
	function getTagColor(npsValue: TProjectDTO["nps"]) {
		if (npsValue == undefined || npsValue == null) return { tag: "NÃO DEFINIDO", color: "bg-primary/60" };
	...
	return (
		<div className="border-primary/20 flex w-[450px] items-center rounded-md border">
			<div className={`h-full w-[5px] rounded-tl-md rounded-bl-md ${getTagColor(nps).color}`}></div>
			<div className="flex grow flex-col p-3">
				<div className="flex w-full items-center justify-between">
					<h1 className="w-full text-center text-sm leading-none font-bold tracking-tight duration-300 ease-in-out hover:text-cyan-500 lg:text-start">
				...
				<div className="justity-center mt-2 flex w-full items-center gap-2">
					<div className="flex items-center gap-2">
						<FaUserAlt />
						<p className="text-primary/60 text-[0.6rem] leading-none tracking-tight">{project.vendedor?.nome}</p>
					</div>
					<div className="flex items-center gap-2">
						<BsTelephone />
						<p className="text-primary/60 text-[0.6rem] leading-none tracking-tight">{project.telefone || "NÃO FORNECIDO"}</p>
					</div>
					<div className="flex items-center gap-2">
						<FaCity />
						<p className="text-primary/60 text-[0.6rem] leading-none tracking-tight">{project.cidade}</p>
					</div>
				</div>
				...
				<div className="w-full">
					<textarea
						...
						className="bg-primary/20 h-[50px] w-full resize-none p-2 text-center outline-hidden"
					/>
				</div>
				<div className="flex w-full items-end justify-between">
					<div className="flex items-end gap-2">
						<div className="flex w-[100px] flex-col gap-1">
							<label htmlFor={"nota"} className={"font-raleway text-primary/60 text-start text-xs font-bold"}>
								NOTA
							</label>
							<select
								...
								className="text-primary/60 w-full bg-transparent text-xs outline-hidden"
							>
					...
					{project.jornada?.dataNps ? (
						<div className="flex items-center gap-2 text-green-500">
							...
							<p className="text-primary/60 text-xs">{formatDateAsLocale(project.jornada.dataNps, true)}</p>
						</div>
					) : null}
					<button
