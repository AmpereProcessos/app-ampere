...
function NPSCard({ project }: NPSCardProps) {
  ...
  function getTagColor(npsValue: TProjectDTO["nps"]) {
    if (npsValue == undefined || npsValue == null)
      return { tag: "NÃO DEFINIDO", color: "bg-primary/60" };
  ...
  return (
    <div className="border-border flex w-[450px] items-center rounded-md border">
      ...
      <div className="flex grow flex-col p-3">
        <div className="flex w-full items-center justify-between">
          ...
          <div
            className={`flex min-w-fit items-center gap-2 rounded-full ${getTagColor(nps).color} px-2 py-1`}
          >
        ...
        <div className="justity-center mt-2 flex w-full items-center gap-2">
          <div className="flex items-center gap-2">
            <FaUserAlt />
            <p className="text-foreground text-[0.6rem] leading-none tracking-tight">
          ...
          <div className="flex items-center gap-2">
            <BsTelephone />
            <p className="text-foreground text-[0.6rem] leading-none tracking-tight">
              {project.telefone || "NÃO FORNECIDO"}
            </p>
          </div>
          <div className="flex items-center gap-2">
            <FaCity />
        ...
        <h1 className="text-foreground mt-2 text-sm">OBSERVAÇÕES</h1>
        ...
        <div className="flex w-full items-end justify-between">
          <div className="flex items-end gap-2">
            <div className="flex w-[100px] flex-col gap-1">
              <label
                htmlFor={"nota"}
                className={"font-raleway text-foreground text-start text-xs font-bold"}
              >
                NOTA
              </label>
              <select
                ...
                className="text-foreground w-full bg-transparent text-xs outline-hidden"
              >
          ...
          {project.jornada?.dataNps ? (
            <div className="flex items-center gap-2 text-green-500">
              ...
              <p className="text-foreground text-xs">
                {formatDateAsLocale(project.jornada.dataNps, true)}
              </p>
            </div>
          ) : null}
          <button
