...
function OSCard({ info, reload }) {
	...
	return (
		<div className="flex flex-col p-2 border border-blue-300 rounded shadow-lg">
			<div className="flex flex-wrap gap-2 items-center justify-around border-b border-gray-300 pb-2">
				<h1 className="font-bold text-[#15599a] text-center">
					{os.qtde} - {os.nomeDoContrato}
				</h1>
				<p className="font-raleway text-sm text-gray-500">TELEFONE: {os.telefone ? os.telefone : "-"}</p>
				<p className="font-raleway text-sm text-gray-500">CONTATO (PAGADOR): {os.pagamento?.contatoPagador ? os.pagamento?.contatoPagador : "-"}</p>
			</div>
			{os.ordensDeServico?.map((ordem, index) => {
				if (ordem.cobrancaRealizada == false) {
					return (
						<div key={index} className={`flex flex-wrap mt-1 ${ordem.dataDeFechamento == undefined && "hidden"} items-center justify-around border-b border-gray-300 pb-2`}>
							...
							<div className="hidden lg:flex flex-col items-center">
								...
								<p className="text-xs uppercase">{ordem.dataDeFechamento != undefined ? new Date(new Date(ordem.dataDeFechamento).setHours(27)).toLocaleDateString() : "-"}</p>
							</div>
							<div className={`flex flex-col items-center ${ordem.cobrancaRealizada != true ? "text-red-500" : "text-gray-500"}`}>
