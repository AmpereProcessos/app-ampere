...
function OperationReportBlock({ session, data, setReportInfo, operationName, operationId }) {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        for (let j = 0; j < files[i].files.length; j++) {
          ...
          const name =
            files[i].files.length > 1 ? `${files[i].name} (${j + 1})` : `${files[i].name}`;
          linkArr = [
            ...linkArr,
            {
              titulo: name,
  ...
  return (
    <div className="flex w-full flex-col items-center py-4">
      <h1 className="text-foreground w-full text-start font-medium">
      ...
      <h3 className="text-foreground w-full pb-2 text-center text-sm font-medium">
      ...
      {reportActivityHolder.error ? (
        <p className="text-xs text-red-500 italic">{reportActivityHolder.error}</p>
      ) : null}
        ...
        {data.atividades.length > 0 ? (
          data.atividades.map((activity, index) => (
            <div className="flex w-full max-w-full items-center justify-between" key={index}>
              <p className="text-foreground grow text-center text-sm break-words break-all">
                {activity}
              </p>
              <button
                onClick={() => {
          ...
          <p className="text-foreground py-4 text-center text-xs font-extralight italic">
      ...
      <textarea
        ...
        className="border-border bg-primary/20 h-[150px] max-h-[150px] w-full resize-none border p-2 text-center outline-hidden lg:h-[100px] lg:max-h-[100px] lg:w-[50%]"
      />
      <h3 className="text-foreground w-full py-2 text-center text-sm font-medium">
        ANEXE ARQUIVOS:
      </h3>
        ...
        <div className="bg-primary/20 relative flex h-fit items-center justify-center rounded-lg border-2 border-dotted border-blue-700 p-2">
          <div className="absolute">
            {fileHolder.file ? (
              ...
              <div className="flex flex-col items-center">
                <i className="fa fa-folder-open fa-4x text-blue-700"></i>
                <span className="block text-xs font-normal text-gray-400 lg:text-sm">
      ...
      {files.length > 0 ? (
        ...
        <p className="text-foreground py-1 text-center text-xs font-extralight italic">
          ...
          reports ? (
            reports.length > 0 ? (
              reports.map((report, index) => (
                <ReportLine report={report} key={index} count={reports.length - index} />
              ))
            ) : (
              <p className="text-foreground w-full animate-pulse py-4 text-center font-medium">
