...
function PPSModalCallInfo({ tipoDeSolicitacao, dados }) {
	...
	return (
		<div className="flex flex-col  gap-x-2 border border-gray-300 p-2 mt-4 w-full">
						...
						<>
							...
							<div className="flex flex-col text-gray-600 w-full">
								...
								{dados.equipamentos.map((item, index) => (
									<div key={index} className="grid grid-cols-6 border border-t-0 border-gray-300">
										<h1 className="text-gray-600 text-xxs text-center border-r p-2 border-gray-300 font-bold">{item.nome}</h1>
										<h1 className="text-gray-600 text-xxs text-center border-r p-2 border-gray-300 font-bold">{item.pot}</h1>
										<h1 className="text-gray-600 text-xxs text-center border-r p-2 border-gray-300 font-bold">{item.qtde}</h1>
										<h1 className="text-gray-600 text-xxs text-center border-r p-2 border-gray-300 font-bold">{item.qtde * item.pot}</h1>
										<h1 className="text-gray-600 text-xxs text-center border-r p-2 border-gray-300 font-bold">{item.horasDiarias}</h1>
										<h1 className="text-gray-600 text-xxs text-center p-2 font-bold">{item.qtde * item.pot * item.horasDiarias}</h1>
									</div>
								))}
								<div className="grid grid-cols-6 border border-t-0 border-gray-300">
									...
									<p className="text-gray-600 text-xxs col-span-1 text-center p-2 font-bold">{getTotalsOfEquipment(dados.equipamentos).totalDailyConsumption}</p>
								</div>
								<div className="grid grid-cols-6 border border-t-0 border-gray-300">
									<p className="bg-gray-600 text-white text-xs col-span-5 text-center p-1 border-r border-white font-bold">POTÊNCIA TOTAL</p>
									<p className="text-gray-600 text-xxs col-span-1 text-center p-2 font-bold">{getTotalsOfEquipment(dados.equipamentos).totalPot}</p>
								</div>
							</div>
						</>
					)}
					{tipoDeSolicitacao == "ANÁLISE DE CRÉDITO" && (
						<>
							...
							<div className="grid grid-cols-2 items-center text-gray-600 w-full">
								...
								<p className="text-xs text-center">{dayjs(dados.dataDeNascimento).add(4, "h").format("DD/MM/YYYY")}</p>
							</div>
							<div className="grid grid-cols-2 items-center text-gray-600 w-full">
								<p className="font-bold text-xs text-center">VALOR FINANCIADO</p>
								<p className="text-xs text-center">R${dados.valorFinanciamento.toFixed(2).replace(".", ",")}</p>
							</div>
							<div className="grid grid-cols-2 items-center text-gray-600 w-full">
								<p className="font-bold text-xs text-center">RENDA</p>
								<p className="text-xs text-center">R${dados.rendaDoCliente.toFixed(2).replace(".", ",")}</p>
							</div>
							<div className="grid grid-cols-2 items-center text-gray-600 w-full">
							...
							{dados.links?.length > 0 && (
								<div className="flex flex-col gap-x-2 p-2 mt-4">
									<span className="font-bold text-center font-raleway">LINKS</span>
