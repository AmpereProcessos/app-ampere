...
function PPSModalCallInfo({ tipoDeSolicitacao, dados }) {
  function getTotalsOfEquipment(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      sumTotalDailyConsumption =
        sumTotalDailyConsumption + arr[i].qtde * arr[i].pot * arr[i].horasDiarias;
    }
    return {
  ...
  return (
    <div className="border-border mt-4 flex w-full flex-col gap-x-2 border p-2">
      <span className="text-center font-bold">INFORMAÇÕES ADICIONAIS</span>
      {tipoDeSolicitacao != "DÚVIDAS E AUXILIOS TÉCNICOS" ? (
        <div className="border-border flex flex-col items-center border p-2">
          <div className="text-foreground grid w-full grid-cols-2 items-center">
            ...
            <>
              ...
              <div className="text-foreground flex w-full flex-col">
                ...
                <div className="grid grid-cols-6">
                  <h1 className="bg-primary/80 border-r border-white p-1 text-center text-xs font-bold text-white">
                    NOME
                  </h1>
                  <h1 className="bg-primary/80 border-r border-white p-1 text-center text-xs font-bold text-white">
                    POTÊNCIA NOMINAL
                  ...
                  <h1 className="bg-primary/80 p-1 text-center text-xs font-bold text-white">
                    Wh DIÁRIO
                  </h1>
                </div>
                {dados.equipamentos.map((item, index) => (
                  <div key={index} className="border-border grid grid-cols-6 border border-t-0">
                    <h1 className="text-foreground text-xxs border-border border-r p-2 text-center font-bold">
                      {item.nome}
                    </h1>
                    <h1 className="text-foreground text-xxs border-border border-r p-2 text-center font-bold">
                      {item.pot}
                    ...
                    <h1 className="text-foreground text-xxs p-2 text-center font-bold">
                      {item.qtde * item.pot * item.horasDiarias}
                    </h1>
                  </div>
                ))}
                <div className="border-border grid grid-cols-6 border border-t-0">
                  <p className="bg-primary/80 col-span-5 border-r border-white p-1 text-center text-xs font-bold text-white">
                    CONSUMO DIÁRIO TOTAL
                  </p>
                  <p className="text-foreground text-xxs col-span-1 p-2 text-center font-bold">
