...
function PosVendaCard({ session, projectId, project, mode }: PosVendaCardProps) {
	...
	function renderDeliveryInfo({ expectedAt, deliveredAt }: { expectedAt?: string | null; deliveredAt?: string | null }) {
		...
		if (expectedAt)
			return (
				<h1 className="text-center text-[0.65rem] leading-none tracking-tight text-gray-500 lg:text-xs">
					PREVISTO PARA: {formatDateAsLocale(expectedAt)} (EM <strong className="text-cyan-500">{getDifferenceBetweenDates({ start: expectedAt, end: new Date() })} DIAS</strong> )
				</h1>
			);
					...
					<div className="flex w-full flex-col items-start justify-between lg:flex-row lg:items-center">
						...
						<div className="mt-2 flex w-full items-center justify-center gap-2 lg:mt-0 lg:w-fit lg:justify-end">
							<div className="flex items-center gap-2 text-gray-500">
								<BsCalendarFill />
								<p className="text-xs font-medium">{project.jornada.dataUltimoContato ? formatDateAsLocale(project.jornada.dataUltimoContato) : "SEM CONTATO"}</p>
							</div>
							<button
								type="button"
								onClick={() => {
					...
					<div className="flex w-full flex-col">
						...
						<div className="mt-1 flex w-full flex-col flex-wrap items-center justify-between gap-2 md:flex-row">
							...
							<div className="flex flex-col items-center rounded p-1 lg:items-start">
								...
								<h1 className="text-center text-[0.65rem] font-medium lg:text-sm">{project.compra.dataPagamento ? formatDateAsLocale(project.compra.dataPagamento) : "NÃO DEFINIDO"}</h1>
							</div>
							...
							<div className="flex flex-col items-center rounded p-1 lg:items-start">
								...
								<h1 className="text-center text-[0.65rem] font-medium lg:text-sm">{project.compra.dataEntrega ? formatDateAsLocale(project.compra.dataEntrega) : "NÃO DEFINIDO"}</h1>
							</div>
							<div className="flex flex-col items-center rounded p-1 lg:items-start">
								<h1 className="text-[0.6rem] leading-none tracking-tight text-gray-500">STATUS DE ENTREGA</h1>
								<h1 className="text-center text-[0.65rem] font-medium lg:text-sm">{project.compra.statusEntrega ? project.compra.statusEntrega : "NÃO DEFINIDO"}</h1>
							</div>
							<div className="flex flex-col items-center rounded p-1 lg:items-start">
              ...
              <p className="text-sm font-medium">
               {project.compra.previsaoEntrega ? formatDateAsLocale(project.compra.previsaoEntrega) : 'SEM PREVISÃO'}
							...
							<div className="flex items-center gap-2 text-gray-500">
								<BsCalendarFill />
								<p className="text-sm font-medium">
									{project.jornada.dataUltimoContato ? `ÚLTIMO CONTATO EM: ${formatDateAsLocale(project.jornada.dataUltimoContato)}` : "SEM REGISTRO DE CONTATO"}
								</p>
							</div>
						</div>
					</div>
