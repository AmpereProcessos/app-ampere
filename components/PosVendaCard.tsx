...
function DeliveryInfo({
  ...
  if (deliveredAt)
    return (
      <h1 className="text-foreground text-center text-[0.65rem] leading-none tracking-tight lg:text-xs">
        ENTREGUE EM: <strong className="text-cyan-500">{formatDateAsLocale(deliveredAt)}</strong>{" "}
        (HÁ{" "}
        <strong className="text-cyan-500">
          {getDifferenceBetweenDates({ start: deliveredAt, end: new Date() })} DIAS
        </strong>{" "}
        )
      </h1>
    );
  ...
  if (expectedAt)
    return (
      <h1 className="text-foreground text-center text-[0.65rem] leading-none tracking-tight lg:text-xs">
        ...
        <strong className="text-cyan-500">
          {getDifferenceBetweenDates({ start: expectedAt, end: new Date() })} DIAS
        </strong>{" "}
        )
      </h1>
    );
...
function AccessInfo({ approvedAt }: { approvedAt?: string | null }) {
  if (approvedAt)
    return (
      <h1 className="text-foreground text-center text-[0.65rem] leading-none tracking-tight lg:text-xs">
        PARECER APROVADO EM:{" "}
        <strong className="text-cyan-500">{formatDateAsLocale(approvedAt)}</strong> (HÁ{" "}
        <strong className="text-cyan-500">
          {getDifferenceBetweenDates({ start: approvedAt, end: new Date() })} DIAS
        </strong>{" "}
        )
      </h1>
    );
              ...
              const updatedStages = stages.map((s, i) =>
                i === index
                  ? {
                      ...s,
                      concluido: value,
                      dataConclusao: value ? new Date().toISOString() : null,
                    }
                  : s,
              );
...
function PosVendaCard({ session, projectId, project, mode, callbacks }: PosVendaCardProps) {
  ...
  const { mutate: handleUpdateProject, isPending } = useMutation({
    ...
    mutationFn: async ({ id, changes }: { id: string; changes: ProjectPatch }) => {
      ...
      if (changes["jornada.dataEfetivacao"])
        await handleOpportunityOnJourneyEndTrigger({ projectId: id });
  ...
  if (mode === "CARD")
    return (
      <div className="font border-border flex w-full gap-2 rounded-md border shadow-xs">
        <div
          className={`h-full w-[7px] ${getBarColor(project.jornada.dataUltimaInteracao)} rounded-tl-md rounded-bl-md`}
        />
        <div className="flex w-full grow flex-col p-3">
              ...
              <div className="text-foreground flex items-center gap-2">
                ...
                <p className="text-xs font-medium">
                  {project.jornada.dataUltimaInteracao
                    ? formatDateAsLocale(project.jornada.dataUltimaInteracao)
                    : "SEM CONTATO"}
                </p>
              </div>
              <button
                ...
                onClick={() => {
                  handleUpdateProject({
                    id: projectId,
                    changes: { "jornada.dataUltimaInteracao": new Date().toISOString() },
                  });
                  setInfo((prev) => ({
                    ...prev,
                    jornada: { ...prev.jornada, dataUltimaInteracao: new Date().toISOString() },
                  }));
                }}
          ...
          <div className="flex w-full flex-col gap-2">
            <h1 className="w-full text-start text-xs leading-none font-bold tracking-tight text-cyan-500">
              INFORMAÇÕES GERAIS
            </h1>
            <div className="flex w-full flex-col flex-wrap items-center justify-between gap-2 md:flex-row">
              ...
              {project.seguro?.aplicavel ? (
                <div className="flex flex-col items-center rounded bg-green-100 p-1 lg:items-start">
                  <h1 className="text-[0.6rem] leading-none tracking-tight text-green-500">
                    POSSUI SEGURO ?
                  </h1>
                  <h1 className="text-center text-[0.65rem] font-medium text-green-600 lg:text-sm">
                    SIM
                  </h1>
                </div>
              ) : null}
              ...
              <div className="flex flex-col items-center rounded p-1 lg:items-start">
                ...
                <h1 className="text-center text-[0.65rem] font-medium lg:text-sm">
                  {project.homologacao.status || "NÃO DEFINIDO"}
                </h1>
              </div>
              <div className="flex flex-col items-center rounded p-1 lg:items-start">
                <h1 className="text-foreground text-[0.6rem] leading-none tracking-tight">
              ...
              <div className="flex flex-col items-center rounded p-1 lg:items-start">
                ...
                <h1 className="text-center text-[0.65rem] font-medium lg:text-sm">
                  {project.compra.dataRequisicaoPagamento
                    ? formatDateAsLocale(project.compra.dataRequisicaoPagamento)
                    : "NÃO DEFINIDO"}
                </h1>
              </div>
              <div className="flex flex-col items-center rounded p-1 lg:items-start">
                <h1 className="text-foreground text-[0.6rem] leading-none tracking-tight">
                  PAGAMENTO DO KIT (CLIENTE)
                </h1>
                <h1 className="text-center text-[0.65rem] font-medium lg:text-sm">
                  {project.compra.dataPagamento
                    ? formatDateAsLocale(project.compra.dataPagamento)
                    : "NÃO DEFINIDO"}
                </h1>
              </div>
              ...
              <div className="flex flex-col items-center rounded p-1 lg:items-start">
                ...
                <h1 className="text-center text-[0.65rem] font-medium lg:text-sm">
                  {project.compra.fornecedor || "NÃO DEFINIDO"}
                </h1>
              </div>
              <div className="flex flex-col items-center rounded p-1 lg:items-start">
                <h1 className="text-foreground text-[0.6rem] leading-none tracking-tight">
                ...
                <h1 className="text-center text-[0.65rem] font-medium lg:text-sm">
                  {project.faturamento?.dataFaturamento
                    ? formatDateAsLocale(project.faturamento.dataFaturamento)
                    : "NÃO DEFINIDO"}
                </h1>
              </div>
            </div>
            ...
            <div className="flex w-full flex-col items-center justify-around gap-3 border border-cyan-500 p-2">
              <div className="flex w-full items-center justify-between gap-2">
                <h1 className="text-start text-xs leading-none font-bold tracking-tight text-cyan-500">
                  JORNADA DO CLIENTE
                </h1>
                <div className="flex items-center gap-2">
                  <Button
                    size="xs"
                    variant={"ghost"}
                    onClick={() => setConclusionMenuIsOpen((prev) => !prev)}
              ...
              <JourneySection
                ...
                onUpdateStages={(updated) =>
                  handleUpdateProject({ id: projectId, changes: { "jornada.estagios": updated } })
                }
              />
            </div>
            <div className="flex w-full flex-col items-center justify-around gap-3 border border-border p-2">
              <h1 className="w-full text-start text-xs leading-none font-bold tracking-tight text-foreground">
                ANOTAÇÕES
              </h1>
              <textarea
                ...
                onChange={(e) => {
                  setInfo((prev) => ({
                    ...prev,
                    jornada: { ...prev.jornada, anotacoes: e.target.value },
                  }));
                }}
                className="field-sizing-content border-border bg-primary/20 text-foreground mt-2 min-h-[50px] w-full resize-none rounded border p-3 text-center text-sm shadow-xs outline-hidden"
              />
            </div>
            ...
            <div className="flex w-full items-center justify-between">
              ...
              <LoadingButton
                ...
                onClick={() => {
                  handleUpdateProject({
                    id: projectId,
                    changes: { "jornada.anotacoes": infoHolder.jornada.anotacoes },
                  });
                }}
              >
                SALVAR ANOTAÇÕES
          ...
          <div className="mt-2 flex w-full flex-wrap items-center justify-start gap-4">
            <div className="flex items-center gap-1">
              ...
              <h1 className="text-foreground text-center text-[0.65rem] leading-none tracking-tight lg:text-xs">
                ASSINADO EM:{" "}
                <strong className="text-cyan-500">
                  {formatDateAsLocale(project.contrato.dataAssinatura)}
                </strong>{" "}
                (HÁ{" "}
                <strong className="text-cyan-500">
                  {getDifferenceBetweenDates({
                    start: project.contrato.dataAssinatura,
                    end: new Date(),
                  })}{" "}
                  DIAS
                </strong>
            ...
            {project.compra.dataPedido ? (
              <div className="flex items-center gap-1">
                ...
                <h1 className="text-foreground text-center text-[0.65rem] leading-none tracking-tight lg:text-xs">
                  COMPRO COM:{" "}
                  <strong className="text-cyan-500">{project.compra?.fornecedor}</strong>
                </h1>
              </div>
            ) : null}
            ...
            <div className="flex items-center gap-1">
              <TbTruckDelivery />
              <DeliveryInfo
                deliveredAt={project.compra.dataEntrega}
                expectedAt={project.compra.previsaoEntrega}
              />
            </div>
            <div className="flex items-center gap-1">
              <BsUnlockFill />
...
function ConclusionMenu({ projectId, project, callbacks, closeMenu }: ConclusionMenuProps) {
  const { mutate, isPending } = useMutation({
    ...
    mutationFn: async () => {
      ...
      await updateProject({
        id: projectId,
        changes: {
          "jornada.estagios": updatedStages,
          "jornada.dataEfetivacao": new Date().toISOString(),
        },
      });
      return "Projeto atualizado com sucesso !";
    },
  ...
  return (
    ...
    >
      <p>
        Você está prestes a concluir a jornada do cliente. Essa ação é irreversível e não poderá ser
        desfeita.
      </p>
