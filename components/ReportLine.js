    ...
    <div className="flex w-full flex-col py-1">
      <div className="flex w-full flex-col items-center justify-between pr-2 lg:flex-row">
        <div className="flex w-full items-center gap-2 lg:w-[60%]">
          ...
          <p className="text-primary/70 text-xs font-medium lg:text-sm">
            {report.nomeAtividade ? `ATUALIZAÇÕES DA ATIVIDADE ${report.nomeAtividade}` : 'ATUALIZAÇÕES DA OPERAÇÃO'}
          </p>
        </div>
        <p className="text-primary/70 w-full text-center text-xs font-medium lg:w-[20%] lg:text-sm">RELATÓRIO Nº {count}</p>
        <div className="flex w-full items-center justify-end gap-2 lg:w-[20%]">
          <AiOutlineCalendar style={{ color: '#fead61' }} />
          <p className="text-primary/60 text-sm font-light">{report.data ? new Date(report.data).toLocaleDateString() : null}</p>
        </div>
      </div>
      {showInfo ? (
        <div className="border-primary/20 mt-1 flex w-full flex-col items-center border-t pt-1">
          <div className="flex w-full flex-col items-center">
            <h1 className="text-primary/80 mb-1 text-sm font-normal">ATIVIDADES REALIZADAS</h1>
            <div className="flex w-full flex-col gap-2 lg:w-[30%]">
              {report.atividades.map((activity, index) => (
                <div className="flex items-center gap-2" key={index}>
                  ...
                  <p className="text-primary/80 text-xs font-thin">
                    {index + 1}) {activity}
                  </p>
                </div>
              ))}
          ...
          <div className="mt-4 flex w-full flex-col items-center lg:mt-2">
            <h1 className="text-primary/80 mb-1 text-sm font-normal">ANEXOS</h1>
          ...
          <div className="mt-4 flex w-full flex-col p-2 lg:mt-2">
            <h1 className="text-primary/80 mb-1 text-center text-sm font-extralight">ANOTAÇÕES</h1>
            <p className="border-primary/20 bg-primary/20 text-primary/80 w-full border p-2 text-center text-xs font-thin">
