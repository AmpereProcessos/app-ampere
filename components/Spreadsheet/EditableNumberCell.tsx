...
export default function EditableNumberCell({
	...
	function handleDisplayButtonFocus(event: FocusEvent<HTMLButtonElement>) {
		...
		if (event.target === document.activeElement && editable) setIsEditing(true);
	}
	...
	const displayButton = (
		<button
			...
			className={cn(
				"h-8 w-full rounded-md px-2 font-mono text-xs tabular-nums text-foreground/80 transition-colors focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring/40",
				alignClass,
