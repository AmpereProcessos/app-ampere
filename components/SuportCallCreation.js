...
function CreateModal({ setModalIsOpen, getCalls }) {
	...
	function validateFields() {
		...
		if (!["PROBLEMAS COM CONCESSIONÁRIA", "GOTEIRA", "DISTRIBUIÇÃO DE CRÉDITOS", "RETRABALHO EM ESTRUTURA"].includes(callInfo.tipoChamado) && !callInfo.linkMonitoramento) {
			setMessage({
					...
					<div className="flex h-full flex-col">
						...
						<div className="flex flex-col overflow-y-auto">
							...
							<div className="mt-4 flex flex-col gap-x-2 border border-gray-300 p-2 lg:flex-row">
								...
								<select value={callInfo.cidade} onChange={(e) => setCallInfo({ ...callInfo, cidade: e.target.value })} className="mt-2 grow text-center text-xs outline-none lg:mt-0">
							...
							<div className="mt-4 flex flex-col gap-x-2 border border-gray-300 p-2 lg:flex-row">
								...
								<select value={callInfo.demanda} onChange={(e) => setCallInfo({ ...callInfo, demanda: e.target.value })} className="mt-2 grow text-center text-xs outline-none lg:mt-0">
