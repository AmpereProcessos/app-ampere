...
function VendedorMetaCard({ vendedor, ano, getVendedoresInfo }) {
  ...
  return (
    <div
      className={`grid grid-cols-14 ${opConcluded ? "bg-green-300" : "bg-background"} border-border w-full items-center border`}
    >
      <div
        className={`text-xxs text-foreground border-border flex h-[60px] items-center justify-center border-r p-1 text-center font-bold lg:text-xs`}
      >
        {infoHolder.nome}
      </div>
      <div
        className={`text-xxs text-foreground border-border flex h-[60px] flex-col items-center justify-center border-r p-1 text-center font-bold lg:text-xs`}
      >
        <input
