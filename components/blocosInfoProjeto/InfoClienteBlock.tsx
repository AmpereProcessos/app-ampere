...
function InfoClienteBlock({ editable, infoHolder, setInfo, clientChanges, setClientChanges }: InfoClienteBlockProps) {
	...
	return (
		<div className="flex w-full flex-col rounded-md border border-primary">
			...
			<AnimatePresence>
				{editMenuIsOpen ? (
					<ClientEditBlock client={infoHolder.cliente} updateClient={updateClient} clientChanges={clientChanges} updateClientChanges={updateClientChanges} />
				) : (
...
function ClientEditBlock({ client, updateClient, clientChanges, updateClientChanges }: ClientEditBlockProps) {
	...
	return (
		<motion.div key={"edit-block"} variants={GeneralVisibleHiddenExitMotionVariants} initial="hidden" animate="visible" exit="exit" className="flex w-full grow flex-col gap-2 p-3">
			...
			<div className="grid grid-cols-1 grid-rows-3 items-center gap-6 px-2 lg:grid-cols-3 lg:grid-rows-1">
				...
				<SelectInput
					label="CIDADE"
					value={client.cidade}
					options={client.uf ? estadosECidades[client.uf as keyof typeof estadosECidades].map((city, index) => ({ id: index + 1, value: city, label: city })) : null}
					handleChange={(value) => {
...
function ClientViewBlock({ client }: ClientViewBlockProps) {
	return (
		<motion.div key={"view-block"} variants={GeneralVisibleHiddenExitMotionVariants} initial="hidden" animate="visible" exit="exit" className="flex w-full grow flex-col gap-2 p-3">
				...
				<div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
					<div className="flex flex-col items-center gap-1 lg:items-start">
						...
						<div className="flex flex-wrap items-center justify-center gap-2 lg:justify-start">
							...
							<div className="flex items-center gap-1">
								<Cake size={15} />
								<p className="text-[0.75rem] font-medium leading-none tracking-tight">{client?.dataNascimento ? formatDateAsLocale(client?.dataNascimento) : "NÃO DEFINIDO"}</p>
							</div>
							<div className="flex items-center gap-1">
								<FaRing size={12} />
