...
function getPrevisionStatus({
  ...
  if (!final || final === 0)
    return (
      <h1 className="border-primary/60 text-foreground relative top-0 right-0 rounded border p-2 text-xs font-bold tracking-tight lg:absolute lg:right-10">
...
function InfoCompraBlock({
  ...
  const isContractAttached = project.links?.contratos
    ?.map((c) => c.title.toUpperCase())
    .includes("CONTRATO ASSINADO");
  ...
  return (
    <div className="flex flex-col rounded-md border border-primary pb-2 shadow-lg gap-6">
      ...
      <div className="w-full flex flex-col gap-2 px-2">
          ...
          <DateInput
            ...
            value={
              infoHolder.compra.dataLiberacao
                ? formatDate(infoHolder.compra.dataLiberacao)
                : undefined
            }
            handleChange={(value) => {
              setChanges((prev) => ({
        ...
        {isContractAttached && isPendingPurchaseAnalysisLiberation ? (
          <div className="px my-2 flex w-full flex-col rounded-xl border border-yellow-500 bg-yellow-100 px-2 py-1 text-yellow-500 italic">
            ...
            <p className="w-full text-center text-[0.65rem]">
              NOTAMOS QUE EXISTE UM CONTRATO ASSINADO ANEXADO, LEMBRE-SE DE LIBERAR O PROJETO PARA
          ...
          >
            ...
            <h1 className="text-xs font-medium tracking-tight">
              {!showLocationInformation
                ? "MOSTRAR LOCALIZAÇÃO DE ENTREGA"
                : "FECHAR DADOS DE LOCALIZAÇÃO DE ENTREGA"}
            </h1>
          </button>
        </div>
        {showLocationInformation ? (
          <InfoBlockSection
            headerTitle="LOCALIZAÇÃO DE ENTREGA"
            headerIcon={<MapPin className="h-4 w-4 min-h-4 min-w-4" />}
          >
                ...
                <SelectInput
                  ...
                  options={
                    infoHolder.compra.localizacaoEntrega?.uf
                      ? estadosECidades[
                          infoHolder.compra.localizacaoEntrega?.uf as keyof typeof estadosECidades
                        ].map((city, index) => ({
                          id: index + 1,
                          value: city,
                          label: city,
        ...
        <InfoBlockSection
          headerTitle="DETALHES DO KIT"
          headerIcon={<Factory className="h-4 w-4 min-h-4 min-w-4" />}
        >
          <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
            <div className="w-full lg:w-1/2">
              <SelectInput
        ...
        <InfoBlockSection
          headerTitle="PEDIDO"
          headerIcon={<ShoppingCart className="h-4 w-4 min-h-4 min-w-4" />}
        >
          <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
              ...
              <DateInput
                ...
                value={
                  infoHolder.compra.dataPedido
                    ? formatDate(infoHolder.compra.dataPedido)
                    : undefined
                }
                handleChange={(value) => {
                  setChanges((prev) => ({
        ...
        <InfoBlockSection
          headerTitle="PAGAMENTO E FATURAMENTO"
          headerIcon={<Banknote className="h-4 w-4 min-h-4 min-w-4" />}
        >
          <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
              ...
              <DateInput
                ...
                value={
                  infoHolder.compra.dataMaxPagamento
                    ? formatDate(infoHolder.compra.dataMaxPagamento)
                    : undefined
                }
                handleChange={(value) => {
                  setChanges((prev) => ({
            ...
            <div className="w-full lg:w-1/4">
              <DateInput
                ...
                editable={editor}
                value={
                  infoHolder.compra.dataRequisicaoPagamento
                    ? formatDate(infoHolder.compra.dataRequisicaoPagamento)
                    : undefined
                }
                handleChange={(value) => {
                  setChanges((prev) => ({
              ...
              <DateInput
                ...
                value={
                  infoHolder.compra.dataPagamento
                    ? formatDate(infoHolder.compra.dataPagamento)
                    : undefined
                }
                handleChange={(value) => {
                  setChanges((prev) => ({
            ...
            <div className="w-full lg:w-1/4">
              <DateInput
                ...
                editable={editor}
                value={
                  infoHolder.compra.dataPagamentoEquipamentos
                    ? formatDate(infoHolder.compra.dataPagamentoEquipamentos)
                    : undefined
                }
                handleChange={(value) => {
                  setChanges((prev) => ({
              ...
              <DateInput
                ...
                value={
                  infoHolder.faturamento.dataFaturamento
                    ? formatDate(infoHolder.faturamento.dataFaturamento)
                    : undefined
                }
                handleChange={(value) => {
                  setChanges((prev) => ({
        ...
        <InfoBlockSection
          headerTitle="ENTREGA"
          headerIcon={<Truck className="h-4 w-4 min-h-4 min-w-4" />}
        >
          <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
            <div className="w-full lg:w-1/4">
              <TextInput
              ...
              <DateInput
                ...
                value={
                  infoHolder.compra.dataEntrega
                    ? formatDate(infoHolder.compra.dataEntrega)
                    : undefined
                }
                handleChange={(value) => {
                  setChanges((prev) => ({
        ...
        {!showDeliveryInfoOnly ? (
          <InfoBlockSection
            ...
            headerIcon={<Box className="h-4 w-4 min-h-4 min-w-4" />}
          >
        ...
        <InfoBlockSection
          ...
          headerIcon={<Logs className="h-4 w-4 min-h-4 min-w-4" />}
        >
          <div className="flex w-full flex-col items-center gap-2 py-2">
            {infoHolder.compra.atualizacoes && infoHolder.compra.atualizacoes.length > 0 ? (
              infoHolder.compra.atualizacoes.map((item, index) => (
                <UpdateCard key={`${item.data}-${index}`} update={item} />
              ))
            ) : (
              <p className="text-foreground w-full text-center text-sm font-medium tracking-tight">
                Nenhuma atualização definida.
              </p>
            )}
          </div>
        </InfoBlockSection>
...
function PurchaseControlsBlock({ session, project }: PurchaseControlsBlockProps) {
  ...
  const { mutate: handleGenerateContractPurchase, isPending: isMutationLoading } =
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      ...
      {isLoading ? (
        <p className="text-foreground w-full animate-pulse text-center text-xs font-bold tracking-tight">
          Carregando...
        </p>
      ) : null}
      ...
      {isSuccess ? (
        purchaseControls && purchaseControls.length > 0 ? (
          purchaseControls.map((purchaseControl) => (
            <PurchaseControlCard key={purchaseControl._id} purchaseControl={purchaseControl} />
          ))
        ) : (
  ...
  return (
    <div className="border-primary bg-background flex w-full flex-col gap-1 rounded-md border p-2 dark:bg-[#121212]">
      <div className="flex w-full items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1">
            <p className="text-foreground text-[0.65rem] font-medium">ATUALIZAÇÃO POR:</p>
            <Avatar
              url={update.autor.avatar_url || undefined}
              width={20}
              height={20}
              fallback={formatNameAsInitials(update.autor.nome)}
            />

            <p className="text-foreground text-[0.65rem] font-medium">{update.autor.nome}</p>
          </div>
          <div className="flex items-center gap-1">
            <BsCalendarPlus />
            <p className="text-foreground text-[0.65rem] font-medium">
              {formatDateAsLocale(update.data, true)}
            </p>
          </div>
        </div>
      </div>
      <p className="bg-primary/10 text-primary rounded-lg p-2 text-center text-xs">
        {update.descricao}
      </p>
    </div>
  );
