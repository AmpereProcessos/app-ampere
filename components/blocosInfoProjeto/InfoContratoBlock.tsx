...
function InfoContratoBlock({ editor, infoHolder, setInfo, changes, setChanges, updateLogs = [], showPaymentInfo = false }: InfoContratoBlockProps) {
	function updateComissionedUser({ index, changes }: { index: number; changes: Partial<TProjectComissionedUser> }) {
		setInfo((prev) => ({
			...prev,
			comissoes: {
				...(prev.comissoes || {}),
				comissionados: prev.comissoes?.comissionados?.map((comissionedUser, i) => (i === index ? { ...comissionedUser, ...changes } : comissionedUser)),
			},
		}));
		setChanges((prev) => ({
			...prev,
	...
	function removeComissionedUser(index: number) {
		setInfo((prev) => ({
			...prev,
			comissoes: { ...(prev.comissoes || {}), comissionados: prev.comissoes?.comissionados?.filter((_, i) => i !== index) },
		}));
		setChanges((prev) => ({ ...prev, "comissoes.comissionados": infoHolder.comissoes?.comissionados?.filter((_, i) => i !== index) }));
	}
	return (
...
function ComissionableUsersCard({ comissionedUser, comissionableValue, updateComissionedUser, removeComissionedUser }: ComissionableUsersCardProps) {
		...
		<div className="border-primary bg-background flex w-full flex-col gap-1 rounded border p-2 shadow-xs dark:bg-[#121212]">
			<div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
				<div className="flex items-center gap-2">
					...
					<Button size="fit" variant="ghost" className="flex items-center gap-1.5 px-2 py-1 text-xs" onClick={() => setEditMenuIsOpen(true)}>
				...
				<div className="flex grow flex-wrap items-center justify-start gap-2">
					<div className="bg-secondary text-xxs text-primary/80 flex items-center gap-1 rounded-lg px-2 py-0.5 text-center font-bold italic">
						...
						<p className="text-[0.57rem] font-black">{formatToMoney(comissionedUser.valor || (comissionedUser.porcentagem * comissionableValue) / 100)}</p>
					</div>
					<div className="bg-secondary text-xxs text-primary/80 flex items-center gap-1 rounded-lg px-2 py-0.5 text-center font-bold italic">
						<FaPercent className={cn("h-3 min-h-3 w-3 min-w-3")} />
						<p className="text-[0.57rem] font-black">{formatDecimalPlaces(comissionedUser.porcentagem)}%</p>
					</div>
					<div
					...
					>
						...
						<p className={cn("text-[0.57rem] font-medium")}>{comissionedUser.dataEfetivacao ? "COMISSÕES EFETIVADAS" : "VALORES NÃO EFETIVADOS"}</p>
					</div>
					<div
						className={cn("bg-secondary text-xxs text-primary/80 flex items-center gap-1 rounded-lg px-2 py-0.5 text-center font-bold italic", {
					...
					>
						...
						<p className={cn("text-[0.57rem] font-medium")}>
							{comissionedUser.dataPagamento ? "PAGAMENTO DE COMISSÕES REALIZADO" : "PAGAMENTO DE COMISSÕES NÃO REALIZADO"}
						</p>
					</div>
					<div
						className={cn("bg-secondary text-xxs text-primary/80 flex items-center gap-1 rounded-lg px-2 py-0.5 text-center font-bold italic", {
					...
					>
						<BadgeCheck className={cn("h-3 min-h-3 w-3 min-w-3")} />
						<p className={cn("text-[0.57rem] font-medium")}>{comissionedUser.dataValidacao ? "VALIDAÇÕES RELIZADAS" : "VALIDAÇÕES PENDENTES"}</p>
					</div>
				</div>
			</div>
			{editMenuIsOpen ? (
	...
	return (
		...
		>
			<div className="flex w-full items-center justify-between">
				...
				>
					<BadgeCheck className={cn("h-3 min-h-3 w-3 min-w-3")} />
					<p className={cn("text-[0.57rem] font-medium")}>{stateHolder.dataValidacao ? "VALIDADO" : "NÃO VALIDADO"}</p>
				</div>
			</div>
			...
			<div className="flex w-full items-center justify-center">
				<div className="bg-secondary text-primary/80 flex items-center gap-1 rounded-lg px-2 py-0.5 text-center text-[0.55rem] font-bold italic">
					...
					<p className="text-[0.57rem] font-black text-[#15599a]">{formatToMoney(comissionableValue * (stateHolder.porcentagem / 100))}</p>
				</div>
			</div>
