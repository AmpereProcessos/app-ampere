...
function InfoDespesasBlock({ projectId, projectName, projectIdentifier }: InfoDespesasBlockProps) {
	...
	const {
		...
		isSuccess,
		isFetching,
	} = useProjectExpenses({ projectId, enabled: !!session?.user.permissoes.financeiro.visualizar, identifier: null });
	...
	return (
		<div className="flex flex-col rounded-md border border-primary pb-2 shadow-lg gap-6">
			...
			{newExpenseMenuIsOpen && session ? (
				<NewExpense
					session={session}
					initialState={{ projeto: { id: projectId, nome: projectName, identificador: projectIdentifier } }}
