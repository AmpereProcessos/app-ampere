...
function InfoMaterialBlock({ editor, infoHolder, setInfo, changes, setChanges, showCircuitBreakers, circuitBreakerAddition }) {
  const [circuitBreakersArr, setCircuitBreakersArr] = useState(infoHolder.material?.disjuntores ? infoHolder.material.disjuntores : null)
      ...
      {showCircuitBreakers ? (
        <div className="flex flex-col w-full">
          ...
          {circuitBreakerAddition ? (
            <div className="grid grid-cols-7 px-2 gap-2 mt-4">
              ...
              <div className="flex flex-col items-center justify-center col-span-1">
                <div onClick={addCircuitBreaker} className="bg-green-300 hover:bg-green-500 text-white p-1 rounded cursor-pointer">
          ...
          {circuitBreakersArr && circuitBreakersArr.length > 0 ? (
            <div className="flex flex-col w-full">
              <div className="grid grid-cols-7 px-2 gap-2 mt-4">
                <h1 className="text-center font-semibold text-[#fead61] col-span-2">TIPO</h1>
                <h1 className="text-center font-semibold text-[#fead61] col-span-2">CORRENTE</h1>
                <h1 className="text-center font-semibold text-[#fead61] col-span-2">QTDE</h1>
                <h1 className="text-center font-semibold text-[#fead61] col-span-1">AÇÕES</h1>
              </div>
              {circuitBreakersArr?.map((circuitBreaker, index) => (
                <div key={index} className="grid grid-cols-7 px-2 gap-2 mt-4">
                  <h1 className="text-center font-semibold text-gray-500 col-span-2">{circuitBreaker.tipo}</h1>
                  <h1 className="text-center font-semibold text-gray-500 col-span-2">{circuitBreaker.corrente}A</h1>
                  <h1 className="text-center font-semibold text-gray-500 col-span-2">{circuitBreaker.qtde} UN</h1>
                  <div className="flex flex-col items-center justify-center col-span-1">
                    {circuitBreakerAddition ? (
                      <div onClick={() => deleteCircuitBreaker(index)} className="bg-red-300 hover:bg-red-500 text-black p-1 rounded cursor-pointer">
            ...
            <p className="text-center text-gray-500 italic text-sm">Sem disjuntores adicionados...</p>
          )}
        </div>
      ) : null}
    </div>
