...
function InfoMaterialBlock({ editor, infoHolder, setInfo, changes, setChanges, updateLogs = [], showCircuitBreakers, circuitBreakerAddition }: InfoMaterialBlockProps) {
	...
	return (
		<div className="flex w-full flex-col rounded-md border border-[#15599a] pb-2 shadow-lg">
			<span className="mb-2 w-full rounded-tr-md rounded-tl-md bg-[#15599a] py-2 text-center font-bold text-white">INFORMAÇÕES SOBRE MATERIAIS DO PROJETO</span>
					...
					{circuitBreakersArr && circuitBreakersArr.length > 0 ? (
						<div className="flex w-full flex-wrap items-start justify-around">
							{circuitBreakersArr?.map((circuitBreaker, index) => (
								<div key={index} className="flex w-full flex-col rounded-md border border-gray-300 p-2 lg:w-[275px]">
									<div className="flex w-full items-center justify-between gap-2">
										...
										<h1 className="min-w-fit rounded-full bg-gray-800 px-2 py-1 text-[0.65rem] font-medium text-white lg:text-xs">{circuitBreaker.corrente}A</h1>
									</div>
