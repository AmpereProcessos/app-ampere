...
function InfoMaterialBlock({
  ...
  function updateAllocation(
    index: number,
    info: Partial<Exclude<TProject["alocacoes"], undefined | null>[number]>,
  ) {
    const prevAllocations = infoHolder.alocacoes || [];
    const newAllocations = prevAllocations.map((allocation, i) =>
      i === index ? { ...allocation, ...info } : allocation,
    );
    setInfo((prev) => ({
      ...prev,
      alocacoes: newAllocations,
...
type InfoMaterialBlockAllocationsProps = {
  ...
  updateAllocation: (
    index: number,
    allocation: Partial<Exclude<TProject["alocacoes"], undefined | null>[number]>,
  ) => void;
};
function InfoMaterialBlockAllocations({
  allocations,
  addAllocation,
  deleteAllocation,
  updateAllocation,
}: InfoMaterialBlockAllocationsProps) {
  const [newAllocationMenuIsOpen, setNewAllocationMenuIsOpen] = useState(false);
  return (
    <InfoBlockSection
      headerTitle="ALOCAÇÕES DE MATERIAL"
      headerIcon={<Box className="h-4 w-4 min-h-4 min-w-4" />}
    >
      <div className="flex items-center justify-end">
        <Button
          onClick={() => setNewAllocationMenuIsOpen((prev) => !prev)}
          size={"sm"}
          variant={"ghost"}
          type="button"
        >
      ...
      <AnimatePresence>
        {newAllocationMenuIsOpen ? (
          <InfoMaterialBlockNewAllocationMenu
            addAllocation={addAllocation}
            closeMenu={() => setNewAllocationMenuIsOpen(false)}
          />
        ) : null}
      </AnimatePresence>
        ...
        {allocations && allocations.length > 0 ? (
          ...
          <div className="flex grow items-center justify-center">
            <p className="text-foreground text-center text-sm italic">
              Nenhuma alocação de material encontrada...
            </p>
          </div>
        )}
      </div>
...
function InfoMaterialBlockNewAllocationMenu({
  addAllocation,
  closeMenu,
}: InfoMaterialBlockNewAllocationMenuProps) {
  const [allocationHolder, setAllocationHolder] = useState<
    Exclude<TProject["alocacoes"], undefined | null>[number]
  >({
    idMaterial: "",
    nome: "",
    unidade: "UN",
  ...
  function handleAddAllocation(info: Exclude<TProject["alocacoes"], undefined | null>[number]) {
    ...
    if (info.quantidadePrevista <= 0)
      return toast.error("A quantidade prevista deve ser maior que zero.");
    addAllocation(info);
    setAllocationHolder({
      idMaterial: "",
      nome: "",
      ...
      <div className="flex w-full grow flex-col gap-2 p-3">
        <MaterialSelector
          initialMaterialState={{
            materialId: allocationHolder.idMaterial || null,
            materialName: allocationHolder.nome,
          }}
          vinculateMaterial={(m) =>
...
function InfoMaterialBlockAllocationCard({
  allocation,
  deleteAllocation,
  updateAllocation,
}: InfoMaterialBlockAllocationProps) {
  const [editMenuIsOpen, setEditMenuIsOpen] = useState(false);
  const [allocationHolder, setAllocationHolder] =
    useState<Exclude<TProject["alocacoes"], undefined | null>[number]>(allocation);
  return (
    <AnimatePresence>
      <div className="w-full flex flex-col p-2 rounded border border-border shadow-xs">
