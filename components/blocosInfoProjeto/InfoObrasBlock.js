...
function InfoObrasBlock({ editor, infoHolder, setInfo, changes, setChanges, project, showMaterialInfo = false, showDeliveryInfo = false }) {
  return (
    <div className="flex flex-col border border-[#15599a] pb-2 shadow-lg rounded-md">
      <span className="w-full bg-[#15599a] text-white text-center font-bold py-2 rounded-tr-md rounded-tl-md mb-2">INFORMAÇÕES SOBRE A OBRA</span>
        ...
        <div className="flex flex-col w-[350px] items-center">
          ...
          <div className="flex">
            ...
            <label className="ml-2" htmlFor="solicitacaoobra">
              {infoHolder.obra?.statusSolicitacao == 'SOLICITADA' ? 'SOLICITADA' : 'NÃO SOLICITADA'}
            </label>
          </div>
        </div>
        {showDeliveryInfo ? (
          <>
            <TextInput
              ...
              editable={false}
              value={infoHolder.compra.statusEntrega ? infoHolder.compra.statusEntrega : '-'}
            />
            <DateInput
      ...
      <div className="flex gap-2 justify-center flex-wrap mt-2">
        <DateInput
          ...
          handleChange={(value) => {
            setChanges({
              ...changes,
              'obra.entrada': isNaN(value) ? new Date(value).toISOString() : null,
            })
            setInfo({
              ...infoHolder,
              obra: {
        ...
        <DateInput
          ...
          editable={editor}
          value={
            infoHolder.obra?.saida != undefined && infoHolder.obra?.saida != '-' ? new Date(infoHolder.obra?.saida).toISOString().slice(0, 10) : 0
          }
          handleChange={(value) => {
            setChanges({
              ...changes,
        ...
        <SelectInput
          ...
          value={
            infoHolder.obra?.equipeResp != undefined && infoHolder.obra?.equipeResp != '-'
              ? infoHolder.obra?.equipeResp == 'TERCEIROS' || infoHolder.obra?.equipeResp == 'TERCERIZADOS' || infoHolder.obra?.equipeResp == 'OUTROS'
                ? 'OUTROS'
