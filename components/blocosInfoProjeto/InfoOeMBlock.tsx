...
function InfoOeMBlock({ editor, infoHolder, setInfo, changes, setChanges }: InfoOeMBlockProps) {
	...
	return (
		<div className="flex flex-col rounded-md border border-primary pb-2 shadow-lg gap-6">
			...
			<AnimatePresence>
				{newMaintenanceMenuIsOpen ? <NewMaintenanceMenu infoHolder={infoHolder} setInfo={setInfo} changes={changes} setChanges={setChanges} /> : null}
			</AnimatePresence>
			<div className="mt-2 flex w-full items-start justify-around gap-4">
				{infoHolder.manutencoes.length > 0 ? (
					...
					<p className="text-primary/60 text-sm font-medium tracking-tight italic">Não há registros de manutenção.</p>
				)}
			</div>
