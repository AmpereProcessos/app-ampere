...
function InfoOeMBlock({ editor, infoHolder, setInfo, changes, setChanges }: InfoOeMBlockProps) {
  ...
  function updateMaintenance({
    ...
    info: TProjectDTO["manutencoes"][number];
    index: number;
  }) {
    const maintenances = [...infoHolder.manutencoes];
  ...
  return (
    <div className="flex flex-col rounded-md border border-primary pb-2 shadow-lg gap-6">
      <div className="flex items-center gap-2 bg-primary/20 px-2 py-2 rounded w-full justify-center">
        <Wrench className="h-4 w-4 min-h-4 min-w-4" />
              ...
              handleChange={(value) => {
                setInfo((prev) => ({
                  ...
                  oem: {
                    ...
                    duracao: Math.round(
                      dayjs(formatDateInputChange(value)).diff(prev.oem?.dataInicio, "days") / 365,
      ...
      <div className="mt-2 flex w-full items-start justify-around gap-4">
        {infoHolder.manutencoes.length > 0 ? (
          ...
          <p className="text-foreground text-sm font-medium tracking-tight italic">
            Não há registros de manutenção.
          </p>
        )}
      </div>
          ...
          <CheckboxWithDate
            ...
            handleChange={(value) => {
              ...
              setChanges((prev) => ({
                ...prev,
                "obra.saida": value,
                "obra.statusDaObra": value ? "CONCLUIDA" : prev.obra.statusDaObra,
              }));
            }}
          />
        </div>
          ...
          <CheckboxInput
            ...
            handleChange={(value) => {
              setInfo((prev) => ({
                ...
                obra: {
                  ...prev.obra,
                  entrada: prev.obra.entrada
                    ? prev.obra.entrada
                    : value
                      ? new Date().toISOString()
                      : prev.obra.entrada,
                  saida: prev.obra.saida
                    ? prev.obra.saida
                    : value
                      ? new Date().toISOString()
                      : prev.obra.saida,
              ...
              setChanges((prev) => ({
                ...
                "obra.entrada": infoHolder.obra.entrada
                  ? infoHolder.obra.entrada
                  : value
                    ? new Date().toISOString()
                    : infoHolder.obra.entrada,
                "obra.saida": infoHolder.obra.saida
