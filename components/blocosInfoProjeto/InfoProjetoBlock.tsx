...
function InfoProjetoBlock({ editor, infoHolder, setInfo, changes, setChanges, comercialEdition = false, updateLogs = [], project, session }: InfoProjetoBlockProps) {
	const isContractAttached = project.links?.contratos?.map((c) => c.title.toUpperCase()).includes("CONTRATO ASSINADO");
	const isPendingProjectInitiation = !!project && (!project.projeto.iniciar || project.projeto.iniciar == "NÃO" || project.projeto.iniciar == "NÃO DEFINIDO");
		...
		<div className="flex flex-col rounded-md border border-[#15599a] pb-2 shadow-lg">
			...
			{isContractAttached && isPendingProjectInitiation ? (
				<div className="px my-2 flex w-full flex-col rounded-xl border border-orange-500 bg-orange-100 py-1 px-2 italic text-orange-500">
					...
					<p className="w-full text-center text-[0.65rem]">NOTAMOS QUE EXISTE UM CONTRATO ASSINADO ANEXADO, LEMBRE-SE DE INICIAR O PROJETO, SE APLICÁVEL.</p>
				</div>
			) : null}
