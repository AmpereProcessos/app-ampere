		...
		<div className="flex flex-col rounded-md border border-primary pb-2 shadow-lg gap-6">
			...
			<div className="flex w-full items-center justify-end gap-2 px-2">
				...
				{newRevenueMenuIsOpen ? (
					<button type="button" onClick={() => setNewRevenueMenuIsOpen(false)} className="rounded-md bg-[#ed174c] px-4 py-1 text-sm font-bold text-white">
						FECHAR
					</button>
				) : (
					<button type="button" onClick={() => setNewRevenueMenuIsOpen(true)} className="rounded-md bg-green-400 px-4 py-1 text-sm font-bold text-white">
						NOVA RECEITA
					</button>
				)}
			</div>
			...
			<div className="mt-4 flex w-full flex-col flex-wrap items-start justify-center gap-4 px-2 lg:flex-row">
				...
				{isSuccess && revenues ? (
					revenues.length > 0 ? (
						revenues.map((revenue) => <RevenueCard key={revenue._id} revenue={revenue} openModal={(id) => setModalRevenue({ isOpen: true, id: id })} />)
					) : (
