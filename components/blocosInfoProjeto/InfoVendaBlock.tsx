...
import { LayoutGrid, LinkIcon, ListCollapse, MapPin, ScrollText, Shield } from "lucide-react";
import Link from "next/link";
import type React from "react";
import toast from "react-hot-toast";
import { FaFile, FaSolarPanel } from "react-icons/fa";
  ...
  return (
    <div className="flex flex-col rounded-md border border-primary pb-2 shadow-lg gap-6">
      <div className="flex items-center gap-2 bg-primary/20 px-2 py-2 rounded w-full justify-center">
        <ScrollText className="h-4 w-4 min-h-4 min-w-4" />
      ...
      <ProjectCoverImage
        ...
        projectCoverImage={project.imagemCapaUrl}
      />
      <div className="w-full flex flex-col gap-2 px-2">
        <InfoBlockSection
          headerTitle="DADOS GERAIS"
          headerIcon={<LayoutGrid className="h-4 w-4 min-h-4 min-w-4" />}
        >
          ...
          <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
            <div className="w-full lg:w-1/3">
              <TextInput
                ...
                value={
                  infoHolder.cpf_cnpj
                    ? formatToCPForCNPJ(infoHolder.cpf_cnpj?.toString() || "")
                    : ""
                }
                handleChange={(value) => {
          ...
          <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
            <div className="w-full lg:w-1/4">
              <SelectInput
                ...
                editable={editor}
                options={serviceTypes.map((tipo, index) => ({
                  id: index + 1,
                  label: tipo.label,
                  value: tipo.value,
                }))}
        ...
        <InfoBlockSection
          headerTitle="ENDEREÇO DE EXECUÇÃO"
        ...
        >
          ...
          <TextareaInput
            ...
            handleChange={(value) => {
              setChanges({ ...changes, obsComercial: value });
              setInfo({
                ...infoHolder,
                obsComercial: value,
              });
        ...
        <InfoBlockSection
          ...
          headerIcon={<ListCollapse className="h-4 w-4 min-h-4 min-w-4" />}
        >
          <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
            <div className="w-full lg:w-1/4">
              <SelectInput
        ...
        <InfoBlockSection
          headerTitle="VINCULAÇÕES"
          headerIcon={<LinkIcon className="h-4 w-4 min-h-4 min-w-4" />}
        >
          <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
            <div className="w-full lg:w-1/4">
              <TextInput
        ...
        <InfoBlockSection
          headerTitle="OPERAÇÃO E MANUTENÇÃO"
          headerIcon={<FaSolarPanel className="h-4 w-4 min-h-4 min-w-4" />}
        >
                ...
                <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
                  <div className="w-full lg:w-1/3">
                    <SelectInput
                      ...
                      selectedItemLabel="NÃO DEFINIDO"
                      options={oemPlans.map((plan, index) => ({
                        id: index + 1,
                        ...plan,
                      }))}
                      handleChange={(value) => {
                      ...
                      handleChange={(value) => {
                        setInfo((prev) => ({
                          ...
                          oem: {
                            ...
                            duracao: Math.round(
                              dayjs(formatDateInputChange(prev.oem?.dataFim)).diff(
                                formatDateInputChange(value),
                                "days",
                              ) / 365,
                            ),
                          },
                      ...
                      handleChange={(value) => {
                        setInfo((prev) => ({
                          ...
                          oem: {
                            ...
                            duracao: Math.round(
                              dayjs(formatDateInputChange(value)).diff(
                                prev.oem?.dataInicio,
                                "days",
                              ) / 365,
                            ), //   dayjs(formatDateInputChange(value)).diff(prev.oem.dataInicio),
                          },
                        }));
                        setChanges((prev) => ({
                          ...prev,
                          ...
                          "oem.duracao": Math.round(
                            dayjs(formatDateInputChange(value)).diff(prev.oem?.dataInicio, "days") /
                              365,
                          ),
                        }));
                      }}
        ...
        <InfoBlockSection
          headerTitle="SEGURO"
          headerIcon={<Shield className="h-4 w-4 min-h-4 min-w-4" />}
        >
          <div className="flex w-full flex-col items-center justify-center gap-2">
            <div className="flex items-center justify-center">
              <div className="w-fit">
                <CheckboxInput
                  ...
                  handleChange={(value) => {
                    setInfo((prev) => ({
                      ...prev,
                      seguro: { ...(prev.seguro || {}), aplicavel: value },
                    }));
                    setChanges((prev) => ({
                      ...prev,
                      "seguro.aplicavel": value,
                    }));
                  }}
                />
              </div>
              ...
              <div className="flex w-full flex-col items-center justify-center gap-2 lg:flex-row">
                ...
                <div className="w-full lg:w-1/3">
                  <DateInput
                    ...
                    value={
                      infoHolder.seguro?.dataInicio
                        ? formatDate(infoHolder.seguro.dataInicio)
                        : undefined
                    }
                    handleChange={(value) => {
                      setInfo((prev) => ({
                      ...
                      setChanges((prev) => ({
                        ...prev,
                        "seguro.dataInicio": formatDateInputChange(value, "string") as string,
                      }));
                    }}
                    width="100%"
                  />
        ...
        {infoHolder.linkDrive && (
          <div className="flex w-full items-center justify-center py-2">
            <a
              className="font-medium text-blue-400 hover:text-[#15599a]"
        ...
        {infoHolder.idSolicitacaoContrato ? (
          <div className="flex w-full flex-col items-center justify-center gap-2 lg:flex-row">
            <Link href={`/comercial/pdfProcuracao/${infoHolder.idSolicitacaoContrato}`}>
              <button
                type="button"
                className="border-primary/80 text-foreground flex items-center gap-2 rounded border px-4 py-2 duration-300 ease-in-out hover:border-gray-400 hover:text-gray-400"
              >
                <FaFile />
