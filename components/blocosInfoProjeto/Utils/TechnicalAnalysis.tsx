...
function TechnicalAnalysis({
	...
	return (
		<div className="flex flex-col rounded-md border border-primary pb-2 shadow-lg gap-6">
			...
			{isError ? (
				<ErrorComponent
					msg={"Erro ao buscar informações da análise técnica."}
				/>
			) : null}
			{isSuccess && analysis ? (
				<div className="flex w-full flex-col gap-2 px-4">
					...
					<div className="flex w-full items-center justify-center gap-2">
						<h1 className="font-raleway text-primary/60 text-xs font-bold">
							ANALISTA
						</h1>
						<Avatar
							url={analysis.analista?.avatar_url}
							fallback={"A"}
							height={30}
							width={30}
						/>
					...
					<div className="flex w-full flex-wrap justify-around">
						<div className="border-primary/60 flex flex-col rounded-md border p-3">
							<p className="text-primary/60 text-[0.6rem] leading-none font-medium tracking-tight uppercase">
								TIPO DE TELHA
							</p>
					...
					{viewPermissions.projetos ? (
						<div className="flex w-full flex-col gap-1">
							...
							<h1 className="text-primary/60 mt-2 text-sm leading-none font-medium tracking-tight">
								PENDÊNCIAS
							</h1>
							{analysis.pendencias && analysis.pendencias.length > 0 ? (
								analysis.pendencias.map((pendency, index) => (
									...
									>
										...
										<h1 className="text-primary/60 bg-primary/20 my-2 rounded-md p-2 text-center text-sm">
											{pendency.descricao}
										</h1>
										<div className="flex w-full items-center justify-start">
											<div className="flex items-center gap-2">
												<div
													className={`text-primary/60 flex items-center gap-2`}
												>
													<BsCalendarFill />
								...
								<p className="text-primary/60 w-full py-2 text-center text-xs font-medium italic">
								...
								{analysis.descritivo?.length ? (
									analysis.descritivo.map((item, index) => (
										<div key={index} className="flex w-full flex-col">
											<div className="flex items-center gap-2">
												<MdTopic />
												<h1 className="text-sm leading-none tracking-tight">
													{item.topico}
												</h1>
											</div>
											<p className="text-primary/60 w-full text-center text-sm">
												{item.descricao}
											</p>
										</div>
									))
