...
export function BuilderStepper({ className }: BuilderStepperProps) {
	const { stageOrder, stages, currentStage, currentIndex, furthestIndex, navigationMode, goToStageByIndex } = useBuilder();
					...
					return (
						<li key={stageId} className="flex min-w-[8rem] flex-1 items-stretch">
							...
							>
								...
								>
									{isComplete && !isActive ? <Check className="h-4 w-4" strokeWidth={2.5} /> : <Icon className="h-4 w-4" />}
								</span>
								<span className="flex min-w-0 flex-col gap-0.5">
									<span className="truncate text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Etapa {idx + 1}</span>
									<span className={cn("truncate text-xs font-semibold", isActive ? "text-foreground" : "text-foreground/80")}>{stage.label}</span>
								</span>
							</button>
							{!isLast ? <span aria-hidden className={cn("my-3 w-px shrink-0 self-stretch", isComplete ? "bg-primary/25" : "bg-border")} /> : null}
						</li>
					);
				})}
			</ol>
