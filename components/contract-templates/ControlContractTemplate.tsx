...
import { createContractTemplate, updateContractTemplate } from "@/utils/methods/mutation/contract-templates";
import { useContractTemplateById } from "@/utils/methods/query/contract-templates";
...
type ControlContractTemplateProps = {
...
function ControlContractTemplate({ contractTemplateId, session, closeMenu, callbacks }: ControlContractTemplateProps) {
	const [mode, setMode] = useState<"editor" | "preview">("editor");
	const { data: contractTemplate, isLoading, isError, isSuccess, error } = useContractTemplateById({ id: contractTemplateId });
...
`,
	...
	const { mutate: handleCreateContractTemplate, isPending } = useMutation({
		mutationKey: ["update-contract-template"],
	...
	useEffect(() => {
		if (contractTemplate) {
			redefineState({ template: contractTemplate });
		}
	}, [contractTemplate, redefineState]);
	return (
		<ResponsiveDialogDrawer
			menuTitle="EDITAR TEMPLATE DE CONTRATO"
			menuDescription="Preencha os campos abaixo para editar um template de contrato."
			menuActionButtonText="ATUALIZAR TEMPLATE DE CONTRATO"
...
export default ControlContractTemplate;
