...
function EtapaEntrada({ session, next, order }) {
  ...
  function validateStage() {
    ...
    if (!files.fotoConjuntoEscada) {
      toast.error(
        "Por favor, anexa um foto do conjunto escada (escada amarrada, cones, corrente e placa de alerta).",
      );
      return false;
    }
    return true;
    ...
    try {
      if (files.fotoConjuntoEscada) {
        for (let i = 0; i < files.fotoConjuntoEscada.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/fotoConjuntoEscada${i + 1}`,
          );
  ...
  return (
    <div className="my-2 flex w-full flex-col">
      <div className="flex flex-col items-center justify-between bg-[#fead61] py-2 text-white">
        ...
        <p className="text-foreground px-2 text-[0.6rem] font-bold italic lg:text-xs">
