...
function EtapaFinalizacao({ session, closeModal, order, queryKey }) {
  ...
  function validateStage() {
    if (!checkFinishingStage) {
      toast.error(
        "Por favor, preencha sobre a conferência de execução dos procedimentos dessa etapa.",
      );
      return false;
    }
    ...
    if (!files.fotoMedicoesStrings) {
      toast.error(
        "Por favor, anexe foto(s) das medições de corrente e tensão CC de todas as strings.",
      );
    ...
    if (!files.fotoMedicoesCAEntrada) {
      toast.error(
        "Por favor, anexe foto(s) das medições de tensão CA fase e linha na entrada de energia.",
      );
    ...
    if (!files.fotoMedicoesCADisjuntorAntes) {
      toast.error(
        "Por favor, anexe foto(s) das medições de tensão CA fase e linha no quadro antes do disjuntor.",
      );
    ...
    if (!files.fotoMedicoesCADisjuntorDepois) {
      toast.error(
        "Por favor, anexe foto(s) das medições de tensão CA fase e linha no quadro depois do disjuntor.",
      );
    ...
    try {
      if (files.filmagemTesteAguaDoTelhado) {
        for (let i = 0; i < files.filmagemTesteAguaDoTelhado.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/filmagemTesteAguaDoTelhado${i + 1}`,
          );
      ...
      if (files.filmagemPosTesteAguaNaLaje) {
        for (let i = 0; i < files.filmagemPosTesteAguaNaLaje.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/filmagemPosTesteAguaNaLaje${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          let url = await getDownloadURL(ref(storage, res.metadata.fullPath));
      ...
      if (files.fotoDataloggerPosConfig) {
        for (let i = 0; i < files.fotoDataloggerPosConfig.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/fotoDataloggerPosConfig${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          let url = await getDownloadURL(ref(storage, res.metadata.fullPath));
      ...
      if (files.fotoMedicoesStrings) {
        for (let i = 0; i < files.fotoMedicoesStrings.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/fotoMedicoesStrings${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          let url = await getDownloadURL(ref(storage, res.metadata.fullPath));
      ...
      if (files.fotoMedicoesCAEntrada) {
        for (let i = 0; i < files.fotoMedicoesCAEntrada.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/fotoMedicoesCAEntrada${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          let url = await getDownloadURL(ref(storage, res.metadata.fullPath));
      ...
      if (files.fotoMedicoesCADisjuntorAntes) {
        for (let i = 0; i < files.fotoMedicoesCADisjuntorAntes.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/fotoMedicoesCADisjuntorAntes${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          let url = await getDownloadURL(ref(storage, res.metadata.fullPath));
      ...
      if (files.fotoMedicoesCADisjuntorDepois) {
        for (let i = 0; i < files.fotoMedicoesCADisjuntorDepois.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/fotoMedicoesCADisjuntorDepois${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          let url = await getDownloadURL(ref(storage, res.metadata.fullPath));
  ...
  return (
    <div className="my-2 flex w-full flex-col">
      <div className="flex flex-col items-center justify-between rounded-sm bg-[#fead61] p-2 text-white">
        <h1 className="w-full text-center font-bold">ETAPA FINALIZAÇÃO DE OBRA</h1>
        <h1 className="text-foreground text-center text-xs font-bold">
          (OBS: TODAS AS FOTOS DEVEM SER TIRADAS ATRAVÉS DO APLICATIVO{" "}
          <strong className="text-[#15599a]">NOTECAM</strong>.)
        </h1>
      </div>
      <div className="border-border my-2 flex flex-col items-center gap-y-2 border-y py-2">
        <div className="flex w-full flex-col gap-y-2">
          ...
          <div className="mt-2 flex flex-col items-center pl-5">
            ...
            <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
              <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
              <p className="col-span-9 font-medium">
                ANTES DE DESCER DO TELHADO CONFERIR SE RETIROU TODAS AS FERRAMENTAS
              </p>
            </div>
            <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
              <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
            ...
            <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
              <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
              <p className="col-span-9 font-medium">
                ANTES DE DESCER DO TELHADO FAZER INSPEÇÃO VISUAL SE TUDO ESTA CORRETO
              </p>
            </div>
            <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
              <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
        ...
        <div className="flex w-full flex-col gap-y-2">
          ...
          <div className="mt-2 flex flex-col items-center pl-5">
            <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
              <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
              <p className="col-span-9 font-medium">
                CONFERIR SE TODA A SOBRA DE MATERIAL FORAM RETIRADAS DO LOCAL DE MONTAGEM DOS
                INVERSORES
              </p>
            </div>
            <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
              <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
              <p className="col-span-9 font-medium">
                CONFERIR SE TODAS AS FERRAMENTAS FORAM RETIRADAS DO LOCAL DA INSTALAÇÃO DOS
                INVERSORES
              </p>
            </div>
            <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
              <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
              <p className="col-span-9 font-medium">
                CONFERIR SE TODO O LIXO FOI RETIRADO DO LOCAL DA INSTALAÇÃO DOS INVERSORES
              </p>
            </div>
            <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
              <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
              <p className="col-span-9 font-medium">
                NO TÉRMINO DA MONTAGEM DOS INVERSORES, LAVAR AS MÃOS E PANOS DE LIMPEZA E REALIZAR A
                HIGIENIZAÇÃO DOS INVERSORES, QDG, STRING, CANOS E CONDULETES
              </p>
            </div>
            <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
              <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
              <p className="col-span-9 font-medium">
                FAZER INSPEÇÃO VISUAL NO LOCAL DA MONTAGEM DOS INVERSORES SE ESTA TUDO CORRETO,
                LEMBRE-SE QUE A MONTAGEM DOS INVERSORES SÃO A VITRINE DA AMPÉRE
              </p>
            </div>
          </div>
        </div>
        <div className="flex w-full flex-col gap-y-2">
          ...
          <div className="mt-2 flex flex-col items-center pl-5">
            <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
              <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
              <p className="col-span-9 font-medium">
                CERTIFICAR-SE QUE TODOS OS LIXOS DE TODAS AS ETAPAS FORAM RETIRADOS E DESCARTADOS
            ...
            <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
              <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
              <p className="col-span-9 font-medium">
                ANTES DE SAIR DA CASA DO CLIENTE FAZER UMA ULTIMA INSPEÇÃO VISUAL PRA VERIFICAR SE
                TUDO ESTÁ CORRETO
              </p>
            </div>
            <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
              <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
              <p className="col-span-9 font-medium">
                ANTES DE SAIR DA CASA DO CLIENTE PERGUNTAR SE ESTA TUDO CORRETO OU SE PODEMOS FAZER
                MAIS ALGUMA COISA POR ELE
              </p>
            </div>
            <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
              <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
              <p className="col-span-9 font-medium">
                DESPEDIR-SE DO CLIENTE E FALAR PRA ELE QUE O ENGENHEIRO RESPONSÁVEL VAI FAZER UMA
                INSPEÇÃO E CONFIGURAÇÕES E AJUSTES FINAIS SE HOUVER NECESSIDADE
              </p>
            </div>
          </div>
        </div>
