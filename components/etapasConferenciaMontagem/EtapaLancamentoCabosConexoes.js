...
function EtapaLancamentoCabosConexos({ session, next, order }) {
  ...
  function validateStage() {
    ...
    if (!files.fotoConexaoSistemaRede) {
      toast.error(
        "Por favor, anexe foto(s) da conexão do sistema solar com a rede elétrica do cliente (no quadro, ou, se foi necessário, no ramal).",
      );
    ...
    try {
      if (files.fotoCabeamentoLaje) {
        for (let i = 0; i < files.fotoCabeamentoLaje.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/fotoCabeamentoLaje${i + 1}`,
          );
      ...
      if (files.fotoConexaoSistemaRede) {
        for (let i = 0; i < files.fotoConexaoSistemaRede.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/fotoConexaoSistemaRede${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
          let url = await getDownloadURL(ref(storage, res.metadata.fullPath));
  ...
  return (
    <div className="my-2 flex w-full flex-col">
      <div className="flex flex-col items-center justify-between bg-[#fead61] p-2 text-white">
        ...
        <p className="text-foreground text-xs font-bold italic">
      ...
      <div className="border-border my-2 flex flex-col items-center gap-y-2 border-y py-2">
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          ...
          <p className="col-span-9 font-medium">
            LANÇAMENTO DE CABOS, FIXAÇÃO DE ROLDANAS, ELETRODUTOS, MANGUEIRA HIPERFLEX E ESTICAR O
            CABEAMENTO
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            APÓS CONCLUÍDO O LANÇAMENTO DOS CABOS, VEM A PARTE DE LIGAÇÃO DOS EQUIPAMENTOS
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            FAZER INSPEÇÃO VISUAL DE TODAS AS CONEXÕES, CC, CA E PRINCIPALMENTE OS TERRAS DO
            INVERSOR INTERNO E EXTERNA ,STRING E DPSs SE ESTÃO FEITAS
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
        ...
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            CONEXÃO CORRETA DOS CABOS DE CORRENTE ALTERNADA E DE CORRENTE CONTINUA (CORES E
            CONECTORES)
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            CONEXÃO DOS CABOS DE CORRENTE ALTERNADA SEMPRE NA REDE MESTRE DO PADRÃO
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            CONEXÃO DOS CABOS DE CORRENTE ALTERNADA SEMPRE COM CONECTORES CORRETOS PARA CONEXÃO,
            CONECTORES DE PERFURAÇÃO OU BIMETÁLICO QUANDO HOUVER NECESSIDADE
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            QUANDO USAR CONECTORES BIMETÁLICO PARA CONEXÃO COM A REDE, FAZER O ISOLAMENTO CORRETO,
            1° ETAPA PASSAR UMA CAMADA DE FITA DE BAIXA TENSÃO, 2° ETAPA CAMADA DE FITA DE ALTA
            TENSÃO E POR ULTIMO 3° ETAPA CAMADA DE FITA DE BAIXA TENSÃO
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            SEMPRE OBSERVAR SE OS CABOS NÃO SE DANIFICOU DURANTE O PROCESSO, DANIFICOU FAZER A
            SUBSTITUIÇÃO
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            CONEXÕES DOS CABOS DE CORRENTE CONTINUA USAR CONECTORES MC4 E CHAVES APROPRIADAS
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            FAZER INSPEÇÃO VISUAL SE TODAS AS CONEXÕES ESTÃO CORRETAS, SE ESTÃO ISOLADAS
            CORRETAMENTE, E SE OS CONECTORES DE PERFURAÇÃO QUEBRARAM OS PARAFUSOS E ISOLAR A PONTA
            DO CABO DE DERIVAÇÃO
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
        ...
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            FAZER UMA INSPEÇÃO VISUAL DE TODO O PROCESSO COM A FINALIDADE DE VERIFICAR SE TUDO ESTA
            CORRETO
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            DURANTE TODO PROCESSO DE MONTAGEM TANTO DOS MÓDULOS, LANÇAMENTO DE CABOS OU MONTAGEM
            MECÂNICA, DEIXAR CEM PORCENTO AS FERRAMENTAS ORGANIZADAS, NÃO DEIXAR FERRAMENTAS
            ESPALHAS POR TODA A CASA DO CLIENTE, ISSO ACONTECE MUITO
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            AO FINAL DE TODO DIA DE OBRA FAZER LISTA DE MATERIAL PARA O DIA SEGUINTE
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap justify-center gap-2">
        <div className="flex w-fit flex-col items-center">
          <label className="ml-2 text-center font-bold text-[#15599a]">
            FOTOS/FILMAGEM DO CABEAMENTO LANÇADO NA LAJE (ROLDANAS FIXADAS, ELETRODUTOS, MANGUEIRA
            HIPERFLEX, ETC)
          </label>
