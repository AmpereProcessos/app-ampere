...
function EtapaMontagemMecanica({ session, next, order }) {
    ...
    try {
      ...
      if (files.fotoInfraEletrica) {
        for (let i = 0; i < files.fotoInfraEletrica.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido.nome}/fotoInfraEletrica${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
      ...
      if (files.fotoPlacaGeracaoDistribuida) {
        for (let i = 0; i < files.fotoPlacaGeracaoDistribuida.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido.nome}/fotoPlacaGeracaoDistribuida${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
  ...
  function validateStage() {
    ...
    if (order?.detalhes.topologia == "INVERSOR" && !files.fotoInfraEletrica) {
      toast.error(
        "Por favor, anexe fotos/filmagem que contemplem a infraestrutura elétrica (inversor, stringbox, conduletes e eletrodutos).",
      );
      ...
      <div className="border-border my-2 flex flex-col items-center gap-y-2 border-y py-2">
        ...
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          ...
          <p className="col-span-9 font-medium">
            ANTES DE COMEÇAR A MONTAGEM DOS INVERSORES FAZER INSPEÇÃO VISUAL PARA EVITAR QUE OS
            EQUIPAMENTOS MOLHEM OU PEGUEM SOL
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            OBSERVAR SE TEM CANO DE AGUA PRÓXIMO OU REGISTRO DE AGUA, SE EXISTE MONTAGEM E CANOS DE
            FIOS OU CABOS ELÉTRICOS TAMBÉM
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
        ...
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            SEMPRE OBSERVAR SE OS ELETRODUTOS E CONDULETES, INVERSORES, STRING E QDG ESTÃO FICANDO
            BEM FIXADOS
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
        ...
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            CONCLUINDO A MONTAGEM FAZER INSPEÇÃO VISUAL DE TODA A MONTAGEM, PARA GARANTIR QUE NÃO
            ESTEJA FALTANDO NADA, PRINCIPALMENTE PARAFUSO DE BOX RETO E ADESIVOS DA AMPÉRE
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
        ...
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            TIRAR UMA FOTO E MANDA-LÁ NO GRUPO DO WHATSAPP DA EQUIPE TÉCNICA, RELATANDO A CONCLUSÃO
            DA SEGUNDA ETAPA
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap justify-center gap-2">
        ...
        {order.detalhes?.topologia == "INVERSOR" && (
          <>
            <div className="flex w-fit flex-col items-center">
              <label className="ml-2 text-center font-bold text-[#15599a]">
                FOTOs/FILMAGEM DA INFRAESTRUTURA ELÉTRICA (INVERSOR, STRINGBOX, CONDULETES,
                ELETRODUTOS)
              </label>
              <div className="bg-primary/20 relative mt-2 flex h-fit items-center justify-center rounded-lg border-2 border-dotted border-blue-700 p-2">
                <div className="absolute">
