    ...
    try {
      if (files.fotoEtiquetaModulos) {
        for (let i = 0; i < files.fotoEtiquetaModulos.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/fotoEtiquetaModulos${i + 1}`,
          );
      ...
      if (files.fotoEtiquetaInversores) {
        for (let i = 0; i < files.fotoEtiquetaInversores.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/fotoEtiquetaInversores${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
      ...
      if (files.filmagemTelhadoPorCima) {
        for (let i = 0; i < files.filmagemTelhadoPorCima.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/filmagemTelhadoPorCima${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
      ...
      if (files.filmagemTelhadoPorBaixo) {
        for (let i = 0; i < files.filmagemTelhadoPorBaixo.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/filmagemTelhadoPorBaixo${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
      ...
      if (files.fotosTrilhosMontados) {
        for (let i = 0; i < files.fotosTrilhosMontados.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/fotosTrilhosMontados${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
      ...
      if (files.fotosInversoresAlocados) {
        for (let i = 0; i < files.fotosInversoresAlocados.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/fotosInversoresAlocados${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
      ...
      if (files.fotosConexoesMicros) {
        for (let i = 0; i < files.fotosConexoesMicros.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/fotosConexoesMicros${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
      ...
      if (files.fotosPaineisInstalados) {
        for (let i = 0; i < files.fotosPaineisInstalados.length; i++) {
          ...
          var imageRef = ref(
            storage,
            `clientes/${order.favorecido?.nome}/fotosPaineisInstalados${i + 1}`,
          );
          let res = await uploadBytes(imageRef, file);
          const size = res.metadata.size;
    ...
    <div className="my-2 flex w-full flex-col">
      ...
      <div className="border-border my-2 flex flex-col items-center gap-y-2 border-y py-2">
        ...
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            FIXAÇÃO DOS TRILHOS COM APERTO DE TODOS OS PARAFUSOS E CONFERIR TELHAS ALTAS E ONDAS
            BAIXAS
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            FIXAÇÃO DA MANTA ASFÁLTICA (PICHE) NAS SAÍDAS DOS GANCHOS, TELHAS ALTAS E ONDAS BAIXAS
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            FAZER ATERRAMENTO DE TODOS OS TRILHOS E COLOCAR AS PONTAS DOS TERRAS PRA DENTRO DA LAJE
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
        ...
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            VERIFICAR SE TODAS AS CONEXÕES DE CORRENTE ALTERANADA DOS MICROS ESTEJAM ESTANHADAS E
            ISOLADAS CORRETAMENTE
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
        ...
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            ANTES DA MONTAGEM DOS MÓDULOS FAZER INSPEÇÃO VISUAL SE ESTÁ TUDO CORRETO E CONFERIR SE
            TEM TELHAS QUEBRADAS
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            DURANTE A FIXAÇÃO DOS MÓDULOS FAZER CONFERÊNCIA SE OS MÓDULOS ESTÃO SENDO CONECTADOS
            CORRETAMENTE TANTO EM SÉRIE OU NOS MICROS
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            DURANTE A FIXAÇÃO DOS MÓDULOS FAZER OUTRA INSPEÇÃO VISUAL MÓDULO A MÓDULO COM O INTUITO
            DE RETIRAR TELHAS QUEBRADAS
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            DURANTE TODA A MONTAGEM NO TELHADO LEVAR SACOLAS PARA IR ARMAZENANDO O LIXO QUE APARECE
            DURANTE O PROCESSO. (LIXO EM RUFOS E CALHAS, LIXO EM SAÍDAS DE ÁGUA, ABRAÇADEIRAS
            DESCARTADAS)
          </p>
        </div>
        <div className="grid w-full grid-cols-10 gap-2 lg:w-[60%]">
          <BsFillSunFill style={{ color: "#fead61", fontSize: "25px" }} />
          <p className="col-span-9 font-medium">
            TIRAR UM FOTO E MANDA-LÁ NO GRUPO DO WHATSAPP DA EQUIPE TÉCNICA, RELATANDO CONCLUSÃO DA
            PRIMEIRA ETAPA.
          </p>
        </div>
      </div>
