...
export function ClienteStage({ draft, updateClient, updateClientData }: ClienteStageProps) {
  ...
  const { data, filters, updateFilters, isFetching } = useClientsSearch({
    initialFilters: { page: 1, search: "", onlyValidPhones: false },
  });
  const clients = data?.clients || [];
  const visibleClients = useMemo(
    () => clients.slice(0, mode === "existing" ? 12 : 4),
    [clients, mode],
  );
      ...
      {mode === "existing" ? (
        <StageSection title="Clientes existentes">
          <ClientSearchField
            ...
            updateSearch={updateSearchByVinculation}
          />
          <ClientResults
            clients={visibleClients}
            selectedClientId={draft.client.clientId}
            selectClient={selectClient}
            emptyLabel={
              filters.search
                ? "Nenhum cliente encontrado para o filtro."
                : "Pesquise por nome, CPF/CNPJ, telefone ou email."
            }
          />
        </StageSection>
      ) : (
        <>
          <StageSection title="Novo cliente">
            <FieldGrid>
              <TextInput
                ...
                handleChange={(value) => updateClientData({ nome: value })}
                width="100%"
              />
              <TextInput
                label="CPF/CNPJ"
              ...
              <TextInput
                ...
                placeholder="Email"
                handleChange={(value) => updateClientData({ email: value })}
                width="100%"
              />
              <TextInput
                label="CEP"
                ...
                placeholder="CEP"
                handleChange={(value) => updateClientData({ cep: formatToCEP(value) })}
                width="100%"
              />
              <SelectInput
                label="UF"
                ...
                handleChange={(value) => updateClientData({ uf: value, cidade: "" })}
                onReset={() => updateClientData({ uf: "", cidade: "" })}
                width="100%"
              />
              <SelectInput
                label="CIDADE"
                ...
                selectedItemLabel="NAO DEFINIDO"
                options={BrazilianCitiesOptionsFromUF(clientData.uf)}
                handleChange={(value) => updateClientData({ cidade: value })}
                onReset={() => updateClientData({ cidade: "" })}
                width="100%"
              />
              <TextInput
                label="BAIRRO"
                ...
                placeholder="Bairro"
                handleChange={(value) => updateClientData({ bairro: value })}
                width="100%"
              />
              <TextInput
                label="LOGRADOURO"
                ...
                handleChange={(value) => updateClientData({ endereco: value })}
                width="100%"
              />
              <TextInput
                label="NÚMERO"
                value={clientData.numeroOuIdentificador}
                placeholder="Número ou identificador"
                handleChange={(value) => updateClientData({ numeroOuIdentificador: value })}
                width="100%"
              />
              <TextInput
                label="COMPLEMENTO"
                ...
                handleChange={(value) => updateClientData({ complemento: value })}
                width="100%"
              />
            </FieldGrid>
            {draft.client.clientId ? (
              <div className="flex justify-end">
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  onClick={() => updateClient({ clientId: null, selectedClient: null })}
                >
                  Remover vínculo CRM
                </Button>
              </div>
          ...
          <StageSection title="Possíveis clientes existentes">
            <ClientResults
              ...
              selectedClientId={draft.client.clientId}
              selectClient={selectClient}
              emptyLabel="Digite CPF/CNPJ ou telefone para consultar possíveis vínculos no CRM."
            />
          </StageSection>
        </>
      )}
      ...
      {clients.map((client) => {
        ...
        return (
          <button
            ...
            className="flex w-full flex-col gap-1 rounded-md border border-primary/15 p-3 text-left transition hover:border-primary/50"
          >
            ...
            <span className="text-xs text-foreground">
              {[client.cpfCnpj, client.telefonePrimario, client.email].filter(Boolean).join(" | ")}
            </span>
