...
const batteryTypes = ["LÍTIO", "ESTACIONÁRIA", "NAO DEFINIDO"].map((value, index) => ({
  id: index + 1,
  label: value === "NAO DEFINIDO" ? "NÃO DEFINIDO" : value,
  value,
}));
const controllerTypes = ["INTEGRADO AO INVERSOR", "SEPARADO", "NAO DEFINIDO"].map(
  (value, index) => ({ id: index + 1, label: value, value }),
);
...
function normalizeSystemByProducts(project: TProject, products: TProductItem[]): TProject {
  ...
  const modulesPeakPower =
    modules.reduce(
      (acc, product) => acc + Number(product.qtde || 0) * Number(product.potencia || 0),
      0,
    ) / 1000;
  const averageModulePower =
    modules.length > 0
      ? modules.reduce((acc, product) => acc + Number(product.potencia || 0), 0) / modules.length
      : "";
  const inverterSummary = inverters
    .map(
      (product) =>
        `${product.qtde}x ${product.fabricante} ${product.modelo} ${product.potencia || 0}W`,
    )
    .join(" | ");
  ...
  return (
    <div className="flex w-full flex-col gap-4">
      <StageSection title="Valores e normalizações">
        <FieldGrid cols={4}>
          <NumberInput
            ...
            value={project.sistema.valorProjeto || 0}
            placeholder="0,00"
            handleChange={(value) =>
              updateProjectWith((prev) => ({
                ...prev,
                sistema: { ...prev.sistema, valorProjeto: value },
              }))
            }
            width="100%"
          />
          <NumberInput
            label="POTÊNCIA PICO (kWp)"
            value={project.sistema.potPico || 0}
            placeholder="Potência"
            handleChange={(value) =>
              updateProjectWith((prev) => ({
                ...prev,
                sistema: { ...prev.sistema, potPico: value },
                homologacao: { ...prev.homologacao, potencia: value },
              }))
            }
            width="100%"
          />
          <SelectInput
            label="TOPOLOGIA"
            value={project.sistema.topologia || null}
            selectedItemLabel="NAO DEFINIDO"
            options={SystemTopologiesTypes}
            handleChange={(value) =>
              updateProjectWith((prev) => ({
                ...prev,
                sistema: { ...prev.sistema, topologia: value as TProject["sistema"]["topologia"] },
              }))
            }
            onReset={() =>
              updateProjectWith((prev) => ({
                ...prev,
                sistema: { ...prev.sistema, topologia: null },
              }))
            }
            width="100%"
          />
          <TextInput
            label="KIT / OBS. EQUIPAMENTOS"
            value={project.compra.kitInfo || ""}
            placeholder="Resumo do kit"
            handleChange={(value) =>
              updateProjectWith((prev) => ({ ...prev, compra: { ...prev.compra, kitInfo: value } }))
            }
            width="100%"
          />
          <TextInput
            label="NÚMERO DE MÓDULOS"
            value={project.sistema.qtdeModulos?.toString() || ""}
            placeholder="Calculado pelos produtos"
            handleChange={(value) =>
              updateProjectWith((prev) => ({
                ...prev,
                sistema: { ...prev.sistema, qtdeModulos: Number(value) || 0 },
              }))
            }
            width="100%"
          />
          <TextInput
            ...
            value={project.sistema.potModulos?.toString() || ""}
            placeholder="Calculado pelos produtos"
            handleChange={(value) =>
              updateProjectWith((prev) => ({
                ...prev,
          ...
          <TextInput
            label="INVERSORES"
            value={project.sistema.inversor || ""}
            placeholder="Calculado pelos produtos"
            handleChange={(value) =>
              updateProjectWith((prev) => ({
                ...prev,
                sistema: { ...prev.sistema, inversor: value },
              }))
            }
            width="100%"
          />
      ...
      <StageSection title="Produtos">
        <div className="flex w-full items-center justify-between gap-2">
          <p className="text-xs font-medium text-foreground">
            {products.length} produto(s) na composição.
          </p>
          <Button
            type="button"
            size="sm"
            variant={productMenuOpen ? "secondary" : "outline"}
        ...
        {productMenuOpen ? (
          <ProductCompositionMenu products={products} updateProducts={updateProducts} />
        ) : null}
        {products.length > 0 ? (
            ...
            {products.map((product, index) => (
              <ProductItem
                ...
                removeProduct={(itemIndex) =>
                  updateProducts(products.filter((_, index) => index !== itemIndex))
                }
              />
            ))}
          </div>
        ) : (
          <p className="rounded-md border border-dashed border-primary/15 p-4 text-center text-sm text-foreground">
            Nenhum produto adicionado. Esta etapa pode seguir sem módulos, inversores ou outros
            equipamentos.
          </p>
        )}
      </StageSection>
      ...
      <StageSection title="Serviços">
        <div className="flex w-full items-center justify-between gap-2">
          <p className="text-xs font-medium text-foreground">
            {services.length} serviço(s) na composição.
          </p>
          <Button
            type="button"
            size="sm"
            variant={serviceMenuOpen ? "secondary" : "outline"}
        ...
        {serviceMenuOpen ? (
          <ServiceCompositionMenu services={services} updateServices={updateServices} />
        ) : null}
        {services.length > 0 ? (
          <div className="grid w-full grid-cols-1 gap-2 lg:grid-cols-2">
            {services.map((service, index) => (
              <ServiceItem
                ...
                service={service}
                index={index}
                removeService={(itemIndex) =>
                  updateServices(services.filter((_, index) => index !== itemIndex))
                }
              />
            ))}
          </div>
        ...
        <StageSection title="Sistema especial">
          {isOffGrid ? (
            <FieldGrid cols={4}>
              <SelectInput
                ...
                value={project.sistema.tipoControlador || null}
                selectedItemLabel="NAO DEFINIDO"
                options={controllerTypes}
                handleChange={(value) =>
                  updateProjectWith((prev) => ({
                    ...prev,
                    sistema: { ...prev.sistema, tipoControlador: value },
                  }))
                }
                onReset={() =>
                  updateProjectWith((prev) => ({
                    ...prev,
                    sistema: { ...prev.sistema, tipoControlador: "" },
                  }))
                }
                width="100%"
              />
              <TextInput
                ...
                value={project.sistema.marcaControlador || ""}
                placeholder="Marca"
                handleChange={(value) =>
                  updateProjectWith((prev) => ({
                    ...prev,
                    sistema: { ...prev.sistema, marcaControlador: value },
                  }))
                }
                width="100%"
              />
              <NumberInput
                label="QTDE CONTROLADORES"
                value={project.sistema.qtdeControlador || 0}
                placeholder="Quantidade"
                handleChange={(value) =>
                  updateProjectWith((prev) => ({
                    ...prev,
                    sistema: { ...prev.sistema, qtdeControlador: value },
                  }))
                }
                width="100%"
              />
              <NumberInput
                label="CORRENTE CONTROLADOR"
                value={project.sistema.correnteControlador || 0}
                placeholder="A"
                handleChange={(value) =>
                  updateProjectWith((prev) => ({
                    ...prev,
                    sistema: { ...prev.sistema, correnteControlador: value },
                  }))
                }
                width="100%"
              />
          ...
          {isPump ? (
            <FieldGrid>
              <TextInput
                ...
                value={project.sistema.marcaBomba || ""}
                placeholder="Marca"
                handleChange={(value) =>
                  updateProjectWith((prev) => ({
                    ...prev,
                    sistema: { ...prev.sistema, marcaBomba: value },
                  }))
                }
                width="100%"
              />
              <NumberInput
                label="QTDE BOMBAS"
                value={project.sistema.qtdeBomba || 0}
                placeholder="Quantidade"
                handleChange={(value) =>
                  updateProjectWith((prev) => ({
                    ...prev,
                    sistema: { ...prev.sistema, qtdeBomba: value },
                  }))
                }
                width="100%"
              />
              <NumberInput
                ...
                value={project.sistema.potBomba || 0}
                placeholder="Potência"
                handleChange={(value) =>
                  updateProjectWith((prev) => ({
                    ...prev,
                    sistema: { ...prev.sistema, potBomba: value },
                  }))
                }
                width="100%"
              />
          ...
          <FieldGrid cols={4}>
            <TextInput
              ...
              value={project.sistema.marcaBateria || ""}
              placeholder="Marca"
              handleChange={(value) =>
                updateProjectWith((prev) => ({
                  ...prev,
                  sistema: { ...prev.sistema, marcaBateria: value },
                }))
              }
              width="100%"
            />
            <NumberInput
              label="QTDE BATERIAS"
              value={project.sistema.qtdeBateria || 0}
              placeholder="Quantidade"
              handleChange={(value) =>
                updateProjectWith((prev) => ({
                  ...prev,
                  sistema: { ...prev.sistema, qtdeBateria: value },
                }))
              }
              width="100%"
            />
            <SelectInput
              ...
              value={project.sistema.tipoBateria || null}
              selectedItemLabel="NAO DEFINIDO"
              options={batteryTypes}
              handleChange={(value) =>
                updateProjectWith((prev) => ({
                  ...prev,
                  sistema: { ...prev.sistema, tipoBateria: value },
                }))
              }
              onReset={() =>
                updateProjectWith((prev) => ({
                  ...prev,
                  sistema: { ...prev.sistema, tipoBateria: "" },
                }))
              }
              width="100%"
            />
            <NumberInput
              label="CAPACIDADE (Ah)"
              value={project.sistema.capacidadeBateria || 0}
              placeholder="Ah"
              handleChange={(value) =>
                updateProjectWith((prev) => ({
                  ...prev,
                  sistema: { ...prev.sistema, capacidadeBateria: value },
                }))
              }
              width="100%"
            />
      ...
      <StageSection title="Observações">
        <TextareaInput
          label="OBSERVAÇÕES DE COMPOSIÇÃO"
          value={project.compra.informacoes || ""}
          placeholder="Itens relevantes, serviços inclusos, particularidades do kit..."
          handleChange={(value) =>
            updateProjectWith((prev) => ({
              ...prev,
              compra: { ...prev.compra, informacoes: value },
            }))
          }
        />
      </StageSection>
...
function ProductCompositionMenu({ products, updateProducts }: ProductCompositionMenuProps) {
  ...
  const [inverterHolder, setInverterHolder] = useState({
    equipment: null as TEquipmentDTO | null,
    qtde: 1,
    garantia: 10,
  });
  ...
  function addCatalogProduct(type: "module" | "inverter") {
    ...
    if (!holder.equipment)
      return toast.error(type === "module" ? "Selecione um módulo." : "Selecione um inversor.");
  ...
  function addCustomProduct() {
    if (customProduct.fabricante.trim().length < 2)
      return toast.error("Informe o fabricante do produto.");
    ...
    <div className="flex w-full flex-col gap-3 rounded-md border border-primary/10 bg-white p-3 dark:bg-[#121212]">
      <div className="grid w-full grid-cols-1 gap-2 lg:grid-cols-[1fr_120px_120px_auto] lg:items-end">
        <SelectVirtualizedInput<TEquipmentDTO>
          ...
          selectedItemLabel="NAO DEFINIDO"
          options={inverters.map((equipment) => ({
            id: equipment._id,
        ...
        <NumberInput
          ...
          handleChange={(value) => setInverterHolder((prev) => ({ ...prev, qtde: Number(value) }))}
          placeholder="QTDE"
          width="100%"
        />
        <NumberInput
          ...
          handleChange={(value) =>
            setInverterHolder((prev) => ({ ...prev, garantia: Number(value) }))
          }
          placeholder="ANOS"
          width="100%"
        />
        <Button type="button" onClick={() => addCatalogProduct("inverter")}>
          ADICIONAR
        </Button>
      </div>
      ...
      <div className="grid w-full grid-cols-1 gap-2 lg:grid-cols-[1fr_120px_120px_auto] lg:items-end">
        ...
        <NumberInput
          ...
          handleChange={(value) => setModuleHolder((prev) => ({ ...prev, qtde: Number(value) }))}
          placeholder="QTDE"
          width="100%"
        />
        <NumberInput
          ...
          handleChange={(value) =>
            setModuleHolder((prev) => ({ ...prev, garantia: Number(value) }))
          }
          placeholder="ANOS"
          width="100%"
        />
        <Button type="button" onClick={() => addCatalogProduct("module")}>
          ADICIONAR
        </Button>
      </div>
      ...
      <div className="grid w-full grid-cols-1 gap-2 lg:grid-cols-[160px_1fr_1fr_120px_100px_100px_120px_auto] lg:items-end">
        <SelectInput
          ...
          value={customProduct.categoria}
          handleChange={(value) =>
            setCustomProduct((prev) => ({ ...prev, categoria: value as TProductItem["categoria"] }))
          }
          onReset={() => setCustomProduct((prev) => ({ ...prev, categoria: "OUTROS" }))}
          width="100%"
        />
        <TextInput
          label="FABRICANTE"
          placeholder="Fabricante"
          value={customProduct.fabricante}
          handleChange={(value) => setCustomProduct((prev) => ({ ...prev, fabricante: value }))}
          width="100%"
        />
        <TextInput
          label="MODELO"
          placeholder="Modelo"
          value={customProduct.modelo}
          handleChange={(value) => setCustomProduct((prev) => ({ ...prev, modelo: value }))}
          width="100%"
        />
        <NumberInput
          label="POTÊNCIA"
          value={customProduct.potencia || 0}
          handleChange={(value) =>
            setCustomProduct((prev) => ({ ...prev, potencia: Number(value) }))
          }
          placeholder="W"
          width="100%"
        />
        <NumberInput
          label="QTDE"
          value={customProduct.qtde}
          handleChange={(value) => setCustomProduct((prev) => ({ ...prev, qtde: Number(value) }))}
          placeholder="QTDE"
          width="100%"
        />
        <NumberInput
          label="GARANTIA"
          value={customProduct.garantia}
          handleChange={(value) =>
            setCustomProduct((prev) => ({ ...prev, garantia: Number(value) }))
          }
          placeholder="ANOS"
          width="100%"
        />
        <NumberInput
          label="VALOR"
          value={customProduct.valor || 0}
          handleChange={(value) => setCustomProduct((prev) => ({ ...prev, valor: Number(value) }))}
          placeholder="0,00"
          width="100%"
        />
...
function ServiceCompositionMenu({ services, updateServices }: ServiceCompositionMenuProps) {
  ...
  function addService() {
    if (serviceHolder.descricao.trim().length < 2)
      return toast.error("Preencha uma descrição de serviço válida.");
    ...
    <div className="flex w-full flex-col gap-2 rounded-md border border-primary/10 bg-white p-3 dark:bg-[#121212]">
      <div className="grid w-full grid-cols-1 gap-2 lg:grid-cols-[1fr_120px_140px_auto] lg:items-end">
        ...
        <NumberInput
          label="VALOR"
          placeholder="0,00"
          value={serviceHolder.valor || 0}
          handleChange={(value) => setServiceHolder((prev) => ({ ...prev, valor: value }))}
          width="100%"
        />
      ...
      <TextareaInput
        ...
        placeholder="Observações, escopo ou particularidades do serviço..."
      />
    </div>
  );
}
