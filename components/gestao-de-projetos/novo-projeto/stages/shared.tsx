...
export function StageSection({
  ...
  return (
    <section
      className={cn(
        "flex w-full flex-col gap-3 rounded-md border border-primary/15 bg-background p-3",
        className,
      )}
    >
      <div className="flex w-full items-center justify-between gap-3 border-b border-primary/10 pb-2">
        <h2 className="text-xs font-black uppercase tracking-wide text-foreground">{title}</h2>
      </div>
      {children}
    </section>
  );
