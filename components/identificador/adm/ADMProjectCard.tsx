      ...
      <div className="flex flex-col p-2">
        ...
        <div className="flex items-center justify-between pb-2">
          <div className="flex flex-col items-start gap-1">
            ...
            <p
              className={`rounded border p-1 text-xs font-black ${project.pagamento?.cobrancaFeita ? 'border border-green-500 text-green-500' : 'border border-red-500 text-red-500'}`}
            >
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <p className={`text-primary/60 rounded p-1 text-sm font-bold`}>
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            <p
              className={`rounded border p-1 text-xs font-black ${project.faturamento?.concluido ? 'border border-green-500 text-green-500' : 'border border-red-500 text-red-500'}`}
            >
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-col items-start gap-1">
            ...
            <p className="text-primary/80 text-xs">{project.pagamento?.forma && project.pagamento.forma}</p>
          </div>
          <div className="flex flex-col items-end gap-1">
            <p className="text-xxs text-end">PAGAMENTO DO KIT</p>
            <p className="text-primary/80 text-end text-xs">{project.compra?.statusLiberacao ? project.compra.statusLiberacao : '-'}</p>
          </div>
        </div>
        <div className="mt-1 flex w-full flex-wrap items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <p className="text-primary/60 text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">
              ...
              {project.receita && project.receita?.fracionamento.length > 1 ? 'RECEBIMENTOS' : 'RECEBIMENTO'}
            </p>
          </div>
