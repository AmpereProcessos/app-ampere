		...
		<div className="flex w-full flex-col rounded border border-gray-500 p-3 shadow-sm lg:w-[650px]">
			...
			<div className="flex w-full items-center justify-between">
				...
				<div className="flex items-center gap-1">
					...
					<p className="text-[0.65rem] font-medium leading-none tracking-tight text-gray-500 lg:text-xs">{formatToMoney(inputMaterial.preco)}</p>
				</div>
			</div>
			<h1 className="my-2 text-[0.65rem] font-bold leading-none tracking-tight text-gray-500 lg:text-xs">INFORMAÇÕES DE ENTRADA</h1>
			...
			{vinculationMaterial ? (
				<div className="mt-2 flex flex-col gap-2 rounded border border-green-300 bg-green-100 p-3">
					...
					<div className="flex w-full items-center justify-between">
						...
						<div className="flex items-center gap-1">
							<ImPriceTag color="rgb(34,197,94)" />
							<p className="text-[0.65rem] font-medium leading-none tracking-tight text-gray-500 lg:text-xs">{formatToMoney(vinculationMaterial.preco)}</p>
							<FaLongArrowAltRight size={10} />
							<p className="text-[0.65rem] font-medium leading-none tracking-tight text-gray-500 lg:text-xs">
								{formatToMoney(
									getNewPrice({
					...
					{vinculationMaterial.grandeza && vinculationMaterial.grandeza != infoHolder.grandeza ? (
						<p className="rounded border-orange-500 bg-orange-100 p-1 text-center text-[0.65rem] font-medium tracking-tight text-orange-500">
							Oops, notamos uma divergência nas grandezas. O material atual está em <strong>{vinculationMaterial.grandeza || "UN"}</strong> , e a entrada está em{" "}
							<strong>{infoHolder.grandeza}</strong>. Verifique as informações e, se necessário, faça ajustes de quantidade e preço.
						</p>
					) : null}
