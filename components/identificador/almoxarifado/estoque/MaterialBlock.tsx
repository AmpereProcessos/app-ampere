...
function renderStatus({ qty, minQty, maxQty }: { qty: number; minQty: number; maxQty: number }) {
	if (qty < minQty) return <h1 className="min-w-fit rounded-full bg-red-600 px-2 py-1 text-[0.58rem] font-medium text-white lg:text-xs">ABAIXO DO MÍNIMO</h1>;

	if (maxQty && qty > maxQty) return <h1 className="min-w-fit rounded-full bg-red-600 px-2 py-1 text-[0.58rem] font-medium text-white lg:text-xs">ACIMA DO MÁXIMO</h1>;

	if (qty < 10) return <h1 className="min-w-fit rounded-full bg-orange-600 px-2 py-1 text-[0.58rem] font-medium text-white lg:text-xs">BAIXA QUANTIDADE</h1>;
	...
	return (
		<div className="flex w-full flex-col rounded-md border border-gray-300 p-3">
			<div className="flex w-full items-center justify-between gap-2">
				<div className="flex w-full items-center justify-between gap-2">
					<h1 className="cursor-pointer text-xs font-black leading-none tracking-tight hover:text-cyan-500 lg:text-sm">{material.nome || "SEM NOME"}</h1>
				</div>
			...
			<div className="mt-2 flex w-full items-center justify-end gap-4">
				...
				<button
					...
					className='enabled:hover:text-white" whitespace-nowrap rounded bg-gray-900 px-4 py-1 text-sm font-medium text-white shadow disabled:bg-gray-500 disabled:text-white enabled:hover:bg-gray-800'
				>
					SALVAR
				</button>
			</div>
