...
function renderStatus({ qty, minQty, maxQty }: { qty: number; minQty: number; maxQty: number }) {
  if (qty < minQty)
    return (
      <h1 className="min-w-fit rounded-full bg-red-600 px-2 py-1 text-[0.58rem] font-medium text-white lg:text-xs">
        ABAIXO DO MÍNIMO
      </h1>
    );
  ...
  if (qty < 10)
    return (
      <h1 className="min-w-fit rounded-full bg-orange-600 px-2 py-1 text-[0.58rem] font-medium text-white lg:text-xs">
        BAIXA QUANTIDADE
      </h1>
    );
...
function MaterialBlock({ material }: MaterialBlockProps) {
  ...
  return (
    <div className="border-border flex w-full flex-col rounded-md border p-3">
      <div className="flex w-full items-center justify-between gap-2">
        <div className="flex w-full items-center justify-between gap-2">
          <h1 className="cursor-pointer text-xs leading-none font-black tracking-tight hover:text-cyan-500 lg:text-sm">
            {material.nome || "SEM NOME"}
          </h1>
        </div>
        {renderStatus({
          qty: infoHolder.qtde,
          minQty: infoHolder.qtdeMinima || 0,
          maxQty: infoHolder.qtdeMaxima || 0,
        })}
      </div>
      <p className="text-foreground w-full text-start text-xs font-medium tracking-tight">
        {material.nomeTecnico || "NOME TÉCNICO NÃO DEFINIDO"}
      </p>
          ...
          <div className="flex items-center gap-2">
            <div className={"flex items-center gap-1"}>
              ...
              <p className="text-foreground text-xs font-medium">
                {formatDateAsLocale(material.alteracao?.dataAlteracao, true) || "N/A"}
              </p>
            </div>
            <div className={"flex items-center gap-1"}>
              <Avatar
              ...
              <p className="text-foreground text-xs font-medium">
                Atualizado por: {material.alteracao.nome}
              </p>
            </div>
          </div>
        ) : null}
