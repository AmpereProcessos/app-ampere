...
function MaterialSelector({
  ...
  renderUnselectedMaterial,
}: MaterialSelectorProps) {
  console.log("renderUnselectedMaterial:", renderUnselectedMaterial);
  const [stateHolder, setStateHolder] = useState<TMaterialState>(
    initialMaterialState || { materialId: null, materialName: "" },
  );
  ...
  const {
    ...
    isStale,
    error,
  } = useVinculationMaterialsBySearch(debouncedQueryParams);
    ...
    return (
      <Dialog open={open} onOpenChange={setOpen}>
          ...
          selectedMaterial ? (
            renderSelectedMaterial && typeof renderSelectedMaterial === "function" ? (
              ...
              <div className="flex w-fit items-center gap-2 self-center rounded-lg bg-green-100 px-2 py-1 dark:bg-green-800">
                <h1 className="text-primary text-sm font-medium tracking-tight">
            ...
            <button
              ...
              className="flex w-fit items-center gap-1 self-center rounded bg-cyan-500 px-4 py-1.5 text-white hover:bg-cyan-700"
            >
              <Link size={12} />
              <h1 className="text-[0.7rem] font-medium tracking-tight">
                CLIQUE AQUI PARA VINCULAR UM ATIVO
              </h1>
            </button>
          )
        ) : (
          <div className="bg-primary/60 flex w-fit items-center gap-1 self-center rounded px-4 py-1.5 text-white">
        ...
        {/* <DialogTrigger asChild>
						<Button
							variant="outline"
							className="w-full rounded-md border border-border p-3 text-xs shadow-xs outline-hidden duration-500 ease-in-out placeholder:italic focus:border-primary"
						>
							{selectedMaterial ? (
								<div className="flex items-center gap-1">
									...
									<div className="flex items-center gap-1">
										<Ruler width={15} height={15} />
										<h1 className="py-0.5 text-center text-[0.6rem] font-medium italic text-foreground">{selectedMaterial.grandeza}</h1>
									</div>
									<div className="flex items-center gap-1">
										<Warehouse width={15} height={15} />
										<h1 className="py-0.5 text-center text-[0.6rem] font-medium italic text-foreground">{selectedMaterial.alocadorDados?.nome || "N/A"}</h1>
									</div>
        ...
        <DialogContent
          className="scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex h-[60%] w-[40%] flex-col gap-2 overflow-y-auto overscroll-y-auto p-3"
        ...
        >
          <MaterialList
            ...
            updateMaterialDescription={(value) =>
              setStateHolder((prev) => ({ ...prev, materialName: value }))
            }
  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      ...
      <DrawerContent
        onClick={(e) => e.stopPropagation()}
        className="flex h-[80%] max-h-[80%] w-full flex-col gap-2 p-3"
      >
        <MaterialList
          vinculatedMaterialId={stateHolder?.materialId}
          materials={materials || []}
    ...
    <>
      ...
      <div className="flex w-full flex-col gap-1">
        <div className="flex w-full items-center justify-center gap-2">
          <Box size={15} />
          <h1 className="text-foreground text-center text-sm font-medium tracking-tight">
            MENU DE MATERIAIS
          </h1>
        </div>
        <input
      ...
      <div className="bg-primary/30 my-2 h-px w-full" />
      {materialsIsLoading ? (
        <h1 className="text-foreground animate-pulse text-center text-xs font-medium italic">
          Carregando...
        </h1>
      ) : null}
      ...
      {materialsIsSuccess ? (
        <div className="scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex w-full grow flex-col gap-2 overflow-y-auto overscroll-y-auto">
          {materials.length > 0 ? (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    <Warehouse width={15} height={15} />
                    <h1 className="text-foreground py-0.5 text-center text-[0.6rem] font-medium italic">
                      {material.alocadorDados?.nome || "N/A"}
                    </h1>
                  </div>
                </div>
                <button
            ...
            <p className="text-foreground text-center text-xs font-medium italic">
...
function NewMaterialMenu({ initialName }: NewMaterialMenuProps) {
  ...
  return (
    <div className="flex w-full grow flex-col items-center justify-center gap-1">
      <h1 className="text-foreground w-full text-center text-xs font-medium tracking-tight">
        Adicione um novo material ao estoque.
      </h1>
      <h1 className="bg-primary text-primary-foreground rounded-lg px-2 py-1 text-[0.65rem]">
        {initialName}
      </h1>
      ...
      <LoadingButton
        loading={isPending}
        onClick={() => handleCreateMaterial({ info: { ...newMaterial, nome: initialName } })}
        size={"xs"}
      >
        CRIAR MATERIAL
      </LoadingButton>
