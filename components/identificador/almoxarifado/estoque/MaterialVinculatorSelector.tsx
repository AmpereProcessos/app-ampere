		...
		return (
			<Dialog open={open} onOpenChange={setOpen}>
				...
				{/* <DialogTrigger asChild>
						<Button
							variant="outline"
							className="w-full rounded-md border border-primary/20 p-3 text-xs shadow-xs outline-hidden duration-500 ease-in-out placeholder:italic focus:border-primary"
						>
							{selectedMaterial ? (
								<div className="flex items-center gap-1">
				...
				<Button variant="outline" className="w-full">
					{selectedMaterial ? (
						<div className="flex items-center gap-1">
							...
							<div className="flex items-center gap-1">
								<Warehouse width={15} height={15} />
								<h1 className="text-primary/80 py-0.5 text-center text-[0.6rem] font-medium italic">{selectedMaterial.alocadorDados?.nome || "N/A"}</h1>
							</div>
							<div className={cn("flex items-center justify-center rounded-full bg-green-600 p-1 text-xs font-medium text-white duration-300 ease-in-out")}>
								<Link size={12} />
							</div>
...
function MaterialList({
	...
	return (
		<>
			...
			{materialsIsLoading ? <h1 className="text-primary/80 animate-pulse text-center text-xs font-medium italic">Carregando...</h1> : null}
...
function NewMaterialMenu({ initialName }: NewMaterialMenuProps) {
	...
	return (
		<div className="flex w-full grow flex-col items-center justify-center gap-1">
			...
			<h1 className="bg-primary text-primary-foreground rounded-lg px-2 py-1 text-[0.65rem]">{initialName}</h1>
