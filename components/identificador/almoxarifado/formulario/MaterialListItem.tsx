			...
			<div className="flex w-full flex-col items-center justify-between gap-1 rounded border border-gray-300 p-2 lg:flex-row">
				<div className="flex w-full flex-row gap-1 lg:w-[40%] lg:flex-col lg:gap-0">
					...
					<div className="flex items-center gap-1">
						...
						{!isFormularyFinished ? (
							<button onClick={() => removeMaterial({ id: material.id, index })} className="rounded-lg border border-red-500 bg-red-100 p-1 text-center text-xxs font-medium text-red-500">
								EXCLUIR
							</button>
						) : null}
						{isFormularyFinished && allowPostFinishEditing ? (
							<button onClick={() => setEditMaterialMenuIsOpen(true)} className="rounded-lg border border-orange-500 bg-orange-100 p-1 text-center text-xxs font-medium text-red-500">
								EDITAR
							</button>
						) : null}
					</div>
				...
				<div className="flex w-full items-center gap-1 lg:w-[60%]">
					<div className="w-[50%]">
						<input
							...
							id={"qtdeRetirada"}
							type="number"
							className="h-full w-full rounded-md border border-gray-300 bg-gray-100 p-3 text-xs outline-none placeholder:italic disabled:bg-gray-400"
						/>
					</div>
					<div className="w-[50%]">
						<input
