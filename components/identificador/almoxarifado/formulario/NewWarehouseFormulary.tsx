			...
			{vinculateClient ? (
				<>
					<p className="text-primary/60 w-full py-2 text-center font-medium tracking-tight">
						...
						<button type="button" onClick={() => setVinculateClient(false)} className="cursor-pointer text-[#fead41]">
							clique aqui
						</button>{" "}
				...
				<>
					<p className="text-primary/60 w-full py-2 text-center font-medium tracking-tight">
						Preencha um titulo para esse formulário de saída de materiais para futura identificação e filtro. Caso o formulário estiver relacionado a um
						cliente,{" "}
						<button type="button" onClick={() => setVinculateClient(true)} className="cursor-pointer text-[#fead41]">
							clique aqui
						</button>
					</p>
					<TextInput
