...
function ViewWarehouseFormularyDraft({
  formularyId,
  session,
  closeModal,
  callbacks,
}: ViewWarehouseFormularyDraftProps) {
  ...
  return (
    <ResponsiveDialogDrawer
      ...
      actionFunction={() =>
        handleUpdate({
          ...
          warehouseFormulary: { ...infoHolder, dataInsercao: new Date().toISOString() },
        })
      }
    ...
    >
      ...
      <InformationItem
        icon={<Text className="h-4 w-4 min-h-4 min-w-4" />}
        label="TÍTULO"
        value={infoHolder.titulo}
      />

      <InformationItem
        icon={<Diamond className="h-4 w-4 min-h-4 min-w-4" />}
        label="MATERIAIS"
        value={infoHolder.categoria}
      />
      <InformationItem
        icon={<UsersRound className="h-4 w-4 min-h-4 min-w-4" />}
        label="RESPONSÁVEIS"
      ...
      <InformationItem
        ...
        label="PROJETO"
        value={infoHolder.projeto.nome || "N/A"}
      />
      <InformationItemLists
        icon={<Box className="h-4 w-4 min-h-4 min-w-4" />}
        label="MATERIAIS"
        value={infoHolder.materiais.map(
          (material) => `- ${material.qtdeRetirada}x ${material.nome}`,
        )}
      />
    </ResponsiveDialogDrawer>
  );
  ...
  return (
    <div className="w-full flex flex-col gap-1">
      <div className="w-full flex items-center gap-1.5">
        {icon}
        <h3 className="text-sm font-bold tracking-tighter text-foreground">{label}</h3>
      </div>
      <h3 className="text-sm tracking-tight">{value}</h3>
    </div>
  );
}
function InformationItemLists({
  icon,
  label,
  value,
}: {
