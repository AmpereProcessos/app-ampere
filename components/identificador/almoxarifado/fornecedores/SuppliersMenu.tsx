...
export function MaterialSuppliersHolder({
  session,
  applicableSuppliersIds,
  handleAddSupplier,
  handleRemoveSupplier,
}: MaterialSuppliersHolderProps) {
...
function SuppliersMenuContent({
  session,
  suppliers,
  applicableSuppliersIds,
  handleAddSupplier,
  handleRemoveSupplier,
}: SuppliersMenuContentProps) {
  ...
  const { mutate: mutationCreateSupplier, isPending: isCreateSupplierLoading } = useMutation({
    mutationKey: [
      "create-supplier",
      `${infoHolder.nome}-${infoHolder.cores.primaria}-${infoHolder.cores.secundaria}`,
    ],
  ...
  function getFilteredSuppliers(suppliers: TMaterialSupplierDTO[]) {
    return {
      ...
      available: suppliers.filter(
        (p) =>
          !applicableSuppliersIds.includes(p._id) &&
          formatWithoutDiacritics(p.nome, true).includes(
            formatWithoutDiacritics(infoHolder.nome, true),
          ),
      ),
    };
  }
          ...
          {available.length > 0 ? (
            ...
            <div className="flex w-full grow flex-col items-center justify-center gap-2 p-2">
              <p className="text-xs font-medium text-foreground">
                Oops, não encontramos nenhum fornecedor. Se precisar, crie um preenchendo os dados
                abaixo.
              </p>
              <div
                style={{
