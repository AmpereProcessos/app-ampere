...
export function MaterialSuppliersHolder({ session, applicableSuppliersIds, handleAddSupplier, handleRemoveSupplier }: MaterialSuppliersHolderProps) {
	...
	return (
		<>
			{isLoading ? <h1 className="text-sm tracking-tight text-primary/80 animate-pulse">Carregando fornecedores...</h1> : null}
					...
					{suppliers
						...
						.map((supplier) => (
							...
							>
								...
								<button type="button" onClick={() => handleRemoveSupplier(supplier._id)} className="items-center justify-center opacity-0 duration-300 ease-in-out group-hover:opacity-100">
...
function SuppliersMenuContent({ session, suppliers, applicableSuppliersIds, handleAddSupplier, handleRemoveSupplier }: SuppliersMenuContentProps) {
	...
	function getFilteredSuppliers(suppliers: TMaterialSupplierDTO[]) {
		return {
			...
			available: suppliers.filter((p) => !applicableSuppliersIds.includes(p._id) && formatWithoutDiacritics(p.nome, true).includes(formatWithoutDiacritics(infoHolder.nome, true))),
		};
	}
				...
				<div className="w-full flex flex-wrap gap-2">
					{available.length > 0 ? (
						...
						<div className="flex w-full grow flex-col items-center justify-center gap-2 p-2">
							<p className="text-xs text-primary/80 font-medium">Oops, não encontramos nenhum fornecedor. Se precisar, crie um preenchendo os dados abaixo.</p>
							<div
								style={{
									border: "1px solid",
