...
import type { TTechnicalAnalysis, TTechnicalAnalysisDTO } from "@/utils/schemas/technical-analysis";
...
function AnalysisVinculationMenu({
  infoHolder,
  setInfoHolder,
  closeMenu,
}: AnalysisVinculationMenuProps) {
  const {
    ...
    isSuccess,
    error,
  } = useOpportunityTechnicalAnalysis({
  ...
  return (
    <AnimatePresence>
      ...
      >
        <h1 className="w-full rounded-md bg-orange-700 p-1 text-center font-medium text-white">
          VINCULAÇÃO DE ANÁLISE EXISTENTE
        </h1>
...
function TechnicalAnalysisVinculationCard({
  analysis,
  selectedId,
  handleClick,
}: TechnicalAnalysisVinculationCardProps) {
  return (
    <div className="border-border flex w-full items-center rounded-md border">
      <div className="flex grow flex-col p-3">
        <div className="flex w-full items-center justify-between gap-2">
          <h1 className="font-raleway leading-none font-bold tracking-tight">{analysis.nome}</h1>
          {selectedId == analysis._id ? (
            <button className="rounded-full bg-green-500 px-2 py-1 text-xs font-bold text-white">
              SELECIONADO
            </button>
          ) : (
            <button
              onClick={() => handleClick(analysis)}
              className="rounded-full bg-cyan-500 px-2 py-1 text-xs font-bold text-white"
            >
              SELECIONAR ANÁLISE
            </button>
          )}
        ...
        <div className="mt-1 flex w-full items-center justify-between">
          <div className="flex items-center gap-2">
            <Avatar
              ...
              fallback={formatNameAsInitials(
                analysis.requerente.nome || analysis.requerente.apelido,
              )}
              height={25}
              width={25}
            />
