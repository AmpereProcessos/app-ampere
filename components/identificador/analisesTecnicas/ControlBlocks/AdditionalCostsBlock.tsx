...
function getSaleValue(totalCost: number, addTaxes: boolean) {
  const saleValue = addTaxes
    ? totalCost / (1 - (STANDARD_PROFIT_MARGIN + STANDARD_TAX))
    : totalCost / (1 - STANDARD_PROFIT_MARGIN);
  return saleValue;
}
...
function AdditionalCostsBlock({
  infoHolder,
  setInfoHolder,
  changes,
  setChanges,
}: AdditionalCostsBlockProps) {
  // Costs holder
        ...
        <div className="mt-2 flex w-full flex-col items-center gap-2 lg:flex-row">
          ...
          <div className="w-full lg:w-1/5">
            <NumberInput
              ...
              value={
                costHolder.qtde && costHolder.custoUnitario
                  ? costHolder.qtde * costHolder.custoUnitario
                  : null
              }
          ...
          <div className="w-full lg:w-1/5">
            <NumberInput
              ...
              handleChange={(value) =>
                setCostHolder((prev) => ({ ...prev, totalVendaSimples: value }))
              }
              width={"100%"}
            />
          </div>
          <div className="w-full lg:w-1/5">
            <NumberInput
              ...
              handleChange={(value) =>
                setCostHolder((prev) => ({ ...prev, totalVendaFaturavel: value }))
              }
              width={"100%"}
            />
          </div>
        ...
        {infoHolder.custos?.length > 0 ? (
          <div className="mt-2 flex w-full flex-col gap-2">
            {infoHolder.custos.map((cost, index) => (
              <div key={index} className="border-border flex w-full flex-col rounded-md border p-3">
                <div className="flex w-full justify-between">
                  <h1 className="text-start leading-none font-bold tracking-tight">
                    {cost.categoria}
                  </h1>
                ...
                <p className="text-foreground text-sm">{cost.descricao}</p>
                <div className="flex w-full items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2 text-green-500">
                      ...
                      <p className="text-foreground text-sm font-medium">
                        {cost.custoUnitario ? formatToMoney(cost.custoUnitario) : "R$ 0,00"} /{" "}
                        {cost.grandeza}
                      </p>
                    </div>
                    {cost.totalVendaSimples ? (
                      <div className="flex items-center gap-2 text-green-500">
                        ...
                        <p className="text-foreground text-sm font-medium">
                          VENDA SIMPLES: {formatToMoney(cost.totalVendaSimples)}
                        </p>
                      </div>
                    ) : null}
                    {cost.totalVendaFaturavel ? (
                      <div className="flex items-center gap-2 text-green-500">
                        <ImPriceTag color="rgb(34,197,94)" />
                        <p className="text-foreground text-sm font-medium">
                          VENDA FATURÁVEL: {formatToMoney(cost.totalVendaFaturavel)}
                        </p>
                      </div>
                    ) : null}
                  </div>
