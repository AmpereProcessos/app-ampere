...
function DescriptiveBlock({
  ...
  changes,
  setChanges,
}: DescriptiveBlockProps) {
  const [descriptiveHolder, setDescriptiveHolder] = useState({
    topico: "",
    descricao: "",
  });
  function addDescriptiveItem() {
    if (descriptiveHolder.topico.trim().length < 2)
      return toast.error("Preencha um tópico de ao menos 2 caracteres.");
    if (descriptiveHolder.descricao.trim().length < 3)
      return toast.error("Preencha um tópico de ao menos 3 caracteres.");
    ...
    <div className="mt-4 flex w-full flex-col">
      ...
      <div className="mt-2 flex w-full flex-col gap-2">
        ...
        <div className="w-full">
          <TextInput
            ...
            handleChange={(value) =>
              setDescriptiveHolder((prev) => ({ ...prev, descricao: value }))
            }
            width="100%"
          />
        </div>
      ...
      {infoHolder.descritivo?.length > 0 ? (
        infoHolder.descritivo.map((item, index) => (
          <div
            key={index}
            className="border-border mb-1 flex w-full flex-col rounded border p-2 shadow-xs"
          >
            <div className="flex w-full items-center justify-between">
              <div className="flex w-full items-center justify-center gap-2">
                <MdTopic />
                <h1 className="text-lg leading-none font-bold tracking-tight text-[#15599a]">
                  {item.topico}
                </h1>
              </div>
              <button
            ...
            <p className="text-foreground mt-1 w-full text-center text-sm">{item.descricao}</p>
          </div>
        ))
      ) : (
