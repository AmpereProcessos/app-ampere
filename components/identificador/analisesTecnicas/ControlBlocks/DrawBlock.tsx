...
function renderInputText(file: File | null) {
  if (!file)
    return (
      <p className="text-sm1 text-foreground mb-2 px-2 text-center dark:text-gray-400">
        <span className="font-semibold">Clique para escolher um arquivo</span> ou o arraste para a
        àrea demarcada
      </p>
    );

  return <p className="text-foreground mb-2 text-sm dark:text-gray-400">{file.name}</p>;
}
      ...
      <div className="flex w-full flex-col">
        <div className="w-full self-center lg:w-1/2">
          <SelectInput
            ...
            onReset={() =>
              setInfoHolder((prev) => ({ ...prev, desenho: { ...prev.desenho, tipo: null } }))
            }
          />
        </div>
        <div className="mt-2 flex w-full flex-col">
          ...
          <textarea
            ...
            onChange={(e) => {
              setInfoHolder((prev) => ({
                ...prev,
                desenho: prev.desenho
                  ? { ...prev.desenho, observacoes: e.target.value }
      ...
      <div className="mt-2 flex w-full flex-col gap-2">
        {previewImageURL || infoHolder.desenho.url ? (
          <div className="flex w-full flex-col items-center gap-2">
            ...
            {!!infoHolder.desenho.url ? (
              <h1 className="text-center leading-none font-bold tracking-tight">IMAGEM</h1>
            ) : null}
          ...
          <div className="flex h-[300px] w-full items-center justify-center">
            <p className="text-foreground text-sm font-medium italic">
              Nenhuma imagem de desenho vinculada.
            </p>
          </div>
        )}
        <div className="relative flex w-full items-center justify-center">
          <label
            ...
            className="dark:hover:bg-bray-800 dark:border-primary/80 dark:hover:bg-primary/80 border-border dark:hover:border-primary/60 hover:bg-primary/20 dark:bg-primary/70 flex h-32 w-full cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed bg-gray-50"
          >
