...
import type { TEquipment, TTechnicalAnalysis, TTechnicalAnalysisDTO } from "@/utils/schemas/technical-analysis";
...
function EquipmentBlock({ infoHolder, setInfoHolder, changes, setChanges }: EquipmentBlockProps) {
	...
	const [equipmentsType, setEquipmentsType] = useState<"modules" | "inverters" | "personalized">("modules");
	...
	function addModuleToEquipments(data: TModule) {
		const newEquipmentsList = [...infoHolder.equipamentos, { ...data, categoria: "MÓDULO" as const }];
	...
	function addInverterToEquipments(data: TInverter) {
		const newEquipmentsList = [...infoHolder.equipamentos, { ...data, categoria: "INVERSOR" as const }];
		setInfoHolder((prev) => ({ ...prev, equipamentos: newEquipmentsList }));
		setChanges((prev) => ({ ...prev, equipamentos: newEquipmentsList }));
	}
			...
			<AnimatePresence>
				{editEnabled ? (
					<motion.div key={"editor"} variants={variants} initial="hidden" animate="visible" exit="exit" className="mt-2 flex w-full flex-col gap-2">
						...
						{equipmentsType === "modules" ? <ModulesBlock addModuleToEquipments={addModuleToEquipments} /> : null}
						{equipmentsType === "inverters" ? <InvertersBlock addInverterToEquipments={addInverterToEquipments} /> : null}
						{equipmentsType === "personalized" ? (
							<PersonalizedProductsBlock addPersonalizedProductToEquipments={addPersonalizedProductToEquipments} />
						) : null}
					</motion.div>
				) : null}
				<div className="mt-4 flex w-full flex-col flex-wrap justify-around gap-4 lg:flex-row">
					{infoHolder.equipamentos.length > 0 ? (
						infoHolder.equipamentos.map((equipment, index) => (
							<div key={`${equipment.id}-${index.toString()}`} className="border-primary/20 mt-1 flex w-full flex-col rounded-md border p-2 lg:w-[350px]">
				...
				<div className="w-full lg:w-2/4">
					<SelectInput
						...
						handleChange={(selectedId) => {
							...
							return updateModule({ id: "", fabricante: "", modelo: "", potencia: 0, qtde: 1, garantia: 10 });
						}}
						onReset={() => updateModule({ id: "", fabricante: "", modelo: "", potencia: 0, qtde: 1, garantia: 10 })}
						options={modules?.map((module) => ({ id: module._id, label: `${module.fabricante} - ${module.modelo}`, value: module._id })) || []}
				...
				<div className="w-full lg:w-2/4">
					<SelectInput
						...
						handleChange={(selectedId) => {
							...
							return updateInverter({ id: "", fabricante: "", modelo: "", potencia: 0, qtde: 1, garantia: 10 });
						}}
						onReset={() => updateInverter({ id: "", fabricante: "", modelo: "", potencia: 0, qtde: 1, garantia: 10 })}
						options={inverters?.map((inverter) => ({ id: inverter._id, label: `${inverter.fabricante} - ${inverter.modelo}`, value: inverter._id })) || []}
						selectedItemLabel="NÃO DEFINIDO"
						width="100%"
					/>
				</div>
...
function PersonalizedProductsBlock({ addPersonalizedProductToEquipments }: PersonalizedProductsBlockProps) {
