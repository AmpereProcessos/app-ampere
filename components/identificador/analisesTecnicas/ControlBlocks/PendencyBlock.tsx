...
function PendencyBlock({
  ...
  const {
    ...
    isError,
    isSuccess,
  } = useActivitiesByTechnicalAnalysisId({ technicalAnalysisId: technicalAnalysisId });
  return (
    <div className="mt-4 flex w-full flex-col">
      ...
      <div className="mt-2 flex w-full flex-col gap-1">
        ...
        {isError ? (
          <ErrorComponent msg="Houve um erro ao buscar atividades da análise técnica." />
        ) : null}
        {isSuccess ? (
          activities.length > 0 ? (
            activities.map((activity, index) => (
              <TechnicalAnalysisActivity
                key={activity._id}
                activity={activity}
                technicalAnalysisId={technicalAnalysisId}
              />
