	...
	async function uploadFiles() {
		...
		try {
			...
			const uploadPromises = Object.keys(files).map(async (key) => {
				const fileObj = files[key];
				var fileRef = ref(storage, `clientes/${requestInfo.nome}-${requestInfo.projeto?.identificador || "NA"}/${fileObj.title}-${(Math.random() * 10000).toFixed(0)}`);
				...
				links.push({
					...
					url: fileUrl,
					formato: uploadResult.metadata.contentType && fileTypes[uploadResult.metadata.contentType] ? fileTypes[uploadResult.metadata.contentType].title : "INDEFINIDO",
				});
			});
					...
					<div className="flex w-full flex-col  bg-[#fff] pb-2">
						...
						<div className="mt-2 flex w-full flex-col gap-2">
							{requestInfo.padrao.map((paInfo, index) => (
								<div key={index} className="flex w-full flex-col border border-gray-300 p-2 shadow-sm">
