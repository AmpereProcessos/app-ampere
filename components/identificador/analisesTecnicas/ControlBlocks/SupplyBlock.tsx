...
function SupplyBlock({ infoHolder, setInfoHolder, changes, setChanges }: SupplyBlockProps) {
  ...
  function handleAddItemsFromKit(itens: TPurchaseControlCompositionKitDTO["itens"]) {
    const newItems: Exclude<TTechnicalAnalysisDTO["suprimentos"], undefined | null>["itens"] =
      itens.map((item) => ({
    ...
    const newItemsList = infoHolder.suprimentos?.itens
      ? [...infoHolder.suprimentos.itens, ...newItems]
      : [...newItems];
    setInfoHolder((prev) => ({
      ...prev,
      suprimentos: prev.suprimentos
        ? { ...prev.suprimentos, itens: newItemsList }
        : { observacoes: "", itens: newItemsList },
    }));
  ...
  function addSupplyItem(
    newItem: Exclude<TTechnicalAnalysisDTO["suprimentos"], undefined | null>["itens"][number],
  ) {
    const newItemsList = infoHolder.suprimentos?.itens
      ? [...infoHolder.suprimentos.itens, newItem]
      : [newItem];
    setInfoHolder((prev) => ({
      ...prev,
      suprimentos: prev.suprimentos
  ...
  function removeSupplyItem(index: number) {
    const newItemsList = infoHolder.suprimentos?.itens
      ? infoHolder.suprimentos.itens.filter((_, i) => i !== index)
      : [];
    setInfoHolder((prev) => ({
      ...prev,
      suprimentos: prev.suprimentos
  ...
  function updateSupplyItem(
    ...
    const newItemsList = infoHolder.suprimentos?.itens
      ? infoHolder.suprimentos.itens.map((_, i) => (i === index ? item : _))
      : [];
    setInfoHolder((prev) => ({
      ...prev,
      suprimentos: prev.suprimentos
          ...
          <textarea
            ...
            onChange={(e) => {
              setInfoHolder((prev) => ({
                ...prev,
                suprimentos: prev.suprimentos
                  ? { ...prev.suprimentos, observacoes: e.target.value }
                  : { observacoes: e.target.value, itens: [] },
              }));
        ...
        <div className="flex w-full flex-wrap items-center justify-end gap-2">
          ...
          <button
            ...
            className={cn(
              ...
              {
                "bg-primary/20 hover:bg-red-300": newSupplyItemMenuIsOpen,
          ...
          >
            ...
            <h1 className="text-xs font-medium tracking-tight">
              {!newSupplyItemMenuIsOpen
                ? "ABRIR MENU DE NOVO ITEM DE SUPRIMENTO"
                : "FECHAR MENU DE NOVO ITEM DE SUPRIMENTO"}
            </h1>
          </button>
        </div>
        <AnimatePresence>
          {newSupplyItemMenuIsOpen && <NewSupplyItemMenu addSupplyItem={addSupplyItem} />}
        </AnimatePresence>
...
type NewSupplyItemMenuProps = {
  addSupplyItem: (
    info: Exclude<TTechnicalAnalysisDTO["suprimentos"], undefined | null>["itens"][number],
  ) => void;
};
function NewSupplyItemMenu({ addSupplyItem }: NewSupplyItemMenuProps) {
  const [itemHolder, setItemHolder] = useState<
    Exclude<TTechnicalAnalysisDTO["suprimentos"], undefined | null>["itens"][number]
  >({
    idMaterial: null,
    descricao: "",
    qtde: 0,
  ...
  function handleAddNewSupplyItem(
    info: Exclude<TTechnicalAnalysisDTO["suprimentos"], undefined | null>["itens"][number],
  ) {
...
function SupplyItemCard({ item, handleUpdate, handleRemove }: SupplyItemCardProps) {
  const [itemHolder, setItemHolder] =
    useState<Exclude<TTechnicalAnalysisDTO["suprimentos"], undefined | null>["itens"][number]>(
      item,
    );
          ...
          <div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
            ...
            <div className="flex items-center gap-1">
              ...
              <h1 className="text-foreground py-0.5 text-center text-[0.6rem] font-medium italic">
                {formatToMoney(item.qtde * (item.preco || 0))}
              </h1>
            </div>
          </div>
          <div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
