	...
	return (
		<div className="bg-background flex w-full grow flex-col px-2">
			...
			<div className="flex w-full grow flex-col gap-2">
				{proposal?.kit ? (
					<div className="border-primary/20 flex w-full flex-col self-center rounded border p-3 shadow-xs lg:w-[60%]">
						...
						<h1 className="text-primary/60 text-xs font-medium">{getModulesStr(proposal.kit.modulos, proposal.kit.tipo)}</h1>
						<div className="flex items-center gap-2">
							<TbTopologyStar />
							<h1 className="text-sm font-medium text-[#fead41]">INVERSORES</h1>
						</div>
						<h1 className="text-primary/60 text-xs font-medium">{getInverterStr(proposal.kit.inversores, proposal.kit.tipo)}</h1>
				...
				<div className="flex w-full flex-col items-center justify-between gap-2 px-1">
					...
					{selectedKits.length > 0 ? (
						<>
							{selectedKits.map((kit, index) => (
								<div key={index} className="flex w-full items-center justify-between pb-1">
									<p className="text-primary/60 italic">{kit.nome}</p>
									<button
										onClick={() => {
											var currentSelectedKitsCopy = [...selectedKits];
						...
						<div className="flex h-[80px] w-full items-center justify-center">
							<p className="text-primary/60 text-sm italic">Sem kits selecionados...</p>
						</div>
					)}
				</div>
