...
import type { TKitDTO } from "@/utils/schemas/kits.schema";
...
function SystemInfo({
  ...
  function handleSearchFilter(value: string) {
    ...
    if (value.trim().length > 0) {
      if (filters.search.trim().length > 0) {
        const filtered = handleOptionFilters();
        const newArr = filtered?.filter((x) => x.nome.toUpperCase().includes(value.toUpperCase()));
        setFilteredKits(newArr);
      }
    } else {
      setFilteredKits(kits);
    ...
    switch (param) {
      case "ASC":
        newArr = dumpyCopyOfKits.sort(
          (a, b) => getPeakPotByModules(a.modulos) - getPeakPotByModules(b.modulos),
        );
        setFilteredKits(newArr);
      ...
      case "DESC":
        newArr = dumpyCopyOfKits.sort(
          (a, b) => getPeakPotByModules(b.modulos) - getPeakPotByModules(a.modulos),
        );
        setFilteredKits(newArr);
  ...
  function formatAndProceed() {
    ...
    setRequestInfo((prev) => ({
      ...
      detalhes: {
        ...prev.detalhes,
        topologia:
          selectedKitsCopy[0].topologia == "MICRO-INVERSOR" ? "MICRO-INVERSOR" : "INVERSOR",
      },
    }));
    goToNextStage();
  }
  ...
  return (
    <div className="bg-background flex w-full grow flex-col px-2">
      <span className="py-2 text-center text-lg font-bold text-[#15599a] uppercase">
        DADOS DO SISTEMA
      </span>
      <div className="flex w-full grow flex-col gap-2">
        {proposal?.kit ? (
          <div className="border-border flex w-full flex-col self-center rounded border p-3 shadow-xs lg:w-[60%]">
            <div className="flex w-full items-center justify-between">
              <h1 className="leading-none font-bold tracking-tight">{proposal.kit.nome}</h1>
              <h1 className="rounded-md border border-blue-500 p-1 text-xs font-medium tracking-tight text-blue-500">
                KIT DA PROPOSTA ATIVA
              </h1>
            </div>
            ...
            <h1 className="text-foreground text-xs font-medium">
              {getModulesStr(proposal.kit.modulos, proposal.kit.tipo)}
            </h1>
            <div className="flex items-center gap-2">
              <TbTopologyStar />
            ...
            <h1 className="text-foreground text-xs font-medium">
              {getInverterStr(proposal.kit.inversores, proposal.kit.tipo)}
            </h1>
        ...
        <div className="flex w-full flex-col items-center justify-between gap-2 px-1">
          ...
          {selectedKits.length > 0 ? (
            <>
              {selectedKits.map((kit, index) => (
                <div key={index} className="flex w-full items-center justify-between pb-1">
                  <p className="text-foreground italic">{kit.nome}</p>
                  <button
                    onClick={() => {
                      var currentSelectedKitsCopy = [...selectedKits];
              ...
              <div className="flex w-full items-center justify-center gap-2">
                <div className="flex flex-col rounded border border-[#15599a] p-2 text-[#15599a]">
                  ...
                  <h1 className="text-center font-medium">
                    {getSelectedKitsPowerPeak(selectedKits).toLocaleString("pt-br", {
            ...
            <div className="flex h-[80px] w-full items-center justify-center">
              <p className="text-foreground text-sm italic">Sem kits selecionados...</p>
            </div>
          )}
        </div>
        <div className="mt-2 flex w-full flex-wrap items-center justify-between gap-1">
          ...
          <div className="flex w-full items-end justify-center gap-2">
            <MultipleSelectInput
              label="FORNECEDORES"
              selected={
                filters.suppliers.length > 0 ? filters.suppliers.map((supplier) => supplier) : null
              }
            ...
            <MultipleSelectInput
              label="TOPOLOGIA"
              selected={
                filters.topology.length > 0 ? filters.topology.map((supplier) => supplier) : null
              }
              options={[
        ...
        <div className="scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex max-h-[500px] grow flex-wrap items-center justify-center gap-2 overflow-y-auto overscroll-y-auto p-2 py-1 lg:justify-between">
