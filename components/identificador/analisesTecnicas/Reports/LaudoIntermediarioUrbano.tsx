...
import { formatDecimalPlaces, formatToMoney, STANDARD_PROFIT_MARGIN, STANDARD_TAX } from '@/utils/constants'
...
function LaudoIntermediarioUrbano({ analysis }: LaudoIntermediarioUrbanoProps) {
  ...
  function getCorrectedGen() {
    ...
    let norte = analysis.alocacaoModulos.norte
      ? (
          (Number(cityFactor['NORTE']) * analysis.alocacaoModulos.norte * getModulesAveragePower(analysis.equipamentos as TProductItem[])) /
          1000
        ).toFixed(2)
      : 0
    let nordeste = analysis.alocacaoModulos.nordeste
      ? (
          (Number(cityFactor['NORDESTE']) * analysis.alocacaoModulos.nordeste * getModulesAveragePower(analysis.equipamentos as TProductItem[])) /
          1000
        ).toFixed(2)
      : 0
    let leste = analysis.alocacaoModulos.leste
      ? (
          (Number(cityFactor['LESTE']) * analysis.alocacaoModulos.leste * getModulesAveragePower(analysis.equipamentos as TProductItem[])) /
          1000
        ).toFixed(2)
      : 0
    let sudeste = analysis.alocacaoModulos.sudeste
      ? (
          (Number(cityFactor['SUDESTE']) * analysis.alocacaoModulos.sudeste * getModulesAveragePower(analysis.equipamentos as TProductItem[])) /
          1000
        ).toFixed(2)
      : 0
    let sul = analysis.alocacaoModulos.sul
      ? ((Number(cityFactor['SUL']) * analysis.alocacaoModulos.sul * getModulesAveragePower(analysis.equipamentos as TProductItem[])) / 1000).toFixed(
          2
        )
      : 0
    let sudoeste = analysis.alocacaoModulos.sudoeste
      ? (
          (Number(cityFactor['SUDOESTE']) * analysis.alocacaoModulos.sudoeste * getModulesAveragePower(analysis.equipamentos as TProductItem[])) /
          1000
        ).toFixed(2)
      : 0
    let oeste = analysis.alocacaoModulos.oeste
      ? (
          (Number(cityFactor['OESTE']) * analysis.alocacaoModulos.oeste * getModulesAveragePower(analysis.equipamentos as TProductItem[])) /
          1000
        ).toFixed(2)
      : 0
    let noroeste = analysis.alocacaoModulos.noroeste
      ? (
          (Number(cityFactor['NOROESTE']) * analysis.alocacaoModulos.noroeste * getModulesAveragePower(analysis.equipamentos as TProductItem[])) /
          1000
        ).toFixed(2)
      : 0
