...
function InstallationInfo({
  ...
  function validateAndProceed() {
    if (infoHolder.distancias.cabeamentoCC.trim().length < 2)
      return toast.error("Por favor, preencha a distância dos módulos até os inversores.");

    if (infoHolder.distancias.cabeamentoCA.trim().length < 2)
      return toast.error("Por favor, preencha a distância dos inversores até os padrão.");
    ...
    if (!files["FOTO DO QUADRO DE DISTRIBUIÇÃO"])
      return toast.error("Anexe uma foto do quadro de distribuição.");
    if (!files["FOTO DO LOCAL DE ATERRAMENTO"])
      return toast.error("Preencha informações sobre o local de aterramento.");
    if (
      infoHolder.detalhes.topologia == "INVERSOR" &&
      !files["FOTO DO LOCAL DE INSTALAÇÃO DO INVERSOR"]
    )
        ...
        <div className="flex w-full flex-col items-center justify-center gap-2 lg:flex-row">
          <div className="w-full lg:w-1/3">
            <DocumentFileInput
              ...
              handleChange={(value) =>
                setFiles((prev) => ({ ...prev, ["FOTO DA FACHADA"]: value }))
              }
            />
          </div>
          <div className="w-full lg:w-1/3">
            <DocumentFileInput
              ...
              handleChange={(value) =>
                setFiles((prev) => ({ ...prev, ["FOTO DO QUADRO DE DISTRIBUIÇÃO"]: value }))
              }
            />
          </div>
          <div className="w-full lg:w-1/3">
            <DocumentFileInput
              ...
              handleChange={(value) =>
                setFiles((prev) => ({
                  ...prev,
                  ["FOTO DO LOCAL DE INSTALAÇÃO DO NOVO DISJUNTOR"]: value,
                }))
              }
            />
          </div>
        ...
        <div className="flex w-full flex-col items-center justify-center gap-2 lg:flex-row">
            ...
            <div className="w-full lg:w-1/2">
              <DocumentFileInput
                ...
                handleChange={(value) =>
                  setFiles((prev) => ({
                    ...prev,
                    ["FOTO DO LOCAL DE INSTALAÇÃO DO INVERSOR"]: value,
                  }))
                }
              />
            </div>
          ...
          <div className="w-full lg:w-1/2">
            <DocumentFileInput
              ...
              handleChange={(value) =>
                setFiles((prev) => ({ ...prev, ["FOTO DO LOCAL DE ATERRAMENTO"]: value }))
              }
            />
          </div>
        </div>
