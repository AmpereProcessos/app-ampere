...
function PAInfo({
  ...
  function validateAndProceed() {
    if (infoHolder.padrao.length == 0)
      return toast.error("Por favor, adicione ao menos um padrão à lista.");

    // VALIDATING FILES
    if (!files["FOTO DO PADRÃO"]) return toast.error("Anexe uma foto do padrão da instalação.");

    if (!files["FOTO DO DISJUNTOR DO PADRÃO"])
      return toast.error("Anexe uma foto do disjuntor do padrão da instalação.");

    if (!files["FOTO DOS CABOS DO PADRÃO"])
      return toast.error("Anexe uma foto dos cabos do padrão da instalação.");

    if (!files["FOTO DO POSTE"])
      return toast.error("Anexe uma foto do poste do padrão da instalação.");
        ...
        <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
          ...
          <div className="w-full lg:w-1/2">
            <DocumentFileInput
              ...
              handleChange={(value) =>
                setFiles((prev) => ({ ...prev, ["FOTO DO DISJUNTOR DO PADRÃO"]: value }))
              }
            />
          </div>
        </div>
        <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
          <div className="w-full lg:w-1/2">
            <DocumentFileInput
              ...
              handleChange={(value) =>
                setFiles((prev) => ({ ...prev, ["FOTO DOS CABOS DO PADRÃO"]: value }))
              }
            />
          </div>
          <div className="w-full lg:w-1/2">
