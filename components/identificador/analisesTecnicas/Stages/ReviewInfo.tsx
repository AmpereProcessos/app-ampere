...
import RoofTiles from "@/utils/images/tipos-telhas.png";
...
function ReviewInfo({
  ...
  function addPersonalizedEquipment() {
    if (personalizedProductHolder.fabricante.trim().length < 3)
      return toast.error("Fabricante do produto não específicado.");
    if (personalizedProductHolder.modelo.trim().length < 3)
  ...
  function addEnergyPA() {
    if (!paInfoHolder.ligacao)
      return toast.error("Por favor, preencha o tipo de ligação do padrão.");
    if (!paInfoHolder.amperagem) return toast.error("Por favor, preencha a amperagem do padrão.");
    if (paInfoHolder.codigoMedidor.trim().length < 5)
      return toast.error("Por favor, preencha um código de medidor válido.");
  ...
  return (
    <div className="bg-background flex w-full grow flex-col px-2">
      <h1 className="w-full rounded-md bg-green-700 px-1 py-2 text-center text-lg font-bold text-white">
        REVISÃO DAS INFORMAÇÕES
      </h1>
      <p className="text-foreground my-2 w-full text-center text-sm leading-none tracking-tight">
        Nessa etapa estão condensadas todas as informações preenchidas ao longo das etapas
        anteriores para que você possa <strong className="text-cyan-500">revisá-las</strong> e então{" "}
        <strong className="text-cyan-500">requisitar</strong> sua análise técnica.
      </p>
      <div className="scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex w-full grow flex-col gap-2 overflow-y-auto overscroll-y-auto px-2 py-1">
        ...
        {/* <p className="w-full text-center text-sm leading-none tracking-tight text-foreground">
        ...
        {/*PA INFORMATION */}
        <h1 className="bg-primary/70 w-full rounded-md p-1 text-center font-medium text-white">
          INFORMAÇÕES DO PADRÃO DE ENERGIA
        </h1>
        <p className="text-foreground my-2 w-full self-center text-center text-sm leading-none tracking-tight lg:w-[60%]">
          Preencha abaixo as informações do padrão de energia instalação e clique em{" "}
          <strong className="font-bold text-green-500">adicionar (+)</strong>. Se necessário, você
          pode adicionar mais de um padrão de energia à lista.
        </p>
        ...
        <div className="flex w-full flex-col justify-around gap-2 lg:flex-row">
          ...
          <div className="w-full lg:w-1/4">
            <SelectInput
              ...
              handleChange={(value) =>
                setPaInfoHolder((prev) => ({ ...prev, modeloCaixaMedidor: value }))
              }
        ...
        <div className="mt-2 flex w-full flex-col flex-wrap items-start justify-around gap-2 lg:flex-row">
          {infoHolder.padrao.length > 0 ? (
            infoHolder.padrao.map((paInfo, index) => (
              <div
                key={index}
                className="border-primary/60 flex w-[450px] flex-col rounded-md border p-2 shadow-xs"
              >
                <div className="flex w-full items-center justify-between gap-1">
                  <div className="flex items-center gap-1">
                    <h1 className="leading-none font-black tracking-tight">
                      PADRÃO {paInfo.ligacao}
                    </h1>
                ...
                <div className="mt-2 flex w-full items-center justify-between gap-2">
                  <div className="flex items-center gap-1">
                    <IoMdBarcode />
                    <p className="text-foreground text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">
                      <strong className="text-cyan-500">Nº DO MEDIDOR:</strong>{" "}
                      {paInfo.codigoMedidor}
                    </p>
                  </div>
                  <div className="flex items-center gap-1">
                    <TbBoxModel2 />
                    <p className="text-foreground text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">
                      <strong className="text-cyan-500">MODELO DA CAIXA:</strong>:{" "}
                      {paInfo.modeloCaixaMedidor || "NÃO DEFINIDO"}
                    </p>
                  </div>
                </div>
                <div className="mt-2 flex w-full flex-wrap items-center justify-around">
                  {paInfo.codigoPosteDerivacao ? (
                    <div className="flex w-1/2 items-center gap-2 lg:w-fit">
                      <IoMdBarcode />
                      <p className="text-foreground text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">
                        <strong className="text-cyan-500">Nº DO POSTE DE DERIVAÇÃO:</strong>{" "}
                        {paInfo.codigoPosteDerivacao}
                      </p>
                    </div>
                  ) : null}
            ...
            <p className="text-foreground w-full text-center text-sm font-medium tracking-tight">
              Nenhum padrão de energia adicionado à lista.
            </p>
          )}
        </div>
        ...
        <div className="w-full self-center lg:w-1/3">
          <CheckboxInput
            ...
            handleChange={(value) =>
              setInfoHolder((prev) => ({
                ...prev,
                transformador: { ...prev.transformador, acopladoPadrao: value },
              }))
            }
          />
        </div>
          ...
          <div className="w-full lg:w-1/2">
            <NumberInput
              ...
              handleChange={(value) =>
                setInfoHolder((prev) => ({
                  ...prev,
                  transformador: { ...prev.transformador, potencia: value },
                }))
              }
            />
          </div>
        ...
        <h1 className="bg-primary/70 w-full rounded-md p-1 text-center font-medium text-white">
          INFORMAÇÕES DA ESTRUTURA DE MONTAGEM
        </h1>
        <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
          <div className="w-full lg:w-1/2">
            <SelectInput
        ...
        <div className="border-border my-3 flex h-[165px] w-[300px] flex-col items-center justify-center self-center border p-1 py-2 lg:h-[250px] lg:w-[450px]">
          ...
          <div className="w-full lg:w-1/3">
            <TextInput
              ...
              handleChange={(value) => {
                setInfoHolder((prev) => ({
                  ...prev,
                  locais: { ...prev.locais, aterramento: value },
                }));
              }}
              width={"100%"}
            />
          ...
          <div className="w-full lg:w-1/3">
            <TextInput
              ...
              handleChange={(value) => {
                setInfoHolder((prev) => ({
                  ...prev,
                  distancias: { ...prev.distancias, cabeamentoCA: value },
                }));
              }}
              width={"100%"}
            />
          ...
          <div className="w-full lg:w-1/3">
            <TextInput
              ...
              handleChange={(value) => {
                setInfoHolder((prev) => ({
                  ...prev,
                  distancias: { ...prev.distancias, cabeamentoCC: value },
                }));
              }}
              width={"100%"}
            />
          ...
          <div className="w-full lg:w-1/3">
            <TextInput
              ...
              handleChange={(value) => {
                setInfoHolder((prev) => ({
                  ...prev,
                  distancias: { ...prev.distancias, conexaoInternet: value },
                }));
              }}
              width={"100%"}
            />
        ...
        <div className="flex w-full flex-col">
          ...
          <textarea
            ...
            className="text-foreground bg-primary/20 min-h-[80px] w-full resize-none rounded-br-sm rounded-bl-sm p-3 text-center text-xs font-medium outline-hidden"
          />
        </div>
      </div>
