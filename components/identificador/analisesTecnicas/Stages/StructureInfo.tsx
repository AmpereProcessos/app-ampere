...
function StructureInfo({
  ...
  function validateAndProceed() {
    if (!infoHolder.detalhes?.tipoEstrutura)
      return toast.error("Preencha a estrutura de montagem.");

    if (!infoHolder.detalhes?.materialEstrutura)
      return toast.error("Preencha o tipo de estrutura.");

    if (infoHolder.detalhes.tipoEstrutura == "TELHADO") {
      if (!infoHolder.detalhes?.tipoTelha)
        return toast.error("Preencha o tipo de telha do cliente.");

      if (!infoHolder.detalhes?.telhasReservas)
        return toast.error("Preencha sobre a existência de telhas reservas.");
    }
    ...
    // FILES
    if (!files["FOTO DO LOCAL DE INSTALAÇÃO"])
      return toast.error("Anexe uma foto do local para instalação.");
    if (!files["FOTO DA ESTRUTURA DE MONTAGEM"])
      return toast.error("Anexe uma foto da estrutura de montagem (laje, solo, etc.).");
    if (!files["FOTO DA ESTRUTURA DE MONTAGEM"])
      return toast.error("Anexe uma foto do local para instalação.");
        ...
        <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
          <div className="w-full lg:w-1/3">
            <DocumentFileInput
              ...
              handleChange={(value) =>
                setFiles((prev) => ({ ...prev, ["FOTO DO LOCAL DE INSTALAÇÃO"]: value }))
              }
            />
          </div>
          <div className="w-full lg:w-1/3">
            <DocumentFileInput
              ...
              handleChange={(value) =>
                setFiles((prev) => ({ ...prev, ["FOTO DA ESTRUTURA DE MONTAGEM"]: value }))
              }
            />
          </div>
          <div className="w-full lg:w-1/3">
            <DocumentFileInput
              ...
              handleChange={(value) =>
                setFiles((prev) => ({ ...prev, ["ESTUDO DE CASO (SE HOUVER)"]: value }))
              }
            />
          </div>
        </div>
