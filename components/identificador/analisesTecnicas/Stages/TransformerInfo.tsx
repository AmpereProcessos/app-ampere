...
function TransformerInfo({
  ...
  function validateAndProceed() {
    if (!infoHolder.transformador.potencia || infoHolder.transformador.potencia <= 0)
      return toast.error("Preencha a potência do transformador.");

    if (!files["FOTO DO TRANSFORMADOR"])
      return toast.error("Anexe uma foto do transformador do cliente.");

    if (!files["FOTO DA LOCALIZAÇÃO DO TRANSFORMADOR"])
      return toast.error("Anexe uma foto da localização do transformador.");

    if (!files["FOTO DO NÚMERO DO TRANSFORMADOR"])
      return toast.error("Anexe uma foto do número do transformador.");
        ...
        <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
          <div className="w-full lg:w-1/3">
            <DocumentFileInput
              ...
              handleChange={(value) =>
                setFiles((prev) => ({ ...prev, ["FOTO DO TRANSFORMADOR"]: value }))
              }
            />
          </div>
          <div className="w-full lg:w-1/3">
            <DocumentFileInput
              ...
              handleChange={(value) =>
                setFiles((prev) => ({ ...prev, ["FOTO DA LOCALIZAÇÃO DO TRANSFORMADOR"]: value }))
              }
            />
          </div>
          <div className="w-full lg:w-1/3">
            <DocumentFileInput
              ...
              handleChange={(value) =>
                setFiles((prev) => ({ ...prev, ["FOTO DO NÚMERO DO TRANSFORMADOR"]: value }))
              }
            />
          </div>
        </div>
