...
function getStatusTag(status: string) {
  ...
  return (
    <h1
      className={`rounded-full ${color} px-2 py-1 text-center text-[0.65rem] font-bold text-white lg:text-xs`}
    >
      {status}
    </h1>
  );
...
function TechnicalAnalysisCard({
  ...
  return (
    <div
      key={analysis._id}
      className="bg-background border-primary/60 flex min-h-[180px] w-full gap-2 rounded-md border shadow-xs lg:w-[500px]"
    >
      <div
        className={`h-full w-[7px] ${getBarColor(analysis.status)} rounded-tl-md rounded-bl-md`}
      ></div>
      <div className="flex w-full grow flex-col p-4">
        <div className="flex w-full grow flex-col">
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-foreground text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">
                ...
                <strong className="text-cyan-500">
                  {analysis.requerente.nome?.toUpperCase() || "NÃO DEFINIDO"}
                </strong>
              </p>
            </div>
            <div className="flex items-end gap-2">
          ...
          {/* <div className="mt-1 flex w-full items-center justify-start gap-2">
                 ...
                 <p className="text-xs font-medium text-foreground">{analysis.requerente.nome || analysis.requerente.apelido}</p>
                </div>
                <div className="mt-2 flex w-full items-center justify-center gap-2">
                 ...
                 <p className="text-xs font-semibold leading-none tracking-tight text-foreground">{analysis.oportunidade.nome || 'NÃO DEFINIDO'}</p>
                </div> */}
        </div>
        <div className="lg:0 mt-2 flex w-full flex-col-reverse items-center justify-between gap-1 lg:flex-row lg:gap-2">
              ...
              <>
                <div className="flex items-center gap-1">
                  ...
                  <p className="text-foreground text-[0.65rem] font-medium">
                    {formatDateAsLocale(analysis.dataEfetivacao, true)}
                  </p>
                </div>
                <div className="flex items-center gap-1">
                  <MdTimer color="rgb(37,99,235)" />
                  <p className="text-foreground text-[0.55rem]">
                    {formatDecimalPlaces(
                      getHoursDiff({
                        start: analysis.dataInsercao,
                        finish: analysis.dataEfetivacao,
                      }),
                      0,
                      2,
                    )}
          ...
          <div className="flex flex-col items-center gap-2 lg:flex-row">
            <div className={`flex items-center gap-1`}>
              <BsCalendarPlus />
              <p className="text-foreground text-[0.65rem] font-medium">
                {formatDateAsLocale(analysis.dataInsercao, true)}
              </p>
            </div>
            <div className="flex items-center gap-1">
              <Avatar
              ...
              />
              <p className="text-foreground text-[0.65rem] font-medium">
                {analysis.analista?.nome || "ANALISTA INDEFINIDO"}
              </p>
            </div>
          </div>
        </div>
