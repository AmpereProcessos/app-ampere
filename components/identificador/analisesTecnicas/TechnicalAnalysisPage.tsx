...
function TechnicalAnalysisPage({ session }: TechnicalAnalysisPageProps) {
  ...
  const [period, setPeriod] = useState<{ after: string | null; before: string | null }>({
    after: null,
    before: null,
  });

  const { data, isLoading, isError, isSuccess, updateFilters } =
    useTechnicalAnalysisByPersonalizedFilters({
      after: period.after,
  ...
  return (
    <div className="flex w-full max-w-full grow flex-col overflow-x-hidden bg-[#f8f9fa] p-6">
      ...
      {editModal.id && editModal.isOpen ? (
        <ControlTechnicalAnalysis
          analysisId={editModal.id}
          session={session}
