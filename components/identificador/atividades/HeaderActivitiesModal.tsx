...
function HeaderActivitiesModal({
  activities,
  userId,
  filters,
  setFilters,
  closeModal,
}: HeaderActivitiesModalProps) {
  const ref = useRef(null);
  useClickOutside(ref, closeModal);
  return (
    <div
      ref={ref}
      className="bg-background border-border absolute top-[35px] -left-[280px] z-200 flex h-[300px] w-[300px] flex-col border p-2 shadow-md lg:-left-[190px]"
    >
      <div className="border-border flex w-full items-center justify-between border-b pb-1">
      ...
      <div className="scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex w-full grow flex-col gap-1 overflow-y-auto overscroll-y-auto py-1 pr-2">
        <input
          ...
          className="border-border w-full rounded-md border p-2 text-xs outline-hidden placeholder:italic"
        />
        ...
        {activities.length > 0 ? (
          ...
          <div className="flex w-full grow items-center justify-center">
            <p className="text-foreground text-center text-xs italic">
