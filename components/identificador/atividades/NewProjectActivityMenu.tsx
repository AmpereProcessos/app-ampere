...
function NewProjectActivityMenu({
  projectId,
  projectName,
  projectIdentifier,
  session,
}: NewProjectActivityMenuProps) {
  ...
  const { mutate: handleCreateActivity } = useMutationWithFeedback({
    ...
    callbackFn: async () =>
      await queryClient.invalidateQueries({
        queryKey: ["activities-by-responsible", session.user.id],
      }),
  });
  return (
            ...
            <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
              ...
              <div className="w-1/2 lg:w-full">
                <div className="flex w-full flex-col gap-1">
                  ...
                  <input
                    value={
                      dayjs(newActivityHolder.dataVencimento).isValid()
                        ? dayjs(newActivityHolder.dataVencimento).format("YYYY-MM-DDTHH:mm")
                        : undefined
                    }
                    onChange={(e) => {
                    ...
                    type="datetime-local"
                    className="border-border w-full rounded-md border p-3 text-sm outline-hidden placeholder:italic"
                  />
                </div>
              </div>
            </div>
                      ...
                      startContent: (
                        <AvatarUI className="h-5 w-5 min-h-5 min-w-5">
                          ...
                          <AvatarFallback className="text-xs">
                            {formatNameAsInitials(resp.usuario)}
                          </AvatarFallback>
                        </AvatarUI>
                      ),
                    })) || []
