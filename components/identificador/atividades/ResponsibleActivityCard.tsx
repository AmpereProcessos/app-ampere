			...
			<div className="mt-1 flex w-full items-center justify-end gap-2">
				{activity.dataVencimento ? (
					<div className="flex items-center gap-1 text-orange-500">
						...
						<p className="text-primary/60 text-[0.6rem] font-medium">{formatDateAsLocale(activity.dataVencimento, true)}</p>
					</div>
				) : null}
				{activity.dataConclusao ? (
					<div className="flex items-center gap-1">
						<BsCalendarCheck color="rgb(34,197,94)" />
						<p className="text-primary/60 text-[0.6rem] font-medium">{formatDateAsLocale(activity.dataConclusao, true)}</p>
					</div>
				) : null}
			</div>
			{activity.projeto.id ? (
				<div className="mt-2 mb-1 flex w-full items-center justify-center gap-2 text-cyan-500">
					...
					<p className="text-xxs text-primary/60 font-medium">{activity.projeto.nome}</p>
				</div>
			) : null}
			<h1 className="text-primary/60 bg-primary/20 mb-2 w-full rounded-md p-2 py-1 text-center text-xs font-medium">{activity.descricao}</h1>
			<div className="flex w-full items-center justify-end gap-2">
				<div className="flex items-center gap-1">
					<BsCalendarPlus />
					<h1 className="text-primary/60 mr-2 text-[0.6rem] font-medium tracking-tight">{formatDateAsLocale(activity.dataInsercao, true)}</h1>
					<Avatar width={20} height={20} url={activity.autor.avatar_url} fallback={formatNameAsInitials(activity.autor.nome)} />
					<h1 className="text-primary/60 text-[0.6rem] font-medium tracking-tight">{activity.autor.nome}</h1>
				</div>
			</div>
		</div>
	);
