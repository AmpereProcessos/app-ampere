...
function getResult({
  ...
  revenues: TProjectFinances["receitas"];
  expenses: TProjectFinances["despesas"];
  power: number;
}) {
...
function AuditingCard({ info, handleClick }: AuditingCardProps) {
  const { liquido, margem, valorWattVenda, valorWattCompra } = getResult({
    revenues: info.receitas,
    expenses: info.despesas,
    power: info.potencia,
  });
  ...
  return (
    <div className="border-border flex w-full flex-col rounded-md border p-4 lg:w-[600px]">
      <div className="flex w-full items-center justify-between gap-2">
        ...
        <div className="bg-primary/80 flex min-w-fit items-center gap-2 rounded-full px-2 py-1">
          <h1 className="text-[0.65rem] font-bold text-white lg:text-xs">
            MARGEM DE {formatDecimalPlaces(margem * 100)} %
          </h1>
        </div>
      </div>
        ...
        <div className="flex items-center gap-2 text-cyan-500">
          ...
          {info.idProjetoCRM ? (
            <a
              ...
              className="text-foreground text-[0.65rem] leading-none font-medium tracking-tight hover:text-cyan-500 lg:text-xs"
            >
              {info.identificador}
            </a>
          ) : (
      ...
      <div className="mt-3 flex w-full items-center justify-between gap-2">
        <h1 className="text-[0.6rem] leading-none font-black tracking-tight text-[#70e000]">
          RECEITAS
        </h1>
        <h1 className="text-[0.7rem] leading-none font-black tracking-tight text-[#70e000]">
          {formatToMoney(totalRevenue)}
        </h1>
      </div>
      ...
      <div className="mt-2 flex w-full flex-wrap items-center gap-2">
        {revenues.map(([key, value]) => (
          <div className="flex items-center gap-2 rounded-md bg-[#006400] px-2 py-1">
            <p className="text-[0.65rem] leading-none font-medium tracking-tight text-white">
              {key}
            </p>
      ...
      <div className="mt-1 flex w-full items-center justify-between gap-2">
        <h1 className="text-[0.6rem] leading-none font-black tracking-tight text-[#ed174c]">
          DESPESAS
        </h1>
        <h1 className="text-[0.7rem] leading-none font-black tracking-tight text-[#ed174c]">
      ...
      <div className="mt-2 flex w-full items-center justify-end gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <p className="text-foreground text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">
            CONCLUIDO EM:{" "}
            {info.dataConclusaoObra ? formatDateAsLocale(info.dataConclusaoObra) : "NÃO DEFINIDO"}
          </p>
        </div>
      </div>
             ...
             {Object.entries(info.despesas).filter(([key, value]) => value != 0).length > 0 ? (
               Object.entries(info.despesas).map(([key, value]) => (
                 <div className="flex w-full items-center justify-between">
                   <p className="text-[0.65rem] font-medium leading-none tracking-tight text-foreground">{key}</p>
                   <p className="text-[0.65rem] font-bold text-[#ed174c]">{formatToMoney(value)}</p>
                 </div>
               ))
             ) : (
               <p className="text-[0.65rem] font-medium leading-none tracking-tight text-foreground">SEM DESPESAS ASSOCIADAS</p>
             )}
            </div> */}
    </div>
  );
  ...
  return (
    <div className="border-border flex w-full gap-2 rounded-md border shadow-xs lg:w-[550px]">
      ...
      <div className="flex grow flex-col p-6">
        <div className="flex w-full items-center justify-between">
          ...
          <div className="border-primary/60 rounded-md border px-2 py-1 text-xs font-medium">
            {formatToMoney(liquido)}
          </div>
        </div>
        <div className="flex w-full grow flex-col">
          <h1 className="mt-4 text-[0.65rem] leading-none font-medium tracking-tighter text-green-600">
            RECEITAS
          </h1>
          {Object.entries(info.receitas)
            ...
            .map(([key, value]) => (
              <div className="mt-1 flex w-full items-center justify-between">
                <p className="text-foreground text-xs leading-none font-medium tracking-tight">
                  {key}
                </p>
                <p className="text-xs font-bold text-green-500">{formatToMoney(value)}</p>
              </div>
            ))}
          <h1 className="text-foreground mt-4 text-[0.65rem] leading-none font-medium tracking-tighter">
            PREVISÃO
          </h1>
          <h1 className="mt-4 text-[0.65rem] leading-none font-medium tracking-tighter text-red-600">
            DESPESAS
          </h1>
        ...
        <div className="mt-2 flex w-full items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-foreground text-xs">
              {formatDateAsLocale(info.dataConclusaoObra)}
            </h1>
            <FaSignature />
            <h1 className="text-foreground text-xs">{formatDateAsLocale(info.dataAssinatura)}</h1>
          </div>
