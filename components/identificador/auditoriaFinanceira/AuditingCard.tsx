...
function AuditingCard({ info, handleClick }: AuditingCardProps) {
	...
	return (
		<div className="flex w-full flex-col rounded-md border border-gray-300 p-4 lg:w-[600px]">
			<div className="flex w-full items-center justify-between gap-2">
				<h1 onClick={() => handleClick(info._id)} className="cursor-pointer text-xs font-black leading-none tracking-tight hover:text-cyan-500 lg:text-sm">
					{info.nome}
				</h1>
			...
			<div className="mt-2 flex w-full items-center justify-center gap-2">
				<div className="flex items-center gap-2">
					<TbActivity />
					<p className="text-[0.65rem] font-medium leading-none tracking-tight text-gray-500 lg:text-xs">Vendido a {formatDecimalPlaces(valorWattVenda)} R$/kWp</p>
				</div>
				{valorWattCompra ? (
					<div className="flex items-center gap-2">
						<TbActivity />
						<p className="text-[0.65rem] font-medium leading-none tracking-tight text-gray-500 lg:text-xs">Compra a {formatDecimalPlaces(valorWattCompra)} R$/kWp</p>
					</div>
				) : null}
			</div>
			...
			<div className="mt-2 flex w-full items-center justify-end gap-4">
				<div className="flex items-center gap-2">
					<FaSignature />
					<p className="text-[0.65rem] font-medium leading-none tracking-tight text-gray-500 lg:text-xs">ASSINADO EM: {formatDateAsLocale(info.dataAssinatura)}</p>
				</div>
				<div className="flex items-center gap-2">
					<FaTools />
	...
	return (
		<div className="flex w-full gap-2 rounded-md border border-gray-300 shadow-sm lg:w-[550px]">
			...
			<div className="flex grow flex-col p-6">
				<div className="flex w-full items-center justify-between">
					<h1 onClick={() => handleClick(info._id)} className="trackig-tight cursor-pointer font-bold leading-none duration-300 ease-in-out hover:text-cyan-500">
						{info.nome}
					</h1>
