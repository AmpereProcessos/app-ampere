...
function ExpenseRevenueListItem({
  session,
  finance,
  tag,
  initialShowItems = false,
}: ExpenseRevenueListItemProps) {
  ...
  return (
    <div className="bg-card border-border flex w-full flex-col gap-1 rounded-xl border px-3 py-4 shadow-2xs">
      <div className="flex w-full items-center justify-between gap-2">
        ...
        <div
          className={`flex min-w-fit items-center gap-2 rounded-full ${tag === "EXPENSE" ? "bg-[#ed174c]" : "bg-[#70e000]"} px-2 py-1`}
        >
          <h1 className="text-[0.65rem] font-medium text-white lg:text-xs">
            {formatToMoney(finance.total)}
          </h1>
        </div>
      </div>
      ...
      <div className="w-full flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          {finance.itens.length > 0 ? (
            ...
            >
              {showItems ? (
                <EyeOffIcon className="w-4 h-4 min-w-4 min-h-4" />
              ) : (
        ...
        ? finance.itens
            ...
            .map((item, index) => (
              ...
              >
                ...
                <h1 className="text-primary text-xs font-black">
      ...
      {isEditing && finance.id ? (
        tag === "EXPENSE" ? (
          <EditExpense
            expenseId={finance.id}
            session={session}
            closeModal={() => setIsEditing((prev) => !prev)}
          />
        ) : (
          <EditRevenue
            revenueId={finance.id}
            session={session}
            closeModal={() => setIsEditing((prev) => !prev)}
          />
        )
