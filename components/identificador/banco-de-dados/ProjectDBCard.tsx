...
function ProjectDBCard({ project, handleClick }: ProjectDBCardProps) {
	return (
		<div className="flex w-full rounded-md border bg-[#fff] md:w-[350px] lg:w-[500px]">
			<div className={`h-full w-[7px] ${getBarColor(project.tipoDeServico)} rounded-bl-md rounded-tl-md`} />
			<div className="flex grow flex-col gap-2 p-3">
				...
				<div className="flex w-full items-center justify-between gap-2">
					<div className="flex items-center gap-2">
						<FaTag />
						<p className="text-[0.65rem] font-medium leading-none tracking-tight text-gray-500 lg:text-xs">{project.vendedor?.nome || "NÃO DEFINIDO"}</p>
					</div>
					<div className="flex items-center gap-2">
						<FaSignature />
						<p className="text-[0.65rem] font-medium leading-none tracking-tight text-gray-500 lg:text-xs">{project.contrato?.status || "NÃO DEFINIDO"}</p>
					</div>
				</div>
			</div>
		</div>
