...
import { useEditApportionment, useInsertApportionment } from "../../../utils/methods/mutation/costApportionments";
import { getErrorMessage } from "../../../utils/methods/handlers";
import { useQueryClient } from "@tanstack/react-query";
import { useCostApportionmentByID } from "../../../utils/methods/query/costApportionments";
...
function EditApportionment({ apportionmentId, closeModal }) {
	const queryClient = useQueryClient();
	const {
		data: apportionment,
		isLoading: apportionmentLoading,
		isSuccess: apportionmentSuccess,
		isError: apportionmentError,
	} = useCostApportionmentByID(apportionmentId);
	const { mutate: editApportionment, isLoading: editApportionmentLoading } = useEditApportionment();
	const [infoHolder, setInfoHolder] = useState(apportionment);
	...
	async function handleApportionmentEdit() {
	...
	useEffect(() => {
		setInfoHolder(apportionment);
	}, [apportionment]);
	return (
		<div style={OVERLAY_STYLES}>
			<div className="h-[90%] w-[80%] lg:h-[80%] lg:w-[50%]" style={MODAL_STYLES}>
				<div className="flex h-full w-full flex-col">
					<div className="border-primary/20 flex items-center justify-between border-b px-2 pb-2 text-lg">
						<div className="flex flex-col items-center">
							<h1 className="pl-6 font-bold text-[#15599a]">EDIÇÃO DE RATEIO DE DESPESAS</h1>
					...
					{apportionmentLoading ? (
						<div className="flex grow items-center justify-center">
							<LoadingPage />
						</div>
					) : null}
					{apportionmentSuccess && infoHolder ? (
						<div className="overscroll-y scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex w-full grow flex-col overflow-y-auto px-2 py-2">
							...
							<div className="my-2 flex w-full flex-col">
								<h1 className="font-raleway bg-primary/60 w-full text-center text-sm font-normal text-white">CATEGORIAS</h1>
									...
									<div className="flex grow items-end gap-2">
										<div className="w-full">
											<TextInput
												...
												handleChange={(value) => setCategoryHolder((prev) => ({ ...prev, nome: value }))}
								...
								{infoHolder.categorias?.length > 0 ? (
									<>
										...
										{infoHolder.categorias.map((category, index) => (
											<div key={index} className="border-primary/20 my-1 flex w-full items-center gap-2 rounded-md border p-2">
									...
									<p className="text-primary/60 w-full py-4 text-center italic">Nenhuma categoria cadastrada</p>
								)}
							</div>

							<div className="flex w-full flex-col items-center">
								<h1 className="font-raleway bg-primary/60 w-full text-center text-sm font-normal text-white">ORÇAMENTOS</h1>
								<div className="mt-2 flex w-full items-end gap-2">
								...
								{infoHolder.orcamentos?.length > 0 ? (
									...
									<p className="text-primary/60 w-full py-4 text-center italic">Nenhum orçamento cadastrada</p>
								)}
							</div>
						</div>
					) : null}
					{apportionmentError ? (
						<div className="flex grow flex-col items-center justify-center gap-1 text-red-500">
							...
							<p className="text-primary/60 text-center text-sm italic">Houve um erro as informações sobre esse centro de custo.</p>
					...
					<div className="flex w-full items-center justify-end py-2">
						<button
							onClick={() => handleApportionmentEdit()}
...
export default EditApportionment;
