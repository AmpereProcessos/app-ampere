...
export default function CertificationReferences({
  ...
  return (
    <ResponsiveDialogDrawerSection
      sectionTitleText="REFERÊNCIAS"
      sectionTitleIcon={<LinkIcon size={15} />}
    >
      <div className="w-full flex items-center justify-center">
        <Button
          ...
          onClick={() => setNewReferenceMenuIsOpen(true)}
        >
          <PlusIcon size={15} />
      ...
      {isValidNumber(editReferenceMenuIndex) ? (
        <ControlCertificationReferenceMenu
          ...
          updateCertificationReference={(info) =>
            updateCertificationReference(editReferenceMenuIndex as number, info)
          }
...
function NewCertificationReferenceMenu({
  certificationValidConfig,
  addCertificationReference,
  closeMenu,
}: NewCertificationReferenceMenuProps) {
  const { data: users } = useUsers();
  const [newReferenceHolder, setNewReferenceHolder] = useState<
    TUseCertificationState["state"]["references"][number]
  >({
    documento: {
      titulo: "",
      formato: "",
    ...
    dataFim: dayjs()
      .add(
        certificationValidConfig.valor,
        TIME_UNITS_MANIPULATION_MAP[certificationValidConfig.medida],
      )
      .toISOString(),
    notificados: [],
    anexo: {},
  ...
  function handleAddCertificationReference(
    info: TUseCertificationState["state"]["references"][number],
  ) {
    if (!info.documento.titulo || info.documento.titulo.trim().length < 3)
      return toast.error("Título não informado.");
    ...
    >
      <TextInput
        ...
        handleChange={(value) =>
          setNewReferenceHolder((prev) => ({
            ...prev,
            documento: { ...prev.documento, titulo: value },
          }))
        }
        width="100%"
      />
      <div className="relative flex h-full w-full flex-col items-center justify-center">
        <label
          ...
          className="dark:hover:bg-primary/80 border-border bg-background hover:bg-primary/10 flex h-full min-h-32 w-full cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed"
        >
          <div className="text-primary flex flex-col items-center justify-center px-2 pt-5 pb-6">
            ...
            <p className="text-center text-xs lg:text-base tracking-tight">
              {newReferenceHolder.anexo.arquivo
                ? newReferenceHolder.anexo.arquivo.name
      ...
      <DatetimeInput
        ...
        handleChange={(value) =>
          setNewReferenceHolder((prev) => ({
            ...prev,
            dataInicio: formatDateInputChange(value, "string", false) as string,
          }))
        }
        width="100%"
      />
      <DatetimeInput
        ...
        handleChange={(value) =>
          setNewReferenceHolder((prev) => ({
            ...prev,
            dataFim: formatDateInputChange(value, "string", false) as string,
          }))
        }
        width="100%"
      />
...
type ControlCertificationReferenceMenuProps = {
  ...
  updateCertificationReference: (
    info: Partial<TUseCertificationState["state"]["references"][number]>,
  ) => void;
  closeMenu: () => void;
};
  ...
  return (
    ...
    >
      <TextInput
        ...
        handleChange={(value) =>
          updateCertificationReference({
            documento: { ...certificationReference.documento, titulo: value },
          })
        }
        width="100%"
      />
        ...
        >
          <div className="text-primary flex flex-col items-center justify-center px-2 pt-5 pb-6">
            <BsCloudUploadFill
              className={cn("size-10", {
                "text-primary":
                  !certificationReference.anexo.arquivo && !certificationReference.documento.url,
                "text-green-500":
                  certificationReference.anexo.arquivo || certificationReference.documento.url,
              })}
            />
...
function CertificationReferenceCard({
  reference,
  removeCertificationReference,
  editCertificationReference,
}: CertificationReferenceCardProps) {
  return (
    <div className="border-border bg-card flex w-full flex-col gap-1 rounded border p-2 shadow-xs">
      <div className="w-full flex items-center justify-between">
        <div className="flex items-center gap-1">
          <LinkIcon className="w-4 h-4" />
          <h1 className="text-sm leading-none font-bold tracking-tight">
            {reference.documento.titulo}
          </h1>
        </div>
        <div className="flex items-center gap-1">
          <Button
            className="hover:bg-blue-200 hover:text-blue-700 p-1"
            size={"fit"}
            variant={"ghost"}
            onClick={() => editCertificationReference()}
          >
            <Pencil className="w-4 h-4" />
          </Button>
          <Button
            className="hover:bg-red-200 hover:text-red-700 p-1"
            size={"fit"}
            variant={"ghost"}
            onClick={() => removeCertificationReference()}
          >
      ...
      <div className="w-full flex flex-col gap-1">
        <p className="text-foreground text-[0.65rem] font-medium tracking-tight">NOTIFICADOS</p>
          ...
          {reference.notificados.length > 0 ? (
            reference.notificados.map((user) => (
              <div key={user.id} className="flex items-center gap-1">
                ...
                <p className="text-foreground text-xs font-medium tracking-tight">{user.nome}</p>
              </div>
            ))
          ) : (
