...
import { EngineeringCallTypes, EngineeringCallsStatus, SlideMotionVariants, formatDate } from "@/utils/constants";
...
export function EngineeringCallsDatabase({ session }: EngineeringCallsDatabaseProps) {
	...
	const { data: engineeringCallsResult, isLoading, isError, isSuccess, error, filters, updateFilters } = useEngineeringCalls();
			...
			<div className="border-primary/20 flex flex-col items-center justify-between gap-2 border-b pb-2">
				<div className="flex w-full items-center justify-between">
					<div className="flex flex-col items-center gap-2 lg:flex-row">
						<p className="text-center text-2xl font-black text-[#15599a] uppercase">BANCO DE CHAMADOS DE ENGENHARIA</p>
					</div>
				...
				<AnimatePresence>
					{filtersMenuIsOpen ? <EngineeringCallsDatabaseFilters filters={filters} updateFilters={updateFilters} /> : null}
				</AnimatePresence>
				...
				<div className="flex flex-wrap justify-around gap-3">
					{engineeringCalls.map((call, index) => (
						<div key={call._id} className="w-full lg:w-[450px]">
							<EngineeringCard call={call} handleClick={() => setEditEngineeringCallMenuId(call._id)} />
						</div>
					))}
				</div>
			) : null}
...
function EngineeringCallsDatabaseFilters({ filters, updateFilters }: EngineeringCallsDatabaseFiltersProps) {
	return (
		<motion.div
			...
			<div className="flex w-full flex-col items-center justify-start gap-2 md:flex-row">
				...
				<div className="w-full md:w-[300px]">
					<MultipleSelectInput
						...
						options={EngineeringCallTypes.map((callType, index) => ({ id: index + 1, label: callType, value: callType }))}
						selectedItemLabel="NENHUM"
...
function EngineeringCallsDatabaseFiltersShowcase({ filters, updateFilters }: EngineeringCallsDatabaseFiltersShowcase) {
	...
	return (
		<div className="flex w-full flex-wrap items-center justify-center gap-2 lg:justify-end">
			...
			{filters.callTypes && filters.callTypes.length > 0 ? (
				<FilterTag label="TIPO DO CHAMADO" value={filters.callTypes.join(", ")} onRemove={() => updateFilters({ callTypes: [] })} />
			) : null}
...
function EngineeringCard({ call, handleClick }: EngineeringCardProps) {
	function renderEngineeringCallStatus(callStatus: TGetEngineeringCallsOutputDefault["calls"][number]["status"]) {
		...
		return (
			<div className={`inline-flex items-center gap-1 rounded-md bg-gray-200 px-1.5 py-0.5 text-[0.65rem] font-bold text-gray-700`}>
	...
	return (
		<div className={"bg-card border-primary/20 flex w-full flex-col gap-3 rounded-xl border px-3 py-4 shadow-xs"}>
			<div className="flex items-center justify-between">
				<h1 className="text-xs font-medium tracking-tight uppercase">{call.projeto || "PROJETO NÃO INFORMADO"}</h1>
				{renderEngineeringCallStatus(call.status)}
			</div>
			<div className="flex w-full flex-wrap items-center justify-start gap-2">
				<EngineeringCardMetadata icon={<FaProjectDiagram className="h-3 min-h-3 w-3 min-w-3" />} text={call.tipoDoChamado || "TIPO NÃO INFORMADO"} />
			</div>
			{call.observacoes && (
				<div className="flex w-full flex-col gap-1">
					<p className="text-primary/80 text-[0.65rem] font-medium uppercase">OBSERVAÇÕES:</p>
					<p className="text-primary/60 text-xs">{call.observacoes}</p>
				</div>
			)}
