...
export function General({ infoHolder, updateInfoHolder }: GeneralProps) {
  ...
  function handleEffectivationUpdate(
    newValue: TEngineeringCall["status"],
    previousData: TEngineeringCall,
  ) {
  ...
  return (
    <div className="flex w-full flex-col gap-2">
        ...
        {EngineeringCallsStatus.map((status) => (
          <button
            ...
            onClick={() =>
              updateInfoHolder({
                status: status.value,
                fechamento: handleEffectivationUpdate(status.value, infoHolder),
              })
            }
      ...
      {infoHolder.fechamento ? (
        <div
          className={
            "flex w-fit items-center gap-1 self-center rounded-md bg-green-200 px-1.5 py-0.5 text-[0.65rem] font-bold text-green-700"
          }
        >
...
type TAttachmensProps = {
  ...
  updateAttachmentItem: (
    index: number,
    item: Partial<TEngineeringCallAttachmentHolderItem>,
  ) => void;
                  ...
                  <input
                    onChange={(e) => {
                      if (e.target.files) {
                        ...
                        const attachments: TEngineeringCallAttachmentHolderItem["attachments"] =
                          files.map((file) => ({
                            file: file,
