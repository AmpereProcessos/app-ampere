  ...
  return (
    <div className="flex w-full grow flex-col gap-6 p-6">
      <div className="border-border flex flex-col items-center justify-between gap-2 border-b pb-2">
        ...
        <AnimatePresence>
          {filtersMenuIsOpen ? (
            <SupportCallsDatabaseFilters filters={filters} updateFilters={updateFilters} />
          ) : null}
        </AnimatePresence>
...
function SupportCallsDatabaseFiltersShowcase({
  filters,
  updateFilters,
}: SupportCallsDatabaseFiltersShowcase) {
  const FilterTag = ({
    label,
    value,
    onRemove,
...
function SupportCard({ call, handleClick }: SupportCardProps) {
  function renderSupportCallStatus(
    callStatus: TGetSupportCallsOutputDefault["calls"][number]["statusChamado"],
  ) {
  ...
  return (
    ...
    >
      ...
      <div className="flex w-full flex-wrap items-center justify-start gap-2">
        <SupportCardMetadata
          icon={<FaCity className="h-3 min-h-3 w-3 min-w-3" />}
          text={call.cidade ?? "CIDADE NÃO INFORMADA"}
        />
        <SupportCardMetadata
          icon={<Diamond className="h-3 min-h-3 w-3 min-w-3" />}
          text={call.tipoChamado ?? "TIPO NÃO INFORMADO"}
        />
        <SupportCardMetadata
          icon={<ArrowUpRight className="h-3 min-h-3 w-3 min-w-3" />}
          text={call.demanda ?? "DEMANDA NÃO INFORMADA"}
        />
      </div>
