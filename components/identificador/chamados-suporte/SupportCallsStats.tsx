...
import { useSupportCallsStats } from "@/utils/methods/query/support-calls";
import { ChartArea, Check, Clock, Sigma, TrendingDown, TrendingUp, X } from "lucide-react";
import { BsCalendarPlus } from "react-icons/bs";

function SupportCallsStats() {
	const { data: stats, filters, updateFilters } = useSupportCallsStats();
	return (
...
export default SupportCallsStats;
...
function CardStat({ title, icon, current, previous, showComparison = true, formatCurrent, formatPrevious, lowerIsBetter, className }: CardStatProps) {
	const change = (() => {
		if (previous === 0) {
	...
	return (
		<div className={cn("bg-card border-primary/20 flex w-full flex-col gap-1 rounded-xl border px-3 py-4 shadow-xs", className)}>
			...
			<div className="flex w-full flex-col">
				...
				{showComparison && (
					<p className="text-primary/60 text-xs tracking-tight">PERÍODO ANTERIOR: {formatPrevious ? formatPrevious(previous) : String(previous || 0)}</p>
				)}
			</div>
		</div>
	);
