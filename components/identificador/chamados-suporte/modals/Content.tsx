...
export function General({ infoHolder, updateInfoHolder }: GeneralProps) {
  ...
  function handleEffectivationUpdate(
    newValue: TSupportCall["statusChamado"],
    previousData: TSupportCall,
  ) {
        ...
        <button
          ...
          onClick={() =>
            updateInfoHolder({
              ...
              fechamento: handleEffectivationUpdate("ABERTO", infoHolder),
            })
          }
          className={cn(
            "flex items-center gap-1 rounded-md bg-orange-300 px-3 py-1.5 text-sm text-orange-800",
            {
        ...
        <button
          ...
          onClick={() =>
            updateInfoHolder({
              ...
              fechamento: handleEffectivationUpdate("EM ANDAMENTO", infoHolder),
            })
          }
          className={cn(
            "flex items-center gap-1 rounded-md bg-blue-300 px-3 py-1.5 text-sm text-blue-800",
            {
        ...
        <button
          ...
          onClick={() =>
            updateInfoHolder({
              ...
              fechamento: handleEffectivationUpdate("RESOLVIDO", infoHolder),
            })
          }
          className={cn(
            "flex items-center gap-1 rounded-md bg-green-300 px-3 py-1.5 text-sm text-green-800",
            {
        ...
        <div className="bg-card flex w-full flex-col items-center justify-center gap-3 p-3">
          ...
          <div className="flex w-full flex-wrap items-center justify-center gap-2">
            <div className="items-canter flex gap-1">
              ...
              <p className="text-foreground text-xs">{infoHolder.equipeResp}</p>
            </div>
            <div className="items-canter flex gap-1">
              <FaCity className="h-3 min-h-3 w-3 min-w-3" />
              <p className="text-foreground text-xs">{infoHolder.cidade}</p>
            </div>
            <div className="items-canter flex gap-1">
              <Tag className="h-3 min-h-3 w-3 min-w-3" />
              <p className="text-foreground text-xs">{infoHolder.plano}</p>
            </div>
          </div>
        </div>
      ) : (
            ...
            startContent: (
              <Avatar className="h-5 w-5 min-h-5 min-w-5">
                ...
                <AvatarFallback className="text-xs">
                  {formatNameAsInitials(user.nome)}
                </AvatarFallback>
              </Avatar>
            ),
          })) || []
      ...
      <div className="flex w-full flex-col gap-1">
        ...
        {infoHolder.linkMonitoramento ? (
          <a
            ...
            className="text-center text-cyan-500 hover:text-cyan-700"
          >
            {infoHolder.linkMonitoramento}
          </a>
        ) : null}
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      ...
      <DateInput
        ...
        handleChange={(value) =>
          updateInfoHolder({
            ultAtualizacaoCliente: formatDateInputChange(value, "string") as string,
          })
        }
        width="100%"
      />
...
export function Attachments({
  ...
  async function handleAttachments({
    ...
    const uploadPromises = uploads.map(async (uploadItem) => {
      const {
        ...
        file: uploadItem.file as File,
        fileName: uploadItem.title,
        vinculationId: undefined,
        prefix: "chamado-suporte",
      });
  ...
  const { mutate: handleAttachmentsMutation, isPending: handleAttachmentsMutationIsPending } =
    useMutation({
    ...
    <div className="flex w-full flex-col gap-2">
      ...
      {newAttachmentMenuIsOpen ? (
        <motion.div
          ...
          animate="visible"
          exit="hidden"
          className="bg-card border-border flex w-full flex-col gap-3 rounded-xl border px-3 py-4 shadow-xs"
        >
            ...
            <div key={index.toString()} className="flex w-full flex-col gap-1">
              ...
              <div className="relative flex w-full items-center justify-center">
                <label
                  ...
                  className="dark:hover:bg-bray-800 border-border hover:bg-primary/10 flex w-full cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed bg-[#fff] px-3 py-1 dark:bg-[#121212]"
                >
                  ...
                  <input
                    onChange={(e) => {
                      if (e.target.files) {
                        ...
                        const attachments: TSupportCallAttachmentHolderItem["attachments"] =
                          files.map((file) => ({
                            file: file,
                ...
                {attachment.attachments.length > 0 ? (
                  attachment.attachments.map((file, index) => (
                    <div
                      ...
                      className="border-primary/50 flex h-[100px] w-[100px] flex-col rounded border"
                    >
                  ...
                  <p className="text-foreground text-xs font-light">Nenhum arquivo encontrado.</p>
                )}
              </div>
            </div>
          ))}
      ...
      {isLoading ? (
        <p className="animate-pulse text-center text-sm font-medium tracking-tight">
          Carregando arquivos...
        </p>
      ) : null}
      {isError ? (
        <p className="text-center text-sm font-medium tracking-tight text-red-500">
