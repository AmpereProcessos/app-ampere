...
function PosVendaCallsFilterMenu({ queryParams, updateQueryParams, closeMenu }: PosVendaCallsFilterMenuProps) {
	const [queryParamsHolder, setQueryParamsHolder] = useState<TGetAllPosVendaCallsInput>(queryParams);
						...
						<div className="flex w-full flex-col gap-2">
							...
							<div className="w-fit self-center">
								<CheckboxInput
									...
									handleChange={(value) => setQueryParamsHolder((prev) => ({ ...prev, ongoingOnly: value ? "true" : "false" }))}
								/>
							</div>
						</div>
						<div className="flex w-full flex-col gap-2">
							...
							<DateInput
								...
								handleChange={(value) => setQueryParamsHolder((prev) => ({ ...prev, periodAfter: formatDateInputChange(value, "string") as string }))}
								width="100%"
							/>
							<DateInput
								label="ANTES DE"
								value={formatDate(queryParamsHolder.periodBefore)}
								handleChange={(value) => setQueryParamsHolder((prev) => ({ ...prev, periodBefore: formatDateInputChange(value, "string") as string }))}
								width="100%"
							/>
							<SelectInput
								label="PARÂMETRO"
								...
								handleChange={(value) => setQueryParamsHolder((prev) => ({ ...prev, periodField: value as "dataInsercao" | "dataEfetivacao" }))}
