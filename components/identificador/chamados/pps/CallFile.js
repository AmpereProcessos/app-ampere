  ...
  return (
    <div className="flex w-full items-center justify-between gap-2 rounded-md border border-cyan-500 p-2">
      <div className="flex items-center gap-2">
        ...
        <Link href={info.link}>
          <div className="text-foreground text-sm font-medium duration-300 ease-in-out hover:text-cyan-500">
            {info.title || "ARQUIVO"}
          </div>
        </Link>
        {info.format ? (
          <p className="text-[0.6rem] font-medium text-[#fead41]">({info.format})</p>
        ) : null}
      </div>
      <div
