...
function ChatsHubTesting({
  ...
  const [selectedPhoneNumber, setSelectedPhoneNumber] = useState<string | null>(
    whatsappConnection?.telefones[0]?.whatsappTelefoneId ?? null,
  );
  ...
  return (
    <div className="w-full max-h-[calc(100vh-200px)] grow flex flex-col items-center justify-center rounded-lg shadow-lg border border-border overflow-hidden">
      {/* Layout Desktop - duas colunas lado a lado */}
      {isDesktop ? (
        <div className="w-full h-full flex">
          <div className="flex flex-col gap-3 w-1/3 h-full border-r border-border">
            <div className="w-full flex items-center justify-between border-b border-border pb-2 px-3 py-3">
              <MessageCircleIcon className="w-5 h-5'" />
              <Button
                onClick={() => setNewChatMenuIsOpen(true)}
                variant={"ghost"}
                size={"fit"}
          ...
          <div className="flex flex-col gap-3 w-2/3 h-full">
            {selectedChatId && selectedPhoneNumber ? (
              ...
              <div className="h-full w-full flex items-center flex-col justify-center">
                <MessageCircleIcon className="w-12 h-12 text-muted-foreground mb-2" />
                <p className="text-foreground text-center text-sm italic">
  ...
  return chats ? (
    chats.map((chat) => (
      <button
        ...
        className={cn(
          "w-full flex gap-3 p-3 hover:bg-primary/10 rounded-lg",
        ...
        <div className="grow flex flex-col min-w-0">
          <div className="flex items-center justify-between w-full min-w-0">
            ...
            {chat.ultimaMensagemData && (
              <span className="text-xs text-foreground ml-2 flex-shrink-0">
          ...
          <div className="flex items-center justify-between w-full min-w-0">
            {chat.ultimaMensagemConteudoTipo === "TEXTO" ? (
              <p className="text-start text-sm text-foreground truncate flex-1 min-w-0">
                {chat.ultimaMensagemConteudoTexto || "Nenhuma mensagem ainda"}
              </p>
            ) : (
              <div className="flex items-center gap-1">
                <ImageIcon className="w-4 h-4" />
                <p className="text-sm text-foreground truncate">MÍDIA</p>
              </div>
            )}
    ...
    <p className="text-foreground text-center text-sm italic">Carregando...</p>
  );
}
...
function ChatsHubTestingVinculation({ handleSelect, closeMenu }: ChatsHubTestingVinculationProps) {
  ...
  function handleVinculate() {
    if (!infoHolder.nome || infoHolder.nome.trim().length < 3)
      return toast.error("Preencha um nome válido.");
    if (!infoHolder.telefone || infoHolder.telefone.trim().length < 3)
      return toast.error("Preencha um telefone válido.");
          ...
          <div className="flex flex-col gap-2 px-4 py-2 bg-card border-t border-primary/10 shadow-sm w-[98%] self-center rounded-full">
            ...
            {isConversationExpired && (
              <div className="flex items-center gap-2 px-3 py-1 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-lg w-[90%] self-center">
                ...
                <p className="text-xs text-amber-800 dark:text-amber-200">
                  Janela de 24h expirada. Envie um template aprovado para reiniciar a conversa.
                </p>
              </div>
            )}
            {/* Input e botões */}
            <div className="flex items-end gap-2">
              ...
              <textarea
                ...
                placeholder={
                  isConversationExpired
                    ? "Envie um template para continuar..."
                    : "Digite uma mensagem..."
                }
                className={cn(
                  "flex-1 px-4 py-2 rounded-lg resize-none text-sm transition-colors focus:outline-none align-top",
                )}
                rows={1}
              ...
              <Popover open={showTemplateSelector} onOpenChange={setShowTemplateSelector}>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    size="icon"
                    variant="ghost"
                    className={cn({ "bg-green-500 hover:bg-green-600": isConversationExpired })}
                  >
      ...
      <div className="p-3 bg-card border-b border-primary/10 flex items-center gap-2">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <h2 className="text-sm lg:text-base font-semibold text-primary">
              {chat.cliente?.nome || "Cliente desconhecido"}
            </h2>
            <div className="text-[0.625rem] lg:text-xs flex items-center gap-2 text-foreground">
              ...
              {chat.atendimentoAberto.dataInicio ? (
                <div className="flex items-center gap-1">
                  ...
                  <p className="text-[0.625rem] lg:text-xs">
                    {formatDateAsLocale(new Date(chat.atendimentoAberto.dataInicio), true)}
                  </p>
                </div>
              ) : null}
              <Button
                ...
                onClick={() =>
                  handleUpdateService({
                    serviceId: chat.atendimentoAberto?._id as Id<"services">,
      ...
      <StickToBottom.Content className="p-3">
        {chatMessages && chatMessages.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center h-full">
            <p className="text-foreground0">Nenhuma mensagem ainda</p>
            <p className="text-muted-foreground0 text-sm mt-1">
