...
import type {
  TCorporatePosition,
  TEmployee,
  TEmployeeAuxiliarContact,
  TWorkingHours,
} from "@/utils/schemas/users";
import {
  InstructionLevels,
  NaturalPersonMaritalStatus,
  VinculationCompanies,
...
function EmployeeCorporativeInformation({
  infoHolder,
  updateInfoHolder,
}: CorporativeInformationProps) {
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      ...
      <DateInput
        ...
        handleChange={(value) =>
          updateInfoHolder({ dataAdmissao: formatDateInputChange(value, "string") })
        }
        width="100%"
      />
      ...
      <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
        <div className="w-full lg:w-1/3">
          <TextInput
            ...
            handleChange={(value) =>
              updateInfoHolder({
                carteiraTrabalho: { ...infoHolder.carteiraTrabalho, pisPaseb: value },
              })
            }
            width="100%"
          />
        ...
        <div className="w-full lg:w-1/3">
          <TextInput
            ...
            handleChange={(value) =>
              updateInfoHolder({
                carteiraTrabalho: { ...infoHolder.carteiraTrabalho, numero: value },
              })
            }
            width="100%"
          />
        ...
        <div className="w-full lg:w-1/3">
          <TextInput
            ...
            handleChange={(value) =>
              updateInfoHolder({
                carteiraTrabalho: { ...infoHolder.carteiraTrabalho, serie: value },
              })
            }
            width="100%"
          />
      ...
      <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
        <div className="w-full lg:w-1/2">
          <TextInput
            ...
            handleChange={(value) =>
              updateInfoHolder({
                carteiraTransito: { ...infoHolder.carteiraTransito, numero: value },
              })
            }
            width="100%"
          />
        ...
        <div className="w-full lg:w-1/2">
          <DateInput
            ...
            value={
              infoHolder.carteiraTransito.dataVencimento
                ? formatDate(infoHolder.carteiraTransito.dataVencimento)
                : undefined
            }
            handleChange={(value) =>
              updateInfoHolder({
                carteiraTransito: {
                  ...infoHolder.carteiraTransito,
                  dataVencimento: formatDateInputChange(value, "string"),
                },
              })
            }
            width="100%"
      ...
      <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
        ...
        <div className="w-full lg:w-1/3">
          <SelectInput
            ...
            selectedItemLabel="NÃO DEFINIDO"
            onReset={() =>
              updateInfoHolder({ localizacao: { ...infoHolder.localizacao, uf: "", cidade: "" } })
            }
        ...
        <div className="w-full lg:w-1/3">
          <SelectInput
            ...
            handleChange={(value) =>
              updateInfoHolder({ localizacao: { ...infoHolder.localizacao, cidade: value } })
            }
            options={
            ...
            selectedItemLabel="NÃO DEFINIDO"
            onReset={() =>
              updateInfoHolder({ localizacao: { ...infoHolder.localizacao, cidade: "" } })
            }
            width="100%"
          />
        </div>
      ...
      <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
        <div className="w-full lg:w-1/2">
          <TextInput
            ...
            handleChange={(value) =>
              updateInfoHolder({ localizacao: { ...infoHolder.localizacao, bairro: value } })
            }
            width="100%"
          />
        </div>
        <div className="w-full lg:w-1/2">
          <TextInput
            ...
            handleChange={(value) =>
              updateInfoHolder({ localizacao: { ...infoHolder.localizacao, endereco: value } })
            }
            width="100%"
          />
        </div>
...
function PositionsMenu({ infoHolder, updateInfoHolder }: PositionsMenuProps) {
  ...
  const [positionHolder, setPositionHolder] = useState<TCorporatePosition>({
    cargo: "",
    cbo: "",
    dataInicio: new Date().toISOString(),
  });
  function handleAddPosition(info: TCorporatePosition) {
    if (info.cargo.trim().length < 3)
      return toast.error("Preencha um cargo de ao menos 3 caracteres.");
      ...
      {newPositionModalIsOpen ? (
        ...
        >
          ...
          <DateInput
            ...
            handleChange={(value) =>
              setPositionHolder((prev) => ({
                ...prev,
                dataInicio: formatDateInputChange(value, "string") || new Date().toISOString(),
              }))
            }
            width="100%"
          />
          <DateInput
            ...
            handleChange={(value) =>
              setPositionHolder((prev) => ({
                ...prev,
                dataFinal: formatDateInputChange(value, "string"),
              }))
            }
            width="100%"
          />
...
function WorkingHoursMenu({ infoHolder, updateInfoHolder }: WorkingHoursMenuProps) {
  ...
  function getWorkload(workingHours: TWorkingHours[]) {
    const totalMinutes = workingHours.reduce(
      (acc, current) =>
        acc + getDifferenceBetweenTimes(current.horarioInicio, current.horarioFinal).minutesTotal,
      0,
    );
      ...
      {infoHolder.horariosTrabalho.length ? (
        <div className="flex items-center gap-2 self-center rounded-md bg-[#15599a] px-2 py-1 font-medium text-white">
          ...
          <h1 className="text-sm tracking-tight">
            {getWorkload(infoHolder.horariosTrabalho).hours} HORAS E{" "}
            {getWorkload(infoHolder.horariosTrabalho).minutes} MINUTOS TOTAIS
          </h1>
        </div>
      ) : null}
      <div className="flex w-full flex-wrap items-start justify-around gap-2">
        ...
        {newWorkingHoursModalIsOpen ? (
          ...
          >
            <TimeInput
              ...
              handleChange={(value) =>
                setWorkingHoursHolder((prev) => ({ ...prev, horarioInicio: value || "" }))
              }
              width="100%"
            />
            <TimeInput
              ...
              handleChange={(value) =>
                setWorkingHoursHolder((prev) => ({ ...prev, horarioFinal: value || "" }))
              }
              width="100%"
            />
          </ResponsiveDialogDrawer>
      ...
      {newAuxiliarContactModalIsOpen ? (
        ...
        >
          ...
          <TextInput
            ...
            handleChange={(value) =>
              setContactsHolder((prev) => ({ ...prev, telefone: formatToPhone(value) }))
            }
            width="100%"
          />
        </ResponsiveDialogDrawer>
