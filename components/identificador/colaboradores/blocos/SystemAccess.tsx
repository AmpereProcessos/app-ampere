    ...
    <div className="flex w-full flex-col gap-2">
      <div className="flex items-center gap-2 bg-primary/20 px-2 py-1 rounded w-fit">
        ...
        <h1 className="text-xs tracking-tight font-medium text-start w-fit">
          INFORMAÇÕES DE ACESSO DE SISTEMA
        </h1>
      </div>
          ...
          <CheckboxInput
            ...
            handleChange={(value) =>
              updateInfoHolder({
                acessoAtivo: value,
                visualizacao: {
                  ...infoHolder.visualizacao,
                  tipo: !infoHolder.visualizacao.tipo
                    ? "OPERACIONAL"
                    : infoHolder.visualizacao.tipo,
                },
              })
            }
        ...
        handleChange={(value) =>
          updateInfoHolder({
            visualizacao: {
              ...infoHolder.visualizacao,
              tipo: value,
              referencia: value === "OPERACIONAL" ? null : infoHolder.visualizacao.referencia,
            },
          })
        }
        onReset={() =>
          updateInfoHolder({
            visualizacao: { ...infoHolder.visualizacao, tipo: null, referencia: null },
          })
        }
        width="100%"
      />
      ...
      {infoHolder.visualizacao.tipo === "VENDAS" ? (
        <SelectInput
          ...
          options={allSellers}
          handleChange={(value) =>
            updateInfoHolder({ visualizacao: { ...infoHolder.visualizacao, referencia: value } })
          }
          onReset={() =>
            updateInfoHolder({ visualizacao: { ...infoHolder.visualizacao, referencia: null } })
          }
          width="100%"
        />
      ) : null}
