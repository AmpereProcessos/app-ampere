...
function ProposeProjectVinculation({
  idProject,
  idSolicitation,
  signatureDate,
  contractRequestDate,
}: ProposeProjectVinculationProps) {
  ...
  return (
    <div className="flex min-w-[350px] flex-col">
      <button
        ...
        className={`text-foreground text-center text-sm duration-300 ease-in-out hover:text-cyan-500`}
      >
        {menuIsOpen ? "FECHAR MENU" : "ABRIR MENU DE VINCULAÇÃO"}
      </button>
      {menuIsOpen ? (
        <div className="border-border flex w-full flex-col gap-2 rounded border p-3">
          <TextInput
