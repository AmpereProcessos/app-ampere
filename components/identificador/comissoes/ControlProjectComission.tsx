...
function ControlProjectComission({
  projectId,
  session,
  closeModal,
  callbacks,
}: ControlProjectComissionProps) {
  ...
  const {
    ...
    error,
    isSuccess,
  } = useComissionDataByProjectId({ projectId });
...
function ProjectComissionDataBlock({
  ...
  function updateComissionableItems(item: TComissionableItemsEnum) {
    ...
    updateInfoHolder({
      ...
      comissionableValue: getContractValue({
        ...
        structureValue: newComissionableItems.includes("ESTRUTURA PERSONALIZADA")
          ? project.valorEstruturaPersonalizada
          : 0,
  ...
  function updateComissioned({
    ...
    updateInfoHolder({
      comissioned: infoHolder.comissioned.map((comissioned, i) =>
        i === index ? { ...comissioned, ...changes } : comissioned,
      ),
    });
  }
  function addComissioned(
    ...
    <div className="flex h-full w-full flex-col gap-6">
      <div className="flex w-full flex-col gap-1.5">
        <div className="flex w-full items-center gap-1.5">
          ...
          <h3 className="text-foreground text-sm font-semibold tracking-tighter">
            NOME DO PROJETO
          </h3>
        ...
        {project.comissoes.comissionados.map((comissioned, index) => (
          <div key={`${comissioned.nome}-${index}`} className="flex w-full items-center gap-1.5">
            ...
            <h3 className="text-xs font-semibold tracking-tight">
              COMISSÃO: {comissioned.porcentagem || 0}%
            </h3>
          </div>
        ))}
        ...
        <div className="flex w-full items-center gap-1.5">
          ...
          <h3 className="text-sm font-semibold tracking-tight">
            {formatToMoney(project.valorProjeto || 0)}
          </h3>
        </div>
        <div className="flex w-full items-center gap-1.5">
        ...
        <div className="flex w-full items-center gap-1.5">
          ...
          <h3 className="text-sm font-semibold tracking-tight">
            {formatToMoney(project.valorEstruturaPersonalizada || 0)}
          </h3>
        </div>
        <div className="flex w-full items-center gap-1.5">
          <FaSolarPanel className="h-4 w-4" />
          <h3 className="text-foreground text-sm font-semibold tracking-tighter">VALOR DO OEM</h3>
          <h3 className="text-sm font-semibold tracking-tight">
            {formatToMoney(project.valorOem || 0)}
          </h3>
        </div>
        <div className="flex w-full items-center gap-1.5">
          ...
          <h3 className="text-sm font-semibold tracking-tight">
            {formatToMoney(project.valorSeguro || 0)}
          </h3>
        </div>
        ...
        <div className="flex w-full items-center gap-1.5">
          ...
          <h3 className="text-foreground text-sm font-semibold tracking-tighter">
            VALOR COMISSIONÁVEL SUGERIDO
          </h3>
          <h3 className="text-sm font-semibold tracking-tight">
            {formatToMoney(project.comissoes?.valorComissionavelSugerido || 0)}
          </h3>
        </div>
      ...
      <div className="flex w-full flex-col gap-3">
        ...
        <div className="flex w-full flex-wrap items-center gap-1.5">
          {ComissionableItems.map((item) => {
            const itemIsInComissionableItems = infoHolder.comissionableItems.includes(
              item as TComissionableItemsEnum,
            );
            return (
              <Button
                key={item}
          ...
          <div className="flex w-full items-center justify-between gap-2">
            ...
            <div className="w-fit">
              <Button
                ...
                onClick={() =>
                  updateInfoHolder({
                    comissioned: infoHolder.comissioned.map((comissioned) => ({
                      ...comissioned,
...
function ComissionedCard({
  comissionableValue,
  comissioned,
  updateComissioned,
  removeComissioned,
}: ComissionedCardProps) {
  return (
    <div className="border-border bg-background flex w-full flex-col gap-1 rounded-md border p-2 shadow-xs dark:bg-[#121212]">
      <div className="flex w-full items-center justify-between">
        ...
        >
          ...
          <p className={cn("text-[0.57rem] font-medium")}>
            {comissioned.dataValidacao ? "VALIDADO" : "NÃO VALIDADO"}
          </p>
        </div>
      </div>
      ...
      <div className="flex w-full items-center justify-center">
        <div className="bg-secondary text-foreground flex items-center gap-1 rounded-lg px-2 py-0.5 text-center text-[0.55rem] font-bold italic">
          ...
          <p className="text-[0.57rem] font-black text-[#15599a]">
            {formatToMoney(comissionableValue * (comissioned.porcentagem / 100))}
          </p>
        </div>
      </div>
