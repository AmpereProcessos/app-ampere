...
import NewCertification from "../certificacoes/NewCertification";

type CertificationsBlockProps = {
  session: TAuthSession;
};
export default function CertificationsBlock({ session }: CertificationsBlockProps) {
  ...
  return (
    <div className="flex h-full grow flex-col gap-3">
      ...
      <AnimatePresence>
        {filtersMenuIsOpen ? (
          <CertificationsBlockFiltersMenu filters={filters} updateFilters={updateFilters} />
        ) : null}
      </AnimatePresence>
      {isLoading ? (
...
function CertificationsBlockFiltersMenu({
  filters,
  updateFilters,
}: CertificationsBlockFiltersMenuProps) {
  return (
    <motion.div
...
function CertificationBlockCard({
  certification,
  handleEditClick,
  userHasEditCertificationsPermission,
}: CertificationBlockCardProps) {
  return (
                ...
                <HoverCardContent className="w-80">
                  ...
                  <div className="flex flex-col gap-3">
                    {certification.referencias.map((usage) => (
                      <div
                        key={usage._id}
                        className="bg-primary/10 flex w-full flex-col gap-1 rounded-lg p-2"
                      >
                        <div className="flex w-full items-center gap-2">
                          <Badge className="rounded-full text-[0.65rem] w-fit">
                            {usage.documento.formato}
                          </Badge>
                          <p className="text-xs font-medium tracking-tight">
                            {usage.documento.titulo}
                          </p>
                        </div>
                        <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-xs font-bold tracking-tight">
                            {formatDateAsLocale(usage.dataFim) || "N/A"}
                          </p>
                        </div>
                        <p className="text-xs font-medium tracking-tight text-foreground">
                          NOTIFICADOS
            ...
            <div className="flex items-center gap-1">
              ...
              <p className="text-xs font-medium text-foreground">
                {formatTimeDurationUnit({
                  ...
                  unit: certification.validade.medida,
                  upperCase: true,
                }) || "N/A"}
              </p>
            </div>
        ...
        <div className="flex w-full items-center justify-between gap-1">
          <div className="flex items-center gap-1">
            ...
            <p className="text-foreground text-xs font-medium">
              {formatDateAsLocale(certification.dataInsercao, true) || "N/A"}
            </p>
          </div>
          {userHasEditCertificationsPermission ? (
            <Button
              ...
              size={"fit"}
              onClick={() => handleEditClick(certification._id)}
            >
