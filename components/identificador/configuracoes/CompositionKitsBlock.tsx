...
function CompositionKits({ session }: CompositionKitsProps) {
	...
	const { data: users, queryKey, isLoading, isError, isSuccess, error } = usePurchaseControlCompositionKits();
		...
		<div className="flex h-full grow flex-col gap-3">
			<div className="border-primary/20 flex w-full flex-col items-center justify-between border-b pb-2 lg:flex-row">
				<div className="flex flex-col">
					...
					<p className="text-sm text-primary/60">Gerencie os kits de composição da plataforma</p>
				</div>
			...
			{isLoading ? <h3 className="text-sm text-center my-4 text-primary/60 animate-pulse">Carregando kits de composição...</h3> : null}
			...
			{isSuccess ? (
				<div className="flex w-full flex-col gap-2 py-2">
					{users.length > 0 ? (
						...
						<div className="text-primary/80 w-full text-center font-medium italic">Nenhum kit de composição encontrado.</div>
					)}
				</div>
			) : null}
...
}: { kit: TPurchaseControlCompositionKitDTO; handleEditClick: (id: string) => void; userHasEditPermission: boolean }) {
	return (
		<div className="border-primary bg-background flex w-full flex-col gap-3 rounded border p-2 shadow-xs dark:bg-[#121212]">
			...
			<div className="flex w-full items-center justify-between gap-1">
				<div className="flex items-center gap-1">
					...
					<p className="text-primary/60 text-xs font-medium">{formatDateAsLocale(kit.dataInsercao, true) || "N/A"}</p>
				</div>
				{userHasEditPermission ? (
					<Button variant={"ghost"} className="flex items-center gap-1 px-2 py-1" size={"fit"} onClick={() => handleEditClick(kit._id)}>
