...
import {
  TGetContractTemplatesOutput,
  type TGetContractTemplatesOutputDefault,
    ...
    <div className="flex h-full grow flex-col">
      <div className="border-border flex w-full flex-col items-center justify-between border-b pb-2 lg:flex-row">
        <div className="flex flex-col">
          ...
          <p className="text-sm text-foreground">Gerencie os templates de contrato</p>
        </div>
...
function ContractTemplatesBlockCard({
  contractTemplate,
  userHasEditPermission,
  handleEditClick,
}: ContractTemplatesBlockCardProps) {
  return (
        ...
        <div className="w-full flex items-center justify-between gap-2">
          <div className="flex flex-wrap items-center gap-2">
            <p className="text-sm leading-none font-bold tracking-tight">
              {contractTemplate.titulo}
            </p>
          </div>
        </div>
        <p className="text-sm text-foreground">{contractTemplate.descricao}</p>
      </div>
      <div className="flex w-full items-center justify-end gap-1">
        {userHasEditPermission ? (
          <Button
            ...
            size={"fit"}
            onClick={() => handleEditClick(contractTemplate._id)}
          >
