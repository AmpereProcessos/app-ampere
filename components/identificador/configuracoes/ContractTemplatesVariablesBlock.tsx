...
function ContractTemplatesVariablesBlock({ session }: ContractTemplatesVariablesBlockProps) {
  ...
  const {
    data: contractTemplateVariables,
    queryKey,
    isLoading,
    isError,
    isSuccess,
  ...
  const [newContractTemplateVariableModalIsOpen, setNewContractTemplateVariableModalIsOpen] =
    useState(false);
  const [editContractTemplateVariableId, setEditContractTemplateVariableId] = useState<
    string | null
  >(null);
    ...
    <div className="flex h-full grow flex-col">
      <div className="border-border flex w-full flex-col items-center justify-between border-b pb-2 lg:flex-row">
        <div className="flex flex-col">
          ...
          <p className="text-sm text-foreground">Gerencie as variáveis de template de contrato</p>
        </div>
      ...
      {isLoading ? (
        <h3 className="text-sm text-foreground animate-pulse">Carregando variáveis...</h3>
      ) : null}
        ...
        <div className="flex w-full flex-col gap-2 py-2">
          {contractTemplateVariables.length > 0 ? (
            ...
            <div className="text-foreground w-full text-center font-medium italic">
              Nenhuma variável de template de contrato encontrada.
            </div>
          )}
        </div>
      ) : null}
  ...
  return (
    <div className="border-primary bg-background flex w-full flex-col gap-3 rounded border p-2 shadow-xs dark:bg-[#121212]">
      <div className="w-full flex flex-col gap-1">
        <div className="w-full flex items-center justify-between gap-2">
          <div className="flex flex-wrap items-center gap-2">
            <p className="text-sm leading-none font-bold tracking-tight">
              {contractTemplateVariable.titulo}
            </p>
            <div className="flex items-center gap-2 px-2 py-0.5 rounded-lg bg-primary/20">
              ...
              <p className="text-[0.65rem] font-bold tracking-tight">
                {contractTemplateVariable.identificador}
              </p>
            </div>
          </div>
          <h1
        ...
        <p className="text-sm text-foreground">{contractTemplateVariable.descricao}</p>
      </div>
