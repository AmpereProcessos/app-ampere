    ...
    <div className="flex h-full grow flex-col gap-3">
      <div className="border-border flex w-full flex-col items-center justify-between border-b pb-2 lg:flex-row">
        <div className="flex flex-col">
          ...
          <p className="text-sm text-foreground">Gerencie os colaboradores da plataforma</p>
        </div>
      ...
      <AnimatePresence>
        {filtersMenuIsOpen ? (
          <UserBlockFiltersMenu filters={filters} updateFilters={(value) => updateFilters(value)} />
        ) : null}
      </AnimatePresence>
      <EmployeesBlockFiltersShowcase
        filters={filters}
...
function EmployeeBlockCard({
  ...
  user: TGetEmployeesOutputDefault[number];
  ...
  return (
    <div className="flex w-full flex-wrap items-center justify-center gap-2 lg:justify-end">
      ...
      {filters.accessActiveOnly ? (
        <FilterTag
          label="APENAS COLABORADORES COM ACESSO ATIVO"
          value="SIM"
          onRemove={() => updateFilters({ accessActiveOnly: false })}
        />
      ) : null}
    </div>
  );
