...
import { formatDateAsLocale, formatDateBirthdayAsLocale, formatNameAsInitials } from "@/utils/methods/formatting";
...
export default function ProfileBlock({ session }: ProfileBlockProps) {
	...
	return (
		<div className="flex h-full grow flex-col">
						...
						<div className="grow flex flex-col gap-1 py-2">
							...
							<div className="flex items-center gap-x-3 gap-y-1 flex-wrap">
								...
								<div className="flex items-center gap-1">
									...
									<p className="text-sm">{profile.dataNascimento ? formatDateBirthdayAsLocale(profile.dataNascimento) : "NÃO DEFINIDO"}</p>
								</div>
							</div>
						</div>
					</div>
			...
			{editProfileModalIsOpen ? <EditProfile session={session} closeModal={() => setEditProfileModalIsOpen(false)} /> : null}
		</div>
	);
}
