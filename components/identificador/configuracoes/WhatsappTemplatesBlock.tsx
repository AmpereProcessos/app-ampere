...
function WhatsappTemplatesBlock({ session }: WhatsappTemplatesBlockProps) {
  ...
  return (
    <div className="flex h-full grow flex-col">
      ...
      {isLoading ? (
        <h3 className="text-sm text-foreground animate-pulse py-4">Carregando templates...</h3>
      ) : null}
      ...
      {isSuccess ? (
        <div className="flex w-full flex-col gap-2 py-2">
          {templates.length > 0 ? (
            ...
            <div className="text-foreground w-full text-center font-medium italic py-8">
              Nenhum template de WhatsApp encontrado.
            </div>
          )}
        </div>
      ) : null}
...
function WhatsappTemplatesBlockCard({
  template,
  userHasEditPermission,
  handleEditClick,
}: WhatsappTemplatesBlockCardProps) {
      ...
      <div className="w-full flex flex-col gap-2">
        <div className="w-full flex items-center justify-between gap-2">
          <div className="flex flex-wrap items-center gap-2">
            <span className={"text-sm leading-none font-bold tracking-tight font-mono"}>
              TEMPLATE
            </span>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <p className="text-xs font-medium text-foreground">
              {LANGUAGE_MAP[template.idioma as keyof typeof LANGUAGE_MAP]}
            </p>
          </div>
          <div className="flex items-center gap-1">
            <Variable className="w-4 h-4 min-w-4 min-h-4" />
            <p className="text-xs font-medium text-foreground">
              {template.formatoParametros === "positional" ? "POSICIONAL" : "NOMEADO"}
            </p>
          </div>
          <div className="flex items-center gap-1">
            <CircleGauge className="w-4 h-4 min-w-4 min-h-4" />
            <p className="text-xs font-medium text-foreground">{template.qualidade}</p>
          </div>
        </div>
      </div>
