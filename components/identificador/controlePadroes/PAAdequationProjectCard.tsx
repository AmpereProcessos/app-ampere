import type {
  TEnergyPAExecution,
  TEnergyPAExecutionWithFiltersOutput,
} from "@/pages/api/gestao-obras/padroes";
...
function PAAdequationProjectCard({ project, callbacks }: PAAdequationProjectCardProps) {
  ...
  async function updatePAAdequationExecutionDate(date: string | null) {
    try {
      await updateProject({
        id: project._id,
        changes: { "padrao.aumentoCarga.dataEfetivacao": date },
      });
  ...
  return (
    <div className="bg-background border-border flex w-full flex-col gap-4 rounded border p-3 font-[Inter]">
      <div className="flex w-full flex-col items-start justify-between gap-2 lg:flex-row lg:items-center">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div
            className={cn(
              "flex items-center gap-1 self-center rounded-lg px-2 py-1",
              getServiceTypeTagColor(project.tipoDeServico || ""),
            )}
          >
            <MdDashboard className="h-3 min-h-3 w-3 min-w-3" />
          ...
          <div className="flex items-center gap-1">
            <FaLocationDot />
            <p className="text-foreground text-[0.65rem] font-medium italic">
        ...
        <div className="flex w-full flex-wrap items-center justify-center gap-2 lg:grow lg:justify-start">
          ...
          >
            ...
            <p className={cn("text-[0.6rem] font-medium")}>
              PARECER DE ACESSO: {project.homologacao.status}
            </p>
          </div>
          <div
            className={cn(
          ...
          >
            ...
            <p className={cn("text-[0.6rem] font-medium")}>
              DOCUMENTAÇÃO:{" "}
              {project.homologacao.documentacao.dataConclusaoElaboracao ? "CONCLUÍDA" : "PENDENTE"}
            </p>
          </div>
        </div>
      </div>
      <div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
        <div className="flex w-full flex-wrap items-center justify-center gap-2 lg:grow lg:justify-start">
          ...
          >
            ...
            <p className={cn("text-[0.6rem] font-medium")}>
              PAGAMENTO: {project.compra.dataPagamento ? "CONCLUÍDO" : "PENDENTE"}
            </p>
          </div>
          <div
            className={cn(
          ...
          >
            ...
            <p className={cn("text-[0.6rem] font-medium")}>
              RESPONSÁVEL: {project.padrao.respInstalacao || "N/A"}
            </p>
          </div>
          <div
            className={cn(
          ...
          {project.ordemDeServico ? (
            <>
              ...
              >
                ...
                <p className={cn("text-[0.6rem] font-medium")}>
                  TÍTULO: {project.ordemDeServico?.descricao || "N/A"}
                </p>
              </div>
              <div
                className={cn(
              ...
              >
                <Contact className={cn("h-3 min-h-3 w-3 min-w-3")} />
                <p className={cn("text-[0.6rem] font-medium")}>
                  RESPONSÁVEL: {project.ordemDeServico?.responsavel?.nome || "N/A"}
                </p>
              </div>
              <div
                className={cn(
              ...
              >
                ...
                <p className={cn("text-[0.6rem] font-medium")}>
                  AGENDAMENTO:{" "}
                  {project.ordemDeServico?.agendamento?.inicio
                    ? formatDateAsLocale(project.ordemDeServico?.agendamento?.inicio)
                    : "N/A"}{" "}
                  -{" "}
                  {project.ordemDeServico?.agendamento?.fim
                    ? formatDateAsLocale(project.ordemDeServico?.agendamento?.fim)
                    : "N/A"}
                </p>
              </div>
              <div
              ...
              >
                ...
                <p className={cn("text-[0.6rem] font-medium")}>
                  EXECUÇÃO:{" "}
                  {project.ordemDeServico?.periodo.inicio
                    ? formatDateAsLocale(project.ordemDeServico?.periodo.inicio)
                    : "N/A"}{" "}
                  -{" "}
                  {project.ordemDeServico?.periodo.fim
                    ? formatDateAsLocale(project.ordemDeServico?.periodo.fim)
                    : "N/A"}
                </p>
              </div>
            </>
            ...
            <div
              className={cn(
                "text-xxs flex items-center gap-1 rounded-lg bg-orange-100 px-2 py-0.5 text-center font-bold text-orange-700 italic",
              )}
            >
