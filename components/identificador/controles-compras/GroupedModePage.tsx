...
import {
  type TPurchaseControlKanbanSimplifiedDTO,
...
function PurchaseControlsGroupedModePage({
  session,
  handleSetMode,
  initialListExpandedModeOptions,
}: PurchaseControlsGroupedModePageProps) {
  ...
  function getPurchaseControlsByStatus(
    ...
    return Object.entries({
      ...
      "AGUARDANDO NOTA FUTURA": purchaseControls.filter(
        (c) => c.status === "AGUARDANDO NOTA FUTURA",
      ),
      "AGUARDANDO PAGAMENTO": purchaseControls.filter((c) => c.status === "AGUARDANDO PAGAMENTO"),
      "AGUARDANDO FATURAMENTO": purchaseControls.filter(
        (c) => c.status === "AGUARDANDO FATURAMENTO",
      ),
...
function PurchaseControlGroupBlock({
  data,
  handleItemClick,
  affectedQueryKey,
  initialListExpandedModeOptions,
}: PurchaseControlsGroupBlockProps) {
      ...
      >
        ...
        <div className="flex items-center gap-1">
          ...
          <h1 className="text-xs font-medium tracking-tight text-white">
            {data.items.length > 1
              ? `${data.items.length} controles de compra`
              : `${data.items.length} controle de compra`}
          </h1>
        </div>
      </div>
      {showItems ? (
...
function PurchaseControlCard({
  purchaseControl,
  handleClick,
  affectedQueryKey,
}: PurchaseControlCardProps) {
  const queryClient = useQueryClient();
...
function PurchaseControlStatusController({
  ...
  const { mutate, isPending } = useMutation({
    ...
    onMutate: async (variables) => {
      if (updateCallbacks?.onMutate)
        updateCallbacks.onMutate({ id: variables.id, updates: variables.changes });
    },
              ...
              <button
                ...
                onClick={() =>
                  mutate({
                    ...
                    changes: {
                      status: status.value as TPurchaseControlKanbanSimplifiedDTO["status"],
                    },
                  })
                }
              >
