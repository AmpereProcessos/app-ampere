...
import type {
  TPurchaseControlKanbanListExpandedModes,
  TPurchasesControlPageModes,
} from "@/pages/suprimentos/controle-compras";
...
function PurchaseControlsKanbanModePage({
  session,
  handleSetMode,
  initialKanbanListExpandedModeOptions,
}: PurchaseControlsKanbanModePageProps) {
  ...
  function getPurchaseControlsByStatus(
    purchaseControls: TPurchaseControlKanbanSimplifiedDTO[] | undefined,
  ): TPurchaseControlByStatus[] {
    if (!purchaseControls)
    ...
    return Object.entries({
      ...
      "EM COTAÇÃO": purchaseControls
        .filter((c) => c.status === "EM COTAÇÃO")
        .sort((a, b) => handleSortByStatusLog("EM COTAÇÃO", a, b)),
  ...
  function onDragEnd(dragEndResult: DropResult) {
    function handleEffectivationUpdate(
      newValue: TPurchaseControlKanbanSimplifiedDTO["status"],
      previousData: TPurchaseControlKanbanSimplifiedDTO,
    ) {
    ...
    function handleStatusLogs(
      newValue: TPurchaseControl["status"],
      previousData: TPurchaseControlKanbanSimplifiedDTO,
    ) {
    ...
    handleUpdatePurchaseControlStatus({
      ...
      changes: {
        ...
        dataEfetivacao: handleEffectivationUpdate(
          destination.droppableId as TPurchaseControlKanbanSimplifiedDTO["status"],
          purchaseControl,
        ),
        registrosStatus: handleStatusLogs(
          destination.droppableId as TPurchaseControlKanbanSimplifiedDTO["status"],
          purchaseControl,
        ),
      },
  ...
  const { mutate: handleUpdatePurchaseControlStatus } = useMutation({
    ...
    onMutate: async (variables) => {
      ...
      const querySnapshot = queryClient.getQueryData(["purchase-controls"]) as
        | TPurchaseControlKanbanSimplifiedDTO[]
        | undefined;

      if (!querySnapshot) return { querySnapshot };
      const update = querySnapshot.map((prev) =>
        prev._id === variables.id
          ? { ...prev, status: variables.changes.status || "INDEFINIDO" }
          : prev,
      );
  ...
  return (
    <div className="w-full h-full flex flex-col gap-3">
        ...
        <div className="flex w-full flex-col items-center justify-end gap-2 gap-y-3 lg:flex-row">
          <div className="flex w-full flex-col items-center gap-1 lg:w-fit lg:flex-row lg:items-end">
            <MultipleSelectInput
              ...
              options={
                tags?.map((tag) => ({ id: tag._id, value: tag._id, label: tag.titulo })) || []
              }
            ...
            <button
              type="button"
              onClick={() =>
                updateQueryParams({ queryPendingConclusion: !queryParams.queryPendingConclusion })
              }
              className={cn(
            ...
            <Button
              onClick={() => setNewPurchaseControlModalIsOpen(true)}
              className="min-h-[46.6px]"
            >
              NOVO CONTROLE
            </Button>
          </div>
      ...
      {vinculationMenuIsOpen ? (
        <NewTransportControl session={session} closeModal={() => setVinculationMenuIsOpen(false)} />
      ) : null}
    </div>
  );
}
...
function FunnelList({
  ...
  const [funnelListItemsExpandedModeActive, setFunnelListItemsExpandedModeActive] =
    useState<boolean>(!(initialKanbanListExpandedMode === "inactive"));
  const deadlineDays = getFunnelListStatusDeadlineDays(
    title as TPurchaseControlKanbanSimplifiedDTO["status"],
  );
  return (
          ...
          <div className="flex w-full flex-col rounded bg-[#15599a] px-2 lg:h-[60px]">
            <div className="flex w-full items-center gap-2">
              ...
              <button
                ...
                className={cn(
                  "hidden items-center rounded p-1 text-white",
                  funnelListItemsExpandedModeActive ? "bg-gray-50" : "bg-transparent",
                )}
              >
                {funnelListItemsExpandedModeActive ? <IoMdContract /> : <IoMdExpand />}
              </button>
...
function FunnelListItem({
  ...
  const [itemExpandedModeActive, setItemExpandedModeActive] = useState(
    funnelListItemsExpandedModeActive,
  );

  function getItemDeadlineStatus(item: TPurchaseControlKanbanSimplifiedDTO) {
    const statusDeadlineDays = PurchaseControlStatus.find(
        ...
        >
          <div className="flex w-full items-center justify-between gap-2">
            <div className="flex items-center gap-1">
              <button
                ...
                className={cn(
                  "flex items-center rounded-full p-1 text-xs text-black",
                  itemExpandedModeActive ? "bg-slate-400" : "bg-transparent",
                )}
              >
                ...
                <div className="flex w-full flex-wrap items-center justify-around gap-2">
                  <div className="flex items-center gap-1">
                    ...
                    <h1 className="text-foreground text-[0.6rem] font-medium">
                      {item.fornecedor.nome || "FORNECEDOR INDEFINIDO"}
                    </h1>
                  </div>
                  <div className="flex items-center gap-1">
                    <ScrollText height={13} width={13} />
                    <h1 className="text-foreground text-[0.6rem] font-medium">
                      FATURAMENTOS {item.faturamentos.filter((f) => !!f.data).length}/
                      {item.faturamentos.length}
                    </h1>
                  </div>
                  <div className="flex items-center gap-1">
                ...
                <div className="flex w-full flex-wrap items-center justify-center gap-2">
                  <div className="flex items-center gap-1">
                    ...
                    <h1 className="text-primary py-0.5 text-center text-[0.6rem] font-bold">
                      {formatDateAsLocale(item.dataPedido) || "N/A"}
                    </h1>
                  </div>
                  <div className="flex items-center gap-1">
          ...
          <div className="flex w-full items-center justify-between gap-2">
            <div className="flex grow flex-wrap items-center gap-2">
              <div className="flex items-center gap-1">
                <Avatar
                  url={item.autor.avatar_url || undefined}
                  fallback={formatNameAsInitials(item.autor.nome)}
                  height={18}
                  width={18}
                />
            ...
            <div className="flex items-center gap-2">
              <div className="flex min-w-fit items-center gap-1">
                <BsCalendarPlus />
                <p className={"text-foreground text-[0.65rem] font-medium"}>
                  {formatDateAsLocale(item.dataInsercao, true)}
                </p>
              </div>
              ...
              {item.dataEfetivacao ? (
                <div className="flex items-center gap-1">
                  ...
                  <p className="text-foreground text-[0.65rem] font-medium">
                    {formatDateAsLocale(item.dataEfetivacao, true)}
                  </p>
                </div>
              ) : null}
            </div>
