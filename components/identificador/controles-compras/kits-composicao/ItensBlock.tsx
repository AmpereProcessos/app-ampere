...
function PurchaseControlCompositionKitItemBlock({ items, addItem, updateItem, removeItem }: PurchaseControlCompositionKitItemBlockProps) {
	...
	return (
		<div className="w-full flex flex-col gap-3">
			<h1 className="w-full text-start tracking-tight text-sm">ITENS</h1>
			<div className="flex w-full items-center justify-end">
				<button
					...
					className={cn("flex items-center gap-1 rounded-lg px-2 py-1 text-black duration-300 ease-in-out", {
						"bg-gray-300  hover:bg-red-300": newItemMenuIsOpen,
				...
				<div className="flex w-full flex-col gap-2 bg-[#fff] p-1 dark:bg-[#121212]">
					{items.length > 0 ? (
						items.map((item, index) => (
							<ItemCard key={`${item.materialId}-${index}`} item={{ ...item }} handleUpdate={(item) => updateItem({ changes: item, index })} handleRemove={() => removeItem(index)} />
						))
					) : (
						<p className="w-full text-center text-sm font-medium tracking-tight text-primary/80">Não há itens de composição definidos.</p>
					)}
				</div>
			</div>
		</div>
