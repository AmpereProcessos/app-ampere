...
type PurchaseControlCompositionKitItemBlockProps = {
  ...
  updateItem: (info: {
    index: number;
    changes: Partial<TPurchaseControlCompositionKit["itens"][number]>;
  }) => void;
  removeItem: (index: number) => void;
};
function PurchaseControlCompositionKitItemBlock({
  items,
  addItem,
  updateItem,
  removeItem,
}: PurchaseControlCompositionKitItemBlockProps) {
      ...
      <div className="flex w-full items-center justify-end">
        ...
        >
          ...
          <h1 className="text-xs font-medium tracking-tight">
            {!newItemMenuIsOpen ? "ABRIR MENU DE NOVO ITEM" : "FECHAR MENU DE NOVO ITEM"}
          </h1>
        </button>
      </div>
      <AnimatePresence>
        {newItemMenuIsOpen ? <NewItemMenu addItem={addItem} /> : null}
      </AnimatePresence>
        ...
        <div className="bg-background flex w-full flex-col gap-2 p-1 dark:bg-[#121212]">
          {items.length > 0 ? (
            ...
            <p className="text-foreground w-full text-center text-sm font-medium tracking-tight">
              Não há itens de composição definidos.
            </p>
          )}
        </div>
      </div>
    ...
    >
      ...
      <div className="flex w-full grow flex-col gap-2 p-3">
        <MaterialSelector
          ...
          unvinculateMaterial={() =>
            setItemHolder((prev) => ({ ...prev, materialId: null, descricao: "", valor: 0 }))
          }
        />
...
function ItemCard({ item, handleUpdate, handleRemove }: ItemCardProps) {
  const [itemHolder, setItemHolder] =
    useState<TPurchaseControlCompositionKit["itens"][number]>(item);
