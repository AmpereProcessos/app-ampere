...
export default function PurchaseControlVinculationMenu({ closeMenu, handleSelect }: PurchaseControlVinculationMenuProps) {
	const { data: purchaseControlsResult, isLoading, isError, isSuccess, error, filters, updateFilters } = usePurchaseControlsByFilters();
				...
				{isSuccess ? (
					<>
						...
						{purchaseControls.length > 0 ? (
							...
							<h1 className="text-primary/80 w-full text-center text-sm font-medium tracking-tight">Nenhum controle de compra encontrado.</h1>
						)}
					</>
				) : null}
			</div>
...
function PurchaseControlCard({ purchase, handleSelect }: PurchaseControlCardProps) {
	return (
		<div className="border-primary/30 bg-background flex w-full flex-col gap-1 rounded-lg border p-3 shadow-xs dark:bg-[#121212]">
			...
			<div className="w-full flex items-center justify-between gap-2">
				<div className="flex flex-wrap items-center gap-2">
					<div className="flex items-center gap-1">
						...
						<p className="text-primary/80 text-[0.65rem] font-medium">{formatDateAsLocale(purchase.dataInsercao, true)}</p>
					</div>
					{purchase.dataEfetivacao ? (
						<div className="flex items-center gap-1">
							<BsCalendarCheck className="w-4 h-4 min-w-4 min-h-4 text-green-500" />
							<p className="text-primary/80 text-[0.65rem] font-medium">{formatDateAsLocale(purchase.dataEfetivacao, true)}</p>
						</div>
					) : null}
					<div className="flex items-center gap-1">
						<Avatar className="w-4 h-4 min-w-4 min-h-4">
							<AvatarImage src={purchase.autor.avatar_url || undefined} />
							<AvatarFallback className="text-xs">{formatNameAsInitials(purchase.autor.nome)}</AvatarFallback>
						</Avatar>
						<p className="text-primary/80 text-[0.65rem] font-medium">{purchase.autor.nome}</p>
					</div>
				</div>
				<Button variant={"ghost"} className="flex items-center gap-1 px-2 py-1" size={"fit"} onClick={() => handleSelect(purchase)}>
					<LinkIcon className="h-4 min-h-4 w-4 min-w-4" />
					<p className="text-primary/80 text-sm font-semibold">VINCULAR</p>
				</Button>
			</div>
		</div>
	);
