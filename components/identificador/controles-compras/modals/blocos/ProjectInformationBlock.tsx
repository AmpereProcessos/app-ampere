	...
	return (
		<div className="flex w-full grow flex-col gap-4">
			...
			<div className="flex w-full grow flex-col gap-2">
				...
				<h1 className="bg-primary/60 w-full p-1 text-center text-xs font-medium text-white">OBSERVAÇÕES DA OBRA</h1>
				<div className="flex w-full items-center justify-end">
					<button
						type="button"
					...
					>
						...
						<h1 className="text-xs font-medium tracking-tight">
							{!newExecutionObservationsMenuIsOpen ? "ABRIR MENU DE NOVA OBSERVAÇÃO" : "FECHAR MENU DE NOVA OBSERVAÇÃO"}
						</h1>
					</button>
				</div>
...
function NewExecutionObservationsMenu({ projectId, currentExecutionObservations, callbacks, closeMenu }: NewExecutionObservationsMenuProps) {
	const [newExecutionObservationsHolder, setNewExecutionObservationsHolder] = useState<TServiceOrder["observacoes"][number]>({
		topico: "",
		descricao: "",
	});
	...
	const { mutate: mutateNewExecutionObservations, isPending } = useMutation({
		mutationFn: async (info: TServiceOrder["observacoes"][number]) => {
			return await updateProject({
				id: projectId,
				changes: {
					"obra.observacoes": [...currentExecutionObservations, info].map((obs) => `${obs.topico}: ${obs.descricao}`).join("/"),
				},
			});
		},
		onMutate: () => {
	...
	return (
		...
		>
			<TextInput
				...
				handleChange={(value) => setNewExecutionObservationsHolder((prev) => ({ ...prev, topico: value }))}
				width="100%"
			/>
			<TextareaInput
				label="DESCRIÇÃO"
				...
				handleChange={(value) => setNewExecutionObservationsHolder((prev) => ({ ...prev, descricao: value }))}
			/>
		</ResponsiveDialogDrawer>
	);
}
