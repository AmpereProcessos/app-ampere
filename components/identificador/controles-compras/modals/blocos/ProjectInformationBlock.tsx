...
import {
  createServiceOrder,
  updateManyServiceOrdersByProjectId,
} from "@/utils/methods/mutation/service-orders";
...
function PurchaseControlProjectInformationBlock({
  ...
  const [newExecutionObservationsMenuIsOpen, setNewExecutionObservationsMenuIsOpen] =
  ...
  const executionObservations = getServiceObservationsFromObras(project.obra.observacoes || "");
  return (
    <div className="flex w-full grow flex-col gap-4">
      ...
      <div className="flex w-full grow flex-col gap-2">
          ...
          <div className="flex flex-col items-center gap-1 lg:items-start">
            ...
            <div className="flex flex-wrap items-center justify-center gap-4 lg:justify-start">
              ...
              <div className="flex items-center gap-1">
                <FaLocationDot />
                <p className="text-[0.6rem] leading-none font-medium tracking-tight">
                  {memoizedProjectLocation || "NÃO DEFINIDA"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center gap-1 lg:items-start">
            ...
            <div className="flex flex-wrap items-center justify-center gap-4 lg:justify-start">
              ...
              <div className="flex items-center gap-1">
                ...
                <p className="text-[0.6rem] leading-none font-medium tracking-tight">
                  {project?.pagamento.pagador.cpfCnpj || "N/A"}
                </p>
              </div>
              <div className="flex items-center gap-1">
                <FaPhone className="w-4 h-4 min-w-4 min-h-4" />
                <p className="text-[0.6rem] leading-none font-medium tracking-tight">
                  {project?.pagamento.pagador.telefone}
                </p>
              </div>
              <div className="flex items-center gap-1">
                <MapPin className="w-4 h-4 min-w-4 min-h-4" />
                <p className="text-[0.6rem] leading-none font-medium tracking-tight">
                  {memoizedPayerLocation || "NÃO DEFINIDA"}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex w-full items-center justify-end">
          <button
            type="button"
            onClick={() => setNewExecutionObservationsMenuIsOpen((prev) => !prev)}
            className={cn(
              "flex items-center gap-1 rounded-lg px-2 py-1 text-black duration-300 ease-in-out",
              {
                "bg-primary/20 hover:bg-red-300": newExecutionObservationsMenuIsOpen,
                "bg-green-300 hover:bg-green-400": !newExecutionObservationsMenuIsOpen,
              },
            )}
          >
            <TextIcon className="w-4 h-4 min-w-4 min-h-4" />
            <h1 className="text-xs font-medium tracking-tight">
              {!newExecutionObservationsMenuIsOpen
                ? "ABRIR MENU DE NOVA OBSERVAÇÃO"
                : "FECHAR MENU DE NOVA OBSERVAÇÃO"}
            </h1>
          </button>
        </div>
        <div className="bg-primary/10 flex lg:w-1/2 w-full flex-col gap-1 self-center items-center justify-center rounded p-2">
                ...
                <div key={index.toString()} className="flex w-full items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-foreground text-xs tracking-tight">
                      {observation.topico}: {observation.descricao}
                    </p>
                  </div>
                </div>
              ))
        ...
        <div className="flex items-center justify-center">
          <div className="w-fit">
            <CheckboxInput
              ...
              handleChange={(value) =>
                updatePurchase({
                  metadata: { pendenciasExecucao: value ? "LEVAR ESTRUTURA NA MONTAGEM" : null },
                })
              }
            />
          </div>
              ...
              >
                ...
                <h1 className="text-xs font-medium tracking-tight">
                  {!technicalAnalysisBlockIsOpen
                    ? "ABRIR INFORMAÇÕES DA ANÁLISE TÉCNICA"
                    : "FECHAR INFORMAÇÕES DA ANÁLISE TÉCNICA"}
                </h1>
              </button>
            </div>
            {technicalAnalysisBlockIsOpen ? (
      ...
      {newExecutionObservationsMenuIsOpen ? (
        <NewExecutionObservationsMenu
          projectId={project._id}
          currentExecutionObservations={executionObservations}
          callbacks={callbacks}
          closeMenu={() => setNewExecutionObservationsMenuIsOpen(false)}
        />
      ) : null}
    </div>
  );
...
type NewExecutionObservationsMenuProps = {
  projectId: string;
  currentExecutionObservations: TServiceOrder["observacoes"][number][];
  callbacks?: {
    onMutate?: () => void;
    onSuccess?: () => void;
    onSettled?: () => void;
...
function NewExecutionObservationsMenu({
  projectId,
  currentExecutionObservations,
  callbacks,
  closeMenu,
}: NewExecutionObservationsMenuProps) {
  const [newExecutionObservationsHolder, setNewExecutionObservationsHolder] = useState<
    TServiceOrder["observacoes"][number]
  >({
    topico: "",
    descricao: "",
  });

  const { mutate: mutateNewExecutionObservations, isPending } = useMutation({
    mutationFn: async (info: TServiceOrder["observacoes"][number]) => {
      return await updateProject({
        id: projectId,
        changes: {
          "obra.observacoes": [...currentExecutionObservations, info]
            .map((obs) => `${obs.topico}: ${obs.descricao}`)
            .join("/"),
        },
      });
    },
  ...
  return (
    <ResponsiveDialogDrawer
      menuTitle="NOVA OBSERVAÇÃO"
      menuDescription="Preencha aqui as informações da nova observação."
      menuActionButtonText="ADICIONAR OBSERVAÇÃO"
      menuCancelButtonText="CANCELAR"
      actionFunction={() => mutateNewExecutionObservations(newExecutionObservationsHolder)}
    ...
    >
      <TextInput
        ...
        value={newExecutionObservationsHolder.topico}
        handleChange={(value) =>
          setNewExecutionObservationsHolder((prev) => ({ ...prev, topico: value }))
        }
        width="100%"
      />
      <TextareaInput
        ...
        value={newExecutionObservationsHolder.descricao}
        handleChange={(value) =>
          setNewExecutionObservationsHolder((prev) => ({ ...prev, descricao: value }))
        }
      />
    </ResponsiveDialogDrawer>
  );
...
function ProjectServiceOrderBlock({ session, project, projectId }: ProjectServiceOrderBlockProps) {
  ...
  const {
    ...
    isError,
    isSuccess,
  } = useProjectServiceOrders({ projectId });
  ...
  const handleMutate = async () =>
    queryClient.cancelQueries({ queryKey: ["project-service-orders", projectId] });
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      ...
      <div className="flex w-full items-center justify-end">
        {!project.idOrdemServico ? (
          ...
          >
            <MdDesignServices />
            <h1 className="text-xs font-medium tracking-tight">
              GERAR ORDEM DE SERVIÇO DO PROJETO
            </h1>
          </button>
        ) : null}
      </div>
      {isLoading ? <LoadingComponent /> : null}
      {isError ? (
        <ErrorComponent msg={"Erro ao buscar informações das ordens de serviço."} />
      ) : null}
      {isSuccess ? (
...
function ServiceOrderCard({ serviceOrder, handleClick }: ServiceOrderCardProps) {
  function getStatusTag(serviceOrder: TServiceOrderSimplifiedDTO) {
    if (serviceOrder.status === "PENDENTE")
      return (
        <div className="text-xxs rounded-full bg-red-600 px-2 py-0.5 font-medium text-white">
          PENDENTE
        </div>
      );
    ...
    if (serviceOrder.status === "AGUARDANDO PLANEJAMENTO")
      return (
        <div className="text-xxs rounded-full bg-blue-800 px-2 py-0.5 font-medium text-white">
    ...
    if (serviceOrder.status === "AGUARDANDO AGENDAMENTO")
      return (
        <div className="text-xxs rounded-full bg-yellow-600 px-2 py-0.5 font-medium text-white">
          AGENDADA
        </div>
      );
    ...
    if (serviceOrder.status === "EM EXECUÇÃO")
      return (
        <div className="text-xxs rounded-full bg-blue-600 px-2 py-0.5 font-medium text-white">
          EM EXECUÇÃO
        </div>
      );
    ...
    if (serviceOrder.status === "CONCLUÍDA PARCIAL")
      return (
        <div className="text-xxs rounded-full bg-purple-600 px-2 py-0.5 font-medium text-white">
          CONCLUÍDA PARCIAL
        </div>
      );
    ...
    if (serviceOrder.status === "CONCLUÍDA")
      return (
        <h1 className="text-xxs min-w-fit rounded-lg bg-green-500 px-2 py-0.5 text-white">
          CONCLUÍDA
        </h1>
      );
    ...
    if (serviceOrder.status === "CANCELADA")
      return (
        <h1 className="text-xxs bg-primary/60 min-w-fit rounded-lg px-2 py-0.5 text-white">
          CANCELADA
        </h1>
      );

    return (
      <h1 className="bg-primary text-xxs min-w-fit rounded-lg px-2 py-0.5 text-white">
        NÃO DEFINIDO
      </h1>
    );
  }
      ...
      <div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
        ...
        <div className="flex w-full flex-wrap items-center justify-center gap-2 lg:min-w-fit lg:justify-end">
          ...
          <div className="flex items-center gap-1">
            ...
            <h1 className="text-primary py-0.5 text-center text-[0.6rem] font-bold">
              {serviceOrder.dataPrevisaoLiberacao
                ? formatDateAsLocale(serviceOrder.dataPrevisaoLiberacao)
                : "N/A"}
            </h1>
          </div>
          <div className="flex items-center gap-1">
