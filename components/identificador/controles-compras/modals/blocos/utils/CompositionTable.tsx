		...
		<div className="border-primary/80 flex w-full flex-col rounded border-0 lg:border">
			...
			<div className="bg-background flex w-full flex-col gap-2 p-1 dark:bg-[#121212]">
				{composition.length > 0 ? (
					...
					<p className="text-primary/80 w-full text-center text-sm font-medium tracking-tight">Não há itens de composição da compra.</p>
				)}
			</div>
		</div>
	);
						...
						<div className="flex w-[30%] items-center gap-1">
							...
							<div className="flex flex-col">
								...
								<div className="flex items-center gap-2">
									<p className="text-primary/80 text-[0.65rem] leading-none tracking-tight italic">{item.categoria}</p>
									{item.qtdeFaturamento && item.valorFaturamento && item.unidadeFaturamento ? (
										<p className="text-primary/80 text-[0.65rem] leading-none tracking-tight italic">
						...
						<div className="flex w-full items-center justify-between gap-2">
							<div className="flex items-center gap-2">
								...
								{item.valor > 0 ? (
									<div className="flex items-center gap-1">
										<FaDollarSign />
										<p className="text-primary/80 text-[0.6rem] italic lg:text-xs">
							...
							{PurchaseCompositionItemCategories.map((category) => (
								<button
									...
									onClick={() =>
										setItemHolder((prev) => ({
											...prev,
											categoria: category.value as "MÓDULO" | "INVERSOR" | "INSUMO" | "ESTRUTURA" | "PADRÃO" | "OUTROS",
										}))
									}
								>
