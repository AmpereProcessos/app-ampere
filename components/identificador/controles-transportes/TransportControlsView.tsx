...
export default function TransportControlsView({
  session,
  handleSetMode,
}: TransportControlsViewProps) {
  ...
  const { mutate: handleDeleteTransportControl, isPending: deleteTransportControlIsPending } =
    useMutation({
      mutationKey: ["delete-transport-control"],
      mutationFn: async (transportControlId: string) =>
        await deleteTransportControl({ transportControlId }),
      onSuccess: () => {
  ...
  const deleteTransportControlReference = transportControls?.find(
    (control) => control._id === deleteTransportControlId,
  );
  return (
    <div className="w-full h-full flex flex-col gap-3">
      <div className="w-full flex items-center justify-end">
        <Button
          onClick={() => setNewTransportControlModalIsOpen(true)}
      ...
      {viewTransportControlId ? (
        <ViewTransportControl
          transportControlId={viewTransportControlId}
          closeModal={() => setViewTransportControlId(null)}
        />
      ) : null}
      ...
      {editTransportControlId ? (
        <EditTransportControl
          transportControlId={editTransportControlId}
          closeModal={() => setEditTransportControlId(null)}
        />
      ) : null}
          ...
          <div className="flex flex-col gap-3">
            <p className="text-sm">
              ...
              <span className="font-semibold">
                #{deleteTransportControlReference?.identificador || "---"}
              </span>{" "}
              {deleteTransportControlReference?.titulo
                ? `- ${deleteTransportControlReference.titulo}`
                : ""}
            </p>
            <LoadingButton
...
function TransportControlCard({
  transportControl,
  handleClick,
  handleValidate,
  handleDelete,
  handleEdit,
}: TransportControlCardProps) {
        ...
        <div className="flex items-center gap-2">
          <div className="flex item-center gap-1">
            ...
            <p className="text-xs font-medium">
              {transportControl.itens.length}{" "}
              {transportControl.itens.length > 1 ? "TRANSPORTES" : "TRANSPORTE"}
            </p>
          </div>
          <h1
            className={cn("flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium", {
              "bg-green-200 text-green-600":
                ...
                new Date(transportControl.dataFim) < new Date(),
              "bg-orange-200 text-orange-600":
                !!transportControl.dataFim &&
                !transportControl.dataValidacao &&
                new Date(transportControl.dataFim) < new Date(),
              "bg-blue-200 text-blue-600":
                !!transportControl.dataInicio && new Date(transportControl.dataInicio) > new Date(),
              "bg-yellow-200 text-yellow-600":
                !transportControl.dataFim && !transportControl.dataInicio,
            })}
          >
            {transportControl.dataFim
              ? transportControl.dataValidacao
      ...
      <div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            onClick={() =>
              copyToClipboard(
                `${process.env.NEXT_PUBLIC_APP_URL}/publico/formulario-transporte/${transportControl._id}`,
              )
            }
            variant={"ghost"}
            size={"fit"}
