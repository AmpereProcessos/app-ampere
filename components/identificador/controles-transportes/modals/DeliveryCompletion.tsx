...
export default function DeliveryCompletionModal({ transportControlId, transportItem, session, closeMenu, onSuccess }: DeliveryCompletionModalProps) {
	...
	async function handleCompleteDelivery() {
		...
		try {
			...
			await queryClient.invalidateQueries({ queryKey: ["transport-control-by-id-public", transportControlId] });
					...
					<div className="flex w-full flex-wrap gap-4">
						{attachments
							...
							.map((attachment, index) => (
								<div key={`${attachment.file?.name}-${index}`} className="border-primary/50 relative flex h-[100px] max-h-[100px] w-[80px] flex-col rounded border">
