		...
		>
			...
			{isSuccess && transportControl ? (
				<>
					<ResponsiveDialogDrawerSection sectionTitleText="INFORMAÇÕES GERAIS" sectionTitleIcon={<LayoutGrid size={15} />}>
						<div className="flex flex-col gap-2">
							<div className="flex items-center gap-1">
								...
								<h2 className="text-sm font-bold tracking-tight">#{transportControl.identificador}</h2>
							</div>
							<div className="flex items-center gap-1">
					...
					<ResponsiveDialogDrawerSection sectionTitleText="QUILOMETRAGEM" sectionTitleIcon={<MapPin size={15} />}>
						<div className="flex flex-col gap-2">
							<div className="flex items-center gap-1">
								...
								<div className="flex items-center gap-1">
									...
									{transportControl.distanciaQuilometragemInicialImagemUrl ? (
										<Button
											...
											onClick={() => window.open(transportControl.distanciaQuilometragemInicialImagemUrl as string, "_blank")}
										>
							...
							<div className="flex items-center gap-1">
								...
								<div className="flex items-center gap-1">
									<h2 className="text-sm font-bold tracking-tight">
										{transportControl.distanciaQuilometragemFinal ? `${formatDecimalPlaces(transportControl.distanciaQuilometragemFinal)} km` : "NÃO DEFINIDA"}
									</h2>
									{transportControl.distanciaQuilometragemFinalImagemUrl ? (
										<Button
											...
											size="fit"
											className="rounded-full p-2"
											onClick={() => window.open(transportControl.distanciaQuilometragemFinalImagemUrl as string, "_blank")}
										>
											<ExternalLink size={15} />
										</Button>
									) : null}
							...
							<div className="flex items-center gap-1">
								...
								<div className="flex items-center gap-1">
									<h2 className="text-sm font-bold tracking-tight">
										{formatDecimalPlaces((transportControl.distanciaQuilometragemFinal ?? 0) - (transportControl.distanciaQuilometragemInicial ?? 0))} km
									</h2>
								</div>
							</div>
						</div>
					...
					<ResponsiveDialogDrawerSection sectionTitleText="ENTREGAS/ITENS" sectionTitleIcon={<Package size={15} />}>
						<div className="flex flex-col gap-2">
										...
										<div className="w-full flex items-center justify-between gap-2">
											...
											{item.dataEfetivacao && (
												<div className="flex items-center gap-1 px-2 py-1 bg-green-200 text-green-600 rounded-lg">
													...
													<p className="text-xs font-medium tracking-tight">{formatDateAsLocale(item.dataEfetivacao)}</p>
												</div>
											)}
										</div>
