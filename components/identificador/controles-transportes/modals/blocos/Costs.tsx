...
export default function CostsBlock({ costs, addCost, removeCost }: CostsBlockProps) {
	...
	return (
		<ResponsiveDialogDrawerSection sectionTitleText="CUSTOS" sectionTitleIcon={<DollarSign size={15} />}>
			...
			{newCostMenuIsOpen ? <NewCostMenu addCost={addCost} closeMenu={() => setNewCostMenuIsOpen(false)} /> : null}
...
function NewCostMenu({ addCost, closeMenu }: NewCostMenuProps) {
	...
	function handleAddCost(info: TUseTransportControlState["state"]["transportControl"]["custos"][number]) {
