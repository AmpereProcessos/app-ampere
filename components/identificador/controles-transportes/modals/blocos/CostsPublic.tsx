...
export default function CostsSection({ transportControlId, costs, session }: CostsSectionProps) {
  ...
  return (
    <div className="flex w-full flex-col gap-4 rounded-lg border border-border bg-card p-6 shadow-sm">
      ...
      {costs.length > 0 ? (
        <div className="space-y-2">
          {costs.map((cost, index) => (
            <div key={index.toString()} className="rounded-lg border border-border bg-muted/50 p-4">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  {cost.anexos && cost.anexos.length > 0 && (
                    <p className="mt-1 text-xs text-muted-foreground">
                      {cost.anexos.length} anexo(s)
                    </p>
                  )}
                </div>
              </div>
