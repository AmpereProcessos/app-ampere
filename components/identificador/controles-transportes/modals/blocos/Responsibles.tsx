...
export default function ResponsiblesBlock({
  responsaveis,
  updateTransportControl,
}: ResponsiblesBlockProps) {
  const {
    data: employeeByCpf,
    isFetching,
    isFetched,
  ...
  const employeeAlreadyAdded =
    !!employeeByCpf && responsaveis.some((responsible) => responsible.id === employeeByCpf.id);
  ...
  function handleRemoveResponsible(responsibleId: string) {
    updateTransportControl({
      responsaveis: responsaveis.filter((responsible) => responsible.id !== responsibleId),
    });
  }

  return (
    <ResponsiveDialogDrawerSection
      sectionTitleText="RESPONSÁVEIS"
      sectionTitleIcon={<UserRound size={15} />}
    >
      <div className="w-full rounded-lg px-3 py-3 flex flex-col items-center gap-2 bg-primary/10">
        <div className="w-full">
          <input
            ...
            value={cpf}
            onChange={(e) =>
              updateQueryParams({ cpf: formatToCPForCNPJ(e.target.value), email: "" })
            }
        ...
        {isFetching ? (
          <p className="text-foreground text-xs font-medium">Validando CPF...</p>
        ) : null}
        {cpfNotFound ? (
          <p className="text-red-500 text-xs font-medium">
            Nenhum colaborador encontrado para o CPF informado.
          </p>
        ) : null}
        {employeeByCpf ? (
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-1">
                    ...
                    <p className="text-[0.65rem] text-foreground">{employeeByCpf.cpf}</p>
                  </div>
                  <div className="flex items-center gap-1">
                    <Phone className="h-3 w-3" />
                    <p className="text-[0.65rem] text-foreground">{employeeByCpf.telefone}</p>
                  </div>
                </div>
              </div>
            </div>
            {employeeAlreadyAdded ? (
              <p className="text-[0.65rem] text-foreground font-medium">Já adicionado</p>
            ) : null}
          </div>
        ) : null}
        <Button
      ...
      <div className="w-full flex flex-col gap-2">
        {responsaveis.length > 0 ? (
          responsaveis.map((responsible) => (
            <div
              key={responsible.id}
              className="w-full border border-border rounded-lg px-2 py-2 flex items-center justify-between gap-2"
            >
              <div className="flex items-center gap-2">
                <Avatar className="h-7 min-h-7 w-7 min-w-7">
                ...
                <div className="flex flex-col">
                  ...
                  <p className="text-[0.65rem] text-foreground">{responsible.telefone}</p>
                </div>
              </div>
              <Button
                type="button"
          ...
          <p className="text-xs text-foreground font-medium">Nenhum responsável adicionado.</p>
        )}
      </div>
    </ResponsiveDialogDrawerSection>
  );
