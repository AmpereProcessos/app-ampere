...
function ClientsVinculationMenu({ initialFilters, handleSelect, closeModal }: ClientsVinculationMenuProps) {
	const { data: clientsResult, isLoading, isError, isSuccess, error, filters, updateFilters } = useClientsSearch({ initialFilters });
		...
		>
			...
			{isSuccess ? (
				<>
					...
					/>
					{clients.length > 0 ? (
						clients.map((client) => <ClientCard key={client._id} client={client} handleSelect={() => handleSelect(client)} />)
					) : (
						<h1 className="text-primary/80 w-full text-center text-sm font-medium tracking-tight">Nenhum cliente encontrado.</h1>
					)}
				</>
			) : null}
		</ResponsiveDialogDrawerViewOnly>
			...
			<div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
				<div className="flex grow flex-wrap items-center justify-center gap-2 lg:justify-start">
					<div className="flex items-center gap-1">
						...
						<h1 className="text-primary/80 py-0.5 text-center text-[0.6rem] font-medium italic">{client.telefonePrimario}</h1>
					</div>
					<div className="flex items-center gap-1">
