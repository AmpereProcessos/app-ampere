...
import HomologationActivity from "./Utils/HomologationActivity";
...
function ActivitiesInformation({ homologation, session, opportunity }: ActivitiesInformationProps) {
  ...
  const {
    ...
    isError,
    isSuccess,
  } = useActivitiesByHomologationId({ homologationId: homologation._id });
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      ...
      <div className="mt-2 flex w-full flex-col gap-1">
        ...
        {isError ? (
          <ErrorComponent msg="Houve um erro ao buscar atividades da homologação." />
        ) : null}
        {isSuccess ? (
          activities.length > 0 ? (
            <>
              {menuEnabled ? (
                ...
                <p className="text-foreground flex w-full grow items-center justify-center gap-2 py-2 text-center font-medium tracking-tight italic">
                  <strong className="text-orange-500">{openActivityCount} </strong> atividades em
                  aberto.
                </p>
              )}
            </>
