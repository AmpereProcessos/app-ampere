...
import type { TFileHolder, TFileReference } from "@/utils/schemas/crm/file-reference.schema";
import type { THomologation } from "@/utils/schemas/crm/homologation.schema";
import type { TOpportunityDTOWithClient } from "@/utils/schemas/crm/opportunity.schema";
  ...
  async function requestHomologation({ info, files }: { info: THomologation; files: TFileHolder }) {
    try {
      ...
      const links = await handleUploadFiles({
        files: files,
        homologationId: insertedHomologationId,
      });
