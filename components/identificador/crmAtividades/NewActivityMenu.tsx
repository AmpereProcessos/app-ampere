...
import { formatDateTime, formatNameAsInitials } from "@/utils/methods/formatting";
import { createActivity } from "@/utils/methods/mutation/crm/activities";
import { useMutationWithFeedback } from "@/utils/methods/mutation/general-hook";
import { useUsers } from "@/utils/methods/query/crm/users";
import { formatDateInputChange } from "@/utils/methods/shared";
import { TActivity } from "@/utils/schemas/crm/activities.schema";
import { TUserDTO } from "@/utils/schemas/crm/users.schema";
...
function NewActivityMenu({
  ...
  const { data: users } = useUsers({ includeDeleted: false });
          ...
          >
            <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
              <div className="w-1/2 lg:w-full">
                <TextInput
                  ...
                  handleChange={(value) =>
                    setNewActivityHolder((prev) => ({ ...prev, titulo: value }))
                  }
                  width="100%"
                />
              </div>
                ...
                <DateTimeInput
                  ...
                  handleChange={(value) =>
                    setNewActivityHolder((prev) => ({
                      ...prev,
                      dataVencimento: formatDateInputChange(value) as string | null,
                    }))
                  }
                  width="100%"
                />
            ...
            <div className="border-primary/20 flex w-full flex-col rounded-md border p-2 shadow-xs">
              ...
              <input
                ...
                onChange={(e) =>
                  setNewActivityHolder((prev) => ({ ...prev, descricao: e.target.value }))
                }
                type="text"
            ...
            <div className="flex w-full items-center gap-2">
              <div className="flex items-end gap-2">
                      ...
                      startContent: (
                        <AvatarUI className="h-5 w-5 min-h-5 min-w-5">
                          ...
                          <AvatarFallback className="text-xs">
                            {formatNameAsInitials(resp.nome)}
                          </AvatarFallback>
                        </AvatarUI>
                      ),
                    })) || []
                ...
                <button
                  onClick={() =>
                    vinculateResponsible({ id: newResponsibleHolder, users: users || [] })
                  }
              ...
              <div className="flex grow items-center justify-start gap-2">
                {newActivityHolder.responsaveis.map((resp, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-2 rounded-lg border border-cyan-500 p-2 shadow-xs"
                  >
                    <Avatar
                      width={25}
                      height={25}
