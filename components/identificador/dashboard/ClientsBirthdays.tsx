...
function ClientsBirthdays() {
	...
	return (
		<div className="mt-4 flex grow flex-col border  border-gray-300 bg-[#fff] p-4 shadow-xl">
			...
			<div className="mt-2 flex w-full grow flex-wrap justify-center gap-y-2 lg:justify-between">
				...
				{isSuccess
					? birthdays.map((birthday, index) => (
							<div key={index} className="flex w-full gap-2 rounded-md border border-gray-300 bg-[#fff] p-3 text-center text-xs shadow-sm lg:w-[450px]">
								...
								<div className="flex items-center justify-center gap-2 ">
									...
									<p className="font-bold text-[#DA0C81]">{birthday.dataNascimento != undefined && new Date(birthday.dataNascimento).toLocaleDateString()}</p>
								</div>
							</div>
						))
					: null}
