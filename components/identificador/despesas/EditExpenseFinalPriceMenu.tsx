...
function EditExpenseFinalPriceMenu({ infoHolder, setInfoHolder }: EditExpenseFinalPriceMenuProps) {
  ...
  function handleValueEditing(newFinalValue: number | null) {
    ...
    const newExpenseItens = infoHolder.itens.map((i) => ({
      ...i,
      preco: i.preco * percentageMultiplier,
    }));
        ...
        {editFinalPriceMenuIsOpen ? (
          ...
          >
            ...
            <p className="text-foreground w-full text-center text-xs tracking-tight">
              Preencha o valor final desejado para despesa e clique em{" "}
              <strong className="text-green-600">CONFIRMAR</strong>
            </p>
            <p className="text-foreground w-full text-center text-xs tracking-tight">
              Ao confirmar, reajustaremos individualmente os valores de todos os itens com base na
              taxa de alteração.
            </p>
