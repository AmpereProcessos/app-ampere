...
function renderPendencyStatus({ expense }: { expense: TExpenseDTO }) {
  ...
  if (!hasPendency)
    return (
      <div className="flex items-center gap-2">
        ...
        <p className="text-primary/60 text-[0.65rem] font-medium lg:text-xs">PAGO</p>
      </div>
    )
  ...
  if (isToday)
    return (
      <div className="flex items-center gap-2">
        ...
        <p className="text-primary/60 text-[0.65rem] font-medium lg:text-xs">PAGAR HOJE</p>
      </div>
    )
  ...
  if (isOverdue)
    return (
      <div className="flex items-center gap-2">
        ...
        <p className="text-primary/60 text-[0.65rem] font-medium lg:text-xs">ATRASADO</p>
      </div>
    )
  ...
  return (
    <div className="flex items-center gap-2">
      <MdOutlineAssignmentLate color="#ffbd00" />
      <p className="text-primary/60 text-[0.65rem] font-medium lg:text-xs">À PAGAR</p>
    </div>
  )
}
...
function ExpenseCard({ expense, openModal }: ExpenseCardProps) {
  return (
    <div className="border-primary/20 flex w-full flex-col rounded-md border p-4 lg:w-[480px]">
      <div className="flex w-full items-center justify-between gap-2">
        <h1
          onClick={() => openModal(expense._id)}
          className="grow cursor-pointer text-xs leading-none font-black tracking-tight hover:text-cyan-500 lg:text-sm"
        >
          {expense.categoria}
        </h1>
      ...
      <div className="mt-1 flex w-full flex-wrap items-center justify-between">
        <div className="flex items-center gap-2">
          <FaDiamond />
          <p className="text-primary/60 text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">{expense.rateio}</p>
        </div>
        {expense.projeto?.id ? (
          <div className="flex items-center gap-2">
            <BsCode size={'20px'} color="rgb(31,41,55)" />
            <p className="text-primary/60 text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">{expense.projeto?.nome}</p>
          </div>
        ) : null}
      </div>
      <div className="mt-1 flex w-full flex-wrap items-center justify-between">
      ...
      <div className="mt-2 flex w-full items-center justify-between">
        <div className={`flex items-center gap-2`}>
          <BsCalendarPlus />
          <p className="text-primary/60 text-xs font-medium">{formatDateAsLocale(expense.dataInsercao)}</p>
        </div>
        <div className="flex items-center justify-center gap-2">
          <Avatar fallback={'U'} height={25} width={25} url={expense.autor?.avatar_url || undefined} />
          <p className="text-primary/60 text-xs font-medium">{expense.autor?.nome}</p>
        </div>
      </div>
    </div>
  )
  ...
  //     className={`flex h-[150px] w-full cursor-pointer items-center gap-2 rounded-lg border border-primary/20 shadow-md duration-300 ease-in-out hover:bg-blue-100 dark:hover:bg-primary/10 lg:w-[450px]`}
  //   >
  //     <div
  ...
  //         <p className={`${expense.projeto ? '' : 'text-primary/20'} font-raleway font-bold`}>
  ...
  //               : 'border-primary/80 text-primary/80  opacity-30'
  //           }`}
  //         >
  //           {expense.idFormularioAlmoxarifado ? (
  ...
  //           <p className="font-raleway text-sm font-medium text-primary/80">{dayjs(expense.dataInsercao).format('DD/MM/YYYY')}</p>
  //         </div>
  //         <div className="flex items-center gap-2">
  //           <FaUserAlt color="#15599a" size="20px" />
  //           <p className="upper font-raleway text-sm font-medium text-primary/80">{expense.autor.nome}</p>
  //         </div>
  //         <div className={`flex items-center gap-2`}>
  //           <RiMoneyDollarCircleFill color="#15599a" size="23px" />
  //           <p className="font-raleway text-sm font-medium text-primary/80">{formatToMoney(expense.total)}</p>
  //         </div>
  //       </div>
  //     </div>
  //   </div>
