...
function ExpenseCard({ expense, openModal }: ExpenseCardProps) {
	return (
		<div className="flex w-full flex-col rounded-md border border-gray-300 p-4 lg:w-[480px]">
			<div className="flex w-full items-center justify-between gap-2">
				<h1 onClick={() => openModal(expense._id)} className="grow cursor-pointer text-xs font-black leading-none tracking-tight hover:text-cyan-500 lg:text-sm">
					{expense.categoria}
				</h1>
				<h1 className="min-w-fit rounded-full bg-gray-800 px-2 py-1 text-[0.58rem] font-medium text-white lg:text-xs">{formatToMoney(expense.total)}</h1>
			</div>
	...
	//     className={`flex h-[150px] w-full cursor-pointer items-center gap-2 rounded-lg border border-gray-300 shadow-md duration-300 ease-in-out hover:bg-blue-100 lg:w-[450px]`}
	//   >
	//     <div
