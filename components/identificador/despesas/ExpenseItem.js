...
function ExpenseItem({ expense }) {
	...
	return (
		<div className="border-primary/20 flex w-full flex-col items-center gap-2 border p-3 shadow-xs lg:w-[40%]">
			...
			<div className="flex w-full items-center justify-center gap-2">
				<div className="flex items-center gap-2">
					...
					<p className="text-primary/60 text-xs font-medium">Criada em: {dayjs(expense.dataInsercao).format("DD/MM/YYYY HH:mm")}</p>
				</div>
				<div className="flex items-center gap-2">
					<FaUserAlt color="rgb(34,197,94)" />
					<p className="text-primary/60 text-xs font-medium">Por {expense.autor.nome}</p>
				</div>
			</div>
			{expense.idFormularioAlmoxarifado ? (
				<Link href={`/almoxarifado/pdfFormulario/${expense.idFormularioAlmoxarifado}`}>
					<div className="mt-2 flex cursor-pointer items-center rounded border border-orange-200 p-2 py-2 pl-2 duration-300 ease-in hover:scale-[1.02] hover:bg-orange-200">
						...
						<p className="text-primary/80 pl-3 text-xs font-medium">FORMULÁRIO (PDF)</p>
					</div>
				</Link>
			) : null}
			{showItems ? (
			...
			{expense.descricao ? <p className="text-primary/60 w-full text-center text-sm italic">{expense.descricao}</p> : null}
			<h1 className="text-primary/60 mt-1 w-full text-center text-sm">TOTAL DA DESPESA</h1>
			<h1 className="font-raleway w-full text-center font-bold text-red-500">
				R${" "}
