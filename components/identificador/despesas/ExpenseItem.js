...
function ExpenseItem({ expense }) {
  ...
  return (
    <div className="border-border flex w-full flex-col items-center gap-2 border p-3 shadow-xs lg:w-[40%]">
      ...
      <div className="flex w-full items-center justify-center gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <p className="text-foreground text-xs font-medium">Por {expense.autor.nome}</p>
        </div>
      </div>
      {expense.idFormularioAlmoxarifado ? (
        <Link href={`/almoxarifado/pdfFormulario/${expense.idFormularioAlmoxarifado}`}>
          <div className="mt-2 flex cursor-pointer items-center rounded border border-orange-200 p-2 py-2 pl-2 duration-300 ease-in hover:scale-[1.02] hover:bg-orange-200">
            ...
            <p className="text-foreground pl-3 text-xs font-medium">FORMULÁRIO (PDF)</p>
          </div>
        </Link>
      ) : null}
      {showItems ? (
        <div className="flex w-full flex-col items-center">
          <button
            ...
            className="flex items-center gap-2 text-sm font-medium text-blue-300 hover:text-blue-700"
          >
      ...
      {expense.descricao ? (
        <p className="text-foreground w-full text-center text-sm italic">{expense.descricao}</p>
      ) : null}
      <h1 className="text-foreground mt-1 w-full text-center text-sm">TOTAL DA DESPESA</h1>
