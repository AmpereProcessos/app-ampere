...
import NewExpense from "./modals/NewExpense";
...
function ExpensesPage({ session }: ExpensesPageProps) {
	...
	const storedExpensesApportionmentsFilter = storedExpensesApportionmentsFilterStr ? (JSON.parse(storedExpensesApportionmentsFilterStr) as string[]) : [];
	...
	const storedPaymentsApportionmentsFilter = storedPaymentsApportionmentsFilterStr ? (JSON.parse(storedPaymentsApportionmentsFilterStr) as string[]) : [];
		...
		<div className="flex grow flex-col gap-2 p-6">
			...
			<div className="flex max-h-[600px] w-full flex-col gap-6 lg:flex-row">
				<div className="w-full lg:w-[40%]">
					<PaymentsBlock session={session} storedPaymentsApportionmentsFilter={storedPaymentsApportionmentsFilter} storedPaymentsCategoriesFilter={storedPaymentsCategoriesFilter} />
				</div>
				<div className="w-full lg:w-[60%]">
					<ExpensesBlock session={session} storedExpensesApportionmentsFilter={storedExpensesApportionmentsFilter} storedExpensesCategoriesFilter={storedExpensesCategoriesFilter} />
				</div>
			</div>
