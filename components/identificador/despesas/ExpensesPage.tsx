...
function ExpensesPage({ session }: ExpensesPageProps) {
  ...
  const storedExpensesApportionmentsFilterStr = localStorage.getItem(
    "expenses-apportionments-filter",
  );
  ...
  const storedExpensesCategoriesFilter = storedExpensesCategoriesFilterStr
    ? (JSON.parse(storedExpensesCategoriesFilterStr) as string[])
    : [];

  const storedPaymentsApportionmentsFilterStr = localStorage.getItem(
    "payments-apportionments-filter",
  );
  ...
  const storedPaymentsCategoriesFilter = storedPaymentsCategoriesFilterStr
    ? (JSON.parse(storedPaymentsCategoriesFilterStr) as string[])
    : [];
  ...
  return (
    <div className="flex grow flex-col gap-2 p-6">
      <div className="border-border flex flex-col items-center justify-between border-b p-1">
        <div className="flex w-full items-center justify-between">
          <div className="flex flex-col items-center gap-2 lg:flex-row">
            <p className="text-center text-2xl font-black text-[#15599a] uppercase">DESPESAS</p>
          </div>
      ...
      {newExpenseModalIsOpen ? (
        <NewExpense session={session} closeModal={() => setNewExpenseModalIsOpen(false)} />
      ) : null}
    </div>
  );
}
