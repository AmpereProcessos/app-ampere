...
function Payments({ infoHolder, setInfoHolder, missingPercentage }: PaymentsProps) {
	const [newFractionnementHolder, setNewFractionnementHolder] = useState<TExpense["pagamentos"][number]>({
		valor: 0,
	...
	function addPayment() {
		...
		if (!newFractionnementHolder.dataPrevisaoPagamento) return toast.error("Preencha uma data de previsão de pagamento.");
			...
			<div className="item-center flex w-full flex-col gap-2 lg:flex-row">
				...
				<div className="w-full lg:w-1/4">
					<DateInput
						...
						labelClassName="text-sm tracking-tight"
						value={newFractionnementHolder.dataPrevisaoPagamento ? formatDate(newFractionnementHolder.dataPrevisaoPagamento) : undefined}
						handleChange={(value) => setNewFractionnementHolder((prev) => ({ ...prev, dataPrevisaoPagamento: formatDateInputChange(value) }))}
						width="100%"
					/>
				</div>
				<div className="w-full lg:w-1/4">
					<DateInput
						...
						labelClassName="text-sm tracking-tight"
						value={newFractionnementHolder.dataPagamento ? formatDate(newFractionnementHolder.dataPagamento) : undefined}
						handleChange={(value) => setNewFractionnementHolder((prev) => ({ ...prev, dataPagamento: formatDateInputChange(value) }))}
						width="100%"
					/>
				</div>
			</div>
...
function PaymentCard({ payment, paymentIndex, infoHolder, setInfoHolder, deletePayment }: PaymentCardProps) {
			...
			<div className="flex w-full flex-col items-center gap-2 lg:flex-row">
				<div className="w-full lg:w-1/4">
					<NumberInput
						...
						handleChange={(value) => updateFractionnementIndex({ info: { ...payment, valor: value, porcentagem: (value / infoHolder.total) * 100 } })}
						width="100%"
					/>
				</div>
				<div className="w-full lg:w-1/4">
					<NumberInput
						...
						handleChange={(value) => updateFractionnementIndex({ info: { ...payment, porcentagem: value, valor: (value * infoHolder.total) / 100 } })}
						width="100%"
					/>
				</div>
				<div className="w-full lg:w-1/4">
					<DateInput
						label="PREVISÃO DE PAGAMENTO"
						labelClassName="text-sm tracking-tight"
						value={payment.dataPrevisaoPagamento ? formatDate(payment.dataPrevisaoPagamento) : undefined}
						handleChange={(value) => updateFractionnementIndex({ info: { ...payment, dataPrevisaoPagamento: formatDateInputChange(value) } })}
						width="100%"
					/>
				</div>
				<div className="w-full lg:w-1/4">
					<DateInput
						...
						handleChange={(value) => updateFractionnementIndex({ info: { ...payment, dataPagamento: formatDateInputChange(value) } })}
						width="100%"
					/>
				</div>
			</div>
