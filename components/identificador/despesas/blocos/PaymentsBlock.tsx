...
import type { TPaymentUnwindSimplifiedDTO } from "@/utils/schemas/expenses";
...
function PaymentsBlock({
  session,
  storedPaymentsApportionmentsFilter,
  storedPaymentsCategoriesFilter,
}: PaymentsBlockProps) {
  ...
  const {
    ...
    initialFilters: {
      search: "",
      apportionments: storedPaymentsApportionmentsFilter,
      categories: storedPaymentsCategoriesFilter,
      previewPeriod: {},
    },
  });
  return (
    <div className="border-primary bg-background flex h-full max-h-full w-full flex-col gap-2 rounded border p-3 shadow-xs dark:bg-[#121212]">
      ...
      <AnimatePresence>
        {filterMenuIsOpen ? <PaymentsFilterMenu filters={filters} setFilters={setFilters} /> : null}
      </AnimatePresence>
...
function PaymentCard({ payment, handleClick }: PaymentCardProps) {
  function getStatusTag(payment: TPaymentUnwindSimplifiedDTO) {
    if (payment.pagamento.dataPagamento)
      return (
      ...
      <div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
        <div className="flex flex-wrap items-center gap-2">
          <p className="text-sm leading-none font-bold tracking-tight">
            {payment.pagamento.titulo}
          </p>
          <div className="bg-secondary text-xxs text-foreground flex items-center gap-1 rounded-lg px-2 py-0.5 text-center font-medium italic">
            <FaPercentage />
            <h1>
              PARCIAL DE{" "}
              <strong>
      ...
      <div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
        <div className="flex w-full flex-wrap items-center justify-center gap-2 lg:grow lg:justify-start">
          ...
          <div className="flex items-center gap-1">
            ...
            <h1 className="text-xxs text-primary py-0.5 text-center font-bold">
              {payment.categoria}
            </h1>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-1">
            ...
            <p className="text-primary py-0.5 text-center text-[0.6rem] font-bold">
              {formatDateAsLocale(payment.pagamento.dataPrevisaoPagamento)}
            </p>
          </div>
        </div>
        <button
  ...
  return (
    ...
    >
      <MultipleSelectInput
        ...
        options={CostApportionments.map((c, index) => ({
          id: index + 1,
          label: c.nome,
          value: c.nome,
        }))}
      ...
      <MultipleSelectInput
        ...
        options={CostApportionments.flatMap((c) => c.categorias).map((c, index) => ({
          id: index + 1,
          label: c.label,
          value: c.value,
        }))}
      ...
      <DateInput
        ...
        handleChange={(value) =>
          setFilters((prev) => ({
            ...prev,
            previewPeriod: { ...prev.previewPeriod, after: formatDateInputChange(value) },
          }))
        }
        width="100%"
      />
      ...
      <DateInput
        ...
        handleChange={(value) =>
          setFilters((prev) => ({
            ...prev,
            previewPeriod: { ...prev.previewPeriod, before: formatDateInputChange(value) },
          }))
        }
        width="100%"
      />
