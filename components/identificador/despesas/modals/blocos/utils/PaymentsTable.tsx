      ...
      <div className="bg-background flex w-full flex-col gap-2 p-1 dark:bg-[#121212]">
        {payments.length > 0 ? (
          ...
          <p className="text-foreground w-full text-center text-sm font-medium tracking-tight">
            Não há registros de pagamentos da despesa.
          </p>
        )}
        {paymentsTotal > expenseTotal ? (
          <p className="w-full rounded border border-orange-400 bg-orange-50 p-1 text-center text-xs tracking-tight text-orange-400 italic">
            Por favor, ajuste os valores dos pagamentos. A somatória dos pagamentos atuais excede o
            valor total estabelecido para a despesa.
          </p>
        ) : null}
      </div>
    </div>
...
function PaymentTableItem({
  item,
  expenseTotal,
  handleUpdate,
  handleRemove,
}: PaymentTableItemProps) {
      ...
      <AnimatePresence>
        <div className="bg-background hidden w-full flex-col gap-1 lg:flex dark:bg-[#121212]">
          <div className="flex w-full items-center gap-2 p-1">
            ...
            <h1 className="w-[20%] text-center text-xs tracking-tight">
              {formatDateAsLocale(item.dataPrevisaoPagamento || undefined) || "-"}
            </h1>
            <h1 className="w-[20%] text-center text-xs tracking-tight">
              {formatDateAsLocale(item.dataPagamento || undefined) || "-"}
            </h1>
          </div>
        </div>
            ...
            <div className="flex items-center gap-2">
              {item.dataPagamento ? (
                <div className="flex items-center gap-1">
                  ...
                  <p className="text-foreground text-[0.6rem] lg:text-xs">PAGO</p>
                </div>
              ) : null}
              <div className="flex items-center gap-1">
                <BsCalendar />
            ...
            <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
                ...
                <NumberInput
                  ...
                  handleChange={(value) =>
                    setItemHolder((prev) => ({
                      ...prev,
                      valor: value,
                      porcentagem: (value / expenseTotal) * 100,
                    }))
                  }
                  width="100%"
                />
              ...
              <div className="w-full lg:w-[20%]">
                <DateInput
                  ...
                  labelClassName="text-xs tracking-tight"
                  value={
                    itemHolder.dataPrevisaoPagamento
                      ? formatDate(itemHolder.dataPrevisaoPagamento)
                      : undefined
                  }
                  handleChange={(value) =>
                    setItemHolder((prev) => ({
              ...
              <div className="w-full lg:w-[20%]">
                <DateInput
                  ...
                  labelClassName="text-xs tracking-tight"
                  value={
                    itemHolder.dataPagamento ? formatDate(itemHolder.dataPagamento) : undefined
                  }
                  handleChange={(value) =>
                    setItemHolder((prev) => ({
                      ...prev,
