...
import { HomologationControlStatus, ServiceOrderStatus, inspectionStatus, serviceTypes } from "@/utils/select-options";
...
function ProjectsAllocationsGrouped({ projectIds }: ProjectsAllocationsGroupedProps) {
	...
	const { data: projectsAllocationsGrouped } = useProjectsAllocationsGrouped({ projectIds, search });

	console.log(projectsAllocationsGrouped);

	return (
		<div className="w-full flex items-center justify-end">
			<Button className="flex items-center gap-1 text-xs" onClick={() => setOpenAllocationsGroupedMenu((prev) => !prev)} size={"fit"} variant={"ghost"}>
				...
				>
					<div className="w-full flex flex-col gap-2 px-2">
						...
						{projectsAllocationsGrouped?.map((allocation) => (
							<div key={allocation.material.id} className="w-full flex flex-col gap-1 p-2 rounded border border-primary/20 shadow-xs">
								<div className="flex items-center gap-2 justify-between">
									<h1 className="text-primary text-sm font-bold tracking-tight">{allocation.material.nome}</h1>
								</div>
