  ...
  const { mutate: handleUpdateProjectStatus } = useMutation({
    ...
    onMutate: async (variables) => {
      ...
      const querySnapshot = queryClient.getQueryData([
        "engineering-projects-kanban",
        filters,
      ]) as TEngineeringProjectsKanbanOutput["data"];
      if (!querySnapshot) return { querySnapshot };

      queryClient.setQueryData(
        ["engineering-projects-kanban", filters],
        (prevData: TEngineeringProjectsKanbanOutput["data"]) => {
          const updatedProject = querySnapshot[source.droppableId].find(
            (p) => p._id === draggableId,
          );
...
function EngineeringKanbanFunnelList({
  listTitle,
  listItems,
  handleItemClick,
}: EngineeringKanbanFunnelListProps) {
  return (
          ...
          <div className="flex w-full grow flex-col gap-2 px-2">
            <div
              className={cn(
                ...
                getServiceTypeTagColor(item.tipoDeServico || ""),
              )}
            >
              <MdDashboard size={12} />
            ...
            <div className="flex items-center justify-between">
              ...
              <div>
                ...
                <p
                  className={cn(
                    "text-foreground text-center text-xs",
                    executiveDrawingFlag.valueColor,
                  )}
                >
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex w-full flex-col">
                <span className="text-xxs text-end">
                  {timeSinceAccessGrantingResponseFlag.label}
                </span>
                <p
                  className={cn(
