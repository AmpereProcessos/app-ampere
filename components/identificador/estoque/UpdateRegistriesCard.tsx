...
function UpdateRegistriesCard({ registry }: UpdateRegistriesCardProps) {
  return (
    <div className="flex w-full flex-col rounded-md border border-gray-200 p-4 lg:w-[300px]">
      ...
      <div className="flex w-full items-center gap-2">
        {registry.qtdeAnterior && registry.qtdeNovo ? (
          <>
            <h1 className="w-1/3 text-end text-lg text-gray-600">{registry.qtdeAnterior}</h1>
            <div className="flex w-1/3 items-center justify-center">
              <FaLongArrowAltRight />
            </div>
            <h1 className="w-1/3 text-start text-lg text-gray-600">{registry.qtdeNovo}</h1>
          </>
        ) : (
          <div className="flex w-full items-center justify-center">{registry.alteracao}</div>
        )}
      </div>
