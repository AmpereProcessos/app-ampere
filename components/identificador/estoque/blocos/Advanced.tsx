...
import { AlignHorizontalJustifyEnd, AlignHorizontalJustifyStart, Combine, Settings } from "lucide-react";
...
export default function Advanced({ materialId, material, deletionData, closeModal }: AdvancedProps) {
	...
	return (
		<div className="flex w-full flex-col gap-3">
			<div className="flex w-fit items-center justify-start gap-2 rounded-lg bg-primary/20 px-4 py-1.5">
				...
				<h1 className="w-full text-start text-xs font-medium leading-none tracking-tight">AVANÇADO</h1>
			</div>
			<div className="flex w-full flex-col gap-1">
				<h3 className="text-sm font-medium">DEPENDÊNCIAS</h3>
				<h3 className="text-xs font-medium tracking-tight">Compras: {deletionData.dependencias?.compras || 0}</h3>
				<h3 className="text-xs font-medium tracking-tight">Formulários: {deletionData.dependencias?.formularios || 0}</h3>
				<h3 className="text-xs font-medium tracking-tight">Análises Técnicas: {deletionData.dependencias?.analisesTecnicas || 0}</h3>
				<h3 className="text-xs font-medium tracking-tight">Projetos: {deletionData.dependencias?.projetos || 0}</h3>
			</div>
			{deletionData.tiposExclusaoHabilitados.includes("hard-delete") ? (
				<div className="flex w-full flex-col gap-1 rounded-lg border border-red-500/20 bg-red-500/20 px-4 py-1.5">
					<div className="flex w-full items-center justify-between gap-2">
						...
						<Button variant="link" size="fit" className="text-xs" onClick={() => setHardDeleteMenuIsOpen(true)}>
							EXCLUIR
						</Button>
					</div>
					<h3 className="text-xs tracking-tight">
						Esse procedimento excluirá todos os registros do material, incluindo os registros de estoque, relatórios e afins. Essa ação pode causar danos à
						integridade de dados (como compras, projetos, etc.).
					</h3>
				</div>
			) : null}
			{deletionData.tiposExclusaoHabilitados.includes("soft-delete") ? (
				<div className="flex w-full flex-col gap-1 rounded-lg border border-yellow-500/20 bg-yellow-500/20 px-4 py-1.5">
					<div className="flex w-full items-center justify-between gap-2">
						...
						<Button variant="link" size="fit" className="text-xs" onClick={() => setSoftDeleteMenuIsOpen(true)}>
							INATIVAR
						</Button>
					</div>
					<h3 className="text-xs tracking-tight">
						Esse procedimento inativaria o material, removendo do estoque, dos relatórios e afins, mas manteria o registro ativo para integridade de dados e
						consultas futuras.
					</h3>
				</div>
			) : null}
			{deletionData.tiposExclusaoHabilitados.includes("merge") ? (
				<div className="flex w-full flex-col gap-1 rounded-lg border border-primary/20 bg-primary/20 px-4 py-1.5">
					<div className="flex w-full items-center justify-between gap-2">
						<h3 className="text-sm font-medium">Deseja mesclar o material com outro?</h3>
						<Button variant="link" size="fit" className="text-xs" onClick={() => setMergeMenuIsOpen(true)}>
							MESCLAR
						</Button>
					</div>
					<h3 className="text-xs tracking-tight">
						Esse procedimento mesclará o material com outro, transferindo os registros dependentes para o material escolhido.
					</h3>
				</div>
			) : null}
			{hardDeleteMenuIsOpen ? (
				<HardDeleteMenu materialId={materialId} material={material} closeMenu={() => setHardDeleteMenuIsOpen(false)} closeModal={closeModal} />
			) : null}
			{mergeMenuIsOpen ? (
				<MergeMenu materialId={materialId} material={material} closeMenu={() => setMergeMenuIsOpen(false)} closeModal={closeModal} />
			) : null}
			{softDeleteMenuIsOpen ? (
				<SoftDeleteMenu materialId={materialId} material={material} closeMenu={() => setSoftDeleteMenuIsOpen(false)} closeModal={closeModal} />
			) : null}
		</div>
	);
}
		...
		<AlertDialog open={true} onOpenChange={closeMenu}>
			<AlertDialogContent>
				<AlertDialogHeader>
					...
					<AlertDialogDescription>
						Esse procedimento excluirá o material, removendo do estoque, dos relatórios e afins, e não poderá ser recuperado.
					</AlertDialogDescription>
				</AlertDialogHeader>
				<AlertDialogFooter>
					<AlertDialogCancel>CANCELAR</AlertDialogCancel>
					<LoadingButton variant="destructive" onClick={() => softDeleteMaterial({ id: materialId })} loading={isPending}>
						EXCLUIR
					</LoadingButton>
				</AlertDialogFooter>
			</AlertDialogContent>
...
function MergeMenu({ materialId, material, closeMenu, closeModal }: MergeMenuProps) {
	...
	const { mutate: mergeMaterial, isPending } = useMutation({
		...
		mutationFn: async ({
			...
			return await deleteMaterial({ id: originId, type: "merge", mergeIntoId: targetId, mergeMethod });
		},
		onSuccess: () => {
		...
		<AlertDialog open={true} onOpenChange={closeMenu}>
			<AlertDialogContent>
					...
					<div className="flex w-full flex-col items-center gap-2">
						<button
							...
							className={cn(
								...
								mergeMethod.mergeMethod === "combine" ? "border-cyan-800 bg-cyan-100 text-cyan-800" : "border-primary/20 text-primary/60",
							)}
						...
						<button
							...
							className={cn(
								"flex w-full items-center gap-1 rounded-lg border px-3 py-1 transition-all duration-300",
								mergeMethod.mergeMethod === "replace-target-with-origin" ? "border-cyan-800 bg-cyan-100 text-cyan-800" : "border-primary/20 text-primary/60",
							)}
							onClick={() => setMergeMethod((prev) => ({ ...prev, mergeMethod: "replace-target-with-origin" }))}
						>
						...
						<button
							...
							className={cn(
								"flex w-full items-center gap-1 rounded-lg border px-3 py-1 transition-all duration-300",
								mergeMethod.mergeMethod === "keep-target" ? "border-cyan-800 bg-cyan-100 text-cyan-800" : "border-primary/20 text-primary/60",
							)}
				...
				<AlertDialogFooter>
					...
					<LoadingButton
						variant="destructive"
						onClick={() => mergeMaterial({ originId: materialId, targetId: mergeMethod.mergeIntoId, mergeMethod: mergeMethod.mergeMethod })}
						loading={isPending}
					>
						MESCLAR
					</LoadingButton>
