...
export function GeneralTagsHolder({ session, applicableTagsIds, handleAddTag, handleRemoveTag, entities }: GeneralTagsHolderProps) {
	...
	return (
		<>
			{isLoading ? <h1 className="text-sm tracking-tight text-primary/80 animate-pulse">Carregando etiquetas...</h1> : null}
					...
					{tags
						...
						.map((tag) => (
							...
							>
								...
								<button type="button" onClick={() => handleRemoveTag(tag._id)} className="items-center justify-center opacity-0 duration-300 ease-in-out group-hover:opacity-100">
									<X height={12} width={12} />
								</button>
							</div>
						))}
					<Button onClick={() => setTagsMenuIsOpen(true)} variant={"ghost"} className="flex items-center gap-2">
						<Tags className="w-4 h-4 min-w-4 min-h-4" />
...
function TagsMenu({ session, tags, applicableTagsIds, handleAddTag, handleRemoveTag, entities, closeModal }: TagsMenuProps) {
	...
	return isDesktop ? (
		<Dialog open onOpenChange={(v) => (!v ? closeModal() : null)}>
			<DialogContent className="flex flex-col h-fit min-h-[50vh] max-h-[70vh]">
				<DialogHeader>
					<DialogTitle>{MENU_TITLE}</DialogTitle>
					<DialogDescription>{MENU_DESCRIPTION}</DialogDescription>
				</DialogHeader>
				<div className="flex-1 overflow-auto">
					<TagsMenuContent session={session} tags={tags} entities={entities} applicableTagsIds={applicableTagsIds} handleAddTag={handleAddTag} handleRemoveTag={handleRemoveTag} />
				</div>
			</DialogContent>
		</Dialog>
	) : (
...
function TagsMenuContent({ session, tags, entities, applicableTagsIds, handleAddTag, handleRemoveTag }: TagsMenuContentProps) {
	...
	function getFilteredTags(tags: TGetTagsRouteOutputDefault) {
		return {
			...
			available: tags.filter((p) => !applicableTagsIds.includes(p._id) && formatWithoutDiacritics(p.titulo, true).includes(formatWithoutDiacritics(infoHolder.titulo, true))),
		};
	}
					...
					{available.length > 0 ? (
						available.map((tag) => (
							<button
								...
								className={cn("group flex items-center gap-1 rounded py-0.5 pl-2 pr-1 hover:scale-[1.02] duration-300 ease-in-out")}
							>
								<Tag width={12} height={12} />
								<h1 className="text-[0.65rem] font-bold tracking-tight">{tag.titulo}</h1>
							</button>
						...
						<div className="flex w-full grow flex-col items-center justify-center gap-2 p-2">
							<p className="text-xs text-primary/80 font-medium">Oops, não encontramos nenhuma etiqueta. Se precisar, crie uma preenchendo os dados abaixo.</p>
							<div
								style={{
									border: "1px solid",
