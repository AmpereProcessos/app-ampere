...
export function GeneralTagsHolder({
  session,
  applicableTagsIds,
  handleAddTag,
  handleRemoveTag,
  entities,
}: GeneralTagsHolderProps) {
  const {
    data: tags,
    isLoading,
    isError,
  ...
  return (
    <>
      {isLoading ? (
        <h1 className="animate-pulse text-sm tracking-tight text-foreground">
      ...
      {isSuccess ? (
        <div className="flex w-full flex-wrap items-center justify-center gap-2">
          ...
          <Button
            onClick={() => setTagsMenuIsOpen(true)}
            variant={"ghost"}
...
function TagsMenuContent({
  ...
  const { mutate: mutationCreateTag, isPending: isCreateTagLoading } = useMutation({
    mutationKey: [
      "create-tag",
      `${infoHolder.titulo}-${infoHolder.cores.primaria}-${infoHolder.cores.secundaria}`,
    ],
  ...
  function getFilteredTags(tags: TGetTagsRouteOutputDefault) {
    return {
      ...
      available: tags.filter(
        (p) =>
          !applicableTagsIds.includes(p._id) &&
          formatWithoutDiacritics(p.titulo, true).includes(
            formatWithoutDiacritics(infoHolder.titulo, true),
          ),
      ),
    };
  }
  ...
  return (
    <div className="flex h-full w-full flex-col gap-6 px-4">
      ...
      <div className="flex w-full flex-col gap-1.5">
        <h1 className="text-sm font-medium tracking-tight text-foreground">ETIQUETAS APLICADAS</h1>
          ...
          {available.length > 0 ? (
            available.map((tag) => (
              <button
                ...
                className={cn(
                  "group flex items-center gap-1 rounded py-0.5 pl-2 pr-1 duration-300 ease-in-out hover:scale-[1.02]",
                )}
              >
                <Tag width={12} height={12} />
                <h1 className="text-[0.65rem] font-bold tracking-tight">{tag.titulo}</h1>
            ...
            <div className="flex w-full grow flex-col items-center justify-center gap-2 p-2">
              <p className="text-xs font-medium text-foreground">
                Oops, não encontramos nenhuma etiqueta. Se precisar, crie uma preenchendo os dados
                abaixo.
              </p>
              <div
                style={{
              ...
              <div className="flex w-full flex-wrap items-center justify-center gap-1">
                {TagsColorPalette.map((tagColor) => (
                  <button
                    ...
                    onClick={() =>
                      updateInfoHolder({
                        cores: { primaria: tagColor.primaria, secundaria: tagColor.secundaria },
                      })
                    }
                    style={{
                      border: "1px solid",
                    ...
                    className={cn(
                      ...
                      tagColor.primaria === infoHolder.cores.primaria
