...
import { useFinancesOverallStats } from "@/utils/methods/query/finances";
...
import { BarChart, CartesianGrid, XAxis, YAxis, Cell, Bar } from "recharts";
import { TGetFinancesOverallStatsOutput } from "@/pages/api/financeiro/stats";
    ...
    <div className="flex w-full flex-col gap-4">
      <div className="w-full flex items-center justify-end">
        <DateIntervalInput
          ...
          value={{
            after: new Date(queryParams.periodAfter),
            before: new Date(queryParams.periodBefore),
          }}
          handleChange={(value) =>
            updateQueryParams({
              periodAfter: value.after?.toISOString(),
              periodBefore: value.before?.toISOString(),
            })
          }
        />
      </div>
        ...
        <div className="flex h-full max-h-full w-full flex-col gap-2 lg:w-[40%]">
          ...
          >
            <div className="flex w-full flex-row items-center justify-between gap-2">
              <div className="flex items-center justify-start gap-2">
                <div
                  className={
                    "flex h-7 w-7 p-1 items-center justify-center rounded-full bg-primary/20 text-primary"
                  }
                >
                  <BadgeDollarSign className="w-4 h-4 min-w-4 min-h-4" />
                </div>
                <h1 className="text-xs font-medium leading-none tracking-tight">
                  POR PLANO DE CONTAS
                </h1>
              </div>
              <button
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <h1 className="text-sm font-medium">
                          (-) {formatToMoney(account.totalCredited || 0)}
                        </h1>
                        <h1 className="text-sm font-medium">
                          (+) {formatToMoney(account.totalDebited || 0)}
                        </h1>
                      </div>
                    </motion.div>
                  ))}
          ...
          >
            <div className="flex w-full flex-row items-center justify-between gap-2">
              ...
              <h1 className="text-sm font-medium">
                {formatToMoney(totalRevenue - (totalExpense + totalCost))}
              </h1>
            </div>
            <AnimatePresence>
              {showLiquidResultDetails ? (
          ...
          <StatCard
            ...
            value={
              <div className="flex items-center gap-2">
                <div
                  className={cn(
                    "flex items-center gap-1 px-2 py-1 rounded-lg bg-red-200 text-red-600",
                  )}
                >
                  <MoveUp className="w-4 h-4 min-w-4 min-h-4" />
                  <h2 className="text-[0.65rem] font-medium">
                    {formatToMoney(stats?.totalPendingTransactions.outflow || 0)}
                  </h2>
                </div>
                <div
                  className={cn(
                    "flex items-center gap-1 px-2 py-1 rounded-lg bg-green-200 text-green-600",
                  )}
                >
                  <MoveDown className="w-4 h-4 min-w-4 min-h-4" />
                  <h2 className="text-[0.65rem] font-medium">
                    {formatToMoney(stats?.totalPendingTransactions.inflow || 0)}
                  </h2>
                </div>
              </div>
            }
            ...
            value={
              <div className="flex items-center gap-2">
                ...
                >
                  <MoveUp className="w-4 h-4 min-w-4 min-h-4" />
                  <h2 className="text-[0.65rem] font-medium">
                    {formatToMoney(stats?.totalPendingTransactionsForToday.outflow || 0)}
                  </h2>
                </div>
                <div
                  className={cn(
                ...
                >
                  <MoveDown className="w-4 h-4 min-w-4 min-h-4" />
                  <h2 className="text-[0.65rem] font-medium">
                    {formatToMoney(stats?.totalPendingTransactionsForToday.inflow || 0)}
                  </h2>
                </div>
              </div>
            }
            ...
            value={
              <div className="flex items-center gap-2">
                ...
                >
                  <MoveUp className="w-4 h-4 min-w-4 min-h-4" />
                  <h2 className="text-[0.65rem] font-medium">
                    {formatToMoney(stats?.totalPendingTransactionsOverdue.outflow || 0)}
                  </h2>
                </div>
                <div
                  className={cn(
                ...
                >
                  <MoveDown className="w-4 h-4 min-w-4 min-h-4" />
                  <h2 className="text-[0.65rem] font-medium">
                    {formatToMoney(stats?.totalPendingTransactionsOverdue.inflow || 0)}
                  </h2>
                </div>
              </div>
            }
...
type TransactionsGraphProps = {
  daily: TGetFinancesOverallStatsOutput["daily"];
};
  ...
  return (
    <ChartContainer config={chartConfig} className="aspect-auto h-full w-full">
      <BarChart accessibilityLayer data={daily}>
        ...
        <ChartLegend
          content={<ChartLegendContent payload={daily} />}
