...
function renderJourneyIcon({ active, icon, index }: { active: boolean; icon: React.ComponentType | IconType; index: number }) {
	...
	return (
		<div className={`flex h-12 w-12 items-center justify-center rounded-full bg-linear-to-b from-[#15599a] to-blue-500 text-white`}>
			{renderIcon(icon)}
		</div>
	);
	...
	return (
		<div className="flex flex-col items-center px-4">
			...
			{active ? (
				...
				<div className="text-primary/60 mt-2 flex items-center gap-1">
					<FaLock />
					<p className="text-sm font-bold">Processo pendente.</p>
				</div>
			)}

			<p className="text-primary/80 mt-1 text-center dark:text-gray-400">{description}</p>
		</div>
	);
}
