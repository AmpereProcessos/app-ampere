...
function MaterialLogs({ materialId }) {
  ...
  return (
    <div className="mt-4 flex w-full flex-col items-center gap-2">
      <h1 className="font-raleway w-full rounded bg-zinc-700 p-2 text-center font-medium text-white">HISTÓRICO DE ALTERAÇÕES</h1>
      {isFetching ? (
      ...
      {isSuccess ? (
        logs.length > 0 ? (
          <div className="flex w-full flex-col gap-2 py-2">
            {logs.map((log, index) => (
              <div key={log._id} className="tex border-primary/20 flex w-full flex-col rounded border p-2 px-4 lg:w-[60%]">
                ...
                <div className="grid grid-cols-3 items-center">
                  {log.qtdeAnterior && log.qtdeNovo ? (
                    <>
                      <h1 className="text-primary/80 text-end text-lg">{log.qtdeAnterior}</h1>
                      <div className="flex items-center justify-center">
                        <FaLongArrowAltRight />
                      </div>
                      <h1 className="text-primary/80 text-start text-lg">{log.qtdeNovo}</h1>
                    </>
                  ) : (
                ...
                <div className="mt-2 flex items-center justify-center gap-4">
                  <div className="flex items-center justify-start gap-2">
                    <FaUserCircle />
                    <h1 className="text-primary/60 text-xs">{log.autor?.nome}</h1>
                  </div>
                  <div className="items-justify-end flex gap-2">
                    <BsCalendarFill color="#15599a" />
                    <h1 className="text-primary/60 text-xs">{dayjs(log.dataInsercao).format('DD/MM/YYYY HH:mm')}</h1>
                  </div>
                </div>
              </div>
            ))}
