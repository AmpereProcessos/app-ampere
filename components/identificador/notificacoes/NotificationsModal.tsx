...
function renderHeader({ projectName, sender }: { projectName: string; sender: string }) {
  if (sender == 'SISTEMA') return <h1 className="text-xs leading-none font-black tracking-tight lg:text-sm">AUTOMAÇÃO</h1>
...
function NotificationsModal({ session, notifications, closeModal }: NotificationsModalProps) {
  ...
  console.log('OLá')
  return (
    <div className="bg-background border-primary/20 fixed top-[230px] right-[-150px] z-100 h-[350px] w-[400px] translate-x-[-50%] translate-y-[-50%] rounded-md border p-[10px]">
      ...
      <div className="scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex h-full max-w-full flex-col overflow-y-auto overscroll-y-auto px-2 py-2">
        {notifications.length > 0 ? (
          notifications.map((notificacao, index) => (
            <div key={index} className={'border-primary/20 flex w-full flex-col gap-1 rounded-md border p-3'}>
              ...
              <div className="border-primary/20 flex items-center justify-center rounded-md border bg-gray-50 p-2 text-center text-xs tracking-tight">
              ...
              <div className="mt-2 flex w-full items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-1">
                    <BsCalendarPlus />
                    <p className="text-primary/60 text-[0.70rem] font-medium">{formatDateAsLocale(notificacao.dataDeEnvio, true)}</p>
                  </div>
                  {notificacao.dataDeLeitura ? (
                    <div className="flex items-center gap-1 text-green-500">
                      <BsCalendarCheck color="rgb(34,197,94)" />
                      <p className="text-primary/60 text-[0.70rem] font-medium">{formatDateAsLocale(notificacao.dataDeLeitura, true)}</p>
                    </div>
                  ) : null}
                </div>
                  ...
                  )}
                  {
                    notificacao.lido ? (
                      ...
                      <button
                        ...
                        className="bg-primary/60 flex items-center gap-1 rounded-full px-2 py-1 text-white duration-300 ease-in-out hover:scale-[1.05] hover:bg-green-500"
                      >
                        <BsCheck />
            ...
            <div>
            <p className="text-xs text-primary/60">{formatDateAsLocale(notificacao.dataDeEnvio)}</p>
            </div>
            <div className="flex items-center gap-2">
            {notificacao.remetenteId && (
            <button onClick={() => {}} className="outline-hidden transition duration-300 ease-in-out hover:scale-125">
            <MdEmail style={{ fontSize: '20px', color: '#15599a' }} />{' '}
            </button>
            )}
            {notificacao.lido ? (
