	...
	return (
		<div className="flex w-full flex-col gap-2">
			...
			<div className="flex w-full items-center justify-end">
				...
				>
					<MdPayment />
					<h1 className="text-xs font-medium tracking-tight">{!newPaymentOptionMenuIsOpen ? "ABRIR MENU DE NOVA OPÇÃO DE PAGAMENTO" : "FECHAR MENU DE NOVA OPÇÃO DE PAGAMENTO"}</h1>
				</button>
			</div>
					...
					{Object.entries(paymentOption.parcelas).map(([key, value]) => (
						<div className={`flex  w-full flex-col gap-1`}>
							...
							<input
								...
								className="h-[30px] w-full rounded-md border border-gray-300 p-3 text-sm outline-none placeholder:italic"
							/>
						</div>
					))}
				</div>
		...
		<AnimatePresence>
			<div className="flex w-full flex-col gap-2 rounded border border-gray-500 p-3">
				<div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
					...
					<div className="flex items-center gap-1">
						<button onClick={() => setEditPaymentOptionMenuIsOpen(true)} className="flex items-center gap-1 rounded-lg bg-blue-600 px-2 py-1 text-[0.6rem] text-white hover:bg-blue-500">
							<Pencil width={10} height={10} />
							<p>EDITAR</p>
						</button>
						<button onClick={() => removePaymentOption()} className="flex items-center gap-1 rounded-lg bg-red-600 px-2 py-1 text-[0.6rem] text-white hover:bg-red-500">
								...
								{Object.entries(paymentOptionInfo.parcelas).map(([key, value]) => (
									<div className={`flex  w-full flex-col gap-1`}>
										...
										<input
											...
											type="number"
											placeholder={`Preencha aqui a taxa para ${key} parcela(s)`}
											className="h-[30px] w-full rounded-md border border-gray-300 p-3 text-xs outline-none placeholder:italic"
										/>
									</div>
								))}
							</div>
