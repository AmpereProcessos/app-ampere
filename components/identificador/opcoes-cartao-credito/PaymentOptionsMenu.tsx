...
function PaymentOptionsMenu({ infoHolder, setInfoHolder }: PaymentOptionsMenuProps) {
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      ...
      <h1 className="text-primary/60 text-sm font-medium tracking-tight">LISTA DE OPÇÕES</h1>
          ...
          {Object.entries(paymentOption.parcelas).map(([key, value]) => (
            <div className={`flex w-full flex-col gap-1`}>
              ...
              <input
                ...
                className="border-primary/20 h-[30px] w-full rounded-md border p-3 text-sm outline-hidden placeholder:italic"
              />
            </div>
          ))}
        </div>
      ...
      <div className="border-primary/60 flex w-full flex-col gap-2 rounded border p-3">
        <div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
          <div className="flex items-center gap-1">
            ...
            <p className="text-[0.6rem] leading-none font-medium tracking-tight lg:text-xs">{option.descricao}</p>
          </div>
          <div className="flex items-center gap-1">
            <button
        ...
        <h1 className="text-primary/60 text-[0.6rem]">TAXAS POR PARCELA</h1>
                ...
                {Object.entries(paymentOptionInfo.parcelas).map(([key, value]) => (
                  <div className={`flex w-full flex-col gap-1`}>
                    ...
                    <input
                      ...
                      type="number"
                      placeholder={`Preencha aqui a taxa para ${key} parcela(s)`}
                      className="border-primary/20 h-[30px] w-full rounded-md border p-3 text-xs outline-hidden placeholder:italic"
                    />
                  </div>
                ))}
              </div>
              <div className="flex w-full flex-col items-center justify-center gap-2 lg:flex-row lg:justify-end">
                <Button variant={'outline-solid'} onClick={() => setEditPaymentOptionMenuIsOpen(false)} size={'xs'}>
                  FECHAR MENU
                </Button>
                <Button
                  onClick={() => {
