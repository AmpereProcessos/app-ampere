...
function PaymentOptionsMenu({ infoHolder, setInfoHolder }: PaymentOptionsMenuProps) {
  ...
  function addPaymentOption(option: TCreditCardOption["opcoes"][number]) {
    if (option.descricao.trim().length == 0)
      return toast.error("Preencha uma descrição/bandeira para a opção de pagamento.");
  ...
  function updatePaymentOption({
    ...
    index: number;
    data: TCreditCardOption["opcoes"][number];
  }) {
    setInfoHolder((prev) => ({
      ...prev,
      opcoes: prev.opcoes.map((op, i) => (i == index ? data : op)),
    }));
  }
  return (
    <div className="flex w-full flex-col gap-2">
      <h1 className="w-full rounded bg-black p-1 text-center font-bold text-white">
        OPÇÕES DE PAGAMENTO
      </h1>
      <div className="flex w-full items-center justify-end">
        <button
          ...
          className={cn(
            ...
            {
              "bg-primary/20 hover:bg-red-300": newPaymentOptionMenuIsOpen,
        ...
        >
          ...
          <h1 className="text-xs font-medium tracking-tight">
            {!newPaymentOptionMenuIsOpen
              ? "ABRIR MENU DE NOVA OPÇÃO DE PAGAMENTO"
              : "FECHAR MENU DE NOVA OPÇÃO DE PAGAMENTO"}
          </h1>
        </button>
      </div>
      <AnimatePresence>
        {newPaymentOptionMenuIsOpen ? (
          <NewPaymentOptionMenu addPaymentOption={addPaymentOption} />
        ) : null}
      </AnimatePresence>
      <h1 className="text-foreground text-sm font-medium tracking-tight">LISTA DE OPÇÕES</h1>
...
function NewPaymentOptionMenu({ addPaymentOption }: NewPaymentOptionMenuProps) {
  ...
  return (
    ...
    >
      <h1 className="w-full bg-green-500 p-1 text-center text-xs font-bold text-white lg:text-base">
        NOVA OPÇÃO PAGAMENTO
      </h1>
          ...
          {Object.entries(paymentOption.parcelas).map(([key, value]) => (
            <div className={`flex w-full flex-col gap-1`}>
              ...
              <input
                ...
                className="border-border h-[30px] w-full rounded-md border p-3 text-sm outline-hidden placeholder:italic"
              />
            </div>
          ))}
        </div>
...
function PaymentOptionCard({
  ...
  const [paymentOptionInfo, setPaymentOptionInfo] =
    useState<TCreditCardOption["opcoes"][number]>(option);
  const [editPaymentOptionMenuIsOpen, setEditPaymentOptionMenuIsOpen] = useState<boolean>(false);
  return (
    <AnimatePresence>
      <div className="border-primary/60 flex w-full flex-col gap-2 rounded border p-3">
        ...
        <h1 className="text-foreground text-[0.6rem]">TAXAS POR PARCELA</h1>
        <div className="flex w-full flex-wrap items-center gap-2">
          {Object.entries(option.parcelas).map(([key, value]) => (
            <div className="bg-primary/60 flex items-center gap-1 rounded-lg px-2 py-0.5 text-[0.6rem] text-white">
              <p>{key}x</p>
        ...
        {editPaymentOptionMenuIsOpen ? (
          ...
          >
            <h1 className="w-full bg-blue-500 p-1 text-center text-xs font-bold text-white lg:text-base">
              EDITAR OPÇÃO PAGAMENTO
            </h1>
            <div className="flex w-full flex-col gap-2 p-3">
              <TextInput
                ...
                placeholder="Preencha aqui a descrição/bandeira da opção..."
                handleChange={(value) =>
                  setPaymentOptionInfo((prev) => ({ ...prev, descricao: value }))
                }
                width="100%"
              />
                ...
                {Object.entries(paymentOptionInfo.parcelas).map(([key, value]) => (
                  <div className={`flex w-full flex-col gap-1`}>
                    ...
                    <input
                      ...
                      type="number"
                      placeholder={`Preencha aqui a taxa para ${key} parcela(s)`}
                      className="border-border h-[30px] w-full rounded-md border p-3 text-xs outline-hidden placeholder:italic"
                    />
                  </div>
                ))}
              </div>
              <div className="flex w-full flex-col items-center justify-center gap-2 lg:flex-row lg:justify-end">
                <Button
                  ...
                  onClick={() => setEditPaymentOptionMenuIsOpen(false)}
                  size={"xs"}
                >
                  FECHAR MENU
                </Button>
