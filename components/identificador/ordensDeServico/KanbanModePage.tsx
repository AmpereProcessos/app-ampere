	...
	function getHomologationAccessTag(acessResponseData: string) {
		...
		if (daysSinceResponse > 110)
			return (
				...
				>
					<TbUrgent size={12} />
					<h1 className="text-xxs font-medium">{daysTillExpiration < 0 ? "PARECER VENCIDO" : `${daysTillExpiration} DIAS ATÉ O VENCIMENTO DO PARECER`}</h1>
				</div>
			);
	}
	return (
		<Draggable draggableId={item._id.toString()} index={index}>
			{(provided) => (
              ...
              {itemExpandedModeActive ? <IoMdContract /> : <IoMdExpand />}
						...
						<button onClick={() => handleClick(item._id)} className="bg-primary text-secondary flex items-center gap-1 rounded-lg px-2 py-1 text-[0.6rem]">
