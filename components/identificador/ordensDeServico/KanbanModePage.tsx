...
import {
  TServiceOrderQueryParams,
  useServiceOrders,
  useServiceOrderTags,
} from "@/utils/methods/query/service-orders";
...
function ServiceOrdersKanbanModePage({ session, handleSetMode }: ServiceOrdersKanbanModePageProps) {
  ...
  function getServiceOrdersByStatus(
    serviceOrders: TServiceOrderSimplifiedDTO[] | undefined,
  ): TServiceOrderByStatus[] {
    if (!serviceOrders)
    ...
    return Object.entries({
      ...
      "AGUARDANDO PLANEJAMENTO": serviceOrders?.filter(
        (s) => s.status == "AGUARDANDO PLANEJAMENTO",
      ),
  ...
  function onDragEnd(dragEndResult: DropResult) {
    function handleEffectivationUpdate(
      newValue: TServiceOrderSimplifiedDTO["status"],
      previousData: TServiceOrderSimplifiedDTO,
    ) {
    ...
    handleUpdateServiceOrderStatus({
      ...
      changes: {
        ...
        dataEfetivacao: handleEffectivationUpdate(
          destination.droppableId as TServiceOrderSimplifiedDTO["status"],
          serviceOrder,
        ),
      },
    });
  ...
  const { mutate: handleUpdateServiceOrderStatus } = useMutation({
    ...
    onMutate: async (variables) => {
      ...
      const querySnapshot = queryClient.getQueryData(["service-orders", queryParams]) as
      ...
      const update = querySnapshot.map((prev) =>
        prev._id == variables.id
          ? { ...prev, status: variables.changes.status || "PENDENTE" }
          : prev,
      );
...
function TagsMenu({ currentApplicableTags, handleClick }: TagsMenuProps) {
  ...
  function getFilteredTags(tags: TServiceOrderTagDTO[]) {
    return tags.filter((t) =>
      formatWithoutDiacritics(t.titulo, true).includes(formatWithoutDiacritics(holder.title, true)),
    );
  }

  const filteredTags = getFilteredTags(tags || []);

  const { mutate: handleCreateServiceOrderTag, isPending: isUpdateLoading } =
        ...
        <div className="flex w-full grow flex-col gap-2">
          {isLoading ? (
            <div className="flex w-full grow items-center justify-center">
              <h1 className="text-foreground animate-pulse text-xs tracking-tight">
                Carregando...
              </h1>
            </div>
          ) : null}
                  ...
                  {TagsColorPalette.map((tagColor) => (
                    <button
                      ...
                      onClick={() =>
                        setHolder((prev) => ({
                          ...prev,
                          colors: { primary: tagColor.primaria, secondary: tagColor.secundaria },
                        }))
                      }
                      style={{
                        border: "1px solid",
                        borderColor: tagColor.primaria,
                        backgroundColor: tagColor.secundaria,
                      }}
                      className={cn(
                        "h-4 w-4 rounded-full opacity-60",
                        tagColor.primaria == holder.colors.primary ? "scale-110 opacity-100" : "",
                      )}
                    ></button>
                  ))}
                </div>
                  ...
                  onClick={() =>
                    ...
                    handleCreateServiceOrderTag({
                      info: {
                        titulo: holder.title,
                        cores: {
                          primaria: holder.colors.primary,
                          secundaria: holder.colors.secondary,
                        },
        ...
        >
          ...
          <h1 className="text-xxs font-medium">
            {daysTillExpiration < 0
              ? "PARECER VENCIDO"
              : `${daysTillExpiration} DIAS ATÉ O VENCIMENTO DO PARECER`}
          </h1>
        </div>
      );
  }
          ...
          <div className="flex w-full grow flex-col gap-2 px-2">
              ...
              <>
                <div
                  className={cn(
                    ...
                    getServiceTypeTagColor(item.projeto.tipo || ""),
                  )}
                >
                  <MdDashboard size={12} />
                      ...
                      <h1 className="text-foreground text-[0.6rem] font-medium uppercase">
                        {item.projeto.contratoDataAssinatura
                          ? `${getFormattedTextFromHoursAmount({
                              hours: getHoursDiff({
                                start: item.projeto.contratoDataAssinatura,
                                finish: new Date(),
                              }),
                              reference: "auto",
                              onlyComplete: false,
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <h1 className="text-foreground text-[0.6rem] font-medium uppercase">
                        {item.projeto.compraEntregaDataEfetivacao
                          ? `${getFormattedTextFromHoursAmount({
                              hours: getHoursDiff({
                                start: item.projeto.compraEntregaDataEfetivacao,
                                finish: new Date(),
                              }),
                              reference: "auto",
                              onlyComplete: false,
            ...
            <div className="flex w-full flex-wrap items-center justify-around gap-2">
              <div className="flex items-center gap-1">
                ...
                <h1 className="text-foreground text-[0.6rem] font-medium">
                  {item.equipamentos.modulos.qtde || 0} MÓDULOS
                </h1>
              </div>
              <div className="flex items-center gap-1">
                <MdRoofing height={13} width={13} />
                <h1 className="text-foreground text-[0.6rem] font-medium">
                  {item.detalhes.tipoTelha ? `TELHA ${item.detalhes.tipoTelha}` : "N/A"}
                </h1>
              </div>
            </div>
            <div className="flex w-full flex-wrap items-center justify-around gap-2">
              <div className="flex items-center gap-1">
                <h1 className="text-foreground text-[0.6rem] font-medium">AGENDAMENTO</h1>
