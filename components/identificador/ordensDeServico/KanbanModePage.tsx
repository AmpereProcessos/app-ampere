	...
	return (
		<div className="flex grow flex-col gap-2 p-6">
			...
			{editServiceOrderModal.id && editServiceOrderModal.isOpen ? (
				<ModalControlServiceOrder session={session} serviceOrderId={editServiceOrderModal.id} closeModal={() => setEditServiceOrderModal({ id: null, isOpen: false })} />
			) : null}
		</div>
	);
}
		...
		<div className="flex items-center justify-center gap-2 lg:justify-end">
			{applicableTags?.map((tag, index) => (
				...
				>
					...
					<button onClick={() => removeTag(tag._id)} className="items-center justify-center opacity-0 duration-300 ease-in-out group-hover:opacity-100">
							...
							<div className="flex w-full flex-wrap gap-2">
								{filteredTags.map((tag) => (
									<button
										...
										className={cn("flex w-fit items-center gap-1 rounded px-2 py-0.5", currentApplicableTags.includes(tag._id) ? "opacity-30" : "opacity-100")}
			...
			{(provided) => (
              ...
              {itemExpandedModeActive ? <IoMdContract /> : <IoMdExpand />}
					...
					<div className="flex w-full grow flex-col gap-2 px-2">
						...
						{item.projeto.id ? (
							<>
								<div className={cn("flex w-fit items-center gap-1 self-center rounded-lg px-2 py-1", getServiceTypeTagColor(item.projeto.tipo || ""))}>
									<MdDashboard size={12} />
									<h1 className="text-[0.5rem] font-medium">{item.projeto.tipo}</h1>
								</div>
								{item.categoria == "MONTAGEM" && item.projeto.homologacaoAcessoDataResposta ? getHomologationAccessTag(item.projeto.homologacaoAcessoDataResposta) : null}
						...
						<div className="flex w-full flex-wrap items-center justify-around gap-2">
							<div className="flex items-center gap-1">
								...
								<h1 className="text-[0.6rem] font-medium text-primary/80">
									{item.agendamento ? `${formatDateAsLocale(item.agendamento.inicio, true)} - ${item.agendamento.fim ? formatDateAsLocale(item.agendamento.fim, true) : "N/A"}` : "N/A"}
								</h1>
							</div>
						</div>
					</div>
