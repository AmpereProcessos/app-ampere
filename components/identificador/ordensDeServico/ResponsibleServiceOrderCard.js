...
function ResponsibleServiceOrderCard({ order, handleOpenModal }) {
	return (
		<div onClick={() => handleOpenModal(order)} className="flex w-[450px] cursor-pointer items-center rounded-md border border-gray-300 hover:bg-blue-50">
			<div className={`h-full w-[7px] min-w-[7px] ${getBarColor(order.periodo)} rounded-tl-md rounded-bl-md`}></div>
			<div className="flex grow flex-col p-3">
				<div className="flex w-full items-center justify-between">
					<h1 onClick={() => handleOpenModal(order)} className="w-full cursor-pointer text-center text-sm font-bold leading-none tracking-tight duration-300 ease-in-out lg:text-start">
