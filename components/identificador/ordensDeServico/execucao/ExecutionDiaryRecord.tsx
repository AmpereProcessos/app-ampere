...
function ExecutionDiaryRecord({ orderId, item, itemIndex, history }: ExecutionDiaryRecordProps) {
  ...
  async function handleRecordUpdate() {
    ...
    if (infoHolder.anotacoes.trim().length < 5)
      return toast.error("Preencha anotações para o registro de execução.");
    if (new Date(infoHolder.saida) < new Date(infoHolder.entrada))
      return toast.error("Horário de check-out não pode ser anterior ao de check-in.");
  ...
  return (
    <div className="border-border flex w-full flex-col rounded-md border p-3 shadow-xs">
        ...
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm leading-none font-thin tracking-tight lg:text-base">
            <strong className="font-bold text-[#fead41]">
              {formatDateAsLocale(item.entrada, true)}
            </strong>{" "}
            até às{" "}
            <strong className="font-bold text-[#fead41]">
              {item.saida ? formatDateAsLocale(item.saida, true) : "?"}
            </strong>
          </p>
        </div>
        <button
          onClick={() => handleRecordDelete(itemIndex)}
      ...
      {!item.saida ? (
        <div className="mt-4 flex w-full flex-col lg:mt-2">
          <h1 className="font-sans leading-none font-bold tracking-tight text-primary">
            ANOTAÇÕES
          </h1>
          <textarea
            ...
            className="border-border bg-primary/20 text-foreground mt-2 min-h-[50px] w-full resize-none rounded border p-3 text-center text-sm shadow-xs outline-hidden"
          />
        </div>
      ) : (
        <h1 className="text-foreground bg-primary/20 my-1 w-full rounded-md py-3 text-center text-sm leading-none tracking-tight">
          {item.anotacoes}
        </h1>
      )}
      ...
      {!item.saida ? (
        <div className="mt-2 flex w-full flex-col items-center justify-end gap-2 lg:flex-row lg:items-end">
          <DatetimeInput
            label="CHECK-OUT"
            labelClassName="text-xs font-sans font-bold  text-primary"
            value={infoHolder.saida}
            handleChange={(value) =>
              setInfoHolder((prev) => ({ ...prev, saida: formatDateInputChange(value) }))
            }
          />
