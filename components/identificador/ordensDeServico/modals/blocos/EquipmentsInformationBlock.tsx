	...
	const isServiceOrderInformationEqualToProject = Object.values({
		...
		invertersQty: project
			? infoHolder.equipamentos.inversor.qtde ===
				project.produtos?.filter((p) => p.categoria === "INVERSOR").reduce((acc, current) => acc + current.qtde, 0)
			: false,
		...
		modulesQty: project
			? infoHolder.equipamentos.modulos.qtde ===
				project.produtos?.filter((p) => p.categoria === "MÓDULO").reduce((acc, current) => acc + current.qtde, 0)
			: false,
	}).every((r) => r);
	return (
...
function ModulesInformation({ infoHolder, updateInfoHolder }: ModulesInformationProps) {
	return (
		<div className="border-primary/20 flex w-full flex-col gap-2 rounded-lg border p-2">
			<div className="flex items-center gap-2">
				<FaSolarPanel />
				<h1 className="text-primary/60 text-xs font-medium tracking-tight">MÓDULOS FOTOVOLTAICOS</h1>
			</div>
			<div className="flex w-full flex-col items-center gap-2 lg:flex-row">
...
function InvertersInformation({ infoHolder, updateInfoHolder }: InvertersInformationProps) {
	return (
		<div className="border-primary/20 flex w-full flex-col gap-2 rounded-lg border p-2">
			<div className="flex items-center gap-2">
				<TbTopologyFull />
				<h1 className="text-primary/60 text-xs font-medium tracking-tight">INVERSORES</h1>
			</div>
			<div className="flex w-full flex-col items-center gap-2 lg:flex-row">
				<div className="w-full lg:w-1/3">
					<TextInput
...
function TakeEquipmentBlock({
	...
	return (
		<div className="border-primary/20 flex h-full w-full flex-col gap-2 rounded-lg border p-2">
			<div className="flex w-full flex-wrap items-center justify-between gap-2">
				<div className="flex items-center gap-2">
					<Box size={15} />
					<h1 className="text-primary/60 text-xs font-medium tracking-tight">MATERIAIS P/ RETIRADA</h1>
				</div>
				{useProjectToTakeEquipmentsInformation && (
					<button
						type="button"
	...
	return (
		<div className="border-primary/20 flex h-full w-full flex-col gap-2 rounded-lg border p-2">
			<div className="flex w-full flex-wrap items-center justify-between gap-2">
				<div className="flex items-center gap-2">
					<CircleDot size={15} />
					<h1 className="text-primary/60 text-xs font-medium tracking-tight">MATERIAIS DISPONÍVEIS IN LOCO</h1>
				</div>
				{useProjectInLocoEquipmentsInformation && (
					<button
						type="button"
			...
			<div className="flex min-h-[50px] w-full grow flex-col gap-2">
				{equipmentsInLoco ? (
					equipmentsInLoco.length > 0 ? (
						...
						<div className="flex w-full grow items-center justify-center">
							<p className="text-primary/60 text-center text-sm italic">Nenhum material adicionado aos disponíveis in loco...</p>
						</div>
					)
				) : (
					<div className="flex w-full grow items-center justify-center">
