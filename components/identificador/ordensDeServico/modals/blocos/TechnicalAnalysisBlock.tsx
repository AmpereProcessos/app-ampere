...
function ServiceOrderTechnicalAnalysisInformationBlock({
  technicalAnalysisId,
}: ServiceOrderTechnicalAnalysisInformationBlockProps) {
  const [technicalAnalysisBlockIsOpen, setTechnicalAnalysisBlockIsOpen] = useState<boolean>(false);

  return (
    <ResponsiveDialogDrawerSection
      sectionTitleText="INFORMAÇÕES DA ANÁLISE TÉCNICA"
      sectionTitleIcon={<TbReportAnalytics size={15} />}
    >
      {technicalAnalysisId ? (
        <div className="flex w-full flex-col gap-2">
          ...
          {technicalAnalysisBlockIsOpen ? (
            <TechnicalAnalysisBlock analysisId={technicalAnalysisId} />
          ) : null}
        </div>
      ) : (
        <div className="text-foreground w-full text-center text-sm font-medium tracking-tight">
          Nenhuma Análise Técnica vinculada.
        </div>
      )}
    </ResponsiveDialogDrawerSection>
  );
