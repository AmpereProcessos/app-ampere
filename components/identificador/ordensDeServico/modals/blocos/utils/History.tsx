...
function ServiceOrderHistoryBlock({ infoHolder, updateInfoHolder }: HistoryBlockProps) {
  ...
  function addHistory(
    history: Exclude<TServiceOrder["periodo"]["historico"], null | undefined>[number],
  ) {
    updateInfoHolder({
      periodo: {
        historico: [...(infoHolder.periodo.historico || []), history].sort(
  ...
  return (
    <ResponsiveDialogDrawerSection
      sectionTitleText="REGISTRO DE ATIVIDADES"
      sectionTitleIcon={<BsCalendar size={12} />}
    >
      <p className="text-foreground my-1 w-full text-center text-sm font-light tracking-tighter">
      ...
      <div className="flex w-full items-center justify-end gap-2">
        ...
        >
          ...
          <h1 className="text-xs font-medium tracking-tight">
            {!historyMenuIsOpen
              ? "ABRIR MENU DE REGISTRO DE ATIVIDADE"
              : "FECHAR MENU DE REGISTRO DE ATIVIDADE"}
          </h1>
        </button>
      </div>
      <AnimatePresence>
        {historyMenuIsOpen ? (
          <NewHistoryMenu addHistory={addHistory} closeMenu={() => setHistoryMenuIsOpen(false)} />
        ) : null}
      </AnimatePresence>
...
function NewHistoryMenu({ addHistory, closeMenu }: NewHistoryMenuProps) {
  const [historyHolder, setHistoryHolder] = useState<
    Exclude<TServiceOrder["periodo"]["historico"], null | undefined>[number]
  >({
    anotacoes: "",
        ...
        <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
          <div className="w-full lg:w-1/2">
            <DateTimeInput
              ...
              handleChange={(value) =>
                setHistoryHolder((prev) => ({
                  ...prev,
                  entrada: formatDateInputChange(value, "string", false) as string,
                }))
              }
              width="100%"
            />
          ...
          <div className="w-full lg:w-1/2">
            <DateTimeInput
              ...
              handleChange={(value) =>
                setHistoryHolder((prev) => ({
                  ...prev,
                  saida: formatDateInputChange(value, "string", false) as string,
                }))
              }
              width="100%"
            />
