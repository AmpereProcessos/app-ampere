...
function ServiceOrderObservationsBlock({
  infoHolder,
  updateInfoHolder,
  useObservationsFromProject,
}: ServiceOrderObservationsBlockProps) {
  ...
  return (
    <ResponsiveDialogDrawerSection
      sectionTitleText="INSTRUÇÕES PARA EXECUÇÃO"
      sectionTitleIcon={<MdAddBox size={15} />}
    >
        ...
        {useObservationsFromProject ? (
          ...
          >
            ...
            <h1 className="text-[0.65rem] font-medium tracking-tight">
              UTILIZAR OBSERVAÇÕES DO PROJETO
            </h1>
          </button>
        ) : null}
        <button
      ...
      <AnimatePresence>
        {newObservationMenuIsOpen ? (
          <NewObservationMenu
            ...
            closeMenu={() => setNewObservationMenuIsOpen(false)}
          />
        ) : null}
      </AnimatePresence>
        ...
        {infoHolder.observacoes.length > 0 ? (
          infoHolder.observacoes.map((obs, index) => (
            <ObservationCard
              key={index}
              observation={obs}
              handleRemove={() => removeObservation(index)}
            />
          ))
        ) : (
