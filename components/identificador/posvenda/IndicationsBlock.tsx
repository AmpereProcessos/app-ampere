...
function IndicationsBlock({ projectId, projectName }: IndicationsBlockProps) {
  ...
  const {
    ...
    isError,
    isSuccess,
  } = useIndicationsByIndicatorId({ projectId });
  ...
  return (
    <div className="flex w-full flex-col">
      ...
      <p className="text-foreground my-2 w-full self-center text-center font-medium tracking-tight lg:w-[60%]">
        Gostou dos nossos serviços ? Indique um conhecido para começar já a economizar na conta de
        luz.
      </p>
      <div className="flex w-full items-center gap-2">
        ...
        <div className="w-full lg:w-1/2">
          <TextInput
            ...
            handleChange={(value) =>
              setIndication((prev) => ({ ...prev, telefone: formatToPhone(value) }))
            }
            width="100%"
          />
        </div>
          ...
          onClick={() =>
            ...
            handleCreateIndication({
              indication: {
                ...
                telefone: indication.telefone,
                idIndicacao: projectId,
              },
            })
          }
      ...
      <div className="flex w-full items-start justify-around gap-2">
        ...
        {isError ? <ErrorComponent msg={"Erro ao buscar indicações."} /> : null}{" "}
        {isSuccess ? (
          indications.length > 0 ? (
            indications.map((indication) => (
              <div
                key={indication._id}
                className="border-primary/60 flex w-[350px] flex-col rounded-md border p-3"
              >
                <h1 className="grow text-xs leading-none font-black tracking-tight lg:text-sm">
                  {indication.nome}
                </h1>
              </div>
            ))
