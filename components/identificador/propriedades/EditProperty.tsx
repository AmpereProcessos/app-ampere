...
function EditProperty({ propertyId, session, closeModal }: EditPropertyProps) {
  ...
  const [imageHolder, setImageHolder] = useState<TSimpleAttachment>({
    file: null,
    previewUrl: null,
  });
  ...
  const { mutate: handleUpdatePropertyMutation, isPending: updateLoading } =
            ...
            <div className="scrollbar-thin scrollbar-track-primary/10 scrollbar-thumb-primary/30 flex-1 overflow-auto">
              <PropertyContent
                ...
                mutateUpdateProperty={(changes) =>
                  handleUpdatePropertyMutation({
                    id: propertyId,
                    changes: { ...infoHolder, ...changes },
...
function SharableUsageLink({ id }: { id: string }) {
  return (
    <Button
      onClick={() =>
        copyToClipboard(
          `${process.env.NEXT_PUBLIC_APP_URL}/publico/uso-temporario-propriedade/${id}`,
        )
      }
...
function PropertyUsageLink({
  ...
  return (
    <div className="flex w-full items-center justify-center">
      <div className="border-border flex items-center justify-center rounded-md border p-2">
