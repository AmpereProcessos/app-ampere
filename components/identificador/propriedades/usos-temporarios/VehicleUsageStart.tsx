...
export default function VehicleUsageStart({ property }: VehicleUsageStartProps) {
  ...
  async function handleStartPropertyUsage({
    ...
    const filesMetadata = await handleMultipleAttachmentsUpdate({
      ...
      onMetrics: (metrics) => {
        ...
        console.info('[PROPERTY_USAGE_UPLOAD_COMPLETED]', { operation: 'start', ...metrics })
      },
    })
  ...
  if (isUsageMutationSuccess) {
    return <VehicleUsageSuccessScreen title="USO INICIADO COM SUCESSO" message="As informações foram registradas com sucesso." />
  }
    ...
    <div className="bg-background mx-auto flex min-h-dvh w-full max-w-lg flex-col justify-start gap-4 px-4 py-6 sm:py-10">
      <div className="bg-card border-border flex w-full flex-col items-center gap-6 rounded-lg border p-4 shadow-xs sm:p-6">
        ...
        <FullscreenUploadProgress
          ...
          messages={['Enviando as imagens…', 'Registrando o uso…', 'Sincronizando os dados…']}
        />
