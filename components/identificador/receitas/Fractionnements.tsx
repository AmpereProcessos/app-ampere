...
function Fractionnements({ infoHolder, setInfoHolder, missingPercentage }: FractionnementsProps) {
  ...
  function addFractionnement() {
    ...
    if (!newFractionnementHolder.dataPrevisaoRecebimento)
      return toast.error("Preencha uma data de previsão de recebimento.");
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      <h1 className="bg-primary/80 w-full rounded text-center font-bold text-white">
        FRACIONAMENTOS
      </h1>
      <h1 className="text-foreground my-2 w-full self-center text-center text-sm font-medium lg:w-[80%]">
        Os fracionamentos se referem aos{" "}
        <strong className="text-[#fead41]">recebimentos da receita</strong> , ou seja, as parcelas
        em que a receita será divida. Portanto, serão utilizadas como parametro para o{" "}
      ...
      <div className="item-center flex w-full flex-col gap-2 lg:flex-row">
        ...
        <div className="w-full lg:w-1/4">
          <DateInput
            ...
            labelClassName="text-sm tracking-tight"
            value={
              newFractionnementHolder.dataPrevisaoRecebimento
                ? formatDate(newFractionnementHolder.dataPrevisaoRecebimento)
                : undefined
            }
            handleChange={(value) =>
              setNewFractionnementHolder((prev) => ({
                ...prev,
                dataPrevisaoRecebimento: formatDateInputChange(value),
              }))
            }
            width="100%"
          />
        ...
        <div className="w-full lg:w-1/4">
          <DateInput
            ...
            labelClassName="text-sm tracking-tight"
            value={
              newFractionnementHolder.dataRecebimento
                ? formatDate(newFractionnementHolder.dataRecebimento)
                : undefined
            }
            handleChange={(value) =>
              setNewFractionnementHolder((prev) => ({
