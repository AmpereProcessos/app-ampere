...
function NewRevenue({ closeModal }) {
  const queryClient = useQueryClient();
  const { data: session } = useSession();
  const initialInfoHolder = {
    tipo: null,
    autor: {
      id: session.user?.id, // id do usuário que criou o referente registro de custos
      nome: session.user?.name, // nome do usuário que criou o referente registro de custos
    },
    descricao: "",
    projeto: null,
    itens: [],
  ...
  function getRevenueTotal(itens) {
  ...
  async function handleInsertRevenue() {
    if (!infoHolder.tipo) {
    ...
    if (!infoHolder.criterioCompetencia && !infoHolder.criterioReferencia) {
      toast.error(
        "Receita deve ser aplicável a ao menos um critério de análise (Competência ou Referência)."
      );
      return false;
    }
    setInsertLoading(true);
    const loadingToastID = toast.loading("Processando...", { duration: 3000 });
    try {
      const user = {
        id: session?.user.id,
  ...
  useEffect(() => {
    const total = getRevenueTotal(infoHolder.itens);
          ...
          <div className="flex py-2 px-2 flex-col w-full grow overflow-y-auto overscroll-y scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
            ...
            <div className="flex flex-col w-full my-2">
              <SelectInput
                ...
                options={fontesDeReceita.map((center, index) => ({
                  id: index + 1,
                  label: center.nome,
                  value: center.nome,
                }))}
                handleChange={(value) =>
                  setInfoHolder((prev) => ({ ...prev, tipo: value }))
                }
                onReset={() =>
                  setInfoHolder((prev) => ({ ...prev, tipo: null }))
                }
                width={"100%"}
              />
            </div>
              ...
              {infoHolder.itens.length > 0 ? (
                <>
                  ...
                  <h1 className="w-full py-2 text-center border-b border-gray-300 border-x px-2 font-black text-lg">
                    {formatToMoney(getRevenueTotal(infoHolder.itens))}
                  </h1>
                </>
              ) : (
