...
function renderStatusTag({
  ...
  total: number;
  realization: TRevenueDTO["efetivacao"];
}) {
...
function renderPendencyStatus({
  fractionnement,
}: {
  fractionnement: TRevenueDTO["fracionamento"];
}) {
  ...
  if (!hasFractionnement)
    return (
      <div className="flex items-center gap-2">
        ...
        <p className="text-foreground text-[0.65rem] font-medium lg:text-xs">SEM FRACIONAMENTOS</p>
      </div>
    );
  ...
  if (!hasPendency)
    return (
      <div className="flex items-center gap-2">
        ...
        <p className="text-foreground text-[0.65rem] font-medium lg:text-xs">TOTALMENTE RECEBIDO</p>
      </div>
    );
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <p className="text-foreground text-[0.65rem] font-medium lg:text-xs">RECEBIMENTO PENDENTE</p>
    </div>
  );
}
...
function RevenueCard({ revenue, openModal }: RevenueCardProps) {
  return (
    <div className="border-border flex w-full flex-col rounded-md border p-4 lg:w-[450px]">
      ...
      <div className="mt-1 flex w-full flex-wrap items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <p className="text-foreground text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">
            {revenue.fracionamento?.length}{" "}
            {revenue.fracionamento.length > 1 ? "RECEBIMENTOS" : "RECEBIMENTO"}
          </p>
        </div>
      ...
      <div className="mt-1 flex w-full items-center justify-between">
        <div className={`flex items-center gap-2`}>
          {revenue.efetivacao.efetivado ? (
            <BsCheck2All color="#2c6e49" />
          ) : (
            <VscChromeClose color="#ed174c" />
          )}
          <p className="text-foreground text-xs font-medium">
            {revenue.efetivacao.efetivado ? "EFETIVADA" : "NÃO EFETIVADO"}
          </p>
        </div>
        <div className="flex items-center justify-center gap-2">
          {revenue.efetivacao.efetivado ? (
            <BsCalendarCheck color="#2c6e49" />
          ) : (
            <BsCalendarEvent color="#ffbd00" />
          )}
          <p className="text-foreground text-xs font-medium">
            {formatDateAsLocale(revenue.efetivacao.data)}
          </p>
        </div>
      </div>
      <div className="mt-2 flex w-full items-center justify-between">
        <div className={`flex items-center gap-2`}>
          <BsCalendarPlus />
          <p className="text-foreground text-xs font-medium">
            {formatDateAsLocale(revenue.dataInsercao)}
          </p>
        </div>
        <div className="flex items-center justify-center gap-2">
          <Avatar
            ...
            height={25}
            width={25}
            url={revenue.autor?.avatar_url || undefined}
          />
          <p className="text-foreground text-xs font-medium">{revenue.autor?.nome}</p>
        </div>
      </div>
    </div>
  );
