...
function renderPendencyStatus({ fractionnement }: { fractionnement: TRevenueDTO["fracionamento"] }) {
	...
	if (!hasFractionnement)
		return (
			<div className="flex items-center gap-2">
				...
				<p className="text-primary/60 text-[0.65rem] font-medium lg:text-xs">SEM FRACIONAMENTOS</p>
			</div>
		);
	...
	if (!hasPendency)
		return (
			<div className="flex items-center gap-2">
				...
				<p className="text-primary/60 text-[0.65rem] font-medium lg:text-xs">TOTALMENTE RECEBIDO</p>
			</div>
		);
	...
	return (
		<div className="flex items-center gap-2">
			...
			<p className="text-primary/60 text-[0.65rem] font-medium lg:text-xs">RECEBIMENTO PENDENTE</p>
		</div>
	);
}
...
function RevenueCard({ revenue, openModal }: RevenueCardProps) {
	return (
		<div className="border-primary/20 flex w-full flex-col rounded-md border p-4 lg:w-[450px]">
			...
			<div className="mt-1 flex w-full flex-wrap items-center justify-between">
				<div className="flex items-center gap-2">
					<FaDiamond />
					<p className="text-primary/60 text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">{revenue.tipo}</p>
				</div>
				<div className="flex items-center gap-2">
					<MdPayment color={"#76c893"} />
					<p className="text-primary/60 text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">{revenue.metodo}</p>
				</div>
			</div>
			<div className="mt-1 flex w-full flex-wrap items-center justify-between">
				<div className="flex items-center gap-2">
			...
			<div className="mt-1 flex w-full items-center justify-between">
				<div className={`flex items-center gap-2`}>
					...
					<p className="text-primary/60 text-xs font-medium">{revenue.efetivacao.efetivado ? "EFETIVADA" : "NÃO EFETIVADO"}</p>
				</div>
				<div className="flex items-center justify-center gap-2">
					{revenue.efetivacao.efetivado ? <BsCalendarCheck color="#2c6e49" /> : <BsCalendarEvent color="#ffbd00" />}
					<p className="text-primary/60 text-xs font-medium">{formatDateAsLocale(revenue.efetivacao.data)}</p>
				</div>
			</div>
			<div className="mt-2 flex w-full items-center justify-between">
				<div className={`flex items-center gap-2`}>
					<BsCalendarPlus />
					<p className="text-primary/60 text-xs font-medium">{formatDateAsLocale(revenue.dataInsercao)}</p>
				</div>
				<div className="flex items-center justify-center gap-2">
					<Avatar fallback={"U"} height={25} width={25} url={revenue.autor?.avatar_url || undefined} />
					<p className="text-primary/60 text-xs font-medium">{revenue.autor?.nome}</p>
				</div>
			</div>
		</div>
	);
