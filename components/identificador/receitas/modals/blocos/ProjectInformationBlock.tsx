...
import { MdLandscape, MdOutlineMiscellaneousServices, MdOutlinePayment, MdPhone } from "react-icons/md";

type RevenueProjectInformationBlockProps = {
	revenue: TRevenue;
	project: TRevenueProjectDTO;
};
function RevenueProjectInformationBlock({ revenue, project }: RevenueProjectInformationBlockProps) {
	return (
		<div className="flex w-full grow flex-col gap-4">
			...
			<div className="flex w-full grow flex-col gap-2">
				...
				<h1 className="bg-primary/60 w-full p-1 text-center text-xs font-medium text-white">SERVIÇOS</h1>
				<div className="flex w-full flex-wrap items-center gap-2">
					{!!project.servicos && project.servicos?.length > 0 ? (
						project.servicos?.map((service) => (
							<div key={service.id} className="border-primary bg-background flex w-full flex-col gap-1 rounded-md border p-2 dark:bg-[#121212]">
								<div className="flex w-full items-center justify-between gap-2">
									<div className="flex items-center gap-1">
										<div className="flex h-[25px] w-[25px] items-center justify-center rounded-full border border-black p-1 dark:border-white">
											<MdOutlineMiscellaneousServices />
										</div>
										<p className="text-sm leading-none font-bold tracking-tight">{service.descricao}</p>
									</div>
									<div className="flex items-center gap-1">
										<AiOutlineSafety size={12} />
										<p className="text-primary/50 text-[0.6rem] font-light lg:text-xs">
...
export default RevenueProjectInformationBlock;
