...
type RevenueReceiptsTableProps = {
  ...
  updateReceipt: (info: {
    index: number;
    item: Partial<TRevenue["fracionamento"][number]>;
  }) => void;
  removeReceipt: (index: number) => void;
};
function RevenueReceiptsTable({
  revenueId,
  revenueTotal,
  receipts,
  updateReceipt,
  removeReceipt,
}: RevenueReceiptsTableProps) {
      ...
      <div className="bg-background flex w-full flex-col gap-2 p-1 dark:bg-[#121212]">
        ...
        {receiptsTotal > revenueTotal ? (
          <p className="w-full rounded border border-orange-400 bg-orange-50 p-1 text-center text-xs tracking-tight text-orange-400 italic">
            Por favor, ajuste os valores dos recebimentos. A somatória dos recebimentos atuais
            excede o valor total estabelecido para a receita.
          </p>
        ) : null}
      </div>
    </div>
        ...
        <div className="bg-background hidden w-full flex-col gap-1 lg:flex dark:bg-[#121212]">
          <div className="flex w-full items-center gap-2 p-1">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-1">
                    ...
                    <p className="text-foreground text-[0.65rem] leading-none font-light tracking-tight italic">
                      {formatDecimalPlaces(((item.valor || 0) / revenueTotal) * 100)}%
                    </p>
                  </div>
                </div>
                {/* <p className="text-[0.65rem] font-light italic leading-none tracking-tight text-foreground">{item.categoria}</p> */}
              </div>
              <button
                type="button"
            ...
            <h1 className="w-[20%] text-center text-xs tracking-tight">
              {formatToMoney(item.valor || 0)}
            </h1>
            <h1 className="w-[20%] text-center text-xs tracking-tight">
              {formatDateAsLocale(item.dataPrevisaoRecebimento || undefined) || "-"}
            </h1>
            <h1 className="w-[20%] text-center text-xs tracking-tight">
              {formatDateAsLocale(item.dataRecebimento || undefined) || "-"}
            </h1>
          </div>
        </div>
            ...
            <div className="flex items-center gap-2">
              {item.dataRecebimento ? (
                <div className="flex items-center gap-1">
                  ...
                  <p className="text-foreground text-[0.6rem] lg:text-xs">RECEBIDO</p>
                </div>
              ) : null}
              <div className="flex items-center gap-1">
                <BsCalendar />
                <p className="text-foreground text-[0.6rem] lg:text-xs">
                  {formatDateAsLocale(item.dataPrevisaoRecebimento || undefined)}
                </p>
              </div>
              {item.dataRecebimento ? (
            ...
            <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
                ...
                <NumberInput
                  ...
                  handleChange={(value) =>
                    setItemHolder((prev) => ({
                      ...prev,
                      valor: value,
                      porcentagem: (value / revenueTotal) * 100,
                    }))
                  }
                  width="100%"
                />
              ...
              <div className="w-full lg:w-[20%]">
                <DateInput
                  ...
                  labelClassName="text-xs tracking-tight"
                  value={
                    itemHolder.dataPrevisaoRecebimento
                      ? formatDate(itemHolder.dataPrevisaoRecebimento)
                      : undefined
                  }
                  handleChange={(value) =>
                    setItemHolder((prev) => ({
                      ...prev,
                      dataPrevisaoRecebimento: formatDateInputChange(value, "string") as string,
                    }))
                  }
                  width="100%"
                />
              ...
              <div className="w-full lg:w-[20%]">
                <DateInput
                  ...
                  labelClassName="text-xs tracking-tight"
                  value={
                    itemHolder.dataRecebimento ? formatDate(itemHolder.dataRecebimento) : undefined
                  }
                  handleChange={(value) =>
                    setItemHolder((prev) => ({
                      ...prev,
                      dataRecebimento: formatDateInputChange(value, "string") as string,
                    }))
                  }
                  width="100%"
                />
