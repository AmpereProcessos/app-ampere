...
type TNewFileAttachmentHolder = {
  fileReference: {
    ...
    idChamado: TFileReference["idChamado"];
...
function NewManyFileReferencesMenu({
  vinculationId,
  prefix,
  session,
  vinculations,
  callbacks,
  closeMenu,
}: NewManyFileReferencesMenuProps) {
  const [infoHolder, setInfoHolder] = useState<TNewFileAttachmentHolder>({
    fileReference: {
      ...
      idOportunidade: vinculations.opportunityId?.blocked
        ? vinculations.opportunityId.value
        : undefined,
      idAnaliseTecnica: vinculations.technicalAnalysisId?.blocked
        ? vinculations.technicalAnalysisId.value
        : undefined,
      idHomologacao: vinculations.homologationId?.blocked
        ? vinculations.homologationId.value
        : undefined,
      ...
      idOrdemServico: vinculations.serviceOrderId?.blocked
        ? vinculations.serviceOrderId.value
        : undefined,
      idChamado: vinculations.callId?.blocked ? vinculations.callId.value : undefined,
  ...
  function addCategory(category: string) {
    setInfoHolder((prev) => ({
      ...
      fileReference: {
        ...prev.fileReference,
        categorias: [...(prev.fileReference.categorias || []), category],
      },
    }));
  }
  function removeCategory(index: number) {
    setInfoHolder((prev) => ({
      ...
      fileReference: {
        ...prev.fileReference,
        categorias: (prev.fileReference.categorias || []).filter((c, i) => i !== index),
      },
    }));
  }
  ...
  async function handleCreateFileReference(reference: TNewFileAttachmentHolder) {
    try {
      const filesMetadata: {
        url: TFileReference["url"];
        formato: TFileReference["formato"];
        tamanho: TFileReference["tamanho"];
      }[] = [];
      const uploadPromises = reference.attachments
        .filter((f) => !!f.file)
        .map(async (file, index) => {
          const fileName =
            reference.attachments.length > 1
              ? `${reference.fileReference.titulo} (${index + 1})`
              : reference.fileReference.titulo;
          const {
            url,
            format: formato,
            size: tamanho,
      ...
      const fileReferences: TFileReference[] = filesMetadata.map((file) => ({
        ...
        idChamado: reference.fileReference.idChamado,
  ...
  return (
    <AnimatePresence>
      ...
      >
        <h1 className="rounded-tl rounded-tr bg-green-600 p-1 text-center text-xs text-white">
          NOVOS ANEXOS
        </h1>
          ...
          <div className="flex w-full flex-col gap-2 lg:w-1/2">
            ...
            <div className="mt-4 flex w-full flex-wrap items-center justify-center gap-2">
              ...
              {vinculations?.callId ? (
                <div className="w-fit">
                  <CheckboxInput
                    labelFalse="VINCULAR AO CHAMADO"
                    labelTrue="VINCULAR AO CHAMADO"
                    checked={!!infoHolder.fileReference.idChamado}
                    editable={!vinculations.callId.blocked}
                    handleChange={(value) =>
                      updateReference({
                        idChamado: value ? vinculations.callId?.value : undefined,
                      })
                    }
                  />
                </div>
              ...
              {infoHolder.attachments
                ...
                .map((attachment, index) => (
                  <div
                    ...
                    className="border-primary/50 flex h-[130px] w-[100px] flex-col rounded border"
                  >
            ...
            <div className="mt-2 flex w-full items-center justify-end gap-2">
              ...
              <LoadingButton loading={isPending} onClick={() => mutate(infoHolder)} type="button">
                {infoHolder.attachments.filter((x) => !!x.file).length > 0
                  ? "ANEXAR ARQUIVOS"
                  : "ANEXAR ARQUIVO"}
              </LoadingButton>
            </div>
...
function CategoriesMenu({
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      <h1 className="text-foreground text-[0.65rem] leading-none font-bold tracking-tight lg:text-xs">
      ...
      <div className="my-1 flex w-full flex-wrap items-center justify-around gap-4 gap-y-1 px-2 lg:justify-center">
        {FileReferenceCategories.filter((c) => allowedCategories.includes(c.value)).map(
          (category, index) => (
            <button
              type="button"
              ...
              className={`rounded-lg ${
                data.fileReference.categorias?.find((c) => c === category.value)
