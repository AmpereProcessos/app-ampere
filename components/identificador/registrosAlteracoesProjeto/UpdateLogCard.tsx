...
function UpdateLogCard({ log, relatedFields }: UpdateLogCardProps) {
  return (
    <div className="border-border flex w-full flex-col gap-1 rounded border p-3">
        ...
        <div className="flex items-center justify-end gap-4">
          <div className={`flex items-center gap-1`}>
            ...
            <p className="text-[0.6rem] font-medium">
              {formatDateAsLocale(log.dataAlteracao, true)}
            </p>
          </div>
          <div className="flex items-center gap-1">
        ...
        {Object.entries(log.alteracoes).map(([field, value]) => {
          ...
          return (
            <div className="border-primary/60 flex w-fit items-center gap-1 rounded-lg border px-2 py-1">
              ...
              <h1 className="text-foreground text-[0.6rem] tracking-tight">
                <strong className="text-orange-500">{updateLog.label}</strong> ALTERADO(A)
                PARA:{" "}
              </h1>
              <h1 className="text-[0.6rem] font-bold tracking-tight text-cyan-600">
                {updateLog.valueFormatted}
              </h1>
            </div>
          );
        })}
