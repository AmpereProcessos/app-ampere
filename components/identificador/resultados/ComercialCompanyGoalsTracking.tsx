...
import type { TCompanyGoalsResults } from '@/utils/schemas/stats'
...
function renderRanking({ competitors, awardedQty }: { competitors: { RESPONSAVEL: string; TOTAL: number }[]; awardedQty: number }) {
  ...
  return podium.map((podiumPosition, index) => (
    <div
      key={`${index}${podiumPosition.competitor}`}
      className={cn('h-full w-1/3 flex-col justify-end gap-2 lg:w-1/5', podiumPosition.positionValue > 3 ? 'hidden lg:flex' : 'flex')}
    >
      {renderAvatarBySeller(podiumPosition.competitor || 'N/A')}
      <h1 className="text-primary/60 text-center text-sm font-bold">{podiumPosition.competitor || 'N/A'}</h1>
      <div className="flex w-full flex-col justify-end">
        <div
          style={{ height: `${podiumPosition.podiumHeight * 280}px` }}
          className={cn(
            'bg-primary/60 flex w-full flex-col items-center justify-center gap-1',
...
function renderAvatarBySeller(sellerName: string) {
  if (!sellerName) {
    return (
      <div className="bg-primary/70 flex h-[30px] max-h-[50px] w-[30px] max-w-[50px] items-center justify-center self-center rounded-full lg:h-[50px] lg:w-[50px]">
  ...
  if (!existingSellerWithPhoto) {
    ...
    return (
      <div className="bg-primary/70 flex h-[30px] max-h-[50px] w-[30px] max-w-[50px] items-center justify-center self-center rounded-full lg:h-[50px] lg:w-[50px]">
        <p className="text-center text-xs font-bold text-white uppercase lg:text-lg">
          {firstLetter}
          {secondLetter}
        </p>
      </div>
...
function ComercialCompanyGoalsTracking({ results }: ComercialCompanyGoalsTrackingProps) {
  ...
  return (
    <div className="bg-background border-primary/20 flex min-h-[110px] w-full flex-col rounded-xl border shadow-xs">
      {/* <div className="flex w-full flex-col items-center justify-between gap-2 rounded-tr-xl rounded-tl-xl bg-blue-800 p-2 px-4 lg:flex-row">
				...
				<div className="flex grow flex-wrap items-center justify-center gap-2 lg:justify-end">
					{Object.keys(GoalsParams).map((key) => (
						<button
							key={key}
							className={cn("rounded-xl border border-white/60 bg-transparent px-2 py-1 text-[0.6rem] font-bold text-white/60", activeGoal == key ? "bg-background text-[#15599a]" : "")}
      ...
      <div className="flex w-full flex-col gap-6 px-6 py-6 lg:px-24">
        ...
        <div className="flex w-full flex-col gap-2">
          ...
          <div className="flex h-[40px] w-full items-center justify-between self-center overflow-hidden rounded-sm border border-primary/60 bg-[#a8a9aa] p-0.5">
            <div
              style={{ width: `${(activeGoalResults.TOTAL / activeGoalParams['META GERAL']) * 100}%` }}
              className={cn('flex h-full flex-col items-center justify-center rounded-sm bg-linear-to-r', {
                'bg-linear-to-r from-yellow-300 to-[#fead41]': activeGoalResults.TOTAL / activeGoalParams['META GERAL'] < 1,
                'bg-linear-to-tl from-lime-400 to-green-600': activeGoalResults.TOTAL / activeGoalParams['META GERAL'] >= 1,
              })}
            >
                ...
                <div className="mb-2 flex w-full flex-col items-center">
                  <div className="flex h-fit w-full items-end justify-center gap-4 p-0 lg:h-[425px] lg:w-[1200px] lg:gap-10 lg:p-6">
                    ...
                    {/* <div className="hidden h-full w-1/5 flex-col justify-end lg:flex">
                         ...
                         <h1 className="text-center text-sm font-bold text-primary/60">{activeGoalResults.RANKING[3]?.RESPONSAVEL}</h1>
                         <div className="flex h-[30%] w-full items-center justify-center bg-primary/60 text-3xl font-bold text-white">4º</div>
                        </div>
                        <div className="flex h-full w-1/3 flex-col justify-end lg:w-1/5">
                         ...
                         <h1 className="text-center text-xs font-bold text-primary/60 lg:text-sm">{activeGoalResults.RANKING[1]?.RESPONSAVEL || 'N/A'}</h1>
                        ...
                        <div className="flex h-full w-1/3 flex-col justify-end lg:w-1/5">
                         ...
                         <h1 className="text-center text-xs font-bold text-primary/60 lg:text-sm">{activeGoalResults.RANKING[0]?.RESPONSAVEL || 'N/A'}</h1>
                        ...
                        <div className="flex h-full w-1/3 flex-col justify-end lg:w-1/5">
                         ...
                         <h1 className="text-center text-xs font-bold text-primary/60 lg:text-sm">{activeGoalResults.RANKING[2]?.RESPONSAVEL || 'N/A'}</h1>
                        ...
                        <div className="hidden h-full w-1/5 flex-col justify-end lg:flex">
                         ...
                         <h1 className="text-center text-sm font-bold text-primary/60">{activeGoalResults.RANKING[4]?.RESPONSAVEL || 'N/A'}</h1>
                         <div className="flex h-[15%] w-full items-center justify-center bg-primary/60 text-3xl font-bold text-white">5º</div>
                        </div> */}
                  </div>
                </div>
              </div>
...
function GoalsAchievedRender() {
  ...
  return (
    <div className="flex w-full items-center justify-center">
      <h1 className="self-center text-4xl leading-20 font-black tracking-[-0.02em] text-green-500 uppercase">META ALCANÇADA !!!</h1>
    </div>
  )
}
