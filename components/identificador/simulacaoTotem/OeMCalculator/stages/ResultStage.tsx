...
function ResultStage({ nome, numModulos, potenciaPico, resetSimulation }: ResultStageProps) {
  ...
  function getEstimatedPossibleLoss({
    moduleQty,
    peakPower,
  }: {
    moduleQty: number;
    ...
    const annualLoss =
      12 * (1 - STANDARD_EFFICIENCE / 100) * estimatedGeneration * STANDARD_ENERGY_TARIFF;
      ...
      <div className="flex w-full grow flex-col">
        ...
        <div className="my-3 flex w-full flex-col">
          <h1 className="text-center font-bold tracking-tight">
            <strong className="text-[#15599a]">{nome}</strong>, a manutenção e limpeza de um sistema
            fotovoltaico é essencial para garantir o seu pleno funcionamento, de forma que você
            obtenha o melhor do seu investimento.
          </h1>
          <div className="flex w-full flex-col items-center gap-2 px-2 lg:flex-row">
            <div className="flex h-full w-full grow flex-col self-center rounded-md border border-red-600 lg:w-1/2">
              <div className="flex w-full items-center gap-1 rounded-br-md rounded-bl-md bg-red-600 p-3">
                <h1 className="w-full text-center text-[0.7rem] font-bold text-white">
                  POTÊNCIAIS PROBLEMAS CAUSADOS PELA FALTA DE MANUTENÇÃO
                </h1>
              </div>
              <div className="flex w-full flex-col gap-2 p-3">
                {potencialProblems.map((problem, index) => (
                  <div key={index} className="flex items-center gap-1">
                    ...
                    <p className="text-foreground text-xs tracking-tight lg:text-base">{problem}</p>
                  </div>
                ))}
              </div>
            </div>
            <div className="flex h-full w-full grow flex-col self-center rounded-md border border-green-600 lg:w-1/2">
              <div className="flex w-full items-center gap-1 rounded-br-md rounded-bl-md bg-green-600 p-3">
                <h1 className="w-full text-center text-[0.7rem] font-bold text-white">
                  BENEFÍCIOS DA MANUTENÇÃO E LIMPEZA
                </h1>
              </div>
              <div className="flex w-full flex-col gap-2 p-3">
                {benefits.map((benefit, index) => (
                  <div key={index} className="flex items-center gap-1">
                    ...
                    <p className="text-foreground text-xs tracking-tight lg:text-base">{benefit}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="my-2 flex w-full flex-col">
            <h1 className="text-foreground w-full text-center tracking-tight">
              A execução de uma manutenção e limpeza, para um sistema como o seu{" "}
              <strong className="text-[#fead41]">({numModulos} módulos)</strong> , pode te fazer{" "}
              <strong className="text-green-600">recuperar um montante anual</strong> estimado de:
            </h1>
            <h1 className="w-full text-center text-xl font-black text-green-600">
              {formatToMoney(
                getEstimatedPossibleLoss({ moduleQty: numModulos, peakPower: potenciaPico }),
              )}
            </h1>
          </div>
        </div>
        <div className="flex w-full items-center gap-1 rounded-br-md rounded-bl-md bg-blue-800 p-3">
          <h1 className="w-full text-center text-[0.7rem] font-bold text-white">
            CONHEÇA NOSSAS OPÇÕES PARA VOCÊ
          </h1>
        </div>
        <div className="flex h-fit w-full flex-wrap items-stretch justify-center gap-2 px-2 py-2">
          {plans.map((plan, index) => (
            <div
              key={index}
              className="bg-background border-primary/60 flex w-[350px] flex-col rounded-lg border p-6 shadow-lg"
            >
              <div className="flex w-full items-center justify-between gap-2">
                <h1 className="text-lg font-black">{plan.nome}</h1>
              </div>
              <p className="text-foreground w-full text-start text-xs">
                {plan?.descricao || "..."}
              </p>
              <div className="my-4 flex w-full items-end justify-center gap-1">
                ...
                <h1 className="text-foreground text-xs font-light">/ {plan?.intervalo.tipo}</h1>
              </div>

              <div className="my-4 flex grow flex-col gap-1">
                <h1 className="text-foreground text-[0.6rem] tracking-tight">DESCRITIVO</h1>
