...
import { getInsuranceInvestmentByReferenceValue } from "@/utils/totem-simulation/helpers";
...
function MainComponent({ resetSimulation }: MainComponentProps) {
	...
	function validateStageChange({ prev, next, data }: { prev: number; next: number; data: TTotemSimulation }) {
		...
		if (prev === 1 && next === 2) {
			if (!data.premissas.valorReferencia || Number(data.premissas.valorReferencia) === 0) {
				toast.error("Por favor, preencha um valor de investimento válido.");
				return false;
			}
		}
		if (prev === 2 && next === 3) {
