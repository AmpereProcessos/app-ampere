...
function ContractRequestControlModal({
  requestId,
  session,
  closeModal,
}: ContractRequestControlModalProps) {
  ...
  const {
    ...
    isSuccess,
    error,
  } = useContractRequestById({ id: requestId });
  ...
  async function approveFormulary(info: TContractRequestDTO) {
    ...
    if (info.idHomologacao) {
      const { projectHomologation, projectHomologationFiles } =
        await getProjectHomologationInformation({
  ...
  return isDesktop ? (
    <Dialog onOpenChange={(v) => (v ? null : closeModal())} open>
      <DialogContent
        className={
          "h-[90%] min-h-[90%] max-h-[90%] lg:max-h-[90%] w-[80%] min-w-[80%] max-w-[80%] lg:max-w-[80%]"
        }
      >
        <DialogHeader>
...
function GenerateContractMenu({
  ...
  const { mutate: handleGetContractDcoument, isPending: isGeneratingContractDocument } =
    useMutation({
  ...
  return (
    <ResponsiveDialogDrawer
      ...
      actionFunction={() => {
        if (!infoHolder.templateId)
          return toast.error("Selecione um template de contrato para gerar o contrato.");
    ...
    >
      ...
      <div className="w-full flex items-center justify-center gap-2">
        <Button
          variant={infoHolder.format === "pdf" ? "default" : "ghost"}
          onClick={() => setInfoHolder({ ...infoHolder, format: "pdf" })}
        >
          PDF
        </Button>
        <Button
          variant={infoHolder.format === "docx" ? "default" : "ghost"}
          onClick={() => setInfoHolder({ ...infoHolder, format: "docx" })}
        >
          DOCX
        </Button>
      </div>
