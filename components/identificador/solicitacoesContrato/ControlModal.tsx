...
function ContractRequestControlModal({
	...
	const userHasEditPermission =
		session.user.permissoes.comercial.editar ||
	...
	const { mutate: handleApproveRequest, isPending: approvalLoading } =
	...
	const { mutate: handleRejectRequest, isPending: rejectLoading } =
		useMutationWithFeedback({
	...
	const { mutate: handleRequestUpdate, isPending: updateLoading } =
		useMutationWithFeedback({
				...
				<div className="flex h-full w-full flex-col">
					<div className="flex items-center justify-between border-b border-gray-200 px-2 pb-2">
						<div className="flex flex-col gap-1">
							<h3 className="text-base font-bold text-[#353432] dark:text-white ">
								CONTROLE DE FORMULÁRIO
							</h3>
							<div className="flex items-center gap-1">
								<BsCode />
								<h1 className="text-xxs leading-none tracking-tight text-gray-500">
									#{requestId}
								</h1>
							</div>
						</div>
					...
					{isError ? (
						<ErrorComponent
							msg={
								"Houve um erro ao buscar informações do formulário de contrato."
							}
						/>
					) : null}
									...
									{infoHolder.aprovacao ? (
										<>
											<div className="flex items-center gap-1">
												...
												<h1 className="text-sm font-medium tracking-tight text-gray-500">
													APROVADO EM:{" "}
													{formatDateAsLocale(infoHolder.dataAprovacao, true)}
												</h1>
											</div>
										...
										<>
											<button
												type="button"
												disabled={
													approvalLoading || updateLoading || rejectLoading
												}
												onClick={() => {
													// @ts-ignore
											...
											<button
												type="button"
												disabled={
													rejectLoading || approvalLoading || updateLoading
												}
												onClick={() => {
													// @ts-ignore
