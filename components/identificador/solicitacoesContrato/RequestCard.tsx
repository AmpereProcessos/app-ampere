...
function RequestCard({ request, openModal }: RequestCardProps) {
	return (
		<div className="border-primary/20 flex w-full flex-col rounded-md border p-4 lg:w-[550px]">
			...
			<div className="mt-2 flex items-center justify-between gap-2">
				<div className="flex items-center gap-2">
					<FaCity />
					<p className="text-primary/60 text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">{request.cidadeInstalacao}</p>
				</div>
				<div className="flex items-center gap-2 text-blue-800">
					...
					<p className="text-primary/60 text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">{request.nomeVendedor || "NÃO DEFINIDO"}</p>
				</div>
			</div>
			<div className="mt-2 flex items-center justify-between gap-2">
				<div className="flex items-center gap-2">
			...
			<div className="mt-2 flex items-center justify-between gap-2">
				<div className={"flex items-center gap-2"}>
					<TbCategoryFilled />
					<p className="text-primary/60 text-xs font-medium">{request.tipoDeServico}</p>
				</div>
				<div className={"flex items-center gap-2"}>
					<MdOutlineBusiness />
					<p className="text-primary/60 text-xs font-medium">{request.nomeParceiro}</p>
				</div>
			</div>
				...
				<div className="flex items-center gap-2">
					{request.dataAprovacao ? (
						<div className={"flex items-center gap-2"}>
							...
							<p className="text-primary/60 text-xs font-medium">{formatDateAsLocale(request.dataAprovacao, true)}</p>
						</div>
					) : null}
					<div className={"flex items-center gap-2"}>
						<BsCalendarPlus />
						<p className="text-primary/60 text-xs font-medium">{formatDateAsLocale(request.dataSolicitacao, true)}</p>
					</div>
				</div>
			</div>
		</div>
