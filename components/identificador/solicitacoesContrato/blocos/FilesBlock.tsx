...
function FilesBlock({ tag, files, vinculateFiles, vinculationPending }: FilesBlockProps) {
  ...
  async function attachFiles(fileHolder: TFileHolder) {
    ...
    if (fileHolder.title.trim().length < 2)
      return toast.error("Preencha um nome de ao menos 2 caractéres para o arquivo.");
    ...
    if (fileHolder.files.length > 0) {
      const promises = Array.from(fileHolder.files).map(async (file, index) => {
        ...
        const fileFormat =
          fileTypes[uploadResult.metadata.contentType || ""]?.title || "NÃO DEFINIDO";
  ...
  return (
    <ResponsiveDialogDrawerSection
      sectionTitleText="DOCUMENTAÇÃO"
      sectionTitleIcon={<FolderOpen className="h-4 w-4 min-h-4 min-w-4" />}
    >
        ...
        <div className="flex w-full grow flex-wrap items-start justify-around gap-2 px-2">
          {files?.map((x, index) => (
            <div key={`${x.title}-${index}`} className="w-full lg:w-[450px]">
              <ArchiveLinkBlock
                obj={x}
                deleteFile={(x: { title: string; link: string; format: string }) =>
          ...
          <div className="relative mb-4 flex w-full items-center justify-center">
            ...
            >
              <div className="text-foreground flex flex-col items-center justify-center pt-5 pb-6">
                ...
                {fileHolder.files ? (
                  <p className="text-foreground mb-2 text-sm dark:text-gray-400">
                  ...
                  <p className="text-foreground mb-2 px-2 text-center text-sm dark:text-gray-400">
                    <span className="font-semibold">Clique para escolher um arquivo</span> ou o
                    arraste para a àrea demarcada
                  </p>
                )}
              </div>
              <input
