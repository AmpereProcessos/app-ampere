...
function AnalysisCard({ analysis, handleOpenModal }) {
  return (
    <div
      ...
      className="flex  gap-2  w-full lg:w-[450px] h-[170px] lg:h-[130px] shadow-sm cursor-pointer border border-gray-300  hover:bg-blue-100 rounded-md"
    >
      ...
      <div className="flex grow flex-col p-3">
        ...
        <div className="flex flex-col lg:flex-row w-full justify-between items-center gap-1 lg:gap-2 mt-2">
          <div className="flex items-center gap-2">
            <MdCategory />
            <p className="text-[0.6rem] lg:text-xs tracking-tight leading-none font-bold text-[#15599a]">
              {analysis.tipoDeSolicitacao || 'NÃO DEFINIDO'}
            </p>
          </div>
          <div className="flex items-center gap-2">
            <FaCity />
            <p className="text-[0.6rem] lg:text-xs text-gray-500 tracking-tight leading-none">{analysis.cidade}</p>
          </div>
        </div>
        <div className="flex flex-col lg:flex-row w-full justify-between items-center gap-1 lg:gap-2 mt-2">
          <div className="flex items-center gap-2">
            <IoMdAlert />
            <p className="text-[0.6rem] lg:text-xs text-gray-500 tracking-tight leading-none">{analysis.tipoDeLaudo}</p>
          </div>
          <div className="flex items-center gap-2">
            <FaUser />
            <p className="text-[0.6rem] lg:text-xs text-gray-500 tracking-tight leading-none">{analysis.nomeVendedor || 'NÃO DEFINIDO'}</p>
          </div>
        </div>
        <div className="w-full gap-4 flex items-center justify-between mt-4">
          <div className={`flex items-center gap-2 ${analysis.dataDeConclusao ? 'text-green-500' : 'text-gray-500'}`}>
            <BsCalendarFill />
            <p className="text-[0.6rem] lg:text-xs font-medium">
              {dayjs(analysis.dataDeConclusao ? analysis.dataDeConclusao : analysis.dataDeAbertura).format('DD/MM/YYYY HH:mm')}
            </p>
          </div>
          <div className={`hidden lg:flex items-center gap-2 ${!!analysis.dataDeConclusao ? 'text-green-500' : 'text-orange-500'}`}>
            <FaHourglass />
            <p className="text-[0.6rem] lg:text-xs tracking-tight leading-none font-bold">
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="font-medium text-gray-500 text-[0.6rem] lg:text-xs">{analysis.analista?.apelido || 'NÃO DEFINIDO'}</p>
          </div>
        </div>
      </div>
    </div>
