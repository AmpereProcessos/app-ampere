...
function DateIntervalInput({
  ...
  const defaultPresets: Preset[] = [
    ...
    },
    {
      label: "Esta semana",
      interval: {
        after: startOfWeek(new Date(), { locale: ptBR }),
        before: endOfWeek(new Date(), { locale: ptBR }),
      },
    },
    {
      label: "Este mês",
    ...
    },
    {
      ...
      interval: {
        ...
        before: endOfMonth(subMonths(new Date(), 1)),
      },
    },
    {
      label: "Este ano",
  ...
  return (
    <div className={cn("flex flex-col gap-1")}>
      {label && (
        <div className={cn("text-foreground text-sm font-medium", labelClassName)}>{label}</div>
      )}
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button
            ...
            variant={"outline"}
            className={cn(
              "min-h-[46.6px] border-border bg-background focus:border-primary w-full justify-start rounded-md border text-left text-sm font-normal ease-in-out dark:bg-[#121212]",
            ...
            {value?.after ? (
              value.before ? (
                <>
                  ...
                  {format(value.before, "dd/MM/yyyy", { locale: ptBR })}
                </>
              ) : (
                format(value.after, "dd/MM/yyyy", { locale: ptBR })
              )
