    ...
    <div ref={ref} className="relative flex w-full flex-col justify-center self-center">
      <div className="flex w-full items-center justify-between gap-2">
        <label
          ...
          className={"text-start font-sans font-bold text-foreground"}
        >
          {label}
        </label>
        {fileReferences ? (
      ...
      <div className="relative mt-2 flex w-full items-center justify-center">
        <label
          ...
          className={`bg-background border-border dark:hover:bg-primary/10 flex min-h-[58px] w-full cursor-pointer flex-col items-center justify-center rounded-md border p-3 hover:border-blue-300 hover:bg-blue-100`}
        >
          <div className="flex w-full items-center gap-2">
            {value ? (
              <p className="text-foreground grow text-center leading-none tracking-tight">
                {typeof value != "string" ? value.name : formatLongString(value, 30)}
              </p>
            ) : (
              <p className="text-foreground grow text-center leading-none tracking-tight">
                <span className="font-semibold text-cyan-500">Clique para escolher um arquivo</span>{" "}
                ou o arraste para a àrea demarcada
              </p>
            )}
            {value ? (
              <BsCheck2All size={30} color={"rgb(34,197,94)"} />
            ) : (
      ...
      {showMenu ? (
        <div className="bg-background scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 border-border absolute top-[95px] z-100 flex h-[250px] max-h-[250px] w-full flex-col gap-2 self-center overflow-y-auto overscroll-y-auto rounded-md border p-2 py-1 shadow-xs">
          {fileReferences?.map((reference) => (
            <div
              key={reference._id}
              className="flex w-full flex-col rounded-md border border-cyan-500 p-2"
            >
              <div className="flex w-full items-center justify-between gap-1">
                <div className="flex grow items-center gap-1">
                  ...
                  <a
                    href={reference.url}
                    className="text-foreground text-[0.65rem] leading-none font-bold tracking-tight duration-300 ease-in-out hover:text-cyan-500"
                  >
                    {reference.titulo}
                  </a>
                </div>
                {value == reference.url ? (
                  <h1 className="rounded-md bg-green-600 px-2 py-1 text-[0.55rem] font-medium text-white">
                    ESCOLHIDO
                  </h1>
                ) : (
                  <button
              ...
              <div className="mt-1 flex w-full items-center justify-between gap-2">
                <h1 className="text-foreground text-center text-[0.6rem] font-medium italic">
                  {reference.formato}
                </h1>
              </div>
            </div>
