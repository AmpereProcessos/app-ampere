	...
	return (
		<div ref={ref} className="relative flex w-full flex-col justify-center self-center">
			<div className="flex w-full items-center justify-between gap-2">
				<label htmlFor={inputIdentifier} className={"font-sans text-start font-bold text-[#353432]"}>
					{label}
				</label>
				{fileReferences ? (
					!showMenu ? (
						<button onClick={() => setShowMenu(true)} className="min-w-fit rounded-md bg-blue-800 px-2 py-1 text-[0.55rem] font-medium text-white hover:bg-blue-600">
							MOSTRAR OPÇÕES
						</button>
					) : (
						<button onClick={() => setShowMenu(false)} className="min-w-fit rounded-md bg-red-600 px-2 py-1 text-[0.55rem] font-medium text-white hover:bg-red-500">
							FECHAR OPÇÕES
						</button>
					)
				) : null}
			...
			<div className="relative mt-2 flex w-full items-center justify-center">
				...
				>
					<div className="flex w-full items-center gap-2">
						{value ? (
							<p className="grow text-center leading-none tracking-tight text-gray-500">{typeof value != "string" ? value.name : formatLongString(value, 30)}</p>
						) : (
			...
			{showMenu ? (
				<div className="absolute top-[95px] z-[100] flex h-[250px] max-h-[250px] w-full flex-col gap-2 self-center overflow-y-auto overscroll-y-auto rounded-md border border-gray-300 bg-[#fff] p-2 py-1 shadow-sm scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300">
						...
						<div key={reference._id} className="flex w-full flex-col rounded-md border border-cyan-500 p-2">
							<div className="flex w-full items-center justify-between gap-1">
								<div className="flex grow items-center gap-1">
									...
									<a href={reference.url} className="text-[0.65rem] font-bold leading-none tracking-tight text-gray-500 duration-300 ease-in-out hover:text-cyan-500">
										{reference.titulo}
									</a>
								</div>
								{value == reference.url ? (
									...
									<button onClick={() => handleChange(reference.url)} className="rounded-md bg-cyan-600 px-2 py-1 text-[0.55rem] font-medium text-white hover:bg-cyan-500">
										USAR ARQUIVO
									</button>
								)}
							</div>
