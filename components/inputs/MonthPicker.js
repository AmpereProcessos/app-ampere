  ...
  return (
    <div className={`relative flex w-full flex-col gap-1 lg:w-[${width ? width : '350px'}]`}>
      ...
      <div
        ...
        className="bg-background border-primary/20 flex min-h-[41px] w-full cursor-pointer items-center justify-between rounded-md border p-3 text-sm shadow-xs"
      >
      ...
      {selectMenuIsOpen ? (
        <div className="bg-background scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 border-primary/20 absolute top-[75px] z-100 flex h-[150px] max-h-[150px] w-full flex-col self-center overflow-y-auto overscroll-y-auto rounded-md border p-2 py-1 shadow-xs">
          <select
          ...
          <div className="grid w-full grow grid-cols-4 gap-2 border-t border-gray-400 p-6">
            {months.map((month, index) => (
              <div
                ...
                className={`p-1 ${selectedMonth == month.value ? 'bg-blue-200' : ''} border-primary/20 dark:hover:bg-primary/10 flex cursor-pointer items-center justify-center rounded border hover:bg-blue-100`}
              >
