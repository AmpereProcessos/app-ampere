...
function extractValue(value, months, years) {
	...
	if (!splittedYear || Number(splittedYear) > 2500 || Number(splittedYear) < 2000) return standardReturn;
	// Passed validations

	return { month: splittedMonth, year: splittedYear };
}
function MonthYearPicker({ width, editable = true, label, labelClassName = "font-sans font-bold  text-[#353432] text-start", showLabel = true, value, handleChange }) {
	...
	const handleYearChange = (event) => {
		...
		const month = selectedMonth ? selectedMonth.toString().padStart(2, "0") : currentMonth.toString().padStart(2, "0");
	...
	return (
		<div className={`relative flex w-full flex-col gap-1 lg:w-[${width ? width : "350px"}]`}>
			...
			<div
				...
				className="flex min-h-[41px] cursor-pointer w-full items-center justify-between rounded-md border border-gray-300 bg-[#fff] p-3 text-sm shadow-sm"
			>
				<p className="grow text-[#353432]">
					{selectedMonth ? selectedMonth.toString().trim().padStart(2, "0") : "MM"}/{selectedYear ? selectedYear : "YYYYY"}
				</p>
				{selectMenuIsOpen ? (
					<IoMdArrowDropup
			...
			{selectMenuIsOpen ? (
				<div className="absolute top-[75px] z-[100] flex h-[150px] max-h-[150px] w-full flex-col self-center overflow-y-auto overscroll-y-auto rounded-md border border-gray-300 bg-[#fff] p-2 py-1 shadow-sm scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300">
					<select className="border-none outline-none w-full text-center font-raleway font-bold pb-2" value={selectedYear ? selectedYear : currentYear} onChange={handleYearChange}>
					...
					<div className="w-full grid grid-cols-4 grow gap-2 p-6 border-t border-gray-400">
						{months.map((month, index) => (
							<div
								...
								className={`p-1 ${selectedMonth == month.value ? "bg-blue-200" : ""} flex items-center justify-center rounded border border-gray-300 hover:bg-blue-100 cursor-pointer`}
							>
