      ...
      <div className="flex items-center">
        <div className="flex -space-x-1">
          {selectedOptions.slice(0, displayCount).map((item, index) => (
            <div key={item.id} className="bg-primary/20 flex items-center gap-1 rounded-lg border-2 border-white p-0.5 px-1">
              <Avatar key={item.id} className="h-4 min-h-4 w-4 min-w-4">
                <AvatarImage src={item.url} alt={item.label} />
                <AvatarFallback className="text-xxs">{formatNameAsInitials(item.label)}</AvatarFallback>
              </Avatar>
