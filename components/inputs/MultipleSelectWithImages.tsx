...
function MultipleSelectWithImages<T>({
      ...
      <div className="flex items-center">
        <div className="flex -space-x-1">
          {selectedOptions.slice(0, displayCount).map((item, index) => (
            <div
              key={item.id}
              className="bg-primary/20 flex items-center gap-1 rounded-lg border-2 border-white p-0.5 px-1"
            >
              <Avatar key={item.id} className="h-4 min-h-4 w-4 min-w-4">
  ...
  if (isDesktop)
    return (
      <div
        ref={ref}
        className={`relative flex w-full flex-col gap-1 lg:w-[${width ? width : "350px"}]`}
      >
        {showLabel ? (
          <label
            ...
            {items ? (
              items.map((item, index) => (
                <button
                  ...
                  className={`hover:bg-primary/20 flex w-full cursor-pointer items-center rounded p-2 px-2 ${selectedIds?.includes(item.id) ? "bg-primary/20" : ""}`}
                >
              ...
              <>
                ...
                >
                  {selectedIds.length > 1
                    ? "MÚLTIPLAS SELEÇÕES"
                    : options?.find((item) => item.id === selectedIds[0])?.label}
                </button>
              </>
            ) : (
              <button
