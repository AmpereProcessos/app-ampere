...
function Header({ toggleSidebar }: HeaderProps) {
  ...
  const userView = session?.user.visualizacao.tipo;
  const pathname = usePathname();
  const publicOrDocumentPath =
    pathname?.includes("pdf") || pathname?.includes("publico") || pathname?.includes("auth");
          ...
          <Link href="/configuracoes">
            <Avatar className="h-6 w-6 lg:h-8 lg:w-8">
              ...
              <AvatarFallback className="text-xs">
                {formatNameAsInitials(session.user?.nome || "USER")}
              </AvatarFallback>
            </Avatar>
          </Link>
        </Button>
