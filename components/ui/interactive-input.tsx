...
function InteractiveInputRoot({
  ...
  const resolvedMode: Exclude<InteractiveInputMode, "auto"> =
...
function InteractiveInputContent({
  children,
  className,
  drawerClassName,
  align = "start",
}: InteractiveInputContentProps) {
  const { mode } = useInteractiveInputContext();
          ...
          {onClear ? (
            <>
              <CommandItem value="__clear__" onSelect={handleClear}>
                {clearLabel}
                <Check
                  className={cn("ml-auto", normalizedValue === null ? "opacity-100" : "opacity-0")}
                />
              </CommandItem>
