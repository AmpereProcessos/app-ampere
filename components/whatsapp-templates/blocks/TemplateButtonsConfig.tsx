...
function TemplateButtonsConfig({ buttons, onButtonsChange }: TemplateButtonsConfigProps) {
  ...
  return (
    <ResponsiveDialogDrawerSection
      sectionTitleText="BOTÕES"
      sectionTitleIcon={<MousePointerClick size={15} />}
    >
      <AnimatePresence>
        <Button
          type="button"
        ...
        {buttons && buttons.length > 0
          ? buttons.map((button, index) => (
              ...
              >
                    ...
                    <Button
                      ...
                      onClick={() =>
                        handleButtonChange(index, {
                          ...button,
                          tipo: option.value as "quick_reply" | "url" | "phone_number",
                        })
                      }
                    >
                ...
                {button.texto.length > 25 && (
                  <p className="text-xs text-red-500">
                    Atenção: O texto do botão deve ter no máximo 25 caracteres.
                  </p>
                )}
              </motion.div>
            ))
