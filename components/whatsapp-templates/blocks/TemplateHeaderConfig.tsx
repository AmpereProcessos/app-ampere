...
function TemplateHeaderConfig({ header, onHeaderChange }: TemplateHeaderConfigProps) {
  ...
  return (
    <ResponsiveDialogDrawerSection
      sectionTitleText="CABEÇALHO"
      sectionTitleIcon={<ImageIcon size={15} />}
    >
      <AnimatePresence>
        {!header ? (
          <Button
            ...
            className="flex items-center gap-1 w-fit self-center"
          >
          ...
          <motion.div
            className="w-full flex flex-col gap-3 p-2 rounded-lg border border-border"
          ...
          >
            ...
            <div className="w-full flex items-center justify-center gap-x-3 gap-y-1 flex-wrap">
              {TemplateHeaderTypeOptions.map((option) => (
                <Button
                  ...
                  onClick={() =>
                    handleHeaderTypeChange(option.value as "text" | "image" | "video" | "document")
                  }
                >
            ...
            {header.tipo === "text" && (
              <TemplateHeaderConfigText header={header} onHeaderChange={onHeaderChange} />
            )}
            {header.tipo === "image" && (
              <TemplateHeaderConfigMidia header={header} onHeaderChange={onHeaderChange} />
            )}
            {header.tipo === "video" && (
              <TemplateHeaderConfigMidia header={header} onHeaderChange={onHeaderChange} />
            )}
