      ...
      <div className="w-full h-full flex flex-col items-center justify-center gap-4 pb-4">
            ...
            <div className="bg-white rounded-lg shadow-md overflow-hidden max-w-[85%] relative">
                  ...
                  {cabecalho.tipo === "text" ? (
                    <div className="px-3 pt-3">
                      <p className="font-semibold text-sm text-gray-900">
                        {cabecalho.conteudo || "Texto do cabeçalho"}
                      </p>
                    </div>
              ...
              <div className="px-3 py-2 pt-3">
                <div className="whitespace-pre-wrap text-sm text-gray-900 break-words">
                  {bodyWithExamples || "Digite o conteúdo da mensagem..."}
                </div>
              ...
              {botoes && botoes.length > 0 && (
                <div className="border-t border-gray-200">
                  {botoes.map((botao, index) => (
                    <div
                      key={index.toString()}
                      className="border-b border-gray-200 last:border-b-0 py-2.5 px-4 text-center"
                    >
                      <button
                        type="button"
                        className="text-sm font-medium text-[#00a884] disabled:cursor-default w-full"
                        disabled
                      >
        ...
        <div className="w-full max-w-md space-y-2 mt-2">
          <div className="flex items-center justify-between text-xs">
            <span className="text-foreground">Caracteres no corpo:</span>
            <span
              className={bodyText.length > 1024 ? "text-red-500 font-semibold" : "text-foreground"}
            >
          ...
          {cabecalho?.tipo === "text" && cabecalho.conteudo && (
            <div className="flex items-center justify-between text-xs">
              <span className="text-foreground">Caracteres no cabeçalho:</span>
              <span
                className={
                  cabecalho.conteudo.length > 60 ? "text-red-500 font-semibold" : "text-foreground"
                }
              >
                {cabecalho.conteudo.length} / 60
              </span>
            </div>
          )}
          ...
          {rodape?.conteudo && (
            <div className="flex items-center justify-between text-xs">
              <span className="text-foreground">Caracteres no rodapé:</span>
              <span
                className={
                  rodape.conteudo.length > 60 ? "text-red-500 font-semibold" : "text-foreground"
                }
              >
                {rodape.conteudo.length} / 60
              </span>
            </div>
          )}
          ...
          {botoes && botoes.length > 0 && (
            <div className="flex items-center justify-between text-xs">
              <span className="text-foreground">Número de botões:</span>
              <span
                className={botoes.length > 10 ? "text-red-500 font-semibold" : "text-foreground"}
              >
                {botoes.length} / 10
              </span>
            </div>
