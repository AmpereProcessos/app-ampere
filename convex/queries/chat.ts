...
import { paginationOptsValidator, CHATS_PAGE_SIZE, MESSAGES_PAGE_SIZE, MESSAGES_INITIAL_LOAD, createCursor, parseCursor } from "../utils/pagination";
...
export const getChats = query({
	...
	handler: async (ctx, args) => {
		...
		if (searchQuery && searchQuery.trim().length > 0) {
			...
			let filteredChats = chatsForMatchingClients.filter((chat) => clientIds.includes(chat.clienteId));
			...
			[...filteredChats, ...messageContentSearch].forEach((chat) => {
...
/**
 * Internal query to get chat data
 */
...
/**
 * Get chat summary for AI/internal use
 */
export const getChatSummary = internalQuery({
	...
	handler: async (ctx, args) => {
		...
		const chatLastOneHundredMessages = await ctx.db
			.query("messages")
			.withIndex("by_chat_and_date", (q) => q.eq("chatId", args.chatId))
			.order("desc")
			.take(100);
