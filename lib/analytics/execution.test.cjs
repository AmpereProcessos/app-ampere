...
test('installed equipment aggregates concluded mounting orders by team and region without mixing string inverters', () => {
  const data = analytics.buildExecutionAnalytics(
    [
      order('micro', at('2025-01-01'), at('2025-01-02'), {
        responsavel: { nome: 'Equipe Azul' },
        detalhes: { topologia: 'MICRO-INVERSOR' },
        equipamentos: { modulos: { qtde: 20 }, inversor: { qtde: 5 } },
      }),
      order('string', at('2025-01-03'), at('2025-01-04'), {
        responsavel: { nome: 'Equipe Verde' },
        detalhes: { topologia: 'INVERSOR' },
        equipamentos: { modulos: { qtde: 10 }, inversor: { qtde: 1 } },
      }),
      order('maintenance', at('2025-01-05'), at('2025-01-06'), {
        categoria: 'MANUTENÇÃO CORRETIVA',
        responsavel: { nome: 'Equipe Azul' },
        detalhes: { topologia: 'MICRO' },
        equipamentos: { modulos: { qtde: 99 }, inversor: { qtde: 30 } },
      }),
      order('missing-team', at('2025-01-07'), at('2025-01-08'), {
        responsavel: { nome: 'NÃO DEFINIDO' },
        detalhes: { topologia: 'MICRO' },
        equipamentos: { modulos: {}, inversor: {} },
      }),
    ],
    [],
    filters,
  ...
  assert.deepEqual(data.equipment.summary, { installations: 3, modules: 30, micros: 5, microInstallations: 2, averageModules: 15 })
  assert.deepEqual(
    data.equipment.groups.teams.map((team) => [team.label, team.modules, team.micros]),
    [
      ['Equipe Azul', 20, 5],
      ['Equipe Verde', 10, 0],
      ['Sem equipe', 0, 0],
  ...
  assert.deepEqual(data.equipment.groups.regions.map((region) => [region.label, region.modules]), [['Uberlândia/MG', 30]])
  assert.deepEqual(data.equipment.quality, { missingModules: 1, missingMicros: 1, missingTeams: 1 })
})
