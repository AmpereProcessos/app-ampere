...
function EmployeesContent({ session }: EmployeesContentProps) {
  ...
  const {
    ...
    filters,
    updateFilters,
  } = useEmployees({ initialFilters: { activeOnly } });
  ...
  return (
    <div className="grow p-6">
      <div className="flex h-full grow flex-col">
        <div className="border-border flex flex-col items-center justify-between border-b p-1 gap-3">
          <div className="flex w-full items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button
                onClick={() => setDropdownMenuVisible((prev) => !prev)}
                type="button"
                variant={"ghost"}
              >
                <ListFilter className="w-4 h-4 min-w-4 min-h-4" />
              </Button>
              {userHasCreateUsersPermission ? (
                <Button onClick={() => setNewEmployeeModalIsOpen(true)}>NOVO COLABORADOR</Button>
              ) : null}
            </div>
          </div>
        ...
        {newEmployeeModalIsOpen ? (
          <NewEmployee closeModal={() => setNewEmployeeModalIsOpen(false)} session={session} />
        ) : null}
        {editEmployeeModal.isOpen && editEmployeeModal.id ? (
          <EditEmployee
            userId={editEmployeeModal.id}
            closeModal={() => setEditEmployeeModal({ isOpen: false, id: null })}
            session={session}
          />
        ) : null}
      </div>
...
function EmployeesFiltersMenu({ filters, updateFilters }: EmployeesFiltersMenuProps) {
  return (
    <motion.div
      ...
      animate="animate"
      exit="exit"
      className="flex w-full flex-col gap-2 border border-border p-3 rounded"
    >
        ...
        <div className="w-full flex flex-col gap-2 grow">
          <div className="w-full flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-1">
              ...
              <p className="text-xs font-medium text-foreground">{getPosition(employee.cargos)}</p>
            </div>
            <div className="flex items-center gap-1">
              <Mail className="w-4 h-4 min-w-4 min-h-4" />
              <p className="text-xs font-medium text-foreground">{employee.email}</p>
            </div>
            <div className="flex items-center gap-1">
              <Phone className="w-4 h-4 min-w-4 min-h-4" />
              <p className="text-xs font-medium text-foreground">
                {employee.telefone ?? "NÃO DEFINIDO"}
              </p>
            </div>
            <div className="flex items-center gap-1">
              ...
              <p className="text-xs font-medium text-foreground">
                {employee.dataNascimento
                  ? formatDateBirthdayAsLocale(employee.dataNascimento)
                  : "NÃO DEFINIDO"}
              </p>
            </div>
          </div>
        ...
        <div className="flex w-full items-center justify-between gap-1">
          <div className="flex items-center gap-1">
            ...
            <p className="text-foreground text-xs font-medium">
              {formatDateAsLocale(employee.dataInsercao, true) || "N/A"}
            </p>
          </div>
          {userHasEditUsersPermission ? (
            <Button
