...
function CreditCardOptionsContent({ session }: { session: TAuthSession }) {
  ...
  const [newCreditCardOptionModalIsOpen, setNewCreditCardOptionModalIsOpen] =
          ...
          <div className="flex w-full items-center justify-between">
            <div className="flex flex-col">
              <p className="text-center text-2xl font-black text-[#15599a] uppercase">
                SIMULAÇÕES DE CARTÃO DE CRÉDITO
              </p>
              <p className="text-foreground text-sm tracking-tight">
                {options?.length || "..."} opções contabilizadas
              </p>
            </div>
            <button
              type="button"
        ...
        {isSuccess ? (
          options.length > 0 ? (
            ...
            <p className="text-foreground w-full text-center font-medium italic">
              Nenhuma opção de simulação foi encontrada...
            </p>
          )
        ) : null}
      </div>
      {newCreditCardOptionModalIsOpen ? (
        <NewCreditCardOption closeModal={() => setNewCreditCardOptionModalIsOpen(false)} />
      ) : null}
      {editModal.id && editModal.isOpen ? (
        <EditCreditCardOption
          optionId={editModal.id}
...
function CreditCardOptionCard({ option, handleClick }: CreditCardOptionCardProps) {
  ...
  function handleSimulation(data: {
    ...
    const fractionment = Object.entries(selected.parcelas).find(
      ([key, value]) => key === data.parcelas,
    );
    ...
    <div className="flex w-full flex-col gap-2 rounded border border-black lg:w-[600px]">
      <div className="flex w-full items-center justify-center bg-black p-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="bg-background rounded-lg px-2 py-0.5 text-[0.6rem] font-black text-black">
            {option.modalidade}
          </h1>
          <button
            type="button"
      ...
      <div className="flex w-full flex-col gap-2 p-3">
        <div className="flex w-full flex-col gap-2 lg:flex-row">
          ...
          <div className="w-full lg:w-1/3">
            <SelectInput
              ...
              handleChange={(value) =>
                setSimulation((prev) => ({ ...prev, opcaoPagamento: value }))
              }
              selectedItemLabel="NÃO DEFINIDO"
        ...
        <div className="flex w-full items-center justify-center">
          <div className="flex w-full flex-col gap-1 overflow-hidden rounded border border-green-600">
            <h1 className="bg-green-600 p-1 text-center text-[0.6rem] font-bold text-white">
              RESULTADO
            </h1>
            <div className="flex w-full flex-col gap-1 p-3">
              <h1 className="font-Poppins w-full text-center font-black">
                {formatToMoney(handleSimulation(simulation).total)}
              </h1>
              <h1 className="text-foreground text-center text-xs font-medium tracking-tight">
                {simulation.parcelas}x PARCELAS DE:{" "}
                {formatToMoney(handleSimulation(simulation).valorParcela)}
              </h1>
            </div>
          </div>
        </div>
