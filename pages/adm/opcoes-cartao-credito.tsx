...
function CreditCardOptions() {
	...
	return (
		<div className="grow p-6">
			...
			{editModal.id && editModal.isOpen ? <EditCreditCardOption optionId={editModal.id} closeModal={() => setEditModal({ id: null, isOpen: false })} /> : null}
		</div>
	);
}
		...
		<div className="flex w-full flex-col gap-2 rounded border border-black lg:w-[600px]">
			<div className="flex w-full items-center justify-center bg-black p-2">
				<div className="flex items-center gap-2">
					...
					<button onClick={() => handleClick(option._id)} className="flex items-center gap-1 rounded-lg bg-blue-800 px-2 py-1 text-[0.6rem] text-white hover:bg-blue-700">
				...
				<div className="flex w-full items-center justify-center">
					<div className="flex w-full flex-col gap-1 overflow-hidden rounded border border-green-600">
						...
						<div className="flex w-full flex-col gap-1 p-3">
							<h1 className="w-full text-center font-Poppins font-black">{formatToMoney(handleSimulation(simulation).total)}</h1>
