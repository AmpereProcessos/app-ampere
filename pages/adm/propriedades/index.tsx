...
function PropertiesContent({ session }: PropertiesContentProps) {
	...
	function handleExportToExcel(properties: TGetPropertiesDefaultOutput | undefined) {
		...
		const excelData = properties.map((property) => ({
			...
			REVISOES: property.metadados.revisoes.map((revision) => `Revisão em ${revision.km}km, feita em ${formatDateAsLocale(revision.data)}.`).join("\n"),
				...
				<div className="flex flex-col items-center justify-between">
					...
					<motion.div variants={SlideMotionVariants} initial="initial" animate="animate" exit="exit" className="mt-4 flex w-full flex-col gap-y-2">
						<div className="flex w-full flex-col gap-y-2">
							<div className="w-full flex items-center gap-2 border border-primary/30 rounded-lg p-2">
								<div className="w-6 h-6 bg-primary text-primary-foreground rounded-full flex items-center justify-center">
									<Search className="h-4 w-4 min-w-4 min-h-4" />
								</div>
								<input
									placeholder="PESQUISAR PROPRIEDADE..."
									value={filters.search || ""}
									onChange={(e) => updateFilters({ search: e.target.value })}
									className="grow outline-none ring-0 focus:ring-0 focus:ring-offset-0 text-xs placeholder:font-medium"
								/>
							</div>
		...
		<div className="w-full flex flex-col gap-2 border border-gray-300 rounded-xl p-3 shadow-sm">
			<div className="w-full flex items-center justify-between">
				...
				<div className="flex items-center gap-2">
					...
					>
						<Eye className="h-4 w-4 min-w-4 min-h-4" />
					</Button>
					<DateIntervalInput
			...
			<AnimatePresence>
				{showStats ? (
					<motion.div variants={SlideMotionVariants} initial="initial" animate="animate" exit="exit" className="flex w-full flex-col items-center justify-center gap-3 lg:flex-row">
						<div className="flex w-full flex-col rounded-xl border border-gray-300 bg-[#fff] p-3 shadow-sm lg:w-1/6">
							<div className="flex items-center justify-between">
								...
								<Landmark className="h-4 w-4 min-w-4 min-h-4" />
							</div>
						...
						<div className="flex w-full flex-col rounded-xl border border-gray-300 bg-[#fff] p-3 shadow-sm lg:w-1/6">
							<div className="flex items-center justify-between">
								...
								<Car className="h-4 w-4 min-w-4 min-h-4 text-yellow-700" />
							</div>
							<div className="flex w-full flex-col">
						...
						<div className="flex w-full flex-col rounded-xl border border-gray-300 bg-[#fff] p-3 shadow-sm lg:w-1/6">
							<div className="flex items-center justify-between">
								...
								<Car className="h-4 w-4 min-w-4 min-h-4 text-red-700" />
							</div>
							<div className="flex w-full flex-col">
						...
						<div className="flex w-full flex-col rounded-xl border border-gray-300 bg-[#fff] p-3 shadow-sm lg:w-1/6">
							<div className="flex items-center justify-between">
								...
								<Calendar className="h-4 w-4 min-w-4 min-h-4 text-blue-700" />
							</div>
							<div className="flex w-full flex-col">
						...
						<div className="flex w-full flex-col rounded-xl border border-gray-300 bg-[#fff] p-3 shadow-sm lg:w-1/6">
							<div className="flex items-center justify-between">
								...
								<Calendar className="h-4 w-4 min-w-4 min-h-4 text-green-700" />
							</div>
							<div className="flex w-full flex-col">
