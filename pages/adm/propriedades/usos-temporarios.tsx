...
function TemporaryUsagesContent({ session }: TemporaryUsagesContentProps) {
	...
	return (
		<div className="grow flex flex-col h-full w-full gap-6 p-6">
							...
							<div className="flex flex-col flex-wrap items-center justify-start gap-2 lg:flex-row">
								<div className="w-full lg:w-[300px]">
									<SelectInput
										...
										handleChange={(value) => updateQueryParams({ periodType: value as "dataInicio" | "dataFim" })}
								...
								<div className="w-full lg:w-[300px]">
									<SelectInput
										...
										selectedItemLabel={"NÃO DEFINIDO"}
										handleChange={(value) => updateQueryParams({ type: value as "all" | "open" | "closed" })}
			...
			{viewPropertyUsageId ? <ViewPropertyUsage usageId={viewPropertyUsageId} closeModal={() => setViewPropertyUsageId(null)} /> : null}
		</div>
	);
}

function TemporaryUsageCard({ usage, handleClick }: { usage: TGetPropertyTemporaryUsagesDefaultOutput[number]; handleClick: (id: string) => void }) {
	...
	const vehicleInfo =
		usage.metadados.tipo === "USO DE VEÍCULO" ? (
			<div className="flex items-center gap-1">
				<Badge className={cn(PROPERTY_METADATA_TYPES_CONFIG.VEÍCULO.stylingClassName, "rounded-full text-[0.65rem]")}>
					{renderIconWithClassNames(PROPERTY_METADATA_TYPES_CONFIG.VEÍCULO.icon, "h-4 w-4 min-w-4 min-h-4")}
					{PROPERTY_METADATA_TYPES_CONFIG.VEÍCULO.name}
				</Badge>
				<div className="flex items-center gap-1">
					<Plus className="w-4 h-4" />
					<p className="text-xs font-medium tracking-tight">KM INICIAL: {usage.metadados.kmInicial}</p>
					{usage.metadados.kmFinal ? (
						<>
							<Check className="w-4 h-4 text-green-600" />
							<p className="text-xs font-medium tracking-tight">KM FINAL: {usage.metadados.kmFinal}</p>
						</>
					) : null}
				</div>
			</div>
		...
		<div className="flex w-full flex-col gap-1 rounded border border-primary bg-[#fff] p-2 shadow-sm dark:bg-[#121212]">
			...
			<div className="flex w-full items-center justify-between flex-col sm:flex-row">
				<div className="flex items-center gap-2 flex-wrap">
					...
					{usage.dataFim ? (
						<div className="flex items-center gap-2">
							<BsCalendarCheck className="w-4 h-4 text-green-600" />
							<p className="text-xs font-medium tracking-tight">FIM: {formatDateAsLocale(usage.dataFim) || "N/A"}</p>
						</div>
					) : null}
					<div className="flex items-center gap-2">
