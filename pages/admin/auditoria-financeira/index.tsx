...
import {
  fetchFinancialAuditingExport,
  useFinancialAuditing,
} from "@/utils/methods/query/financial-auditing";
...
function FinancesAuditingContent({ session }: { session: TAuthSession }) {
  const [projectFinancesModal, setProjectFinancesModal] = useState<{
    isOpen: boolean;
  ...
  function getStats({ info }: { info?: TProjectFinances[] }) {
    ...
    const totalRevenues = info.reduce((acc, current) => {
      const currentTotalRevenue = Object.values(current.receitas).reduce(
    ...
    const totalExpenses = info.reduce((acc, current) => {
      const currentTotalRevenue = Object.values(current.despesas).reduce(
        (acc, current) => acc + current,
        0,
      );
      return acc + currentTotalRevenue;
  ...
  return (
    <div className="flex grow flex-col p-6">
      <div className="border-border flex flex-col items-center border-b px-1 py-2 gap-1">
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-2 gap-y-2">
            ...
            <div className="w-full lg:w-[250px]">
              <SelectInput
                ...
                onReset={() =>
                  setDateFilter((prev) => ({ ...prev, field: "contrato.dataAssinatura" }))
                }
                width="100%"
              />
            </div>
        ...
        <div className="mt-2 flex w-full flex-col flex-wrap items-center justify-center gap-2 lg:flex-row">
          ...
          <div className="w-full lg:w-[350px]">
            <MultipleSelectInput
              ...
              options={cidadesAtendidas.map((city, index) => ({
                id: index + 1,
                label: city,
                value: city,
              }))}
          ...
          <div className="w-full lg:w-[350px]">
            <MultipleSelectInput
              ...
              options={allSellers.map((seller, index) => ({
                id: index + 1,
                label: seller.value,
                value: seller.value,
              }))}
              handleChange={(value) =>
                setFilters((prev) => ({ ...prev, sellerName: value as string[] }))
              }
              ...
              <div className="w-[120px]">
                <NumberInput
                  ...
                  handleChange={(value) =>
                    setFilters((prev) => ({
                      ...prev,
                      moduleQty: { ...prev.moduleQty, min: value },
                    }))
                  }
                  width="100%"
                />
              ...
              <div className="w-[120px]">
                <NumberInput
                  ...
                  handleChange={(value) =>
                    setFilters((prev) => ({
                      ...prev,
                      moduleQty: { ...prev.moduleQty, max: value },
                    }))
                  }
                  width="100%"
                />
          ...
          <div className="bg-background border-border flex min-h-[110px] w-full flex-col rounded-xl border p-3 shadow-xs lg:w-1/4">
            ...
            <div className="mt-2 flex w-full flex-col">
              <div className="text-2xl font-bold text-[#15599a]">
                {getStats({ info: auditing }).projetos}
              </div>
            </div>
          </div>
          <div className="bg-background border-border flex min-h-[110px] w-full flex-col rounded-xl border p-3 shadow-xs lg:w-1/4">
            ...
            <div className="mt-2 flex w-full flex-col">
              <div className="text-2xl font-bold text-[#15599a]">
                {formatToMoney(getStats({ info: auditing }).receitas)}
              </div>
            </div>
          </div>
          <div className="bg-background border-border flex min-h-[110px] w-full flex-col rounded-xl border p-3 shadow-xs lg:w-1/4">
            ...
            <div className="mt-2 flex w-full flex-col">
              <div className="text-2xl font-bold text-[#15599a]">
                {formatToMoney(getStats({ info: auditing }).despesas)}{" "}
              </div>
            </div>
          </div>
          <div className="bg-background border-border flex min-h-[110px] w-full flex-col rounded-xl border p-3 shadow-xs lg:w-1/4">
            ...
            <div className="mt-2 flex w-full flex-col">
              <div className="text-2xl font-bold text-[#15599a]">
                {formatDecimalPlaces(getStats({ info: auditing }).margem)}%
              </div>
            </div>
          </div>
        </div>
        <div className="bg-background border-border flex min-h-[70px] w-full gap-2 rounded-xl border p-3 shadow-xs">
          ...
          <div className="mt-2 flex grow flex-wrap items-center justify-around">
            {Object.entries(getStats({ info: auditing }).despesasPorTipo).map(([key, value]) => (
              <p key={key} className="text-foreground text-[0.8rem]">
      ...
      <div className="mt-4 flex flex-wrap justify-around gap-3">
        ...
        {isError ? (
          <ErrorComponent msg={"Erro ao buscar dados para auditoria financeira."} />
        ) : null}
        {isSuccess
          ? auditing.map((info) => (
              <AuditingCard
                ...
                handleClick={(id: string) =>
                  setProjectFinancesModal({ isOpen: true, projectId: id })
                }
              />
            ))
          : null}
