...
function Acompanhamento() {
	...
	function getPotenciaVendida() {
		...
		if (dateFilter.after && dateFilter.before && dateFilter.field1 != null) {
			...
			filteredArr = filteredArr.filter((x) => x[dateFilter.field1][dateFilter.field2] >= dateFilter.after && x[dateFilter.field1][dateFilter.field2] <= dateFilter.before);
		}
	...
	function getPotenciaHomologada() {
		var filteredArr = info;
		filteredArr = filteredArr.filter((x) => x.homologacao.acesso.dataResposta != null && x.homologacao.acesso.dataResposta != undefined && x.homologacao.acesso.dataResposta != "-");
		if (dateFilter.after && dateFilter.before && dateFilter.field1 != null) {
			if (!filteredArr) filteredArr = info;
			filteredArr = filteredArr.filter((x) => x[dateFilter.field1][dateFilter.field2] >= dateFilter.after && x[dateFilter.field1][dateFilter.field2] <= dateFilter.before);
		}
	...
	function renderClients() {
		...
		return (
			<div className="flex w-full flex-col">
				<h1 className="text-center font-medium text-[#fead61]">{filteredArr.length > 1 ? `${filteredArr.length} CLIENTES` : `${filteredArr.length} CLIENTE`}</h1>
	...
	return (
		<div className="flex grow flex-col gap-2 p-6">
			...
			<div className="flex w-full items-center justify-end py-2">
				<button onClick={() => setShowClientsNames((prev) => !prev)} className={`flex items-center gap-2 rounded p-1 font-medium ${renderShowClientsButtonStyles(showClientsNames)}`}>
			...
			<div className="grid grid-cols-1 grid-rows-5  gap-x-3 lg:grid-cols-5 lg:grid-rows-1">
				...
				<div className="col-span-1 flex h-[300px] flex-col border border-gray-300 bg-[#fff] p-4 shadow-xl">
					...
					<p className="flex grow items-center justify-center text-2xl font-bold text-[#15599a]">R$ {getTotalVendido().total.toLocaleString("pt-BR")}</p>
				</div>
				<div className="col-span-1 flex h-[300px] flex-col border border-gray-300 bg-[#fff] p-4 shadow-xl">
					<div className="flex justify-between">
						<h1 className="uppercase text-gray-600">TICKET MÉDIO</h1>
					</div>
					<p className="flex grow items-center justify-center text-2xl font-bold text-[#15599a]">R$ {getTotalVendido().ticketMedio.toLocaleString("pt-BR")}</p>
				</div>
				<div className="col-span-1 flex h-[300px] flex-col border border-gray-300 bg-[#fff] p-4 shadow-xl">
			...
			<div className="grid grid-cols-1 grid-rows-2 gap-x-3 lg:grid-cols-2 lg:grid-rows-1">
				<div className="col-span- flex h-[300px] flex-col border border-gray-300 bg-[#fff] p-4 shadow-xl">
					...
					<p className="flex grow items-center justify-center text-2xl font-bold text-[#15599a]">R$ {getCustos().totalValorDoKit.toLocaleString("pt-BR")}</p>
				</div>
				<div className="col-span- flex h-[300px] flex-col border border-gray-300 bg-[#fff] p-4 shadow-xl">
					<div className="flex justify-between">
					...
					<p className="flex grow items-center justify-center text-2xl font-bold text-[#15599a]">R$ {getCustos().totalEfetivoCustos.toLocaleString("pt-BR")}</p>
