...
export default function UsersControl() {
  ...
  const [editionModalObj, setEditionModalObj] = useState({});
  ...
  return (
    <div className="flex flex-col grow bg-[#fff] overflow-x-hidden">
      <div className="w-full bg-gray-800 flex items-center justify-center h-[80px] p-2">
      ...
      {users ? (
        <>
          <div className="w-full flex justify-around flex-wrap my-4 gap-2">
            {users.map((user, index) => (
              <CardControleUsuarios
                key={user._id}
                userInfo={user}
                admin={user.manager}
      ...
      <div
        ...
        className="fixed bg-[#15599a] cursor-pointer hover:bg-[#fead61] text-white hover:text-[#15599a] ml-10 p-3 rounded-lg bottom-10 left-150"
      >
        <p className="uppercase font-bold text-sm">NOVO USUÁRIO</p>
      </div>
      {newUserModalIsOpen && (
        <ModalNovoUsuario closeModal={() => setNewUserModalIsOpen(false)} />
      )}
      {editionModalIsOpen && (
        <ModalEdicaoUsuario
          userInfo={editionModalObj}
          getUsers={getUsers}
          closeModal={() => setEditionModalIsOpen(false)}
        />
      )}
    </div>
  );
