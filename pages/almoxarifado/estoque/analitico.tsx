...
function StockAnalytics() {
	...
	const { data: stockAnalytics, isLoading: isStockAnalyticsLoading, isError: isStockAnalyticsError, isSuccess: isStockAnalyticsSuccess } = useStockAnalytics();
					...
					{dropdownMenuVisible ? (
						<motion.div variants={SlideMotionVariants} initial="initial" animate="animate" exit="exit" className="mt-4 flex w-full flex-col gap-y-2">
							<div className="flex flex-col flex-wrap items-end justify-center gap-2 lg:flex-row">
								<TextInput label={"NOME"} value={filters.name} placeholder={"Digite o nome do material..."} handleChange={(value) => updateFilters({ name: value })} />
