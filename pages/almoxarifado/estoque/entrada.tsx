...
export default function NewMaterialsEntrance() {
  ...
  function updateItem({
    ...
    index: number;
    changes: Partial<TMaterialEntranceInput["itens"][number]>;
  }) {
    setInfoHolder((prev) =>
      prev
        ? {
            ...prev,
            itens: prev.itens.map((item, idx) => (idx === index ? { ...item, ...changes } : item)),
          }
        : null,
    );
  }
  async function handleFileInputChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    const NFItens = Array.isArray(resultExtracted.nfeProc.NFe.infNFe.det)
      ? resultExtracted.nfeProc.NFe.infNFe.det
      : [resultExtracted.nfeProc.NFe.infNFe.det];
  ...
  return (
    <div className="flex w-full grow flex-col gap-12 p-6">
      <div className="border-border flex flex-col items-center justify-between border-b p-1">
        <p className="w-full text-start text-2xl font-black text-[#15599a] uppercase">
      ...
      <div className="flex w-full flex-col gap-3">
        <div className="flex flex-col items-center justify-center gap-1">
          ...
          <p className="text-foreground w-full text-sm italic">
            Anexe a nota fiscal para iniciar o procedimento de entrada de materiais.
          </p>
        </div>
        <label
            ...
            {infoHolder ? (
              <div className="flex w-full items-center justify-center gap-1">
                ...
                <p className="text-primary text-sm font-medium">
                  NF ({infoHolder.idNF}) anexada com sucesso !
                </p>
              </div>
            ) : (
              <p className="text-primary text-sm font-medium">
                Clique aqui para anexar o arquivo XML.
              </p>
            )}
          </div>
          <input
        ...
        {infoHolder ? (
          <div className="border-primary bg-background flex w-full flex-col gap-3 rounded border p-3 shadow-lg dark:bg-[#121212]">
            ...
            <div className="flex w-full flex-col gap-1">
              <h1 className="text-muted-foreground w-full text-start text-xs tracking-tight">
                EMISSOR
              </h1>
              <div className="flex w-full flex-wrap items-center gap-4 gap-y-2">
                <div className="flex items-center gap-1">
                  ...
                  <p className="text-primary text-sm font-bold tracking-tight">
                    {infoHolder.emissor.nome || "NOME DO EMISSOR NÃO ENCONTRADO"}
                  </p>
                </div>
                <div className="flex items-center gap-1">
                  <IdCard className="h-4 min-h-4 w-4 min-w-4" />
                  <p className="text-primary text-sm font-bold tracking-tight">
                    {infoHolder.emissor.cpfCnpj || "CNPJ DO EMISSOR NÃO ENCONTRADO"}
                  </p>
                </div>
                <div className="flex items-center gap-1">
                    ...
                    {formatLocation({
                      location: {
                        ...
                        numeroOuIdentificador:
                          infoHolder.destinatario.localizacao.numeroOuIdentificador,
                      },
                      includeCEP: true,
                      includeCity: true,
                      includeUf: true,
            ...
            <div className="flex w-full flex-col gap-1">
              ...
              <div className="flex w-full flex-col gap-3">
                {infoHolder.itens.map((item, index) => (
                  <NewMaterialsEntranceItemCard
                    key={item.index}
                    item={item}
                    updateItem={(changes) => updateItem({ index, changes })}
                  />
                ))}
              </div>
            </div>
...
function NewMaterialsEntranceItemCard({ item, updateItem }: TNewMaterialsEntranceItemCardProps) {
  function getNewPrice({
    ...
    const newPrice =
      (previousQty * previousPrice + inputQty * inputPrice) / (previousQty + inputQty);
    return newPrice;
  }
  return (
    <div className="border-primary/50 flex w-full flex-col gap-2 rounded-lg border p-2">
      <div className="flex w-full items-center justify-between gap-2">
        ...
        <MaterialSelector
          initialMaterialState={{
            ...
            materialName: item.material?.nome || "",
          }}
          ...
          renderSelectedMaterial={({ selectedMaterial, handleMaterialUnvinculation }) => (
            <div className="flex w-fit items-center gap-2 self-center rounded-lg px-2 py-1">
              <h1 className="text-primary text-[0.65rem] font-medium tracking-tight">
          ...
          renderUnselectedMaterial={({ setOpen }) => (
            <button
              ...
              className="flex w-fit items-center gap-1 self-center rounded px-2 py-1 transition-colors hover:bg-cyan-300"
            >
              <Link size={12} />
      ...
      <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
        <div className={"flex w-full flex-col gap-1 lg:w-1/2"}>
          <label
            htmlFor={"qty-price"}
            className={"font-sans text-[0.6rem] font-bold tracking-tight text-primary"}
          >
          ...
          <input
            ...
            name="qty-price"
            type="number"
            className="border-border text-foreground rounded-lg border p-1 text-center text-[0.6rem] tracking-tight shadow-xs outline-hidden placeholder:italic"
          />
        </div>
        <div className={"flex w-full flex-col gap-1 lg:w-1/2"}>
          <label
      ...
      {item.material ? (
        <div className="border-border bg-secondary/50 mt-2 flex flex-col gap-2 rounded border p-3">
          ...
          <div className="flex w-full items-center justify-between">
            <div className="flex items-center gap-1">
              ...
              <p className="text-foreground text-[0.65rem] leading-none font-medium tracking-tight lg:text-xs">
                {formatDecimalPlaces(item.material.quantidadeAtual + item.quantidade)}{" "}
          ...
          {item.material.unidade && item.material.unidade !== item.unidade ? (
            <p className="rounded border-orange-500 bg-orange-100 p-1 py-2 text-center text-[0.7rem] font-medium text-orange-500">
              Oops, notamos uma divergência nas grandezas. O material atual está em{" "}
              <strong>{item.material.unidade || "UN"}</strong> , e a entrada está em{" "}
              <strong>{item.unidade}</strong>. Verifique as informações e, se necessário, faça
              ajustes de quantidade e preço.
            </p>
          ) : null}
        </div>
      ) : null}
  ...
  return (
    <div className="border-primary bg-background flex w-full flex-col gap-3 rounded border p-3 shadow-lg dark:bg-[#121212]">
      ...
      <div className="scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex w-full grow flex-wrap items-start justify-around gap-2 overflow-y-auto overscroll-y-auto p-3">
        ...
        {isSuccess
          ? logs.map((log) => (
              <UpdateRegistriesCard key={log._id} registry={log} showMaterialName={true} />
            ))
          : null}
      </div>
    </div>
