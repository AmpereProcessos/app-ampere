...
import { Box, Check, DollarSign, IdCard, MapPin, User, X, Link, MoveRight, ReceiptCent } from 'lucide-react'
        ...
        <div className="mt-2 flex flex-col gap-2 rounded border border-primary/30 bg-secondary/50 p-3">
          ...
          {item.material.unidade && item.material.unidade !== item.unidade ? (
            <p className="rounded border-orange-500 bg-orange-100 p-1 py-2 text-center text-[0.7rem] font-medium text-orange-500">
              Oops, notamos uma divergência nas grandezas. O material atual está em <strong>{item.material.unidade || 'UN'}</strong> , e a entrada
              está em <strong>{item.unidade}</strong>. Verifique as informações e, se necessário, faça ajustes de quantidade e preço.
            </p>
          ) : null}
        </div>
      ) : null}
