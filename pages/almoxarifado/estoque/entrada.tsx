...
import { Box, Check, DollarSign, IdCard, Link, MapPin, MoveRight, ReceiptCent, User, X } from "lucide-react";
					...
					<div className="border-primary bg-background flex w-full flex-col gap-3 rounded border p-3 shadow-lg dark:bg-[#121212]">
						...
						<div className="flex w-full flex-col gap-1">
							...
							<div className="flex w-full flex-wrap items-center gap-4 gap-y-2">
								<div className="flex items-center gap-1">
									<User className="h-4 min-h-4 w-4 min-w-4" />
									<p className="text-primary text-sm font-bold tracking-tight">{infoHolder.destinatario.nome || "NOME DO DESTINATÁRIO NÃO ENCONTRADO"}</p>
								</div>
								<div className="flex items-center gap-1">
									<IdCard className="h-4 min-h-4 w-4 min-w-4" />
									<p className="text-primary text-sm font-bold tracking-tight">{infoHolder.destinatario.cpfCnpj || "CNPJ DO DESTINATÁRIO NÃO ENCONTRADO"}</p>
								</div>
								<div className="flex items-center gap-1">
									<MapPin className="h-4 min-h-4 w-4 min-w-4" />
									<p className="text-primary text-sm font-bold tracking-tight">
		...
		<div className="border-primary/50 flex w-full flex-col gap-2 rounded-lg border p-2">
			...
			<div className="flex w-full flex-col items-center gap-2 lg:flex-row">
				<div className={"flex w-full flex-col gap-1 lg:w-1/2"}>
					<label htmlFor={"qty-price"} className={"font-sans text-[0.6rem] font-bold tracking-tight text-primary"}>
				...
				<div className={"flex w-full flex-col gap-1 lg:w-1/2"}>
					<label htmlFor={"input-price"} className={"font-sans text-[0.6rem] font-bold tracking-tight text-primary"}>
						PREÇO DE ENTRADA
					</label>
					<input
						id={"input-price"}
				...
				<div className="border-primary/30 bg-secondary/50 mt-2 flex flex-col gap-2 rounded border p-3">
					...
					{item.material.unidade && item.material.unidade !== item.unidade ? (
						<p className="rounded border-orange-500 bg-orange-100 p-1 py-2 text-center text-[0.7rem] font-medium text-orange-500">
							Oops, notamos uma divergência nas grandezas. O material atual está em <strong>{item.material.unidade || "UN"}</strong> , e a entrada está em{" "}
							<strong>{item.unidade}</strong>. Verifique as informações e, se necessário, faça ajustes de quantidade e preço.
						</p>
					) : null}
				</div>
			) : null}
