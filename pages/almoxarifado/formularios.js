...
function Formularios() {
  ...
  const { data: forms, isSuccess, hasNextPage, fetchNextPage, refetch, isFetching } = useForms(validateAuthorization(session, 'Almoxarifado'))
  ...
  function filterProjects() {
    ...
    if (filters.pesquisa.trim().length > 0) {
      ...
      newArr = newArr.filter(
        (form) =>
          form.nomeDoContrato.toUpperCase().includes(filters.pesquisa.toUpperCase()) ||
          form.nomeTerceiro?.toUpperCase().includes(filters.pesquisa.toUpperCase())
      )
    }
    if (filters.codigo.trim().length > 0) {
      if (!newArr) newArr = forms
      newArr = newArr.filter((form) => form.codigoProjeto?.toString().includes(filters.codigo))
    }
  ...
  async function handleOpenModal(form) {
    let { data } = await axios.get(`/api/almoxarifado/getFormularios?id=${form._id}`)
    setModalForm(data)
    setModalIsOpen(true)
  }
  useEffect(() => {
    ...
    return (
      <div className="p-6 grow">
        <div className="flex flex-col items-center gap-2 border-b border-gray-200 pb-2">
          <h1 className="text-[#fead61] font-raleway font-bold text-xl">FORMULÁRIOS ({filteredForms.length})</h1>
          <div className="flex flex-col items-center w-full gap-2">
            <div className="w-full flex flex-col lg:flex-row items-center justify-center gap-2">
              <input
                ...
                onChange={(e) => setFilters((prev) => ({ ...prev, pesquisa: e.target.value }))}
              />
              <input
                type={'number'}
            ...
            <div className="w-full flex flex-col lg:flex-row items-center justify-center gap-2">
              ...
              <button
                onClick={() => setFilters({ ...filters, efetivados: !filters.efetivados })}
                className={`p-2 rounded w-fit border border-[#15599a] font-bold ${
                  filters.efetivados ? 'bg-[#15599a] text-white' : 'text-[#15599a] bg-transparent'
                }`}
              >
                NÃO EFETIVADOS
              </button>
        ...
        {modalIsOpen && <ModalFormAlmoxarifado info={modalForm} setModalIsOpen={setModalIsOpen} getForms={refetch} />}
        {createModalIsOpen && <ModalNovoFormAlmoxarifado getForms={refetch} setModalIsOpen={setCreateModalIsOpen} />}
        <div
