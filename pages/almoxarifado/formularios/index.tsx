      ...
      <div className="border-border flex flex-col items-center justify-between border-b p-1">
        <div className="flex w-full flex-wrap items-center justify-between gap-2">
          <p className="text-center text-2xl font-black text-[#15599a] uppercase lg:text-left">
            ...
            <strong className="text-[#fead41]">({warehouseFormulariesMatched || "..."})</strong>
          </p>
          ...
          {warehouseFormularies.length > 0 ? (
            warehouseFormularies.map((form) => (
              <FormularyCard
                key={form._id}
                formulary={form}
                openModal={(id) => setModalForm({ isOpen: true, id: id })}
              />
            ))
          ) : (
...
function WarehouseFormulariesDrafts({
  ...
  const {
    data: warehouseFormulariesDraftsResult,
    queryKey,
    isLoading,
    isError,
    isSuccess,
  } = useWarehouseFormsDrafts();
  const handleOnMutate = async () => {
  ...
  return (
    <div className="flex w-full flex-col gap-2 rounded-lg bg-orange-200 p-3">
      ...
      {warehouseFormulariesDraftsResult.length > 0 ? (
        ...
        <p className="w-full text-center font-medium text-orange-600 italic">
          Nenhum formulário em rascunho encontrado.
        </p>
      )}
      ...
      <div className="flex min-w-0 items-center gap-2 truncate">
        ...
        <div className="hidden items-center gap-1 lg:flex">
          ...
          <p className="text-muted-foreground text-xs font-medium tracking-tight">
            {form.materiais.length} materiais
          </p>
        </div>
      </div>
