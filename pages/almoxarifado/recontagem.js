...
function Recontagem() {
  ...
  async function sendAlterations() {
    ...
    try {
      ...
      alert(
        "Oops, houve um erro na alteração dos itens. Por favor, tente novamente."
      );
    }
  }
  useEffect(() => {
                  ...
                  {material.recontagem ? (
                    <p className="text-gray-400 text-xs italic">
                      ...
                      <strong className="text-[#fead61]">
                        {dayjs(material.recontagem.data).format(
                          "DD/MM/YYYY HH:mm"
                        )}
                      </strong>{" "}
                      por{" "}
                      {material.recontagem.responsavel
                        ? material.recontagem.responsavel
                        : session.user?.name}
                    </p>
                  ) : null}
