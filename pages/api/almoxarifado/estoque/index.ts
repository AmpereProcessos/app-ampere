...
const updateMaterial: NextApiHandler<PutResponse> = async (req, res) => {
	...
	const updateResponse = await collection.updateOne({ _id: new ObjectId(id) }, { $set: { ...changes, alteracao: { ...author, dataAlteracao: new Date().toISOString() } } });
		...
		await dbSession.withTransaction(async () => {
			...
			for (const item of payload.itens) {
				...
				if (existingMaterial) {
					...
					const newPrice = (existingMaterial.qtde * existingMaterial.preco + item.quantidade * item.valorUnitario) / (existingMaterial.qtde + item.quantidade);
