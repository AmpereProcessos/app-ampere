...
const createForm: NextApiHandler<PostResponse> = async (req, res) => {
  ...
  const db: Db = await connectToDatabase(process.env.DB_KEY);
  const collection: Collection<TNewWarehouseFormulary> = db.collection("formularios");
  const form = WarehouseFormularySchema.parse(req.body);
  if (!form)
    throw new createHttpError.BadRequest("Informações para a criação formulário não fornecidas.");

  const insertResponse = await collection.insertOne(form);

  if (!insertResponse.acknowledged)
    throw new createHttpError.InternalServerError("Oops, houve um erro na inserção do formulário.");
...
const editForm: NextApiHandler<PutResponse> = async (req, res) => {
  ...
  const changes = WarehouseFormularySchema.partial().parse(req.body);
  ...
  if (!updateResponse.acknowledged)
    throw new createHttpError.InternalServerError(
      "Oops, houve um erro na atualização do formulário.",
    );

  return res.status(201).json({
    data: "Formulário atualizado com sucesso!",
