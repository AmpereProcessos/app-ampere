...
const GetEmployeesInputSchema = z.object({
  ...
  simplified: z
    .string({
      invalid_type_error: "Tipo não válido para filtro de simplificado.",
    })
    .optional()
    .nullable()
    .transform((val) => val === "true"),
...
export type TGetEmployeesDefaultInput = Omit<TGetEmployeesInput, "id" | "simplified">;
export type TGetEmployeesByIdInput = Pick<TGetEmployeesInput, "id">;
export type TGetEmployeesSimplifiedInput = Omit<TGetEmployeesInput, "id">;
...
async function getEmployees({
  ...
  if ("id" in input) {
    // For getting a specific employee by id, we must check if user has human resources access
  ...
  const accessActiveOnlyQuery: Filter<TEmployee> = input.accessActiveOnly
    ? { acessoAtivo: true }
    : {};
  ...
  if (input.simplified) {
    const colaborators = await usersCollection
      .find(query, {
        sort: { nome: 1 },
        projection: {
          nome: 1,
    ...
    return {
      data: {
        default: undefined,
        byId: undefined,
        simplified: colaborators.map((colaborator) => ({
          _id: colaborator._id.toString(),
          nome: colaborator.nome,
          email: colaborator.email,
          telefone: colaborator.telefone,
          avatar_url: colaborator.avatar_url,
          dataNascimento: colaborator.dataNascimento,
          dataInsercao: colaborator.dataInsercao,
        })),
      },
    };
  }

  // For getting all employees, we must check if user has human resources access
  if (!session?.user.permissoes.recursosHumanos.visualizar)
    throw new createHttpError.Unauthorized("Usuário não possui permissão para essa requisição.");

  const colaborators = await usersCollection.find(query, { sort: { nome: 1 } }).toArray();
  const colaboratorsFormatted = colaborators.map((colaborator) => ({
    ...colaborator,
    _id: colaborator._id.toString(),
  }));
  return {
...
export type TGetEmployeesOutputById = Exclude<
  Awaited<ReturnType<typeof getEmployees>>["data"]["byId"],
  undefined
>;
export type TGetEmployeesOutputDefault = Exclude<
  Awaited<ReturnType<typeof getEmployees>>["data"]["default"],
  undefined
>;
export type TGetEmployeesOutputSimplified = Exclude<
  Awaited<ReturnType<typeof getEmployees>>["data"]["simplified"],
  undefined
>;
...
async function createColaborator({
  ...
  if (!insertResponse.acknowledged)
    throw new createHttpError.InternalServerError("Oops, houve um erro ao inserir colaborador.");
...
async function editColaborator({
  session,
  input,
}: {
  session: TAuthSession;
  input: TEditColaboratorInput;
}) {
  if (!session?.user.permissoes.usuarios.editar)
    throw new createHttpError.Unauthorized("Usuário não possui permissão para essa requisição.");
  ...
  if (!updateResponse.acknowledged)
    throw new createHttpError.InternalServerError(
      "Oops, houve um erro desconhecido ao atualizar colaborador.",
    );
