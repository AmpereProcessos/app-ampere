...
const handlePurchaseControlAllocations: NextApiHandler<
  HandlePurchaseControlAllocationsResponse
> = async (req, res) => {
  ...
  if (
    !purchaseControlId ||
    typeof purchaseControlId !== "string" ||
    !ObjectId.isValid(purchaseControlId)
  )
  ...
  const purchaseControl = await purchaseControlCollection.findOne({
  ...
  const pendingAllocations = purchaseControl.composicao.filter(
    (item) => !!item.alocadorDestinoId && !!item.materialId && !item.dataAlocacao,
  );
  // If there are no pending allocations, return a success message
  if (pendingAllocations.length == 0)
    return res.status(200).json({ message: "Nenhuma alocação pendente." });
  ...
  try {
    await dbSession.withTransaction(async () => {
      const currentMaterials = await materialStockCollection
        .find({
          _id: { $in: pendingAllocations.map((item) => new ObjectId(item.materialId as string)) },
        })
        .toArray();
      ...
      for (const allocation of pendingAllocations) {
        ...
        const currentMaterial = currentMaterials.find(
          (item) => item._id.toString() == allocation.materialId,
        );
        ...
        if (!allocation.alocadorOrigemId) {
          ...
          const newPrice =
            (currentMaterial.qtde * currentMaterial.preco +
              allocation.qtde * (allocation.valor || 0)) /
            (currentMaterial.qtde + allocation.qtde);
          ...
          minimumNotificationChanges.push({
            materialId: currentMaterial._id.toString(),
            materialName: currentMaterial.nome,
            previousQuantity: currentMaterial.qtde,
            newQuantity: newQtde,
            minimumQuantity: currentMaterial.qtdeMinima,
          });
        } else {
          ...
          await materialStockCollection.updateOne(
            { _id: new ObjectId(allocation.materialId as string) },
            { $inc: { qtde: -allocation.qtde }, $set: { preco: currentMaterial.preco } },
          );
          minimumNotificationChanges.push({
            materialId: currentMaterial._id.toString(),
            materialName: currentMaterial.nome,
            previousQuantity: currentMaterial.qtde,
            newQuantity: currentMaterial.qtde - allocation.qtde,
            minimumQuantity: currentMaterial.qtdeMinima,
          });
        }
      }
