...
import { InsertFileReferenceSchema, type TFileReference, type TFileReferenceEntity } from "@/utils/schemas/crm/file-reference.schema";
...
const getFileReferences: NextApiHandler<GetResponse> = async (req, res) => {
	...
	const { opportunityId, clientId, analysisId, homologationId, callId } = req.query;
	...
	if (callId) {
		if (typeof callId !== "string" || !ObjectId.isValid(callId)) throw new createHttpError.BadRequest("ID de chamado inválido.");
		const fileReferences = await getFileReferencesByCallId({ collection: collection, callId: callId });
		return res.status(200).json({ data: fileReferences });
	}
