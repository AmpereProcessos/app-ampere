...
export default async function handler(req, res) {
	if (req.method == "POST") {
		try {
			...
			if (!sellerName || typeof sellerName != "string") throw new createHttpError.BadRequest("Nome do vendedor não informado.");
			if (!idCRMProject || typeof idCRMProject != "string") throw new createHttpError.BadRequest("Nome do vendedor não informado.");
			if (!message || typeof message != "string" || message.trim().length < 5) throw new createHttpError.BadRequest("Mensagem inválida. ");
			...
			const sellerInfo = allUsersInCRM.find((user) => {
				const similarityPercentage = calculateStringSimilarity(sellerName.toUpperCase(), user.nome.toUpperCase());
			...
			const crmProjectInfo = await projectsCollection
				.aggregate([{ $match: { _id: new ObjectId(idCRMProject) } }, { $project: { nome: 1, identificador: 1, email: 1 } }])
				.toArray();

			if (!crmProjectInfo[0]) throw new createHttpError.NotFound("Projeto no CRM não encontrado.");
			...
			const insertNotificationResponse = await notificationsCollection.insertOne(notificationObj);
			if (insertNotificationResponse.insertedId) res.status(200).json({ data: "Notificação criado com sucesso!" });
			else throw new createHttpError.InternalServerError("Houve um erro na criação da notificação.");
		} catch (error) {
