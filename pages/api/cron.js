...
export default async function handler(req, res) {
  ...
  const filteredCodes = signedContractCodes.filter((obj) => !!obj.codigoSVB || !isNaN(obj.codigoSVB))
  ...
  const dbResp = await collection2.updateMany({ codigoSVB: { $in: arr } }, { $set: { contratoAssinado: true, estagioFunil: 3 } })
