...
export default async function handler(req, res) {
	...
	try {
		...
		const value_cf_qual_o_valor_da_sua_conta_de_luz = cf_qual_o_valor_da_sua_conta_de_luz.find(
			(rdParam) => consumo > rdParam.parametros.min && consumo <= rdParam.parametros.max,
		);
		...
		const { data: rdResponse } = await axios.post(`https://api.rd.services/platform/conversions?api_key=${process.env.RD_API_KEY}`, {
			...
			payload: {
				...
				name: nome,
				cf_qual_o_valor_da_sua_conta_de_luz: value_cf_qual_o_valor_da_sua_conta_de_luz ? value_cf_qual_o_valor_da_sua_conta_de_luz.value : undefined,
				state: uf,
				city: cidade,
			},
		});
