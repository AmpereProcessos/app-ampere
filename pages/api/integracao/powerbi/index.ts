...
const exportData: NextApiHandler<GetResponse> = async (req, res) => {
  ...
  const result = data.map((project) => {
    ...
    return {
      ...
      'VALOR PREVISTO PRO KIT': project.compra.previsaoValorDoKit || 0,
      ...
      'STATUS DO PARECER DE ACESSO': project.homologacao.status,
      'DATA DE APROVAÇÃO DO PARECER': project.homologacao.acesso.dataResposta,
      'DESENHO DE TELHADO FEITO': project.homologacao.pendencias.desenhos ? 'SIM' : 'NÃO',
      'MAPA DE MICRO FEITO': project.homologacao.pendencias.mapasDeMicro ? 'SIM' : 'NÃO',
      'STATUS DA OBRA': project.obra.statusDaObra,
      'ENTRADA NA OBRA': project.obra.entrada ? formatDateAsLocale(project.obra.entrada) : null,
      'SAÍDA DA OBRA': project.obra.saida ? formatDateAsLocale(project.obra.saida) : null,
      'EQUIPE DE EXECUÇÃO': project.obra.equipeResp,
      'STATUS DA VISTORIA': !!project.homologacao.vistoria.dataEfetivacao ? 'FEITA' : 'PENDENTE',
      'DATA DE PEDIDO DA VISTORIA': project.vistoria.dataPedido ? formatDateAsLocale(project.vistoria.dataPedido) : null,
