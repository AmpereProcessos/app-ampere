...
const handleUpdateManyServiceOrders: NextApiHandler<any> = async (req, res) => {
  ...
  if (!session.user.permissoes.ordensDeServico.editar)
    throw new createHttpError.Unauthorized(
      "Usuário não possui permissão para editar ordens de serviço.",
    );
  ...
  if (projectId) {
    ...
    return res
      .status(201)
      .json({
        data: "Ordens de serviço atualizadas com sucesso.",
        message: "Ordens de serviço atualizadas com sucesso.",
      });
  }
  throw new createHttpError.BadRequest(
    "Operação inválida. Por favor, especifique o ID do projeto.",
  );
};
