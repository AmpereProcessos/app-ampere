...
import { TProposal } from '@/utils/schemas/crm/proposal.schema'

function getContractValue(valorProjeto: number, valorPadrao: number, valorEstrutura: number) {
  var totalSum = 0
...
const getComercialAnalyticalData: NextApiHandler<GetResponse> = async (req, res) => {
  ...
  const appDb: Db = await connectToDatabase(process.env.DB_KEY, 'projetos')
  ...
  const analytical = appProjects.map((project) => {
    ...
    return {
      ...
      valorContrato: getContractValue(project.sistema?.valorProjeto, project.padrao?.valor || 0, project.estruturaPersonalizada?.valor || 0),
      proposta: {
