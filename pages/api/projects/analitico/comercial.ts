...
const QuerySchema = z.object({
  after: z.string({
    required_error: "Parâmetro de data não informado.",
    invalid_type_error: "Tipo não válido para parâmetro de data.",
  }),
  before: z.string({
    required_error: "Parâmetro de data não informado.",
    invalid_type_error: "Tipo não válido para parâmetro de data.",
  ...
  field: z.string({
    required_error: "Campo de filtro não informado.",
    invalid_type_error: "Tipo não válido para o campo de filtro.",
  }),
});
type GetResponse = {
...
const GetComercialAnalyticsDataInputSchema = z.object({
  search: z.string({
    required_error: "Parâmetro de busca não informado.",
    invalid_type_error: "Tipo não válido para parâmetro de busca.",
  }),
  periodField: z.enum(["contrato.dataAssinatura", "contrato.dataSolicitacao"]),
  periodAfter: z
    .string({
      required_error: "Parâmetro de data não informado.",
      invalid_type_error: "Tipo não válido para parâmetro de data.",
  ...
  sellerIds: z
    .string({
      required_error: "Parâmetro de IDs de vendedores não informado.",
      invalid_type_error: "Tipo não válido para parâmetro de IDs de vendedores.",
    })
    .transform((val) => val.split(",").filter((v) => !!v)),
  pendingVinculation: z
    .string({
      required_error: "Parâmetro de vinculação pendente não informado.",
      invalid_type_error: "Tipo não válido para parâmetro de vinculação pendente.",
    })
    .transform((val) => val === "true"),
});
export type TGetComercialAnalyticsDataInput = z.infer<typeof GetComercialAnalyticsDataInputSchema>;

const getComercialAnalyticalData = async (filters: TGetComercialAnalyticsDataInput) => {
  console.log("[INFO][GET COMERCIAL ANALYTICAL DATA] Filters:", filters);
  ...
  const appProjects = await getProjects({
    collection: appProjectsCollection,
    filters,
  });
  const crmOpportunities = await getOpportunities({
    collection: crmOpportunitiesCollection,
    filters,
  });

  const analytical = appProjects
    .map((project) => {
    ...
    .filter((p) => (filters.pendingVinculation ? !p.proposta.id : true));

  const totalInApp = analytical.reduce((acc, curr) => acc + curr.valorContrato, 0);
  const totalInCRM = analytical.reduce((acc, curr) => acc + curr.proposta.valor, 0);
  console.log("[INFO][GET COMERCIAL ANALYTICAL DATA] Total in App:", totalInApp);
  console.log("[INFO][GET COMERCIAL ANALYTICAL DATA] Total in CRM:", totalInCRM);
  return analytical;
};

const getComercialAnalyticalDataHandler: NextApiHandler<TGetComercialAnalyticalDataOutput> = async (
  req,
  res,
) => {
  const session = await validateAuthenticationWithSession(req, res);
  const filters = GetComercialAnalyticsDataInputSchema.parse(req.query);
  const data = await getComercialAnalyticalData(filters);
  return res.status(200).json(data);
};
export type TGetComercialAnalyticalDataOutput = Awaited<
  ReturnType<typeof getComercialAnalyticalData>
>;
export default apiHandler({ GET: getComercialAnalyticalDataHandler });

type GetProjectsProps = {
  collection: Collection<TProject>;
  filters: TGetComercialAnalyticsDataInput;
};
async function getProjects({ collection, filters }: GetProjectsProps) {
  const { search, periodField, periodAfter, periodBefore, sellerIds } = filters;

  const searchMatch: Filter<TProject> =
    search.trim().length > 0
      ? {
          $or: [
            { nomeDoContrato: { $regex: search, $options: "i" } },
            { codigoSVB: { $regex: search, $options: "i" } },
            { telefone: { $regex: search, $options: "i" } },
            { cpfCnpj: { $regex: search, $options: "i" } },
          ],
        }
      : {};
  const periodMatch: Filter<TProject> = {
    $and: [{ [periodField]: { $gte: periodAfter } }, { [periodField]: { $lte: periodBefore } }],
  };
  const sellersMatch: Filter<TProject> =
    sellerIds.length > 0 ? { "vendedor.idCRM": { $in: sellerIds } } : {};

  const finalMatch: Filter<TProject> = {
...
async function getOpportunities({ collection, filters }: GetOpportunitiesProps) {
  const { periodAfter, periodBefore, sellerIds } = filters;

  try {
    const periodMatch = {
      $and: [{ "ganho.data": { $gte: periodAfter } }, { "ganho.data": { $lte: periodBefore } }],
    };
    const sellersMatch = sellerIds.length > 0 ? { "responsaveis.id": { $in: sellerIds } } : {};
    const finalMatch = {
      ...periodMatch,
      ...sellersMatch,
    };
