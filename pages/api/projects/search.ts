...
import { PersonalizedFiltersSchema, ProjectDBSimplifiedProjection, type TProject, type TProjectDTODBSimplified } from "@/utils/schemas/projects";
...
const getProjectsByPersonalizedFilters: NextApiHandler<PostResponse> = async (req, res) => {
	...
	const payerNameQuery: Filter<TProject> | null =
		filters.payerName.trim().length > 0 ? { $or: [{ "pagamento.pagador": { $regex: filters.payerName, $options: "i" } }, { "pagamento.pagador": filters.payerName }] } : null;
	const dateQuery: Filter<TProject> | null =
		filters.period.after && filters.period.before && filters.period.field
			...
			: null;
	const tagsQuery: Filter<TProject> | null = filters.tagIds.length > 0 ? { "etiquetas.id": { $in: filters.tagIds } } : null;
	...
	const neighborhoodQuery: Filter<TProject> | null =
		filters.neighborhood.trim().length > 0 ? { $or: [{ bairro: { $regex: filters.neighborhood, $options: "i" } }, { bairro: filters.neighborhood }] } : null;
	...
	const acquisitionChannelQuery: Filter<TProject> | null = filters.acquisitionChannel.length > 0 ? { canalVenda: { $in: filters.acquisitionChannel } } : null;
	const modulesQtyQuery: Filter<TProject> | null =
		filters.modulesQty.greater != null && filters.modulesQty.less != null ? { "sistema.qtdeModulos": { $gte: filters.modulesQty.greater, $lte: filters.modulesQty.less } } : null;
...
async function getProjectsByFilter({ collection, query, skip, limit }: GetProjectsByFilterParams) {
	try {
		...
		const sort = { qtde: -1 };
		const match = { ...query };
		const projects = await collection.aggregate([{ $sort: sort }, { $match: match }, { $skip: skip }, { $project: ProjectDBSimplifiedProjection }, { $limit: limit }]).toArray();
