...
const getProjectsByPersonalizedFilters: NextApiHandler<PostResponse> = async (req, res) => {
  ...
  const nameQuery: Filter<TProject> | null =
    filters.name.trim().length > 0
      ? {
          $or: [
            { nomeDoContrato: { $regex: filters.name, $options: "i" } },
            { nomeDoContrato: filters.name },
          ],
        }
      : null;
  const payerNameQuery: Filter<TProject> | null =
    filters.payerName.trim().length > 0
      ? {
          $or: [
            { "pagamento.pagador": { $regex: filters.payerName, $options: "i" } },
            { "pagamento.pagador": filters.payerName },
          ],
        }
      : null;
    ...
    filters.period.after && filters.period.before && filters.period.field
      ? {
          [filters.period.field]: {
            $gte: formatDateQuery(filters.period.after, "start"),
            $lte: formatDateQuery(filters.period.before, "end"),
          },
        }
      : null;
  const tagsQuery: Filter<TProject> | null =
    filters.tagIds.length > 0 ? { "etiquetas.id": { $in: filters.tagIds } } : null;
  const stateQuery: Filter<TProject> | null =
    filters.state.length > 0 ? { uf: { $in: filters.state } } : null;
  const cityQuery: Filter<TProject> | null =
    filters.city.length > 0 ? { cidade: { $in: filters.city } } : null;
  const neighborhoodQuery: Filter<TProject> | null =
    filters.neighborhood.trim().length > 0
      ? {
          $or: [
            { bairro: { $regex: filters.neighborhood, $options: "i" } },
  ...
  const addressQuery: Filter<TProject> | null =
    filters.address.trim().length > 0
      ? {
          $or: [
            { logradouro: { $regex: filters.address, $options: "i" } },
            { logradouro: filters.address },
          ],
        }
      : null;
  const serviceTypeQuery: Filter<TProject> | null =
    filters.serviceType.length > 0 ? { tipoDeServico: { $in: filters.serviceType } } : null;
  const sellerQuery: Filter<TProject> | null =
    filters.seller.length > 0 ? { "vendedor.idCRM": { $in: filters.seller } } : null;
  ...
  const modulesQtyQuery: Filter<TProject> | null =
    filters.modulesQty.greater != null && filters.modulesQty.less != null
      ? {
          "sistema.qtdeModulos": {
            $gte: filters.modulesQty.greater,
            $lte: filters.modulesQty.less,
          },
        }
      : null;
  ...
  const { projects, projectsMatched } = await getProjectsByFilter({
    collection,
    query,
    skip,
    limit,
...
async function getProjectsByFilter({ collection, query, skip, limit }: GetProjectsByFilterParams) {
  try {
    ...
    const projects = await collection
      ...
      .toArray();
    return { projects, projectsMatched } as {
      projects: WithId<TProjectDTODBSimplified>[];
      projectsMatched: number;
    };
  } catch (error) {
    throw error;
