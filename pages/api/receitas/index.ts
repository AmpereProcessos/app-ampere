...
import {
  GeneralRevenueSchema,
  type TRevenue,
  type TRevenueWithProjectDTO,
...
const getRevenues: NextApiHandler<GetResponse> = async (req, res) => {
  ...
  if (projectId) {
    ...
    const revenues = await collection
      .find({ "projeto.id": projectId }, { sort: { "efetivacao.data": -1, dataInsercao: 1 } })
      .toArray();
    return res.status(200).json({ data: revenues });
  }
  // Query for all revenues
  const revenues = await collection
    .find({}, { sort: { "efetivacao.data": -1, dataInsercao: 1 } })
    .toArray();
  return res.status(200).json({ data: revenues });
};
...
const createRevenue: NextApiHandler<PostResponse> = async (req, res) => {
  ...
  if (revenueProjectId) {
    ...
    const projectResponse = await projectsCollection.findOne({
      _id: new ObjectId(revenueProjectId),
    });
    ...
    if (!projectResponse) {
      ...
      if (projectResponse?.idClienteCRM) {
        console.log("[INFO] [CREATE REVENUE] Fetching client data.", {
          clientCRMId: projectResponse.idClienteCRM,
        });
        const clientResponse = await clientsCollection.findOne({
          _id: new ObjectId(projectResponse.idClienteCRM),
        });
        client = clientResponse;

        if (!clientResponse) {
          console.warn("[WARN] [CREATE REVENUE] Client not found.", {
            clientCRMId: projectResponse.idClienteCRM,
          });
        } else {
          console.log("[INFO] [CREATE REVENUE] Revenue attached to client.", {
            ...
            existingContaAzulCliente:
              (clientResponse as TClient & { idContaAzulCliente?: string }).idContaAzulCliente ||
              null,
          });
        }
      } else {
        console.log(
          "[INFO] [CREATE REVENUE] Project has no CRM client attached, skipping client fetch.",
        );
      }
    }
  } else {
    console.log(
      "[INFO] [CREATE REVENUE] Revenue has no project attached, will skip Conta Azul integration.",
    );
  }
  ...
  // 	}
  // }
  const revenueReceivedCompletely =
    revenue.fracionamento.length > 0
      ? revenue.fracionamento.every((f) => !!f.dataRecebimento)
      : false;
...
const editRevenue: NextApiHandler<PutResponse> = async (req, res) => {
  ...
  if (!updateResponse.acknowledged)
    throw new createHttpError.InternalServerError("Oops, houve um erro ao atualizar receita.");
  ...
  const revenueReceivedCompletely =
    revenueUpdated.fracionamento.length > 0
      ? revenueUpdated.fracionamento.every((f) => !!f.dataRecebimento)
      : false;
  if (revenueReceivedCompletely) {
    await collection.updateOne(
      { _id: new ObjectId(id) },
      { $set: { dataEfetivacao: new Date().toISOString() } },
    );
  }

  return res
    .status(201)
    .json({ data: "Receita atualizada com sucesso!", message: "Receita atualizada com sucesso!" });
};
