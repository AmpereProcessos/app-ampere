...
import connectToVendedoresDatabase from "../../../utils/services/mongodb/auxiliaries";
...
export default async function handler(req, res) {
	if (req.method === "GET") {
		...
		const filtered = result.filter((x) => {
			const sellerInfo = vendedores.find((seller) => seller.nome == x._id.vendedor && seller.ativo == true);
		...
		var newObj = await collection.updateOne({ _id: ObjectId(id) }, { $set: { [ano]: novasMetas } });
