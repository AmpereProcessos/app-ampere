...
async function getSDRRanking(params: TGetSDRRankingInput) {
		...
		.find(
			{
				$or: [
					{
						$and: [{ "responsaveis.dataInsercao": { $gte: startDate.toISOString() } }, { "responsaveis.dataInsercao": { $lte: endDate.toISOString() } }],
					},
	...
	const ranking = Object.entries(reducedOpportunities)
		.sort((a, b) => {
			...
			if (params.rankBy === "opportunities-created-qty") return createdB - createdA;
			if (params.rankBy === "opportunities-won-qty") return wonB - wonA;
			if (params.rankBy === "opportunities-send-qty") return sendB - sendA;
		...
		.map(([sdrId, { created, won, send }], index) => {
			...
			const value = valueMap[params.rankBy];
...
const getSDRRankingHandler: NextApiHandler<TGetSDRRankingOutput> = async (req, res) => {
	await validateAuthenticationWithSession(req, res);
	const { type, rankBy } = req.query;

	const params = GetSDRRankingQueryParams.parse({
		type: type as unknown,
		rankBy: rankBy as unknown,
	});

	const data = await getSDRRanking(params);
