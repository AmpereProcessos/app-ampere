...
export default async function handler(req, res) {
	if (req.method == "POST") {
		...
		const { projectId, contractName, email, phone_number, documentation } = req.body;
		const formattedFileName = contractName ? contractName.split(" ").join("_") : "CONTRATO";
		try {
			...
			if (typeof filePath === "string") {
				...
				await collection.updateOne({ _id: ObjectId(projectId) }, { $set: { assinaturaDigital: digitalSignObject } });
				...
				const { data: sendEmailClientResponse } = await axios.post(
					...
					{
						request_signature_key: addClientSignerResponse.list.request_signature_key,
						message: "Prezado,\nPor favor assine o documento.\n\nQualquer dúvida estou à disposição.\n\nAtenciosamente,\nVolts",
					},
				);
