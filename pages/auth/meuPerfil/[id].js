				...
				<div className="grid-rows-10 grid w-full grid-cols-1 gap-y-4 p-6 lg:grid-cols-10 lg:grid-rows-1  lg:gap-2 xl:w-[70%]">
					<div className="row-span-1 flex min-h-[350px] w-full items-center justify-center rounded-md border border-gray-300 p-4 shadow-lg lg:col-span-2">
						<div className="flex flex-col items-center">
								...
								{!newImage && info.avatar_url ? (
									<>
										...
										<div className="absolute flex h-[150px] w-[150px] items-center justify-center rounded-full bg-gray-400 opacity-0 hover:opacity-60">
											<input onChange={(e) => setNewImage(e.target.files[0])} className="h-full w-full opacity-0" type="file" accept="image/png, image/jpeg" />
							...
							{newImage ? (
								<button onClick={saveChanges} className="mt-4 rounded-lg border border-blue-500 p-2 font-bold text-blue-500 hover:bg-blue-500 hover:text-white">
									Salvar
								</button>
							) : null}
						</div>
					...
					<div className="row-span-9 flex min-h-[350px] flex-col rounded-md border border-gray-300 p-4 shadow-lg lg:col-span-8">
						...
						<div className="flex grow flex-col gap-4 pt-2">
							<div className="grid w-full grid-cols-1 grid-rows-2 items-center gap-2 border-b border-gray-300 py-2 lg:grid-cols-10 lg:grid-rows-1">
							...
							<div className="grid w-full grid-cols-1 grid-rows-2 items-center gap-2 border-b border-gray-300 py-2 lg:grid-cols-10 lg:grid-rows-1">
								...
								<a href={info.avatar_url} className="cursor-pointer break-words text-center text-gray-500 hover:text-blue-300 lg:col-span-7 lg:text-start">
							...
							<div className="grid w-full grid-cols-1 grid-rows-2 items-center gap-2 border-b border-gray-300 py-2 lg:grid-cols-10 lg:grid-rows-1">
								...
								<p className="text-center text-gray-500 lg:col-span-7 lg:text-start">{info.birthday ? dayjs(info.birthday).add(4, "hours").format("DD/MM/YYYY") : "-"}</p>
							</div>
							<div className="grid w-full grid-cols-1 grid-rows-2 items-center gap-2 border-b border-gray-300 py-2 lg:grid-cols-10 lg:grid-rows-1">
								...
								not.length > 0 ? (
									not.map((notificacao, index) => (
										<div
											...
											className={` ${
												notificacao.lido
													? "flex max-w-full flex-col border-b border-gray-300 bg-green-100 p-1 hover:bg-blue-100"
													: "flex max-w-full flex-col border-b border-gray-300 p-1 hover:bg-blue-100"
											}`}
										>
											<h1 className="text-center text-sm font-bold italic text-gray-600">
												<strong className="text-[#15599a]">{notificacao.remetente}</strong> diz {notificacao.projetoReferencia ? `sobre o projeto ${notificacao.projetoReferencia}` : ""}{" "}
											...
											<div className="flex items-center justify-end gap-2 pr-4">
												...
												{notificacao.lido ? (
													...
													<button onClick={() => setAsRead(notificacao._id, index)} className="outline-none transition duration-300 ease-in-out hover:scale-150">
														<BsCheck
															style={{
																fontSize: "20px",
																color: "gray",
