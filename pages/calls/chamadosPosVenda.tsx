...
function PosVendaCalls({ session }: PosVendaCallsProps) {
	...
	return (
		<div className="flex grow flex-col p-6">
			...
			{filterMenuIsOpen ? <PosVendaCallsFilterMenu queryParams={filters} updateQueryParams={updateFilters} closeMenu={() => setFilterMenuIsOpen(false)} /> : null}
		</div>
	);
}
		...
		<div className="flex w-full flex-col gap-1 rounded-md border border-primary bg-[#fff] p-2 dark:bg-[#121212]">
			<div className="w-full flex flex-col gap-2">
				<div className="w-full flex items-center gap-1">
					...
					<h1 className="text-sm tracking-tight font-bold">ESTATÍSTICAS</h1>
				</div>
			...
			<div className="w-full flex flex-col items-center justify-center gap-2 lg:flex-row">
				<div className="flex w-full flex-col rounded-xl border border-primary/30 bg-[#fff] dark:bg-[#121212] p-3 shadow-sm lg:w-1/3">
		...
		<div className="flex items-center justify-center lg:justify-end flex-wrap gap-2">
			...
			{filters.search && filters.search.length > 0 ? (
				<div className="flex items-center gap-1 bg-secondary text-[0.65rem] rounded-lg px-2 py-1">
					...
					<button type="button" onClick={() => updateFilters({ search: null })} className="bg-transparent text-primary hover:bg-primary/20 rounded-lg p-1">
						<X size={12} />
					</button>
				</div>
			) : null}
			{filters.ongoingOnly ? (
				<div className="flex items-center gap-1 bg-secondary text-[0.65rem] rounded-lg px-2 py-1">
					...
					<button type="button" onClick={() => updateFilters({ ongoingOnly: null })} className="bg-transparent text-primary hover:bg-primary/20 rounded-lg p-1">
						<X size={12} />
					</button>
				</div>
			) : null}
