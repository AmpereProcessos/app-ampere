...
function ChamadosProjetos() {
  ...
  return <ChamadosProjetosContent session={session} />
}

export default ChamadosProjetos

function ChamadosProjetosContent({ session }: { session: TAuthSession }) {
  // Utils
            ...
            filteredClosedCalls.map((call) => (
              ...
              >
                <div className="grid w-full grid-cols-3 items-center gap-x-2">
                  ...
                  <p
                    className={`col-span-1 rounded-lg border p-1 text-xs font-bold ${statusStyles[call.status].textColor} ${statusStyles[call.status].borderColor}`}
                  >
                    {call.status}
                  </p>
                </div>
