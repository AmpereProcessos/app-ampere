...
function ChamadosSuporte() {
	...
	function getCalls() {
		axios.get(`/api/chamados/suporte/mainData?closedAfter=${closedCallsFilters.afterDateFilter}&closedBefore=${closedCallsFilters.beforeDateFilter}`).then((res) => {
	...
	function filterInProgressCalls() {
		...
		if (inProgressCallsFilters.statusFilter.length > 0 && inProgressCallsFilters.respFilter.length > 0) {
			newArr = inProgress.filter((call) => inProgressCallsFilters.respFilter.includes(call.responsavel) && inProgressCallsFilters.statusFilter.includes(call.statusChamado));
	...
	if (status == "authenticated") {
		return (
			<div className="flex w-full grow flex-col gap-y-2 bg-gray-100 p-6">
					...
					<div className="overscroll-y mt-2 flex grow flex-wrap justify-around gap-2 overflow-y-auto scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300">
						{filteredClosedCalls ? (
							filteredClosedCalls.map((call) => (
								<div onClick={() => handleOpenModal(call._id)} key={call._id} className="max-h-[180px] w-[370px] cursor-pointer border border-gray-300 p-3 hover:bg-blue-100">
									<div className="flex w-full items-center justify-between gap-2">
										...
										<p className={`rounded-lg border p-1 text-xs font-bold ${statusStyles[call.statusChamado].textColor} ${statusStyles[call.statusChamado].borderColor}`}>
											{call.statusChamado}
										</p>
									</div>
									<div className="mt-2 flex w-full items-center justify-between">
				...
				<div onClick={() => setCreationModal(true)} className="left-150 fixed bottom-10 cursor-pointer rounded-lg bg-[#15599a] p-3 text-white hover:bg-[#fead61] hover:text-[#15599a]">
				...
				{modalIsOpen && <ModalCallSuporte modalIsOpen={modalIsOpen} session={session} updateModalInfo={updateModalInfo} setModalIsOpen={setModalIsOpen} info={modalCall} />}
			</div>
		);
	}
}
