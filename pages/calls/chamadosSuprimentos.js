	...
	if (status == "authenticated") {
		return (
			<div className="flex w-full grow flex-col gap-y-2 bg-gray-100 p-6">
						...
						{openCalls.filtered ? (
							openCalls.filtered?.length > 0 ? (
								openCalls.filtered.map((chamado) => (
									<div key={chamado._id} onClick={() => handleOpenModal(chamado)} className={`h-[200px] w-[420px] cursor-pointer border border-gray-300 p-3 hover:bg-blue-100`}>
										<div className="flex w-full items-center justify-between gap-3">
											<h1 className="text-base font-bold uppercase">
												<strong className="text-[#15599a]">{chamado.codigoProjeto}</strong> {chamado.nomeDoContrato}
											</h1>
											<p className={`border-2 text-center text-xs font-bold ${statusStyles[chamado.status].text} ${statusStyles[chamado.status].border} rounded-lg p-1`}>{chamado.status}</p>
										</div>
										...
										<div className="mt-3 flex justify-between">
											<div className="flex flex-col items-center">
												...
												<p className="text-xs font-bold text-red-500">{chamado.dataEntrega ? dayjs(chamado.dataEntrega).format("DD/MM/YYYY") : "-"}</p>
											</div>
											<div className="flex flex-col items-center">
												<p className="text-xs font-bold text-gray-500">DIAS DESDE ENTREGA</p>
												<p className="text-xs font-bold text-gray-500">{chamado.dataEntrega ? dayjs().diff(new Date(chamado.dataEntrega), "days") : "-"}</p>
											</div>
										</div>
									</div>
								))
						...
						{closedCalls.filtered ? (
							closedCalls.filtered?.length > 0 ? (
								closedCalls.filtered.map((chamado) => (
									<div onClick={() => handleOpenModal(chamado)} key={chamado._id} className={`h-[200px] w-[420px] cursor-pointer border border-gray-300 p-3 hover:bg-blue-100`}>
										<div className="flex w-full items-center justify-between gap-3">
											<h1 className="text-base font-bold uppercase">
												<strong className="text-[#15599a]">{chamado.codigoProjeto}</strong> {chamado.nomeDoContrato}
											</h1>
										...
										<div className="mt-3 flex justify-between">
											<p className="text-xs font-bold text-gray-500">DATA DE ENTREGA</p>
											<p className="text-xs font-bold text-[#15599a]">{chamado.dataEntrega ? dayjs(chamado.dataEntrega).format("DD/MM/YYYY") : "-"}</p>
										</div>
										<div className="mt-3 flex justify-between">
											<p className="text-xs font-bold text-gray-500">AVARIAS</p>
											<p className="text-xs font-bold text-red-500">{chamado.avarias ? "SIM" : "NÃO"}</p>
				...
				{modalIsOpen && <ModalCallSuprimentos info={modalCall} modalIsOpen={modalIsOpen} setModalIsOpen={() => setModalIsOpen(false)} getCalls={getCalls} />}
			</div>
		);
	}
}
