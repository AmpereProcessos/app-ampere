...
import {
  useGetAllPosVendaCalls,
  useGetPosVendaCallsStatistics,
} from "@/utils/methods/query/pos-venda-calls";
...
function PosVendaCalls({ session }: PosVendaCallsProps) {
  ...
  const [editCallModal, setEditCallModal] = useState<{ id: string | null; isOpen: boolean }>({
    id: null,
    isOpen: false,
  });
  const {
  ...
  const { mutate: handleUpdateCallStatus } = useMutation({
    ...
    onMutate: async (variables) => {
      ...
      const querySnapshot = queryClient.getQueryData(["pos-venda-calls", filters]) as
        | TPosVendaCallDTO[]
        | undefined;

      if (!querySnapshot) return { querySnapshot };
      const update = querySnapshot.map((prev) =>
        prev._id === variables.id ? { ...prev, status: variables.call.status || "PENDENTE" } : prev,
      );
  ...
  function onDragEnd(dragEndResult: DropResult) {
    function handleEffectivationUpdate(
      newValue: TPosVendaCall["status"],
      previousData: TPosVendaCall,
    ) {
    ...
    handleUpdateCallStatus({
      ...
      call: {
        ...
        dataEfetivacao: handleEffectivationUpdate(
          destination.droppableId as TPosVendaCall["status"],
          call,
        ),
      },
    });
  ...
  const handleOnSettled = async () =>
    await queryClient.invalidateQueries({ queryKey: ["pos-venda-calls", filters] });
  return (
    <div className="flex grow flex-col p-6">
      <div className="border-border flex flex-col items-center border-b px-1 py-2">
        <div className="flex w-full items-center justify-between">
          <div className="flex flex-col items-center gap-2 lg:flex-row">
            <p className="text-center text-2xl font-black text-[#15599a] uppercase">
              CHAMADOS DE POS VENDA
            </p>
          </div>
  ...
  return (
    <div className="border-primary bg-background flex w-full flex-col gap-1 rounded-md border p-2 dark:bg-[#121212]">
      <div className="flex w-full flex-col gap-2">
        ...
        <h3 className="text-muted-foreground w-full text-sm tracking-tight">
          Estatísticas de chamados de pos venda
        </h3>
      </div>
      ...
      <div className="flex w-full flex-col items-center justify-center gap-2 lg:flex-row">
        <div className="border-border bg-background flex w-full flex-col rounded-xl border p-3 shadow-xs lg:w-1/3 dark:bg-[#121212]">
        ...
        <div className="border-border bg-background flex w-full flex-col rounded-xl border p-3 shadow-xs lg:w-1/3 dark:bg-[#121212]">
          ...
          <div className="mt-2 flex w-full flex-col">
            <div className="text-xl font-bold text-[#15599a]">
              {formatToMoney(statistics?.valorTotal || 0)}
            </div>
          </div>
        </div>
      </div>
...
function FiltersShowcase({ filters, updateFilters }: FiltersShowcaseProps) {
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-2 lg:justify-end">
      {filters.periodAfter && filters.periodBefore && filters.periodField ? (
        <div className="bg-secondary flex items-center gap-1 rounded-lg px-2 py-1 text-[0.65rem]">
          <p className="text-foreground">
            PERÍODO:{" "}
            <strong>
              {PeriodMap[filters.periodField]} {formatDateAsLocale(filters.periodAfter)} a{" "}
              {formatDateAsLocale(filters.periodBefore)}
            </strong>
          </p>
          <button
            type="button"
            onClick={() =>
              updateFilters({ periodAfter: null, periodBefore: null, periodField: null })
            }
      ...
      {filters.ongoingOnly ? (
        <div className="bg-secondary flex items-center gap-1 rounded-lg px-2 py-1 text-[0.65rem]">
          <p className="text-foreground">APENAS CHAMADOS EM ANDAMENTO</p>
          <button
            type="button"
      ...
      {(provided) => (
        ...
        >
          ...
          <div className="bg-primary/10 flex w-full items-center justify-center rounded p-2">
            <h1 className="text-[0.6rem] font-medium whitespace-pre-line">
              {item.descricao || "DESCRIÇÃO NÃO DEFINIDA"}
            </h1>
          </div>
          <div className="flex w-full flex-wrap items-center justify-center">
            <div
              className={cn(
                ...
                {
                  "bg-primary/20 text-foreground": !item.metadados.cobravel,
            ...
            >
              ...
              <p className={cn("text-[0.57rem] font-medium")}>
                {item.metadados.cobravel ? "CHAMADO COBRÁVEL" : "CHAMADO NÃO COBRÁVEL"}
              </p>
            </div>
          </div>
          <div className="flex w-full items-center justify-between gap-2">
