...
export default function ChatsTesting() {
  ...
  const {
    ...
    isError,
    isSuccess,
  } = useConvexQuery(api.queries.connections.getWhatsappConnection);

  return (
    <div className="flex flex-col gap-6 grow p-6">
      <div className="border-border flex items-center justify-between border-b p-1">
        <h1 className="text-start text-2xl font-black text-[#15599a] uppercase">Chats</h1>
      </div>
      {isPending ? (
        <h3 className="text-sm text-foreground animate-pulse py-4">Carregando conexão...</h3>
      ) : null}
...
type WhatsappConnectionBlockConnectedProps = {
  whatsappConnection: Exclude<
    typeof api.queries.connections.getWhatsappConnection._returnType,
    null
  >;
};
function WhatsappConnectionBlockConnected({
  whatsappConnection,
}: WhatsappConnectionBlockConnectedProps) {
  ...
  const disconnectWhatsappConnectionMutation = useMutation(
    api.mutations.connections.disconnectWhatsappConnection,
  );
  return (
    <div className="flex w-full flex-col gap-2 py-2">
      <div className="w-full flex items-center justify-between gap-2 flex-col lg:flex-row">
        ...
        <LoadingButton
          ...
          loading={false}
          onClick={() =>
            disconnectWhatsappConnectionMutation({ whatsappConnectionId: whatsappConnection._id })
          }
        >
          DESCONECTAR
        </LoadingButton>
      ...
      <div className="w-full flex flex-col gap-1.5">
        <p className="text-sm text-foreground">Detalhes da sua Conexão:</p>
        <div className="w-full flex flex-col gap-3">
          <div className="flex items-start lg:items-center gap-x-2 gap-y-1 flex-col lg:flex-row">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-foreground">ID da conta do WhatsApp Business:</p>
            </div>
          ...
          <div className="flex items-start lg:items-center gap-x-2 gap-y-1 flex-col lg:flex-row">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-foreground">Data de expiração do token:</p>
            </div>
            <p className="text-sm font-bold">
              {formatDateAsLocale(new Date(whatsappConnection.dataExpiracao), true) || "N/A"}
            </p>
          </div>
          <div className="flex items-start lg:items-center gap-x-2 gap-y-1 flex-col lg:flex-row">
            <div className="flex items-center gap-2">
              <Key className="w-4 h-4 min-w-4 min-h-4" />
              <p className="text-sm text-foreground">Permissões que você concedeu:</p>
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {whatsappConnection.metaEscopo.map((scope) => (
                <Badge
                  key={scope}
                  className="text-xs text-foreground bg-primary/10 rounded-md px-2 py-1"
                >
          ...
          <div className="flex items-start lg:items-center gap-x-2 gap-y-1 flex-col lg:flex-row">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-foreground">Telefones conectados:</p>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
  ...
  return (
    <div className="flex w-full flex-col gap-2 py-2">
      <p className="text-sm text-foreground">
        Oops, parece que você não está conectado ao WhatsApp Business.
      </p>
