	...
	return (
		<div className="flex grow flex-col gap-2 p-6">
			<div className="flex flex-col items-center justify-between border-b border-gray-200 p-1">
				...
				{filterMenuIsOpen ? (
					<ContractRequestsFilterMenu filters={filters} updateFilters={updateFilters} queryLoading={isLoading} resetSelectedPage={() => updateFilters({ page: 1 })} />
				) : null}
			</div>
			<GeneralPaginationComponent
				...
				itemsMatchedText={contractRequestsMatched > 1 ? `${contractRequestsMatched} solicitações encontradas.` : `${contractRequestsMatched} solicitação encontrada.`}
				itemsShowingText={contractRequestsShowing > 1 ? `Mostrando ${contractRequestsShowing} solicitações.` : `Mostrando ${contractRequestsShowing} solicitação.`}
			/>
			...
			<div className="flex w-full flex-wrap items-center justify-between gap-2">
				...
				{isSuccess ? (
					contractRequests && contractRequests.length > 0 ? (
						contractRequests.map((request) => <RequestCard key={request._id} request={request} openModal={(id) => setEditContractRequestModal({ id: id, isOpen: true })} />)
					) : (
			...
			{editContractRequestModal.id && editContractRequestModal.isOpen ? (
				<ContractRequestControlModal session={session} requestId={editContractRequestModal.id} closeModal={() => setEditContractRequestModal({ id: null, isOpen: false })} />
			) : null}
		</div>
	);
}
