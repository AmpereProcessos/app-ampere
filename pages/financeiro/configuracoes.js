...
function Configuracoes() {
	...
	const { data: apportionments, isFetching: apportionmentsFetching, isSuccess: apportionmentsSuccess } = useCostApportionments(validateAuthorization(session, "ADM"));
	const { data: receiptAccounts, isFetching: receiptAccountsFetching, isSuccess: receiptAccountsSuccess } = useReceiptAccounts(validateAuthorization(session, "ADM"));
	...
	return (
		<div className="flex grow flex-col gap-2 p-6">
				...
				<div className="flex h-[500px] max-h-[500px]  w-[50%] flex-col rounded border border-gray-300 shadow-lg">
					...
					<div className="overscroll-y flex grow flex-col gap-3 overflow-y-auto px-2 py-2 scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300">
						...
						{apportionmentsSuccess
							? apportionments?.map((apportionment, index) => <ApportionmentItem key={index} apportionment={apportionment} openEditModal={handleOpenEditApportionment} />)
							: null}
					</div>
				</div>
				<div className="flex h-[500px] max-h-[500px]  w-[50%] flex-col  rounded border border-gray-300 shadow-lg">
					...
					<div className="overscroll-y flex grow flex-col gap-3 overflow-y-auto scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300">
						...
						{receiptAccountsSuccess ? receiptAccounts?.map((account, index) => <AccountItem key={index} account={account} openEditModal={handleOpenEditAccount} />) : null}
					</div>
				</div>
			</div>
			...
			{editAccount.id && editAccount.isOpen ? <EditReceiptAccount receiptAccountId={editAccount.id} closeModal={() => setEditAccount({ id: null, isOpen: false })} /> : null}
