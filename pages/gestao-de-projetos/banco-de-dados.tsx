...
function MainDatebasePageContent({ session }: { session: TAuthSession }) {
  ...
  const { data, isLoading, isError, isSuccess, filters, updateFilters } =
    useProjectsByPersonalizedFilters({ page });
  const handleOnMutate = async () =>
    await queryClient.cancelQueries({ queryKey: ["projects-by-filters", page, filters] });
  const handleOnSettled = async () =>
    await queryClient.invalidateQueries({ queryKey: ["projects-by-filters", page, filters] });
  ...
  return (
    <div className="grow p-6">
      <div className="border-border flex w-full flex-col gap-2 border-b p-1">
        ...
        {userHasOverallAccess || userHasCommercialProjectCreationAccess ? (
          <div className="flex w-full flex-wrap items-center justify-end gap-2">
            {userHasCommercialProjectCreationAccess ? (
              <Button
                variant={"default"}
                onClick={() => router.push("/gestao-de-projetos/novo-projeto")}
                size={"xs"}
                className="gap-1"
              >
        ...
        <div className="flex w-full flex-wrap items-center justify-around gap-2">
          {Object.entries(ProjectTypesCollors).map(([key, value]) => (
            <div key={key} className="flex items-center gap-1">
              ...
              <p className="text-foreground text-xs font-bold tracking-tight">{key}</p>
            </div>
          ))}
        </div>
      </div>
        ...
        {isSuccess && projects
          ? projects.map((project) => (
              <ProjectDBCard
                key={project._id}
                project={project}
                handleClick={(id) => setModalProject({ isOpen: true, projectId: id })}
              />
            ))
          : null}
      </div>
      ...
      {exportationMenuIsOpen && (
        <ProjectExportationMenu closeMenu={() => setExportationMenuIsOpen(false)} />
      )}
    </div>
  );
}
