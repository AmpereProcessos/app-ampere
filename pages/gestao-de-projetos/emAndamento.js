  ...
  return (
    <>
      <div className="items flex w-full grow flex-col-reverse gap-2 p-6 lg:flex-row">
        <ol className="self-center border-l-2 border-[#15599a]">
          <li>
            <div className="flex-start flex items-center">
              ...
              <h4 className="text-foreground -mt-2 text-xs font-semibold lg:text-xl">
        ...
        <div className="bg-background border-border top-0 left-0 mb-2 flex h-[450px] w-full flex-col rounded-sm border p-3 shadow-lg lg:sticky lg:top-[10%] lg:left-[60%] lg:mb-0 lg:h-[600px] lg:w-[500px]">
          <div className="border-border flex flex-col items-center border-b pb-2">
          ...
          <div className="overscroll-y scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex max-h-[520px] grow flex-col overflow-y-auto">
            {selectedProjects.projetos?.length > 0 ? (
              ...
              <p className="text-foreground flex h-full items-center justify-center text-center text-sm italic">
...
export async function getServerSideProps(context) {
  ...
  let assContrato = arr.filter(
    (x) => x.contrato.status == "SOLICITADO" || x.contrato.status == "NÃO ASSINADO",
  );
  ...
  let assDocumentacoes = arr.filter(
    (x) => !!x.homologacao.documentacao.dataLiberacao && !x.homologacao.documentacao.dataAssinatura,
  );
  let libConc = arr.filter(
    (x) => !!x.homologacao.acesso.dataSolicitacao & !x.homologacao.acesso.dataSolicitacao,
  );
  let agendamentoObra = arr.filter((x) =>
    ["AGUARDANDO AGENDAMENTO", "CASA EM CONSTRUÇÃO"].includes(x.obra.statusDaObra),
  );
  let terminoObra = arr.filter((x) => ["AGENDADA", "EM ANDAMENTO"].includes(x.obra.statusDaObra));
  let vistoriaConcessionaria = arr.filter(
    (x) => !!x.homologacao.vistoria.dataSolicitacao && !x.homologacao.vistoria.dataResposta,
  );
