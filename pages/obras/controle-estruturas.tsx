...
function InstallationStructureControls() {
	...
	function getStats(info: TInstallationStructureExecution[]) {
		const pending = info.reduce((acc, current) => (!current.estruturaPersonalizada.dataMontagem && current.estruturaPersonalizada.status != "PRONTA" ? acc + 1 : acc), 0);
		const pendingPaid = info.reduce(
			(acc, current) => (!current.estruturaPersonalizada.dataMontagem && current.estruturaPersonalizada.status != "PRONTA" && !!current.compra.dataPagamento ? acc + 1 : acc),
			0,
		);
