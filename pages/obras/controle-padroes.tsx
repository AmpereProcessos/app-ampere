...
function EnergyPAControls() {
	...
	function getStats(info: TEnergyPAExecution[]) {
		const pending = info.reduce((acc, current) => (!current.padrao.aumentoCarga.dataEfetivacao && !current.homologacao.vistoria.dataEfetivacao ? acc + 1 : acc), 0);
		const pendingPaid = info.reduce(
			(acc, current) => (!current.padrao.aumentoCarga.dataEfetivacao && !current.homologacao.vistoria.dataEfetivacao && !!current.compra.dataPagamento ? acc + 1 : acc),
			0,
		);
		return {
