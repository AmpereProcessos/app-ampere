...
import {
  useEnergyPAExecutionWithFilters,
  usePAExecutionStats,
} from "@/utils/methods/query/execution";
  ...
  return (
    <div className="grow bg-slate-50 p-6">
      ...
      <div className="flex w-full flex-col gap-2 py-2">
        ...
        {isError ? (
          <ErrorComponent msg={"Erro ao encontrar projetos para adequação de padrão."} />
        ) : null}
        {isSuccess && projects ? (
          projects.length > 0 ? (
    ...
    <div className="border-border flex w-full flex-col gap-2 rounded-xl border p-3 shadow-xs">
      ...
      <div className="flex w-full flex-col items-center justify-center gap-3 lg:flex-row">
        ...
        <div className="bg-background border-border flex min-h-[130px] w-full flex-col rounded-xl border p-3 shadow-xs lg:w-1/3">
          <div className="flex items-center justify-between">
            <h1 className="text-xs font-medium tracking-tight uppercase">
              ADEQUAÇÕES DE PADRÃO CONCLUÍDAS
            </h1>
        ...
        <div className="bg-background border-border flex min-h-[130px] w-full flex-col rounded-xl border p-3 shadow-xs lg:w-1/3">
          <div className="flex items-center justify-between">
            <h1 className="text-xs font-medium tracking-tight uppercase">
              ADEQUAÇÕES DE PADRÃO PENDENTES
            </h1>
            <LayoutGrid className="h-4 min-h-4 w-4 min-w-4" />
          </div>
          <div className="flex w-full flex-col">
            <div className="text-2xl font-bold text-[#15599a]">
              {stats?.totalAdequacoesPendentes}
            </div>
            <p className="text-foreground text-xs">{stats?.totalAdequacoesPendentesPagas} pagos</p>
          </div>
        </div>
      </div>
      <div className="bg-background border-border flex w-full flex-col gap-1 rounded-xl border p-3 shadow-xs">
        ...
        <div className="flex w-full flex-wrap justify-start gap-2">
          {stats?.porResponsabilidade.map((responsability) => (
            <div
              key={responsability.tipo}
              className="bg-background border-border flex items-center gap-2 rounded-xl border p-3 shadow-xs"
            >
              <h1 className="text-xs font-medium tracking-tight uppercase">
                {responsability.tipo}
              </h1>
              <div className="text-foreground text-xs">
                {responsability.totalAdequacoesPendentes} pendentes
              </div>
              <div className="text-foreground text-xs">
                {responsability.totalAdequacoesPendentesPagas} pagos
              </div>
            </div>
          ))}
        </div>
