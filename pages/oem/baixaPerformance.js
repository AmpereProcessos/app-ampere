		...
		return (
			<div className="flex grow flex-col p-6">
				<div className="border-b border-gray-300 p-2">
					<h1 className="text-xl font-bold text-[#15599a]">IDENTIFICAÇÃO DE BAIXA PERFORMANCE {badPerformers.length > 0 && `(${badPerformers.length})`}</h1>
				</div>
				<div className="flex items-center justify-center gap-4">
					<div className="mt-2 flex items-center justify-center gap-2">
						<TextInput editable={true} label={"Token Solar View"} placeholder="Preencha aqui o token SolarView" value={token} handleChange={(value) => setToken(value)} />
						<button
				...
				<div className="mt-4 flex flex-col items-center">
					...
					<h1 className="text-center text-xl font-bold text-[#fead41]">{badPerformers.length > 0 ? `USINAS A ANALISAR (${badPerformers.length})` : "SEM USINAS A ANALISAR..."}</h1>
						...
						{filteredMonitoramentoBook.map((item, index) => (
							...
							>
								...
								<div className="col-span-2 flex items-center justify-center gap-4 border-r border-[#15599a] p-1">
									...
									{item.problema == "PROBLEMA COM CONEXÃO" && <MdSignalWifiStatusbarConnectedNoInternet4 style={{ color: "#FF9D00", fontSize: "20px" }} />}
