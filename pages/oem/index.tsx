	...
	function getStats({ info }: { info?: TProjectDTO[] }) {
		...
		const overdueMaintenance = info.reduce((acc, current) => {
			...
			const isOverDue = current.manutencoes.some(
				(m, index) => !m.dataEfetivacao && referenceDate && getDifferenceBetweenDates({ start: new Date(referenceDate), end: new Date() }) > (index + 1) * 365,
			);
			if (isOverDue) acc += 1;
			return acc;
		}, 0);
								...
								<div className="flex flex-col p-2 pb-3">
									...
									<div className="mt-2 flex items-center justify-between">
										<div className="flex flex-col items-start">
											...
											<p className={`text-xs font-medium tracking-tight ${checkOeMEnding(project.medidor.data, project.tipoDeServico, project.contrato.dataAssinatura).color}`}>
