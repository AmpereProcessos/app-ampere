...
function getFilteredData(data: TProjectDTO[], dueDate: string) {
  if (!data) return [];
  return data.filter(
    (p) => p.medidor.data && new Date(p.medidor.data) < getCorrespondentDateInPreviousYear(dueDate),
  );
}
...
function RelatorioSetorContent({ session }: { session: TAuthSession }) {
  ...
  function getTotals(
    data: TProjectDTO[],
    dueDate: string,
    dilutionDate: string | null,
  ): TotalsResults {
    if (!data) return { general: { plants: 0, modules: 0 } };
    const modules = data.reduce(
      (acc, current) => acc + formatAsNumber(current.sistema?.qtdeModulos || 0),
      0,
    );
  ...
  function renderStatsByCity(data: TProjectDTO[], dueDate: string, dilutionDate: string | null) {
    return Object.entries(getTotalsByCity(data, dueDate, dilutionDate)).map(
      ([key, value], index) => (
        <div
          key={`${key}-${index.toString()}`}
          className="border-border flex w-full items-center gap-2 border-b py-1"
        >
          <h1 className="text-foreground w-1/3 text-center text-xs font-medium tracking-tight lg:text-sm">
            {key}
          </h1>
    ...
    return data.map((project) => (
      <div key={project._id} className="flex w-full flex-col gap-1">
        <div className="flex w-full items-center justify-between gap-2">
          <h1 className="tracking-tightlg:text-sm text-xs leading-none font-black">
  ...
  return (
    <div className="grow p-6">
      <div className="border-border flex flex-col gap-1 border-b p-1">
        <p className="text-2xl font-black text-[#15599a] uppercase">
          RELATÓRIO DE OPERAÇÃO E MANUTENÇÃO
        </p>
          ...
          <div className="flex w-full flex-col gap-4">
            ...
            {showClientsNames ? (
              <motion.div
                ...
                className="overscroll-y flex h-fit max-h-[400px] w-full flex-col self-center overflow-y-auto border border-border p-2 scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 lg:w-[50%] gap-4"
              >
            ...
            <div className="flex w-full flex-col gap-4">
              <div className="flex flex-col items-center justify-center bg-[#15599a] p-2">
                ...
                <p className="text-center text-sm font-medium text-white italic">
                  ...
                  {scenarios.first.dilutionDate
                    ? `, com diluição até ${formatDateAsLocale(scenarios.first.dilutionDate)}`
                    : null}
                </p>
              </div>
              ...
              <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
                ...
                <div className="w-full lg:w-1/3">
                  <DateInput
                    ...
                    value={
                      scenarios.first.dilutionDate
                        ? formatDate(scenarios.first.dilutionDate)
                        : undefined
                    }
                    handleChange={(value) =>
                      setScenarios((prev) => ({
                ...
                <div className="w-full lg:w-1/3">
                  <div className="flex w-full flex-col gap-1">
                    <h1 className={"font-sans font-bold text-primary"}>
              ...
              <AnimatePresence>
                {scenarios.first.showClients ? (
                  <motion.div
                    initial={{ scale: 0.8, opacity: 0.6 }}
                    animate={{ scale: 1, opacity: 1 }}
                    className="overscroll-y border-border scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex h-fit max-h-[400px] w-full flex-col gap-4 self-center overflow-y-auto border p-2 lg:w-[50%]"
                  >
              ...
              <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
                <div className="border-border flex w-full flex-col items-center rounded border p-3 shadow-lg lg:w-1/3">
                  <h1 className="font-Poppins text-center text-lg font-medium text-[#15599a]">
                    ...
                    <p className="font-Poppins text-center text-2xl font-black text-[#fead41]">
                      {
                        getTotals(
                          firstScenarioData,
                          scenarios.first.dueDate,
                          scenarios.first.dilutionDate || null,
                        ).general.plants
                      }
              ...
              <div className="border-border flex w-full flex-col rounded border shadow-lg">
            ...
            <div className="flex w-full flex-col gap-4">
              <div className="flex flex-col items-center justify-center bg-[#fead41] p-2">
                ...
                <p className="text-center text-sm font-medium text-white italic">
                  ...
                  {scenarios.second.dilutionDate
                    ? `, com diluição até ${formatDateAsLocale(scenarios.second.dilutionDate)}`
                    : null}
                </p>
              </div>
              ...
              <div className="flex w-full flex-col items-center gap-2 lg:flex-row">
                ...
                <div className="w-full lg:w-1/3">
                  <DateInput
                    ...
                    value={
                      scenarios.second.dilutionDate
                        ? formatDate(scenarios.second.dilutionDate)
                        : undefined
                    }
                    handleChange={(value) =>
                      setScenarios((prev) => ({
                    ...
                    <p className="font-Poppins text-center text-2xl font-black text-[#fead41]">
                      {
                        getTotals(
                          secondScenarioData,
                          scenarios.second.dueDate,
                          scenarios.second.dilutionDate || null,
                        ).general.plants
                      }
