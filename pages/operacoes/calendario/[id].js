  ...
  function handleFilter(newView) {
    ...
    if (newView == 'ATIVIDADES') {
      ...
      activities = activities.map((activity, index) => {
        return {
          ...activity,
          title: activity.dataConclusao ? `(CONCLUIDO) ${activity.nome}` : activity.nome,
          ...
          backgroundColor: activity.dataConclusao ? 'rgb(34,197,94)' : colors[index].cor,
          borderColor: activity.dataConclusao ? 'rgb(34,197,94)' : colors[index].cor,
        }
      })
      ...
      operation.atividades.forEach((activity, index) => {
        let subs = activity.subAtividades.map((x) => {
          return {
            ...
            backgroundColor: x.dataConclusao ? 'rgb(34,197,94)' : colors[index].cor,
      ...
      <div className="flex flex-col w-full border-b border-gray-200 pb-2 mb-2">
        ...
        <div className="flex flex-col w-full">
          ...
          {showReportHistory ? (
            reports ? (
              reports.length > 0 ? (
                reports.map((report, index) => <ReportLine report={report} key={index} />)
              ) : (
