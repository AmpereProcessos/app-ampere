...
function BancoDeOS() {
	...
	const { data: orders, filters, setFilters } = useServiceOrders({ after: dateFilter.after, before: dateFilter.before, enabled: status == "authenticated" });
	console.log(filters);
		...
		return (
			<div className="grow p-6">
							...
							<motion.div initial={{ scale: 0.8, opacity: 0.6 }} animate={{ scale: 1, opacity: 1 }} className="mt-4 flex w-full flex-col gap-y-2">
								...
								<div className="flex flex-col flex-wrap items-center justify-center gap-2 lg:flex-row">
									<div
										...
										className={`${filters.inProgress ? "bg-[#15599a]" : "bg-blue-300"} flex h-[36px] cursor-pointer items-center justify-center rounded px-2 font-bold text-white`}
									>
										EM PROCESSO
									</div>
									<div
										...
										className={`${filters.inExecution ? "bg-[#15599a]" : "bg-blue-300"} flex h-[36px] cursor-pointer items-center justify-center rounded px-2 font-bold text-white`}
									>
										EM EXECUÇÃO
									</div>
									<div
										...
										className={`${filters.unassigned ? "bg-[#15599a]" : "bg-blue-300"} flex h-[36px] cursor-pointer items-center justify-center rounded px-2 font-bold text-white`}
									>
										PARA DESIGNAR
									</div>
								</div>
				...
				{modalInfo.isOpen && modalInfo.orderId ? (
					<ModalOrdemServico modalIsOpen={modalInfo.isOpen} orderId={modalInfo.orderId} closeModal={() => setModalInfo({ isOpen: false, orderId: null })} />
				) : null}
