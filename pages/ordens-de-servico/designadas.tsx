				...
				<div className="mt-4 flex flex-col justify-around gap-3">
					...
					{isSuccess ? (
						orders && orders.length > 0 ? (
							orders?.map((order, index) => <ServiceOrderCard key={index} serviceOrder={order} handleClick={() => setEditModal({ isOpen: true, id: order._id })} />)
						) : (
...
function ServiceOrderCard({ serviceOrder, handleClick }: ServiceOrderCardProps) {
	function getStatusTag(serviceOrder: TServiceOrderSimplifiedDTO) {
		...
		if (serviceOrder.status === "EM PLANEJAMENTO") return <div className="rounded-full bg-blue-800 px-2 py-0.5 text-[0.5rem] font-medium text-white">EM PLANEJAMENTO</div>;

		if (serviceOrder.status === "AGENDADA") return <div className="rounded-full bg-yellow-600 px-2 py-0.5 text-[0.5rem] font-medium text-white">AGENDADA</div>;
	...
	return (
		<div className="flex w-full flex-col gap-1 rounded border border-primary bg-[#fff] p-2 shadow-sm dark:bg-[#121212]">
			...
			<div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
				...
				<button onClick={() => handleClick(serviceOrder._id)} className="flex items-center gap-1 rounded-lg bg-primary px-2 py-1 text-[0.6rem] text-secondary">
