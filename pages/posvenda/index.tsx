...
function PosvendaContent({ session }: { session: TAuthSession }) {
  ...
  function getStats({ info }: { info: TProjectDTO[] | undefined }) {
    ...
    const docsToSign = info.reduce((acc, current) => {
      const missingDocSigning =
        !!current.homologacao.documentacao.dataLiberacao &&
        !current.homologacao.documentacao.dataAssinatura;
    ...
    const deliveries = info.reduce(
      (acc, current) => {
        ...
        const isThisWeek =
          dayjs().isSame(deliveryPrevision, "week") && dayjs(deliveryPrevision).isAfter(new Date());
    ...
    <div className="grow p-6">
      <div className="border-border flex flex-col items-center justify-between border-b p-1">
        ...
        <div className="my-2 flex w-full flex-col items-center justify-center gap-3 lg:flex-row">
          <div className="bg-background border-border flex min-h-[110px] w-full flex-col rounded-xl border p-3 shadow-xs lg:w-1/5">
          ...
          <div className="bg-background border-border flex min-h-[110px] w-full flex-col rounded-xl border p-3 shadow-xs lg:w-1/5">
            ...
            <div className="mt-2 flex w-full flex-col">
              ...
              <p className="text-foreground text-xs">{stats.entregaSemana} essa semana</p>
            </div>
          </div>
          <div className="bg-background border-border flex min-h-[110px] w-full flex-col rounded-xl border p-3 shadow-xs lg:w-1/5">
            <div className="flex items-center justify-between">
