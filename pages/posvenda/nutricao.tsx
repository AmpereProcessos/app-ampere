...
import {
  createOpportunityInteraction,
  updateOpportunityFunnelReference,
} from "@/utils/methods/mutation/crm/opportunities";
...
function StageBlock({ session, stage, opportunities }: StageBlockProps) {
  function getOpportunitiesInStage({
    ...
    return opportunities.filter(
      (opportunity) => opportunity.funil.idEstagio.toString() === stageId.toString(),
    );
  }
...
function OpportunityCard({
  ...
  return (
    <div className="border-border bg-background flex w-full flex-col gap-2 rounded border p-2 shadow-xs">
      ...
      <div className="flex w-full items-center justify-between gap-2">
              ...
              return (
                <div className="flex items-center gap-1">
                  <Avatar
                    ...
                    width={18}
                  />
                  <p className="text-muted-foreground text-[0.65rem] font-light">{resp.nome}</p>
                </div>
              );
            return null;
          })}
          {opportunity.responsaveis.length > 2 ? (
            <p className="text-muted-foreground text-[0.65rem] font-light">...</p>
          ) : null}
        </div>
        ...
        <div className="ites-center flex min-w-fit gap-1">
          <BsCalendarPlus />
          <p className={"text-muted-foreground text-[0.65rem] font-medium"}>
            {formatDateAsLocale(opportunity.dataInsercao, true)}
          </p>
        </div>
      </div>
    </div>
