...
function OpportunityCard({
	...
	opportunity,
	stageTitle,
}: { session: Session; opportunity: TOpportunitySimplifiedDTOWithProposalAndActivitiesAndFunnels; stageTitle: string }) {
	...
	const { mutate: handleNotifyInteraction } = useMutation({
		...
		mutationFn: async () => {
			...
			await updateOpportunityFunnelReference({ id: opportunity.funil.id, changes: { idEstagioFunil: (Number(opportunity.funil.idEstagio) + 1).toString() } });
	...
	return (
		<div className="flex w-full flex-col rounded p-2 border border-primary/20 shadow-sm bg-[#fff] gap-2">
			...
			<div className="w-full flex flex-col grow gap-2">
				{opportunity.ultimaInteracao?.data ? (
					<div className="w-full flex items-center justify-center gap-1">
						<BsCalendar />
						<p className={"text-[0.65rem] font-medium text-primary/50"}>Última interação em: {formatDateAsLocale(opportunity.ultimaInteracao?.data, true)}</p>
					</div>
				) : null}
