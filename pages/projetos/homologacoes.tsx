					...
					<div className="flex items-center gap-1">
						...
						<div className="flex flex-col gap-1">
							...
							<p className="text-sm leading-none tracking-tight text-gray-500">
								{homologations?.length ? (homologations.length > 0 ? `${homologations.length} homologações contabilizadas` : `${homologations.length} homologação contabilizada`) : "..."}
							</p>
						</div>
					</div>
					<button
				...
				{isSuccess ? (
					homologations.length > 0 ? (
						homologations.map((homologation) => (
							<HomologationCard key={homologation._id} homologation={homologation} handleClick={(id) => setEditModal({ id: id, isOpen: true })} userHasEditPermission={true} />
						))
					) : (
						<p className="flex w-full grow items-center justify-center py-2 text-center font-medium italic tracking-tight text-gray-500">Não foram encontradas homologações...</p>
					)
				) : null}
			</div>
			{newHomologationModalIsOpen ? <NewHomologation session={session} affectedQueryKey={["homologations"]} closeModal={() => setNewHomologationModalIsOpen(false)} /> : null}
			{editModal.isOpen && editModal.id ? <ControlHomologation session={session} homologationId={editModal.id} closeModal={() => setEditModal({ id: null, isOpen: false })} /> : null}
		</div>
	);
}
