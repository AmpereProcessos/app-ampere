	...
	return (
		<div className="flex w-full max-w-full grow flex-col overflow-x-hidden bg-[#f8f9fa] p-6">
			...
			<div className="flex flex-wrap justify-between gap-2 py-2">
				...
				{isSuccess && technicalAnalysis ? technicalAnalysis.map((analysisInfo) => <TechnicalAnalysisCard key={analysisInfo.id} analysis={analysisInfo} />) : null}
			</div>
		</div>
	);
}
...
function TechnicalAnalysisCard({ analysis }: TechnicalAnalysisCardProps) {
	function getTechnicalAnalysisStatusTag(technicalAnalysis: TIGreenTechnicalAnalysis[number]) {
		if (technicalAnalysis.status === "CONCLUÍDO" && technicalAnalysis.dataEfetivacao)
			return <h1 className={cn("min-w-fit rounded-lg px-2 py-0.5 text-[0.5rem] bg-green-500 text-white")}>{analysis.status}</h1>;

		const currentDate = new Date();
		if (technicalAnalysis.dataPrevisaoEfetivacao && technicalAnalysis.dataPrevisaoEfetivacao < currentDate)
			return <h1 className={cn("min-w-fit rounded-lg px-2 py-0.5 text-[0.5rem] bg-orange-500 text-white")}>EM ATRASO</h1>;

		if (status === "EM ANDAMENTO") return <h1 className={cn("min-w-fit rounded-lg px-2 py-0.5 text-[0.5rem] bg-blue-500 text-white")}>{status}</h1>;

		return <h1 className={cn("min-w-fit rounded-lg px-2 py-0.5 text-[0.5rem] bg-gray-500 text-white")}>{status}</h1>;
	}
	return (
		<div className="flex w-full flex-col gap-1 rounded border border-primary bg-[#fff] p-2 shadow-sm dark:bg-[#121212]">
				...
				<div className="flex w-full items-center justify-between gap-1 lg:w-fit">
					...
					<div className="flex -space-x-1 overflow-hidden">
						{analysis.analistas.map((analyst) => (
							<Avatar key={analyst.id} url={analyst.usuario.avatar} width={20} height={20} fallback={formatNameAsInitials(analyst.usuario.nome || "")} />
						))}
					</div>
				</div>
			</div>
			...
			<div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
				<div className="flex flex-wrap items-center gap-2">
					...
					<div className="flex items-center gap-1">
						<Avatar key={analysis.autor.id} url={analysis.autor.avatar} width={20} height={20} fallback={formatNameAsInitials(analysis.autor.nome || "")} />
