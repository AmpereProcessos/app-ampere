...
function Projetos() {
	...
	function getTotalCircuitBreakers() {
		const circuitBreakerObj = {};
		if (projects) {
			for (let i = 0; i < projects.length; i++) {
				const cbArr = projects[i].material?.disjuntores;
				if (cbArr) {
					for (let j = 0; j < cbArr.length; j++) {
						const tag = `${cbArr[j].tipo} ${cbArr[j].corrente}A`;
						const currentTagQtde = circuitBreakerObj[tag] ? circuitBreakerObj[tag] : 0;
		...
		return (
			<div className="grow p-6">
						...
						>
							...
							<div className="flex flex-col p-2">
								...
								<div className="flex items-center justify-between">
									...
									<div>
										<span className="text-center text-xxs">{project.compra.statusEntrega === "ENTREGUE" ? "DATA DE ENTREGA" : "PREV. DE ENTREGA"}</span>
								...
								<div className="flex items-center justify-between">
									...
									<div className="flex w-full flex-col">
										...
										<p className={"text-end text-xs uppercase text-red-500"}>
											{project.homologacao.acesso.dataResposta ? `${getDateDiff(new Date(), new Date(project.homologacao.acesso.dataResposta))} DIAS` : "-"}
										</p>
									</div>
								</div>
								{project.homologacao.acesso.dataSolicitacao ? (
									<div className="flex w-full items-center justify-between">
										<p className="text-xxs ">{project.homologacao.acesso.dataResposta ? "ATÉ APROVAÇÃO DO PARECER" : "DESDE A SOLICITAÇÃO DE ACESSO"}</p>
										<p className="text-start text-xs text-gray-600">
											{project.homologacao.acesso.dataResposta
												? `${getDateDiff(new Date(project.homologacao.acesso.dataResposta), new Date(project.homologacao.acesso.dataSolicitacao))} DIAS`
				...
				{modalProject.isOpen && modalProject.projectId && (
					<ModalProjetos projectId={modalProject.projectId} modalIsOpen={modalProject.isOpen} closeModal={() => setModalProject({ isOpen: false, projectId: null })} />
				)}
			</div>
		);
	}
