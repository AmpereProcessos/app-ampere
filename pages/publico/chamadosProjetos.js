...
import { EngineeringCallTypes, projetistas } from "../../utils/constants";
	...
	return (
		<section className="flex min-h-screen items-center justify-center bg-[#15599a]">
			<div className="bg-background flex flex-col rounded p-4">
				<Link href="/chamados/chamadosProjetos">
				...
				<div className="border-primary/20 mt-4 flex flex-col gap-x-2 border p-2 lg:flex-row">
					...
					<select value={responsavel} onChange={(e) => setResponsavel(e.target.value)} className="mt-2 grow text-center text-xs outline-hidden lg:mt-0">
				...
				<div className="border-primary/20 mt-4 flex flex-col gap-x-2 border p-2">
					...
					<select value={tipoDoChamado} onChange={(e) => setTipoDoChamado(e.target.value)} className="mt-2 text-center text-xs outline-hidden lg:mt-0">
