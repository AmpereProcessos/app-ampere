...
import {
  ...
  updateTransportControlItem,
} from "@/utils/methods/mutation/transport-controls";
...
type TransportControlIdentifiedResponsible =
  TGetTransportControlByIdPublicOutput["data"]["responsaveis"][number] & {
    cpf: string;
  };

export default function TransportControlFormulary({
  transportControlId,
  error,
}: TransportControlFormularyProps) {
  ...
  const {
    ...
    isError,
    error: queryError,
  } = useTransportControlByIdPublic({ id: transportControlId });
  const [identifiedResponsible, setIdentifiedResponsible] =
    useState<TransportControlIdentifiedResponsible | null>(null);
  const {
    data: employeeByCpf,
  ...
  const cpfNotFound =
    cpfLooksComplete && employeeByCpfIsFetched && !employeeByCpf && !isFetchingEmployeeByCpf;

  if (isLoading) return <LoadingComponent />;
  if (isError)
    return (
      <ErrorComponent msg={queryError?.message || "Erro ao carregar controle de transporte."} />
    );
    ...
    return (
      <div className="h-full w-full flex flex-col items-center justify-center gap-6 bg-background container mx-auto py-12 px-6">
        <div className="w-full flex flex-col gap-6 items-center w-full lg:w-[60%]">
          <div className="w-full flex flex-col items-center gap-1.5 p-3 border border-border rounded-lg shadow-sm">
            ...
            <div className="w-full flex items-center justify-center gap-2">
              <div className="flex items-center gap-1">
                ...
                <p className="text-foreground text-[0.65rem] font-medium">
                  {formatDateAsLocale(transportControl.dataInsercao, true)}
                </p>
              </div>
              <div className="flex items-center gap-1">
          ...
          <div
            className={cn(
              "flex w-fit items-center justify-center gap-1 self-center rounded bg-blue-200 px-6 py-3 text-blue-800",
            )}
          >
            <PlusIcon size={15} />
            <h1 className="w-fit text-start text-xs font-medium tracking-tight">
              Você está iniciando o transporte de mercadorias.
            </h1>
          </div>
...
function TransportControlFormularyStarting({
  transportControlId,
  transportControl,
  callbacks,
}: TransportControlFormularyStartingProps) {
  ...
  const { mutate: handleStartTransport, isPending: isStarting } = useMutation({
    ...
    mutationFn: async (info: TransportControlFormularyStartingPayload) => {
      ...
      if (!info.initialKilometersAttachment.file)
        throw new Error("Imagem da quilometragem inicial é obrigatória.");
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center px-6 py-6 border border-border rounded-lg shadow-sm gap-6">
      <NumberInput
        label="QUILOMETRAGEM INICIAL"
        value={startingPayload.initialKilometers}
        placeholder="Digite a quilometragem inicial..."
        handleChange={(value) =>
          setStartingPayload({ ...startingPayload, initialKilometers: value })
        }
        width="100%"
      />
      <div className="border-border bg-background flex w-full grow flex-col gap-4 rounded-md border p-6 shadow-xs dark:bg-[#121212]">
        <div className="flex w-full flex-col gap-1">
          <div className="flex w-full flex-col items-start gap-2 lg:flex-row lg:items-center">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-sm leading-none font-bold tracking-tight">
                IMAGEM DA QUILOMETRAGEM INICIAL
              </h1>
            </div>
        ...
        <p className="text-foreground text-xs leading-none font-light">
          Anexe um foto do painel do veículo.
        </p>
        <div className="relative flex w-full items-center justify-center">
          <label
            htmlFor="dropzone-file"
            className="relative flex aspect-square h-auto w-full max-w-[300px] cursor-pointer flex-col items-center justify-center overflow-hidden rounded-lg border-2 border-dashed border-border bg-background hover:bg-primary/10 dark:bg-[#121212] dark:hover:bg-bray-800"
          >
            {startingPayload.initialKilometersAttachment.previewUrl ? (
              <Image
                src={startingPayload.initialKilometersAttachment.previewUrl}
                alt="Imagem da quilometragem inicial."
                fill
            ...
            <input
              onChange={(e) => {
                if (e.target.files) {
                  ...
                  setStartingPayload({
                    ...startingPayload,
                    initialKilometersAttachment: { file, previewUrl: URL.createObjectURL(file) },
                  });
                }
                return;
              }}
...
function TransportControlFormularyConcluding({
  transportControlId,
  transportControl,
  callbacks,
}: TransportControlFormularyConcludingProps) {
  ...
  return (
    <>
      <div className="w-full flex flex-col items-center justify-center px-3 py-6 border border-border rounded-lg shadow-sm gap-6">
        <ResponsiveDialogDrawerSection
          sectionTitleText="ENTREGAS"
          sectionTitleIcon={<Package className="h-4 w-4 min-w-4 min-h-4" />}
        >
        ...
        <ResponsiveDialogDrawerSection
          ...
          sectionTitleIcon={<DollarSign className="h-4 w-4 min-w-4 min-h-4" />}
        >
      ...
      <div className="w-full flex flex-col items-center justify-center px-3 py-6 border border-border rounded-lg shadow-sm gap-6">
        <ResponsiveDialogDrawerSection
          sectionTitleText="RESUMO DA ROTA"
          sectionTitleIcon={<Route className="h-4 w-4 min-w-4 min-h-4" />}
        >
          <div className="flex w-full flex-wrap items-stretch justify-center gap-4">
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <div className="relative flex aspect-square h-auto w-full min-w-[120px] max-w-[120px] overflow-hidden rounded-lg border border-border bg-primary/10">
                {transportControl.distanciaQuilometragemInicialImagemUrl ? (
                  <Image
                    src={transportControl.distanciaQuilometragemInicialImagemUrl}
                    alt="Quilometragem inicial"
                    fill
                    className="object-cover"
                    unoptimized
                  />
                  ...
                  <div className="flex h-full w-full items-center justify-center">
                    <MdAttachFile className="h-8 w-8 text-muted-foreground" />
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col items-center gap-2">
              <span className="w-full rounded-lg border border-dashed border-primary/40 bg-primary/5 px-3 py-1.5 text-center text-sm font-medium text-foreground">
                {transportControl.distanciaQuilometragemFinal != null
                  ? `${transportControl.distanciaQuilometragemFinal.toLocaleString("pt-BR")} km`
                  : "NÃO DEFINIDO"}
              </span>
              <div className="relative flex aspect-square h-auto w-full max-w-[120px] items-center justify-center overflow-hidden rounded-lg border-2 border-dashed border-border bg-primary/5">
        ...
        <div className="w-full flex flex-col gap-1.5 items-center justify-center">
          <Button
            disabled={!isElegibleToConclude}
            onClick={() => setConclusionMenuIsOpen(true)}
          ...
          {!isElegibleToConclude ? (
            <div
              className={cn(
                "flex w-fit items-center justify-center gap-1 self-center rounded bg-red-200 px-2 py-1 text-red-800",
              )}
            >
              <AlertCircle className="w-4 h-4 min-w-4 min-h-4" />
              <h1 className="w-fit text-start text-xs font-medium tracking-tight">
                Você precisa concluir todas as entregas para concluir o transporte.
              </h1>
            </div>
          ) : null}
        </div>
...
function TransportControlFormularyConcludingMenu({
  transportControlId,
  closeMenu,
  callbacks,
}: TransportControlFormularyConcludingMenuProps) {
  const [finalKilometers, setFinalKilometers] =
    useState<TransportControlFormularyConcludingMenuPayload["finalKilometers"]>(null);
  ...
  const { mutate: handleCompleteDelivery, isPending: isCompletingDelivery } = useMutation({
    ...
    mutationFn: async (info: TransportControlFormularyConcludingMenuPayload) => {
      ...
      if (!info.finalKilometersAttachment.file)
        throw new Error("Imagem da quilometragem final é obrigatória.");
  ...
  return (
    ...
    >
      ...
      <div className="border-border bg-background flex w-full grow flex-col gap-4 rounded-md border p-6 shadow-xs dark:bg-[#121212]">
        ...
        <p className="text-foreground text-xs leading-none font-light">
          Anexe uma foto do painel do veículo.
        </p>
        <div className="relative flex w-full items-center justify-center">
          <label
          ...
          >
            {finalKilometersAttachment.previewUrl ? (
              <Image
                src={finalKilometersAttachment.previewUrl}
                alt="Imagem da quilometragem final."
                fill
                className="object-cover"
              />
            ) : (
...
function TransportControlFormularyConcludingDeliveryCard({
  ...
  return (
    <div className="bg-background border-border relative flex w-full flex-col justify-between gap-1 rounded-lg border p-3 shadow-xs">
      <div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[0.625rem] font-medium tracking-tight text-foreground">
            {delivery.id}
          </h3>
        </div>
        <div className="flex items-center gap-2">
            ...
            {formatLocation({
              location: {
                ...
                numeroOuIdentificador:
                  delivery.controleCompra?.entrega.localizacao.numeroOuIdentificador || null,
      ...
      <div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
        <div className="flex flex-wrap items-center gap-2">
          {delivery.dataEfetivacao && (
            <div className="flex items-center gap-1">
              ...
              <p className="text-foreground text-[0.65rem] font-medium">
                {formatDateAsLocale(delivery.dataEfetivacao, true)}
              </p>
            </div>
          )}
        </div>
        ...
        {!delivery.dataEfetivacao ? (
          <Button
            ...
            onClick={() => setDeliveryMenuIsOpen(true)}
          >
            <CheckCircle2 className="w-4 h-4 min-w-4 min-h-4" />
            <p className="text-foreground text-[0.65rem] font-medium">CONCLUIR ENTREGA</p>
          </Button>
        ) : null}
      </div>
      {deliveryMenuIsOpen ? (
...
function TransportControlFormularyConcludingDeliveryConcludeMethodButton({
  ...
  const [deliveryDate, setDeliveryDate] = useState<string | undefined>(
    delivery.dataEfetivacao || new Date().toISOString(),
  );
  const defaultAttachmentDefinitions = [
    { key: "km", title: "ENTREGA - FOTO DO KM", label: "FOTO DO KM NA ENTREGA" },
    {
      key: "materiais",
      title: "ENTREGA - FOTO DOS MATERIAIS ENTREGUES",
      label: "FOTO DOS MATERIAIS ENTREGUES",
    },
  ...
  >([]);
  const allDefaultAttachmentsReady = defaultAttachmentDefinitions.every(
    (item) => !!defaultAttachments[item.key].file,
  );
  ...
  return (
    ...
    >
      ...
      />
      <div className="w-full flex flex-col gap-2">
        <h3 className="text-foreground text-xs font-medium tracking-tight">
              ...
              <label
                ...
                className={cn(
                  "relative h-[140px] w-full rounded-lg overflow-hidden cursor-pointer border border-dashed flex items-center justify-center",
                  {
                    ...
                    "border-border bg-primary/10": !hasFile,
                  },
                )}
              >
      ...
      <div className="w-full flex flex-col gap-1">
        <h3 className="text-foreground text-xs font-medium tracking-tight">OUTROS ANEXOS</h3>
        <div className="relative flex h-full w-full flex-col items-center justify-center">
          <label
            htmlFor={`dropzone-file-${delivery.id}`}
            className="dark:hover:bg-bray-800 border-border bg-background hover:bg-primary/10 flex h-full min-h-64 w-full cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed dark:bg-[#121212]"
          >
            <div className="text-primary flex flex-col items-center justify-center px-2 pt-5 pb-6">
              ...
              <p className="text-center text-xs lg:text-base">
                Clique para escolher um ou mais arquivos ou os arraste para a àrea demarcada
              </p>
            </div>
            <input
              onChange={(e) => {
        ...
        <div className="flex w-full flex-wrap items-start justify-start gap-4">
          {otherAttachments.length > 0 ? (
            otherAttachments
              ...
              .map((attachment) => (
                <div
                  ...
                  className="border-primary/50 flex h-[100px] max-h-[100px] w-[80px] flex-col rounded border"
                >
                  <div className="relative flex h-[80] w-full grow items-center justify-center bg-linear-to-b from-sky-400 to-sky-200">
                    {attachment.previewUrl ? (
                      ...
                      <h1 className="rounded-lg bg-blue-600 px-4 py-1 text-[0.65rem] font-bold text-white">
                        {getFileTypeTitle(attachment.file?.type || "")}
                      </h1>
                    )}
                  </div>
            ...
            <p className="text-foreground w-full text-start font-medium tracking-tight">
