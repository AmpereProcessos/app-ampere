...
function EnergyDistributionReport() {
  const [newDistributionEntryHolder, setNewDistributionEntryHolder] =
    useState<TDistributionEntryHolder>({
      installationTitle: "",
  ...
  return (
    <div className="flex w-full max-w-full grow flex-col overflow-x-hidden bg-[#f8f9fa] p-6">
      <div className="flex flex-col items-center gap-2">
        ...
        <div className="bg-background border-border flex w-full flex-col rounded-lg border p-3 lg:w-[60%] dark:bg-[#121212]">
          ...
          <div className="flex w-full flex-col gap-3">
                ...
                <div className="text-primary flex flex-col items-center justify-center px-2 pt-5 pb-6">
                  ...
                  {!newDistributionEntryHolder.distributionFile ? (
                    <p className="text-center text-xs lg:text-base">
                      Clique para escolher um arquivo ou o arraste para a àrea demarcada
                    </p>
                  ) : (
                    <p className="text-center text-xs lg:text-base">
                      {formatLongString(newDistributionEntryHolder.distributionFile.name)}
                    </p>
                  )}
                </div>
                <input
            ...
            <TextInput
              ...
              handleChange={(value) =>
                updateNewDistributionEntryHolder({ installationTitle: value })
              }
              width="100%"
            />
            <NumberInput
            ...
            <div className="flex w-full items-center justify-end">
              <Button onClick={() => handleDistributionNewEntry(newDistributionEntryHolder)}>
      ...
      <div className="flex w-full flex-col gap-2">
        {Object.entries(installationRecords).map(([period, records]) => (
          <div
            key={period}
            className="border-border bg-background flex w-full flex-col rounded-lg border p-3 dark:bg-[#121212]"
          >
            ...
            <div className="flex w-full flex-col gap-2 rounded border border-[#15599a]">
              <div className="flex items-center gap-2 rounded bg-[#15599a]">
                ...
                <p className="text-xxs w-[10%] p-1 text-center font-bold text-white">
                  QUOTA DE RECEBIMENTO
                </p>
              ...
              {records.map((record, recordIndex) => (
                <div
                  key={`${record.identificador}-${recordIndex}`}
                ...
                >
                  <div className="flex w-[10%] flex-col gap-1">
                    <div className="flex w-full items-center gap-2">
                      ...
                      />
                      <p className={"text-[0.45rem] font-bold md:text-sm"}>
                        {record.identificador}
                      </p>
                    </div>
                  </div>
                  <div className="flex w-[10%] items-center justify-center">
                    <p className="text-center] text-[0.45rem] font-bold md:text-sm">
                      {record.modalidade}
                    </p>
                  </div>
                  <p className="w-[10%] text-center text-[0.45rem] font-bold md:text-sm">
                    {record.recebimentoQuota
                      ? `${formatDecimalPlaces(record.recebimentoQuota, 2)}%`
                      : "-"}
                  </p>
                  <p className="w-[10%] text-center text-[0.45rem] font-bold md:text-sm">
                    {record.injecaoValor !== 0
                      ? `${formatDecimalPlaces(record.injecaoValor, 2)}kWh`
                      : "-"}
                  </p>
                  <p className="w-[10%] text-center text-[0.45rem] font-bold md:text-sm">
                    {record.consumoValor !== 0
                      ? `${formatDecimalPlaces(record.consumoValor, 2)}kWh`
                      : "-"}
                  </p>
                  <p className="w-[10%] text-center text-[0.45rem] font-bold md:text-sm">
                    {record.transferenciaValor !== 0
                      ? `${formatDecimalPlaces(record.transferenciaValor, 2)}kWh`
                      : "-"}
                  </p>
                  <p className="w-[10%] text-center text-[0.45rem] font-bold md:text-sm">
                    {record.recebimentoValor !== 0
                      ? `${formatDecimalPlaces(record.recebimentoValor, 2)}kWh`
                      : "-"}
                  </p>
                  <p className="w-[10%] text-center text-[0.45rem] font-bold md:text-sm">
                    {record.compensacaoValor !== 0
                      ? `${formatDecimalPlaces(record.compensacaoValor, 2)}kWh`
                      : "-"}
                  </p>
                  <p className="w-[10%] text-center text-[0.45rem] font-bold md:text-sm">
                    {record.saldoAtual !== 0
                      ? `${formatDecimalPlaces(record.saldoAtual, 2)}kWh`
                      : "-"}
                  </p>
                  <p className="w-[10%] text-center text-[0.45rem] font-bold md:text-sm">
                    {record.tarifaEnergia !== 0
                      ? `${formatDecimalPlaces(record.tarifaEnergia, 2)}kWh`
                      : "-"}
                  </p>
				...
				{Object.keys(instalacoes).length > 0
					? Object.keys(instalacoes).map((key, index) => (
							<div key={index} className="flex flex-col">
								...
								<div className="grid grid-cols-6 lg:grid-cols-12 border-b border-border">
								...
								{instalacoes[key].map((item, index2) => (
									<div key={index2} className="grid grid-cols-6 lg:grid-cols-12 gap-1 border-b border-border">
										<p className="hidden lg:block text-xxs font-bold text-foreground text-center border-r border-border p-1">{formatPeriodo(item.periodo)}</p>
										<p
											style={{
												color: COLORS[index2],
											}}
											className={`text-xxs font-bold text-center border-r border-border p-1`}
										>
											{item.instalacao}
										</p>
										<p className="hidden lg:block text-xxs font-bold text-foreground text-center border-r border-border p-1">
											{item.modalidade == "Auto consumo-Geradora" ? "GERADORA" : "RECEBEDORA"}
										</p>
										<p className="hidden lg:block text-xxs font-bold text-foreground text-center border-r border-border p-1">
											{item.quota ? `${item.quota.substr(0, item.quota.length - 4)}%` : "-"}
										</p>
										<p className="text-xxs font-bold text-foreground text-center border-r border-border p-1">
										...
										<div className="hidden lg:flex items-center border-r border-border">
											<input
												type={"number"}
												placeholder="-"
												...
												className="text-xxs font-bold text-foreground text-center p-1 outline-hidden h-full w-full bg-transparent"
											/>
											<p className="text-xxs font-bold text-foreground text-center mr-2">R$/kWh</p>
										</div>
										<div className="flex items-center">
											<p className="text-xxs font-bold text-foreground text-center">R$</p>
											<input
												type={"number"}
												placeholder="-"
        ...
        <div classNameName="flex flex-col bg-background">
          {entregues.map((obj) => (
            <div classNameName="grid grid-cols-6 h-[36px]">
              <div classNameName="text-center flex items-center justify-center text-xxs p-1 text-foreground text-center font-bold border-b border-border">
                {obj.nomeDoProjeto}
              </div>
              <div classNameName="text-center flex items-center justify-center text-xxs p-1 text-foreground font-bold border-b border-border">
                {obj.nomeDoContrato}
              </div>
              <div classNameName="text-center flex items-center justify-center text-xxs p-1 text-foreground font-bold border-b border-border">
