...
function SimulacaoFaturaEPayback() {
	...
	function simulate() {
		...
		const geracao = Object.keys(irradianca).map((key, index) => Number(((irradianca[key] * info.potModulos * info.qtdeModulos) / 1000).toFixed(2)));
		const consumoInstantaneo = consumo.map((obj, index) => {
			let consumo = geracao[index] > obj.valor * info.simultaneidade ? obj.valor * info.simultaneidade : geracao[index];
			return consumo;
		});
		const liquidoMensal = geracao.map((geracao, index) => Number((geracao - consumo[index].valor).toFixed(2)));
		const progressaoKWH = {
			2023: info.valorkWh,
		...
		var meses;
		meses = (new Date(2029, 12, 31).getFullYear() - new Date(info.anoInicio, info.mesInicio, 1).getFullYear()) * 12;
		...
		for (let i = 0; i <= meses; i++) {
			...
			var injetadoMesComparacao = geracao[mesComparacao - 1] - consumoInstantaneo[mesComparacao - 1];

			var usoConcessionariaMesComparacao = consumo[mesComparacao - 1].valor - consumoInstantaneo[mesComparacao - 1];

			var saldoParaAdicionarDireitoAdquirido = injetadoMesComparacao + disponibilidadePorTipoDeLigacao[info.tipoDeLigacao] - usoConcessionariaMesComparacao;

			saldoCumulado = saldoCumulado + liquidoMensal[mesComparacao - 1] > 0 ? saldoCumulado + liquidoMensal[mesComparacao - 1] : 0;

			saldoCumuladoDireitoAdquirido = saldoCumuladoDireitoAdquirido + saldoParaAdicionarDireitoAdquirido > 0 ? saldoCumuladoDireitoAdquirido + saldoParaAdicionarDireitoAdquirido : 0;
			...
			var compensacao = liquidoMesComparacao >= 0 ? usoConcessionariaMesComparacao : compensacaoSeComUsoDeSaldo > 0 ? compensacaoSeComUsoDeSaldo : 0;

			var fioB = compensacao * progressaoFioB[anoComparacao];
			var outrosCustos = compensacao >= usoConcessionariaMesComparacao ? fioB : fioB + (usoConcessionariaMesComparacao - compensacao) * progressaoKWH[anoComparacao];

			var custoDisponibilidade = disponibilidadePorTipoDeLigacao[info.tipoDeLigacao] * progressaoKWH[anoComparacao];

			var valorFatura = custoDisponibilidade > outrosCustos ? custoDisponibilidade + info.iluminacaoPublica : outrosCustos + info.iluminacaoPublica;
			var valorFaturaDireitoAdquirido =
				saldoParaAdicionarDireitoAdquirido > 0
					...
					: saldoParaAdicionarDireitoAdquirido + saldoPassadoDireitoAdquirido >= 0
						? custoDisponibilidade + info.iluminacaoPublica
						: Math.abs(saldoParaAdicionarDireitoAdquirido + saldoPassadoDireitoAdquirido) * progressaoKWH[anoComparacao] + info.iluminacaoPublica + custoDisponibilidade;

			var economia = consumo[mesComparacao - 1].valor * progressaoKWH[anoComparacao] + info.iluminacaoPublica - valorFatura;
			var economiaDireitoAdquirido = consumo[mesComparacao - 1].valor * progressaoKWH[anoComparacao] + info.iluminacaoPublica - valorFaturaDireitoAdquirido;
			...
			if (custoDisponibilidade > outrosCustos) saldoCumulado = saldoCumulado + disponibilidadePorTipoDeLigacao[info.tipoDeLigacao];

			tabelaFinal.push({
				ANO: anoComparacao,
				MÊS: mesComparacao,
				TAG: mesComparacao >= 10 ? `${mesComparacao}/${anoComparacao}` : `0${mesComparacao}/${anoComparacao}`,
		...
		return (
			<div className="p-6 grow flex flex-col">
				<div className="flex items-center justify-center gap-2 w-full mb-3">
					<h1 className="text-center font-bold text-[#15599a] text-xl font-['Roboto']">ANÁLISE</h1>
					<button onClick={() => setData(null)} className="p-2 rounded border border-gray-600 text-gray-600 hover:bg-gray-600 hover:text-white hover:scale-110 duration-300 ease-in-out">
						<MdRestartAlt />
					</button>
				</div>
				<div className="flex items-center justify-center lg:justify-end w-full">
					<button onClick={() => getYearlyGraphData(data)} className="p-2 text-xs rounded text-white bg-[#15599a] hover:scale-110 duration-300 ease-in-out mb-2">
				...
				<div className="w-full grid grid-cols-6 lg:grid-cols-8">
					<div className="text-center text-xxs lg:text-base flex items-center justify-center bg-black text-white p-1 font-bold border-r border-white">PERÍODO</div>
					...
					<div className="hidden lg:flex text-center text-xxs lg:text-base items-center justify-center bg-black text-white p-1 font-bold border-r border-white">SALDO NOVA LEI</div>
					<div className="text-center text-xxs lg:text-base flex items-center justify-center bg-black text-white p-1 font-bold border-r border-white">FATURA DIREITO ADQUIRIDO</div>
					<div className="text-center text-xxs lg:text-base flex items-center justify-center bg-black text-white p-1 font-bold border-r border-white">FATURA NOVA LEI </div>
					<div className="text-center text-xxs lg:text-base flex items-center justify-center bg-black text-white p-1 font-bold border-r border-white">PAYBACK DIREITO ADQUIRIDO</div>
					<div className="text-center text-xxs lg:text-base flex items-center justify-center bg-black text-white p-1 font-bold border-r border-white">PAYBACK NOVA LEI</div>
					<div className="text-center text-xxs lg:text-base flex items-center justify-center bg-black text-white p-1 font-bold">POUPANÇA</div>
				</div>
				{data?.map((obj, index) => (
					<div key={index} className="w-full grid grid-cols-6 lg:grid-cols-8 border-x border-b border-gray-300">
						<h1 className="text-center text-xxs lg:text-sm text-gray-600 border-r border-gray-300 p-1">{obj["TAG"]}</h1>
						<h1 className="hidden lg:block text-center text-xxs lg:text-sm text-gray-600 border-r border-gray-300 p-1">{obj["SALDO ACUMULADO (DIREITO ADQUIRIDO)"].toFixed(2)}</h1>
						<h1 className="hidden lg:block text-center text-xxs lg:text-sm text-gray-600 border-r border-gray-300 p-1">{obj["SALDO ACUMULADO (NOVA LEI)"].toFixed(2)}</h1>
						<h1 className="text-center text-xxs lg:text-sm text-gray-600 border-r border-gray-300 p-1">{obj["VALOR FATURA (DIREITO ADQUIRIDO)"].toFixed(2)}</h1>
						<h1 className="text-center text-xxs lg:text-sm text-gray-600 border-r border-gray-300 p-1">{obj["VALOR FATURA (NOVA LEI)"].toFixed(2)}</h1>
						<h1 className={`text-center text-xxs lg:text-sm ${obj["PAYBACK (DIREITO ADQUIRIDO)"] > 0 ? "text-green-500" : "text-red-500"} border-r border-gray-300 p-1`}>
							{obj["PAYBACK (DIREITO ADQUIRIDO)"].toFixed(2)}
						</h1>
						<h1 className={`text-center text-xxs lg:text-sm ${obj["PAYBACK (NOVA LEI)"] > 0 ? "text-green-500" : "text-red-500"} p-1`}>{obj["PAYBACK (NOVA LEI)"].toFixed(2)}</h1>
						<h1 className="text-center text-xxs lg:text-sm text-gray-600 border-r border-gray-300 p-1">{obj["INVESTIMENTO POUPANÇA"].toFixed(2)}</h1>
					</div>
				))}
			</div>
		);
		...
		return (
			<div className="p-6 grow flex flex-col lg:flex-row items-center justify-center">
				...
				<div className="w-full lg:w-[40%] flex flex-col">
					<h1 className="text-center text-xl text-[#15599a] font-bold font-['Roboto']">SIMULAÇÃO DE FATURAS E PAYBACK</h1>
					<div className="w-full flex flex-col py-2 px-12 gap-2">
						<div className="grid grid-rows-2 grid-cols-1 lg:grid-rows-1 lg:grid-cols-2 w-full gap-2">
							<div className="flex flex-col gap-2 w-full">
								<label className="text-start italic text-md text-gray-500">Quantidade de Módulos</label>
								<input
							...
							<div className="flex flex-col gap-2 w-full">
								<label className="text-start italic text-md text-gray-500">Potência dos Módulos</label>
								<input
						...
						<div className="grid grid-rows-2 grid-cols-1 lg:grid-rows-1 lg:grid-cols-2 w-full gap-2">
							<div className="flex flex-col gap-2 w-full">
								<label className="text-start italic text-md text-gray-500">Tipo de ligação</label>
								<select
							...
							<div className="flex flex-col gap-2 w-full">
								<label className="text-start italic text-md text-gray-500">Cidade</label>
								<select
									value={info.cidade}
						...
						<div className="grid grid-rows-2 grid-cols-1 lg:grid-rows-1 lg:grid-cols-2 w-full gap-2">
							<div className="flex flex-col gap-2 w-full">
								<label className="text-start italic text-md text-gray-500">Ano de início</label>
								<select
							...
							<div className="flex flex-col gap-2 w-full">
								<label className="text-start italic text-md text-gray-500">Mês de início</label>
								<select
						...
						<div className="grid grid-rows-2 grid-cols-1 lg:grid-rows-1 lg:grid-cols-2 w-full gap-2">
							<div className="flex flex-col gap-2 w-full">
								<label className="text-start italic text-md text-gray-500">Valor do kWh</label>
								<input
									value={info.valorkWh}
							...
							<div className="flex flex-col gap-2 w-full">
								<label className="text-start italic text-md text-gray-500">Valor do Fio B</label>
								<input
						...
						<div className="grid grid-rows-2 grid-cols-1 lg:grid-rows-1 lg:grid-cols-2 w-full gap-2">
							<div className="flex flex-col gap-2 w-full">
								<label className="text-start italic text-md text-gray-500">Iluminação pública</label>
								<input
							...
							<div className="flex flex-col gap-2 w-full">
								<label className="text-start italic text-md text-gray-500">Simultaneidade</label>
								<input
						...
						<div className="grid grid-rows-2 grid-cols-1 lg:grid-rows-1 lg:grid-cols-2 w-full gap-2">
							<div className="flex flex-col gap-2 w-full">
								<label className="text-start italic text-md text-gray-500">Consumo médio (kWh)</label>
								<input
							...
							<div className="flex flex-col gap-2 w-full">
								<label className="text-start italic text-md text-gray-500">Valor investido (R$)</label>
								<input
						...
						<div className="w-full justify-center items-center flex">
							<div className="flex flex-col gap-2 w-full">
								<label className="text-start italic text-md text-gray-500">Rendimento mensal da Poupança (%)</label>
								<input
