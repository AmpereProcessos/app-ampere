  ...
  if (data) {
    return (
      <div className="p-6 grow flex flex-col">
        ...
        {data?.map((obj, index) => (
          <div key={index} className="w-full grid grid-cols-6 lg:grid-cols-8 border-x border-b border-gray-200">
            ...
            <h1
              className={`text-center text-xxs lg:text-sm ${
                obj['PAYBACK (DIREITO ADQUIRIDO)'] > 0 ? 'text-green-500' : 'text-red-500'
              } border-r border-gray-300 p-1`}
            >
              {obj['PAYBACK (DIREITO ADQUIRIDO)'].toFixed(2)}
            </h1>
            <h1 className={`text-center text-xxs lg:text-sm ${obj['PAYBACK (NOVA LEI)'] > 0 ? 'text-green-500' : 'text-red-500'} p-1`}>
