...
function PublicPropertyTemporaryUsagePageContentForm({ openUsage }: { openUsage: TGetTemporaryUsageByPropertyOutput['data'] }) {
  ...
  async function handleUsageMutationMethod({
    ...
    if (obligatoryAttachmentsNotFilled.length > 0) {
      throw new Error(
        `A(s) anexação(ões) ${obligatoryAttachmentsNotFilled.map((obligatoryAttachment) => obligatoryAttachment.title).join(', ')} são obrigatórias.`
      )
    }
    const filesMetadata = await handleMultipleAttachmentsUpdate({ attachments, vinculationId: info.propriedade.id })
    if (propertyUsageId) {
      return await updatePropertyUsage({
        id: propertyUsageId,
        changes: { ...info, arquivos: [...info.arquivos, ...filesMetadata], dataFim: new Date().toISOString() },
      })
    }
  ...
  return (
    <div className="bg-background flex h-full justify-center px-0 py-6 lg:px-6 lg:py-12">
      {isUsageMutationSuccess ? (
        <div className="flex h-full w-full flex-col items-center justify-center gap-1">
          <div className="flex flex-col items-center gap-1">
            ...
            <h1 className="text-primary text-center text-lg font-bold tracking-tight">Formulário finalizado com sucesso !</h1>
          </div>
        ...
        <div className="container flex flex-col gap-4">
          <div className="bg-background border-primary/20 flex w-full flex-col gap-1.5 rounded-lg border p-3.5 shadow-xs dark:bg-[#121212]">
            <div className="flex w-full items-center justify-center gap-2">
              <Image src={Logo} alt="Logo" width={35} height={35} />
              <h1 className="text-center text-sm leading-none font-bold tracking-tight lg:text-lg">UTILIZAÇÃO DE PROPRIEDADE</h1>
            </div>
            <div className="flex w-full items-center justify-center gap-2">
              {propertyImageUrl ? (
              ...
              <h1 className="text-center text-sm leading-none font-bold tracking-tight lg:text-lg">{propertyName}</h1>
              <div className="bg-primary/20 flex w-fit items-center gap-2 rounded-lg px-2 py-1">
                <Code size={15} />
                <h1 className="w-fit text-start text-[0.6rem] font-medium tracking-tight">{propertyIdentifier}</h1>
              </div>
            </div>
          </div>
          ...
          <div className="bg-background border-primary/20 flex w-full flex-col gap-1.5 rounded-lg border p-3.5 shadow-xs dark:bg-[#121212]">
            {openUsage.openUsage ? (
              <div className={cn('flex w-fit items-center justify-center gap-1 self-center rounded bg-green-200 px-2 py-1 text-green-800')}>
                <Check size={15} />
                <h1 className="w-fit text-start text-xs font-medium tracking-tight">Você está finalizando o uso temporário da propriedade.</h1>
              </div>
            ) : (
              <div className={cn('flex w-fit items-center justify-center gap-1 self-center rounded bg-blue-200 px-2 py-1 text-blue-800')}>
                <Plus size={15} />
                <h1 className="w-fit text-start text-xs font-medium tracking-tight">Você está iniciando o uso temporário da propriedade.</h1>
              </div>
            )}
            <UserSelection />
            {propertyMetadataType === 'USO DE VEÍCULO' ? (
              <VehicleMetadata property={openUsage.property} hasOpenUsage={!!openUsage.openUsage} />
            ) : null}
      ...
      {currentAuthor.id !== '' ? (
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-center text-sm leading-none font-medium tracking-tight">{currentAuthor.nome}</h1>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="flex min-w-fit items-center gap-1">
              ...
              <p className={'text-primary/80 text-xs font-medium'}>{currentAuthor.telefone}</p>
            </div>
          </div>
        </motion.div>
      ) : null}
...
function VehicleMetadata({ property, hasOpenUsage }: VehicleMetadataProps) {
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      <div className="bg-primary/20 flex w-fit items-center gap-2 rounded px-2 py-1">
        <Car size={15} />
        <h1 className="w-fit text-start text-xs font-medium tracking-tight">DETALHES DO USO DO VEÍCULO</h1>
      </div>
      {vehicleReviewAlertLevel ? (
        <div className={cn('flex w-full items-center justify-center gap-1 rounded px-2 py-1', vehicleReviewAlertLevel.color)}>
          <AlertCircle size={15} />
      ...
      <div className="flex w-full flex-col gap-2">
        <h1 className="text-primary/80 text-sm font-medium tracking-tight">ANEXOS</h1>
        {attachments.length > 0 ? (
          attachments.map((attachment, index) => (
            <PropertyAttachmentCard
              key={attachment.identificador}
              index={index}
              attachment={attachment}
              propertyUsageMetadataType={propertyUsageMetadataType}
            />
          ))
        ) : (
    ...
    <div className="border-primary/20 bg-background flex w-full grow flex-col gap-4 rounded-md border p-6 shadow-xs dark:bg-[#121212]">
      <div className="flex w-full flex-col gap-1">
        <div className="flex w-full flex-col items-start gap-2 lg:flex-row lg:items-center">
          <div className="flex items-center gap-2">
            {filePredefinition ? renderIconWithClassNames(filePredefinition.icon) : <Paperclip className="h-4 min-h-4 w-4 min-w-4" />}
            <h1 className="text-sm leading-none font-bold tracking-tight">{attachment.titulo}</h1>
          </div>
          <div className="flex items-center gap-2">
            {filePredefinition ? (
              <>
                ...
                {filePredefinition.multiple ? (
                  <div className="bg-primary/20 text-primary flex items-center gap-1 rounded-lg px-2 py-1">
                    <FileStack className="h-3 min-h-3 w-3 min-w-3" />
        ...
        <p className="text-primary/80 text-xs leading-none font-light">{filePredefinition?.call || `Anexo do arquivo ${attachment.titulo}.`}</p>
      </div>
      <div className="relative flex w-full items-center justify-center">
        <label
          htmlFor="dropzone-file"
          className="dark:hover:bg-bray-800 border-primary/20 bg-background hover:bg-primary/10 flex w-full cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed px-3 py-1 dark:bg-[#121212]"
        >
      ...
      <h3 className="text-primary/80 text-[0.6rem] font-medium tracking-tight">ARQUIVOS ANEXADOS</h3>
      <div className="flex w-full flex-wrap items-center gap-2">
        {attachment.arquivos.length > 0 ? (
          attachment.arquivos.map((file, index) => (
            <div key={`${file.arquivo?.name}-${index}`} className="border-primary/50 flex h-[100px] w-[100px] flex-col rounded border">
