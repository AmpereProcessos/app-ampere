...
function MainUFVInsurancePage() {
	...
	return (
		<div className="flex grow flex-col gap-2 p-6">
			...
			{isSuccess ? (
				projects.length > 0 ? (
					projects.map((project) => <UFVInsuranceProjectCard key={project._id} project={project} onRevenueEditClick={(id) => setRevenueModal({ id, isOpen: true })} />)
				) : (
					<div className="w-full text-center text-sm font-medium tracking-tight text-primary/80">Nenhum projeto encontrado.</div>
				)
			) : null}
			{revenueModal.id && revenueModal.isOpen ? <EditRevenue revenueId={revenueModal.id} session={session} closeModal={() => setRevenueModal({ id: null, isOpen: false })} /> : null}
		</div>
	);
}
			...
			<div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
				...
				<div className="flex w-full flex-wrap items-center justify-center gap-2 lg:min-w-fit lg:justify-end">
					...
					<div className="flex items-center gap-1">
						...
						<h1 className="py-0.5 text-center text-[0.6rem] font-bold  text-primary">{formatToMoney(project.seguro?.valor || project.sistema.valorProjeto || 0)}</h1>
					</div>
					<div className="flex items-center gap-1">
					...
					<div className="flex items-center gap-2">
						...
						{receiptStatus.fractionationStr ? (
							<button
								...
								className={cn("rounded bg-blue-100 py-0.5 px-2 text-[0.5rem] font-bold text-primary/80 duration-300 ease-in-out", showReceipts ? "bg-blue-300" : "")}
							>
								...
								<div key={index} className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
									<div className="flex flex-wrap items-center justify-center gap-2 lg:justify-start">
										...
										<div className="hidden items-center gap-1 rounded-lg bg-secondary px-2 py-0.5 text-center text-[0.5rem] font-medium italic text-primary/80 lg:flex">
											...
											<h1>
												PARCIAL DE{" "}
												<strong>{receipt.porcentagem ? formatDecimalPlaces(receipt.porcentagem) : formatDecimalPlaces(100 * (receipt.valor || 0 / (project.receita?.total || 0)))}%</strong>
											</h1>
										</div>
									...
									<div className="hidden flex-wrap items-center justify-center gap-2 lg:flex lg:justify-end">
										<div className="flex items-center gap-1">
											...
											<p className="py-0.5 text-center text-[0.6rem] font-bold  text-primary">{formatDateAsLocale(receipt.dataPrevisaoRecebimento)}</p>
										</div>
										...
										<h1 className="rounded-lg bg-primary px-2 py-0.5 text-center text-[0.65rem] font-medium text-secondary">{formatToMoney(receipt.valor || 0)}</h1>
									</div>
								</div>
							))
						) : (
