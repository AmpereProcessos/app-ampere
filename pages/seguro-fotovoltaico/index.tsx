...
import {
  type UseUFVInsuranceProjectsFiltersParams,
  useUFVInsuranceProjects,
} from "@/utils/methods/query/ufv-insurance";
  ...
  return (
    <div className="flex grow flex-col gap-2 p-6">
          ...
          <div className="flex flex-col items-center gap-1 lg:flex-row">
            <div className="flex items-center gap-1">
              ...
              <p className="text-center text-2xl font-black text-[#15599a] uppercase">
                PROJETOS DE SEGURO FOTOVOLTAICO
              </p>
            </div>
          </div>
        </div>
      ...
      {isSuccess ? (
        projects.length > 0 ? (
          projects.map((project) => (
            <UFVInsuranceProjectCard
              key={project._id}
              project={project}
              onRevenueEditClick={(id) => setRevenueModal({ id, isOpen: true })}
            />
          ))
        ) : (
      ...
      {revenueModal.id && revenueModal.isOpen ? (
        <EditRevenue
          revenueId={revenueModal.id}
          session={session}
          closeModal={() => setRevenueModal({ id: null, isOpen: false })}
        />
      ) : null}
    </div>
  );
...
function UFVInsuranceProjectCard({ project, onRevenueEditClick }: UFVInsuranceProjectCardProps) {
  ...
  function getRevenueStatus(revenue: TProjectUFVInsuranceDTO["receita"]) {
    ...
    const totalReceived = receipts.reduce(
      (acc, current) => (current.dataRecebimento ? acc + (current.valor || 0) : acc),
      0,
    );
  ...
  function getReceiptStatus(receipt: TRevenue["fracionamento"][number]) {
    ...
    if (isOverDue)
      return (
        <h1 className="text-xxs min-w-fit rounded-lg bg-red-600 px-2 py-0.5 text-white">
          EM ATRASO
        </h1>
      );

    return (
      <h1 className="text-xxs min-w-fit rounded-lg bg-blue-500 px-2 py-0.5 text-white">
        A RECEBER
      </h1>
    );
  }
      ...
      <div className="flex w-full flex-col items-center justify-between gap-2 lg:flex-row">
        ...
        <div className="flex w-full flex-wrap items-center justify-center gap-2 lg:min-w-fit lg:justify-end">
          ...
          <div className="flex items-center gap-1">
            ...
            <h1 className="text-primary py-0.5 text-center text-[0.6rem] font-bold">
              {formatDecimalPlaces(project.seguro?.duracao || 0, 0)}
            </h1>
          </div>
          <div className="flex items-center gap-1">
            ...
            <h1 className="text-primary py-0.5 text-center text-[0.6rem] font-bold">
              {formatDateAsLocale(project.seguro?.dataInicio)}
            </h1>
          </div>
          <div className="flex items-center gap-1">
            ...
            <h1 className="text-primary py-0.5 text-center text-[0.6rem] font-bold">
              {formatDateAsLocale(project.seguro?.dataFim)}
            </h1>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {receiptStatus.fractionationStr ? (
              <button
                ...
                className={cn(
                  "text-xxs text-foreground rounded bg-blue-100 px-2 py-0.5 font-bold duration-300 ease-in-out",
        ...
        {project.receita ? (
          showReceipts ? (
            project.receita?.fracionamento.length > 0 ? (
              project.receita?.fracionamento.map((receipt) => (
                ...
                >
                  <div className="flex flex-wrap items-center justify-center gap-2 lg:justify-start">
                    <p className="text-[0.65rem] leading-none font-medium tracking-tight">
                      {receipt.titulo}
                    </p>
                    <div className="bg-secondary text-xxs text-foreground hidden items-center gap-1 rounded-lg px-2 py-0.5 text-center font-medium italic lg:flex">
                      <FaPercentage />
                      <h1>
                        PARCIAL DE{" "}
                        <strong>
                          {receipt.porcentagem
                            ...
                            : formatDecimalPlaces(
                                100 * (receipt.valor || 0 / (project.receita?.total || 0)),
                              )}
                          %
                        </strong>
                      </h1>
                  ...
                  <div className="hidden flex-wrap items-center justify-center gap-2 lg:flex lg:justify-end">
                    ...
                    {receipt.dataRecebimento ? (
                      <div className="flex items-center gap-1">
                        ...
                        <p className="text-primary py-0.5 text-center text-[0.6rem] font-bold">
                          {formatDateAsLocale(receipt.dataRecebimento)}
                        </p>
                      </div>
                    ) : null}
              ...
              <div className="text-foreground w-full text-start text-[0.65rem] font-medium tracking-tight">
                Nenhum recebimento definido.
              </div>
            )
          ) : null
        ) : (
