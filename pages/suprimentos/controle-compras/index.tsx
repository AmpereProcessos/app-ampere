...
import { ChartColumn, DiamondIcon, KanbanIcon, ListIcon, Truck } from "lucide-react";
...
function PurchasesControl({ initialMode, kanbanListExpandedModeOptions }: PurchasesControlProps) {
  ...
  return (
    <PurchasesControlContent
      session={session}
      initialMode={initialMode}
      kanbanListExpandedModeOptions={kanbanListExpandedModeOptions}
    />
  );
}
...
function PurchasesControlContent({
  session,
  initialMode,
  kanbanListExpandedModeOptions,
}: PurchasesControlContentProps) {
    ...
    <div className="w-full flex flex-col gap-2 p-6 grow">
      <div className="w-full flex items-center justify-between flex-col lg:flex-row gap-2">
        ...
        <div className="flex items-center gap-1 px-2 py-1 bg-primary/20 rounded-lg">
          ...
          >
            <ChartColumn className="w-3 h-3 min-w-3 min-h-3" />
            ANALÍTICO
          </Link>
          <button
            type="button"
      ...
      {mode === "card" ? (
        <PurchaseControlsCardModePage session={session} handleSetMode={handleSetMode} />
      ) : null}
      {mode === "grouped" ? (
        <PurchaseControlsGroupedModePage
          initialListExpandedModeOptions={kanbanListExpandedModeOptions}
          session={session}
          handleSetMode={handleSetMode}
        />
      ) : null}
      {mode === "transport-controls" ? (
        <TransportControlsView session={session} handleSetMode={handleSetMode} />
      ) : null}
    </div>
  );
  // if (mode === "kanban")
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  function getCookieDefinitionByListTitle({
    cookiesEntries,
    title,
  }: {
    cookiesEntries: [string, string][];
    title: string;
  }) {
    const entry = cookiesEntries.find(
      (entry) => getTextBetweenParentheses(entry[0]) === formatWithoutDiacritics(title),
    );
