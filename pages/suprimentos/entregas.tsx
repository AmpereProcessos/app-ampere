...
import {
  GeneralVisibleHiddenExitMotionVariants,
  fileTypes,
  formatDate,
  formatLongString,
  getFileTypeTitle,
...
import {
  usePurchaseControlById,
  usePurchaseControlsTags,
} from "@/utils/methods/query/purchase-controls";
...
export const getServerSideProps: GetServerSideProps<DeliveriesPageProps> = async (context) => {
  ...
  const parsedDeliveredRecentOnly =
    typeof deliveredRecentOnly === "string" ? deliveredRecentOnly === "true" : false;
  return {
    props: {
      tagIds: parsedTagIds,
...
function DeliveriesPage({ tagIds, purchasedOnly, deliveredRecentOnly }: DeliveriesPageProps) {
  ...
  return (
    <DeliveriesPageContent
      ...
      deliveredRecentOnly={deliveredRecentOnly}
    />
  );
}
...
function DeliveriesPageContent({
  ...
  purchasedOnly,
  deliveredRecentOnly,
}: DeliveriesPageContentProps) {
  const [dropdownMenuVisible, setDropdownMenuVisible] = useState<boolean>(false);
  const [acknowlegdeMenu, setAcknowlegdeMenu] = useState<{ id: string | null; isOpen: boolean }>({
    id: null,
    isOpen: false,
  });
  const {
        ...
        <div className="flex w-full items-center justify-between">
          <div className="flex flex-col items-center gap-2 lg:flex-row">
            <p className="text-center text-2xl font-black text-[#15599a] uppercase">
              PROJETOS EM PROCESSO DE ENTREGA
            </p>
          </div>
          {dropdownMenuVisible ? (
          ...
          >
            ...
            <div className="flex w-full flex-col flex-wrap items-center justify-start gap-2 lg:flex-row">
              ...
              <MultipleSelectInput
                ...
                handleChange={(value) =>
                  setFilters((prev) => ({ ...prev, deliveryStatus: value as string[] }))
                }
              ...
              <MultipleSelectInput
                ...
                handleChange={(value) =>
                  setFilters((prev) => ({ ...prev, cities: value as string[] }))
                }
                options={AllCities}
              ...
              <MultipleSelectInput
                ...
                handleChange={(value) =>
                  setFilters((prev) => ({ ...prev, ufs: value as string[] }))
                }
                options={AllStates}
              ...
              <MultipleSelectInput
                ...
                handleChange={(value) =>
                  setFilters((prev) => ({ ...prev, tagIds: value as string[] }))
                }
                options={
                  tags?.map((tag) => ({ id: tag._id, label: tag.titulo, value: tag._id })) || []
                }
            ...
            <div className="flex w-full flex-wrap items-center gap-2">
              <div className="w-fit">
                <CheckboxInput
                  ...
                  handleChange={(value) =>
                    setFilters((prev) => ({ ...prev, purchasedOnly: value }))
                  }
                />
              </div>
              <div className="w-fit">
                <CheckboxInput
                  ...
                  handleChange={(value) =>
                    setFilters((prev) => ({ ...prev, deliveredRecentOnly: value }))
                  }
                />
              </div>
            </div>
          ...
          projects.length > 0 ? (
            projects.map((project) => (
              <ProjectCard
                key={project._id}
                project={project}
                handleClick={(id) => setAcknowlegdeMenu({ id, isOpen: true })}
              />
            ))
          ) : (
            <p className="text-foreground w-full text-center tracking-tight">
              Nenhuma compra em processo de entrega encontrado.
            </p>
          )
        ) : null}
      </div>
...
function ProjectCard({ project, handleClick }: ProjectCardProps) {
  function getDeliveryStatusTag(status: TPurchaseControlDeliveryTrackingDTO["entrega"]["status"]) {
    if (status === "AGUARDANDO COMPRA")
      return (
        <h1 className="text-xxs min-w-fit rounded-lg bg-orange-600 px-2 py-0.5 text-white">
          {status}
        </h1>
      );
    ...
    if (status === "EM ROTA")
      return (
        <h1 className="text-xxs min-w-fit rounded-lg bg-blue-600 px-2 py-0.5 text-white">
          {status}
        </h1>
      );
    ...
    if (status === "AGUARDANDO DESPACHE")
      return (
        <h1 className="text-xxs min-w-fit rounded-lg bg-orange-500 px-2 py-0.5 text-white">
          {status}
        </h1>
      );
    ...
    <div className="border-primary/60 flex w-full flex-col gap-1 rounded border p-2">
        ...
        <div className="flex w-full flex-wrap items-center justify-center gap-2 lg:grow lg:justify-start">
          <div className="flex items-center gap-1">
            ...
            <h1 className="text-foreground py-0.5 text-center text-[0.6rem] font-medium italic">
              {project.projetoDados?.vendedor?.nome}
            </h1>
          </div>
          <div className="flex items-center gap-1">
            <FaPhone width={10} height={10} />
            <h1 className="text-foreground py-0.5 text-center text-[0.6rem] font-medium italic">
              {project.projetoDados?.telefone}
            </h1>
          </div>
          <div className="flex items-center gap-1">
              ...
              {formatLocation({
                location: {
                  ...
                  numeroOuIdentificador:
                    project.entrega.localizacao.numeroOuIdentificador?.toString(),
                },
                includeUf: true,
                includeCity: true,
              })}
        ...
        <div className="flex w-full flex-wrap items-center justify-center gap-2 lg:min-w-fit lg:justify-end">
          <div className="flex items-center gap-1">
            ...
            <h1 className="text-primary py-0.5 text-center text-[0.6rem] font-bold">
              {project.fornecedor?.nome || "NÃO DEFINIDO"}
            </h1>
          </div>
          <div className="flex items-center gap-1">
            ...
            <h1 className="text-primary py-0.5 text-center text-[0.6rem] font-bold">
              {formatDateAsLocale(project.dataPedido) || "NÃO POSSUI"}
            </h1>
          </div>
          <div className="flex items-center gap-1">
      ...
      <div className="flex w-full items-center justify-between">
        <div className="flex items-center gap-1">
          ...
          <h1 className="text-primary py-0.5 text-center text-[0.6rem] font-bold">
            {formatDateAsLocale(project.dataInsercao, true)}
          </h1>
        </div>
        <button
          type="button"
...
function AcknowledgeDeliveryMenu({
  session,
  purchaseControlId,
  closeModal,
  affectedQueryKey,
}: AcknowledgeDeliveryMenuProps) {
  const queryClient = useQueryClient();

  const {
    data: purchaseControl,
    isLoading,
    isError,
    isSuccess,
    error,
  } = usePurchaseControlById({ id: purchaseControlId });
  ...
  async function handleAcknowledgeDelivery(info: TAcknowlegdeDeliveryInfo) {
    ...
    await updatePurchaseControl({
      ...
      changes: {
        ...purchaseControl,
        entrega: {
          ...purchaseControl.entrega,
          status: "ENTREGUE",
          dataEfetivacao: info.deliveryDate,
        },
      },
    });
            ...
            <>
              <div className="scrollbar-thin scrollbar-track-primary/20 scrollbar-thumb-primary/20 flex h-full flex-col gap-y-2 overflow-y-auto overscroll-y-auto p-2 py-1">
                <div className="flex items-center gap-1 self-center">
                  ...
                  <h1 className="py-0.5 text-center text-sm font-bold italic">
                    {purchaseControl.titulo}
                  </h1>
                </div>
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-4 py-1">
                      <div className="flex items-center gap-1">
                        ...
                        <p className="text-xs leading-none font-medium tracking-tight">
                          {purchaseControl.projetoDados?.nomeDoContrato}
                        </p>
                      </div>
                      <div className="flex items-center gap-1">
                        <MdPhone />
                        <p className="text-xs leading-none font-medium tracking-tight">
                          {purchaseControl.projetoDados?.telefone}
                        </p>
                      </div>
                      <div className="flex items-center gap-1">
                        <BsPersonVcard />
                        <p className="text-xs leading-none font-medium tracking-tight">
                          {purchaseControl.projetoDados?.cpf_cnpj}
                        </p>
                      </div>
                      <div className="flex items-center gap-1">
                        <MdLandscape />
                        ...
                        <p className="text-xs leading-none font-medium tracking-tight">
                          {formatLocation({
                            location: {
                              ...
                              numeroOuIdentificador:
                                purchaseControl.projetoDados?.numeroResidencia?.toString() || "",
                              complemento: null,
                              latitude: null,
                              longitude: null,
                            },
                ...
                <div className="flex flex-wrap items-center justify-center gap-4 py-1">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <p className="text-xs leading-none font-medium tracking-tight">
                      {purchaseControl.fornecedor?.contato || "N/A"}
                    </p>
                  </div>
                </div>
                <h1 className="bg-primary/80 w-full rounded p-1 text-center text-[0.65rem] font-bold tracking-tight text-white">
                ...
                <h1 className="w-full rounded bg-blue-800 p-1 text-center text-sm font-bold tracking-tight text-white">
                  ACUSAR ENTREGA
                </h1>
                <DateInput
                  ...
                  handleChange={(value) =>
                    setInfoHolder((prev) => ({
                      ...prev,
                      deliveryDate: formatDateInputChange(value) as string,
                    }))
                  }
                  width="100%"
                />
                ...
                <div className="flex w-full flex-col gap-2">
                  <div className="relative flex w-full items-center justify-center">
                    <label
                      ...
                      className="dark:hover:bg-bray-800 border-border bg-background hover:bg-primary/10 flex h-64 w-full cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed dark:bg-[#121212]"
                    >
                    ...
                    {infoHolder.files
                      ...
                      .map((attachment, index) => (
                        <div
                          ...
                          className="border-primary/50 flex h-[180px] w-[150px] flex-col rounded border"
                        >
