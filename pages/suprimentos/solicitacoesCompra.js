		...
		return (
			<div className="flex grow flex-col p-6">
								...
								>
									...
									<div className="flex items-center justify-between">
										<div className="flex flex-col  items-start">
											...
											<h1 className={`text-xs ${getStatusColor(solicitation.status)} rounded-md p-1 font-medium`}>{solicitation.status ? solicitation.status : "EM ABERTO"}</h1>
										</div>
										<div className="flex flex-col  items-end">
											<h1 className="text-end text-xs font-medium text-gray-500">RESPONSÁVEL</h1>
											<h1 className={`rounded-md p-1 text-end text-xs font-medium`}>{solicitation.responsavel ? solicitation.responsavel : "NÃO DEFINIDO"}</h1>
										</div>
									</div>
									...
									<div className="flex items-center justify-between">
										<div className="flex flex-col items-start">
											...
											<div className="flex items-center gap-2">
												...
												<h1 className="text-xs font-medium text-gray-700 lg:text-base">{solicitation.dataInsercao ? dayjs(solicitation.dataInsercao).format("DD/MM/YY HH:mm") : null}</h1>
											</div>
										</div>
										<div className="flex flex-col items-end">
									...
									<div className="flex w-full flex-col">
										...
										{solicitation.itens.length > 4 ? <p className="w-full text-center text-sm font-light text-gray-700">Clique para ver demais itens</p> : null}
									</div>
								</div>
							))
						) : (
				...
				{modal.isOpen && modal.info ? (
					<PurchaseSolicitationModal info={modal.info} isOpen={modal.isOpen} closeModal={() => setModal({ info: null, isOpen: false })} getSolicitations={getSolicitations} />
				) : null}
			</div>
		);
	}
