        ...
        <div className="mt-4 flex flex-wrap justify-around gap-3">
          {filteredForms.map((solicitacao) => (
            <div
              ...
              className={`flex flex-col ${getCardColor(solicitacao.aprovacao)} border-primary/20 dark:hover:bg-primary/10 w-[250px] cursor-pointer border p-3 hover:bg-blue-100 lg:w-[450px]`}
            >
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-primary/80 text-xs">{solicitacao.nomeVendedor && solicitacao.nomeVendedor}</p>
                </div>
                <div>
                  <span className="text-xxs">CIDADE</span>
                  <p className="text-primary/80 text-xs">{solicitacao.cidade ? solicitacao.cidade : '-'}</p>
                </div>
              </div>
            </div>
          ))}
