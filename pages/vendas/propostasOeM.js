...
function PropostasOeMVendedor() {
  ...
  async function getProposes(seller) {
    setProposes({ status: "loading", data: null });
    try {
      const { data } = await axios.get(`/api/o&m/proposes?seller=${seller}`);
      let obj = {
  ...
  useEffect(() => {
    if (session?.user) {
      if (!proposes.data) {
        if (session.user.vendedor) getProposes(session.user.vendedor);
      }
    }
  }, [session]);
      ...
      <div className="flex flex-col p-6 grow bg-[#fff]">
        ...
        <div className="flex py-2 gap-4 mt-5  shadow-lg w-full max-w-full overflow-x-auto scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300">
          <ListPropostas
            ...
            listId={1}
            fetchProposes={() => getProposes(session.user.vendedor)}
            proposes={
        ...
        {newProposeModalIsOpen ? (
          <ModalNewPropostaOeM
            getProposes={() => getProposes(session.user.vendedor)}
            closeModal={() => setNewProposeModalIsOpen(false)}
            seller={session.user.vendedor}
...
export default PropostasOeMVendedor;
