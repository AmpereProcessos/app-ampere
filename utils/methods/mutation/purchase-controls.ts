import type { TCreatePurchaseControlCompositionKitInput, TCreatePurchaseControlCompositionKitOutput } from "@/pages/api/controles-compras/kits";
...
export async function createPurchaseControlCompositionKit(input: TCreatePurchaseControlCompositionKitInput) {
	try {
		const { data }: { data: TCreatePurchaseControlCompositionKitOutput } = await axios.post("/api/controles-compras/kits", input);
