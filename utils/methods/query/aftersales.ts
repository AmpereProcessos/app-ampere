...
function getNestedFieldValue(project: TProjectDTO, path: string) {
	// @ts-ignore
	return path.split(".").reduce((acc, part) => acc?.[part as keyof TProjectDTO], project);
}
...
async function fetchProjects() {
	try {
		...
		console.error("Error fetching after sales projects", error);
		throw error;
	}
}
...
export function useAfterSalesProjects() {
	...
	function orderByContact(projects: TProjectDTO[]) {
		...
		switch (filters.contactOrder) {
			case "ASC":
				// @ts-ignore
				return [...nulls, ...filtered.sort((a, b) => new Date(a.jornada.dataUltimoContato) - new Date(b.jornada.dataUltimoContato))];
			case "DESC":
				// @ts-ignore
				return [...filtered.sort((a, b) => new Date(b.jornada.dataUltimoContato) - new Date(a.jornada.dataUltimoContato)), ...nulls];
	...
	function handelModelData(data: TProjectDTO[]) {
		const orderedData = orderByContact(data);
