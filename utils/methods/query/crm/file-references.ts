...
async function fetchFileReferencesByQuery({ clientId, opportunityId, analysisId, homologationId, projectId, purchaseId, revenueId, serviceOrderId }: TFileReferencesQueryParams) {
	try {
		...
		const param = [clientParam, opportunityParam, analysisParam, homologationParam, projectParam, purchaseParam, revenueParam, serviceOrderParam].filter((q) => !!q).join("&");
		if (!param) return [];
