...
async function fetchOpportunityHomologations({ opportunityId }: { opportunityId: string }) {
  try {
    const { data } = await axios.get(`/api/crm/homologations?opportunityId=${opportunityId}`)
...
async function fetchHomologationById({ id }: { id: string }) {
  try {
    const { data } = await axios.get(`/api/crm/homologations?id=${id}`)
...
async function fetchHomologations() {
  try {
    const { data } = await axios.get(`/api/crm/homologations`)
