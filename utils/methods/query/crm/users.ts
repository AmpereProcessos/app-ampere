...
async function fetchUsers({ includeDeleted = false }: UseUsersParams) {
	try {
		const { data } = await axios.get(`/api/crm/users?includeDeleted=${includeDeleted}`);
		return data.data as TUserDTO[];
	} catch (error) {
		throw error;
	}
}
export function useUsers({ includeDeleted = false }: UseUsersParams) {
	return useQuery({
		queryKey: ["users"],
		queryFn: async () => await fetchUsers({ includeDeleted }),
	});
}
