...
import type {
  TEngineeringProjectsKanbanInput,
  TEngineeringProjectsKanbanOutput,
} from "@/pages/api/projects/engenharia/kanban";

async function fetchProjects() {
  try {
    const { data }: { data: TGetEngineeringProjectsOutput } = await axios.get(
      "/api/projects/engenharia",
    );
    return data.data;
  } catch (error) {
    throw error;
...
export function useEngineeringProjects() {
  ...
  function matchPendingVistory(project: TGetEngineeringProjectsOutput["data"][number]) {
    ...
    return (
      !!project.homologacao.vistoria.dataSolicitacao && !project.homologacao.vistoria.dataEfetivacao
    );
  }
  ...
  function matchPaidOnly(project: TGetEngineeringProjectsOutput["data"][number]) {
    if (!filters.paidOnly) return true;
...
async function fetchEngineeringProjectsKanban(input: TEngineeringProjectsKanbanInput) {
  try {
    const { data }: { data: TEngineeringProjectsKanbanOutput } = await axios.post(
      "/api/projects/engenharia/kanban",
      input,
    );
    return data.data;
  } catch (error) {
