import { TGetServiceOrdersItineraryInput, TGetServiceOrdersItineraryOutput } from '@/pages/api/ordensDeServico/roteiro'
...
export function useServiceOrdersByPersonalizedFilters({ initialFilters }: UseServiceOrdersByPersonalizedFiltersParams) {
  const [filters, setFilters] = useState<TPersonalizedServiceOrderFilter>({
    ...
    projectHomologationApproved: initialFilters.projectHomologationApproved ?? false,
    orderBy: {
...
async function fetchServiceOrdersItinerary({ params }: { params: TGetServiceOrdersItineraryInput }) {
  const searchParams = new URLSearchParams()
  if (params.search) searchParams.set('search', params.search)
  if (params.page) searchParams.set('page', params.page.toString())
  if (params.responsibleIds) searchParams.set('responsibleIds', params.responsibleIds.join(','))
  if (params.pendingConclusionOnly) searchParams.set('pendingConclusionOnly', params.pendingConclusionOnly.toString())
  const { data } = await axios.get<TGetServiceOrdersItineraryOutput>(`/api/ordensDeServico/roteiro?${searchParams.toString()}`)
  if (!data.data.default) throw new Error('Erro ao buscar ordens de serviço')
  return data.data.default
}

export function useServiceOrdersItinerary({ initialFilters }: { initialFilters: Partial<TGetServiceOrdersItineraryInput> }) {
