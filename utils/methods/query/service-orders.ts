import {
  TGetServiceOrdersItineraryInput,
  TGetServiceOrdersItineraryOutput,
} from "@/pages/api/ordensDeServico/roteiro";
import type { TServiceOrdersByFiltersResult } from "@/pages/api/ordensDeServico/search";
import type {
  TServiceOrderStatsInput,
  TServiceOrderStatsOutput,
} from "@/pages/api/ordensDeServico/stats";
import type {
...
async function fetchServiceOrders({ queryTags, queryPendingConclusion }: TServiceOrderQueryParams) {
  const { data } = await axios.get(
    `/api/ordensDeServico?queryTags=${queryTags}&queryPendingConclusion=${queryPendingConclusion}`,
  );
...
async function fetchServiceOrdersByPersonalizedFilters({
  page,
  filters,
}: {
  page: number;
...
export function useServiceOrdersByPersonalizedFilters({
  initialFilters,
}: UseServiceOrdersByPersonalizedFiltersParams) {
  ...
  return {
    ...useQuery({
      ...
      queryFn: async () =>
        await fetchServiceOrdersByPersonalizedFilters({ page: filters.page, filters }),
    }),
    filters,
    updateFilters,
  };
...
async function fetchServiceOrdersByResponsible({ responsibleName }: { responsibleName: string }) {
  const { data } = await axios.get(
    `/api/ordensDeServico?responsibleName=${responsibleName}&queryPendingConclusion=true`,
  );
  return data.data as TServiceOrderSimplifiedDTO[];
}
...
async function fetchServiceOrdersByTechnicalAnalysis({
  technicalAnalysisId,
}: {
  technicalAnalysisId: string;
}) {
  const { data } = await axios.get(
    `/api/ordensDeServico?technicalAnalysisId=${technicalAnalysisId}`,
  );
  return data.data as TServiceOrderSimplifiedDTO[];
}

export function useServiceOrdersByTechnicalAnalysis({
  technicalAnalysisId,
}: {
  technicalAnalysisId: string;
}) {
...
async function fetchServiceOrdersStats(input: TServiceOrderStatsInput) {
  const { data }: { data: TServiceOrderStatsOutput } = await axios.post(
    "/api/ordensDeServico/stats",
    input,
  );
  return data.data;
}
...
async function fetchServiceOrdersItinerary({
  params,
}: {
  ...
  const { data } = await axios.get<TGetServiceOrdersItineraryOutput>(
    `/api/ordensDeServico/roteiro?${searchParams.toString()}`,
  );
  if (!data.data.default) throw new Error("Erro ao buscar ordens de serviço");
