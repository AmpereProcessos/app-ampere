...
async function fetchTechnicalAnalysisByOpportunityId({ opportunityId, concludedOnly }: { opportunityId: string; concludedOnly: boolean }) {
  try {
    const { data } = await axios.get(`/api/analises-tecnicas?opportunityId=${opportunityId}&concludedOnly=${concludedOnly}`)
...
async function fetchTechnicalAnalysis() {
  try {
    const { data } = await axios.get(`/api/analises-tecnicas`)
    return data.data as TTechnicalAnalysisDTO[]
  } catch (error) {
    throw error
  }
...
export async function fetchTechnicalAnalysisById({ id }: { id: string }) {
  try {
    const { data } = await axios.get(`/api/analises-tecnicas?id=${id}`)
    return data.data as TTechnicalAnalysisDTO
  } catch (error) {
    throw error
  }
...
async function fetchTechnicalAnalysisByPersonalizedFilters({
  ...
  try {
    const { data } = await axios.post(`/api/analises-tecnicas/search?after=${after}&before=${before}&page=${page}`, {
      applicants,
      analysts,
      partners,
      filters,
