...
export function renderProductCategoryIcon(
  category: TProductItem["categoria"],
...
export function handleRenderFileIconWithClassNames(format: string, className?: string) {
  const extensionInfo = Object.values(fileTypes).find((f) => f.title === format);
  if (!extensionInfo)
    return (
      <div className="text-primary">
        <AiFillFile className={cn("h-4 min-h-4 w-4 min-w-4", className)} />
      </div>
    );
  return (
    <div className="text-primary">{renderIconWithClassNames(extensionInfo.icon, className)}</div>
  );
}
    ...
    <button
      ...
      className={`${
        activePage === p
          ...
          : "text-primary hover:bg-primary/50 border-transparent"
